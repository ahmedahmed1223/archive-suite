...
- Combined Control Center/adapter/probe run reached 59/60; the unrelated `confirmed JSON migration executes through the manifest-backed release Compose runtime` test is currently blocked by local Docker credential/daemon access (`<redacted-user>\.docker\config.json`, npipe permission denied). The focused probe suite is green.
