...
- **السبب الجذري**: `/api/v1/auth/login` محدود بـ`throttle:10,1` (10 طلبات/دقيقة، V1-104). 5 اختبارات × مشروعَي Playwright (chromium + mobile-chrome) = 10 طلبات تسجيل دخول بالضبط عند حد الحد الأقصى، وأي إعادة محاولة (`retries: 2` في CI) تتجاوزه فورًا — فيفشل تسجيل الدخول بـ429 لا بخطأ حقيقي في الصفحة المُختبرة.
- **الإصلاح**: نُقل تسجيل الدخول من `beforeEach` (شبكة فعلية في كل اختبار) إلى `beforeAll` (مرة واحدة لكل مشروع Playwright، عبر `request.newContext()` مستقل)، مع استخراج كوكي `va_refresh` من `storageState()` وحقنه محليًا في كل اختبار عبر `context.addCookies()` — بلا أي طلب شبكة إضافي. يخفّض هذا عدد نداءات `/auth/login` الفعلية من 10 إلى 2 لكل تشغيل، بعيدًا جدًا عن الحد. لم يُغيَّر `throttle:10,1` نفسه — يبقى ضابط أمان إنتاجي حقيقي ضد هجمات القوة الغاشمة.
- التحقق: `pnpm --filter @archive/next exec tsc --noEmit` نظيف (لا أخطاء نوع جديدة).
