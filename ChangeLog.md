...
- أصبح أمر `rehearse` نفسه ينفذ load ثم `compose up -d --wait` وHTTPS، ويضمن `finally` لتنظيف containers/network/volumes ضمن project فريد ثم يفحص غياب الموارد بالـproject label. evidence المنقح الملتزم في `docs/v1-206-rehearsal-evidence.json` يسجل الصور الخمس والصحة وHTTPS والتنظيف، بينما بقيت logs/PID خارج payload المتحقق منه.
