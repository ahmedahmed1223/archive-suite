...
- إصلاح مراجعة P1 إضافي: يطلب collector سجلات الخدمات structured مع `--no-log-prefix`، ويبقى parser قادرًا على fixture Compose الحقيقي `service | {json}` ويفشل مغلقًا عند event JSON مشوه بدل إرجاع صفر. يسجل Caddy الآن `X-Request-ID` النهائي من response عبر `{http.response.header.X-Request-ID}`، فيحافظ Next/Laravel على validation ويظهر المعرّف المولد نفسه عند غياب header وارد. أثبت Docker E2E harness المعرّف نفسه (36 محرفًا) في response وسجل Next وسجل Laravel وCaddy JSON، ونظف حاوياته وشبكته في `finally`. التحقق المحدود: unit+Control ‏30/30 وLaravel correlation ‏6/6 (31 assertion)؛ لم تُعد البوابة الكاملة لأن الإصلاح محدود وبعد نجاحها السابق.
