...
- تُعرض على خريطة Leaflet حقيقية (بلاطات OpenStreetMap، أصول العلامات مستضافة ذاتيًا لتعمل في التثبيت المعزول/intranet).
- قائمة جانبية لكل سجل موقَّع ورابط فتحه في OpenStreetMap.
- دالة نقية جديدة `geoTaggedRecords` في `lib/geotag.ts` مع 3 اختبارات TDD؛ 6/6 فحوصات الجرد خضراء.

## V1-711 رفع مجزأ قابل للاستئناف للملفات الكبيرة — 2026-07-19

- 5 نقاط نهاية جديدة (`/uploads/sessions` إنشاء/حالة/إلغاء، `.../chunks/{index}` رفع جزء، `.../complete` تجميع).
- التجزئة تُستخرج منطق التحقق المشترك مع الرفع المباشر إلى خدمتين بلا تكرار.
- الاستئناف الحقيقي: العميل يستعلم `GET .../{id}` بعد انقطاع ليعرف الأجزاء الناقصة فقط.
- الواجهة: `lib/chunked-upload.ts` (توجيه تلقائي للملفات ≥100MB، استئناف عبر localStorage).
- 15 اختبار Laravel + 5 اختبارات Vitest جديدة، عقد OpenAPI محدَّث.

## V1-303B fixtures مصادقة حسب الدور — 2026-07-19

- fixtures أدوار مع تسجيل دخول حي لكل سياق (refresh token يُدوَّر عند كل استخدام فلا يُعاد تشغيله).
- رفع throttle للدخول/التحديث لملاءمة IP مشترك، وحصانة العميل ضد فشل refresh العابر.
- بيانات records/rights/backups/jobs معزولة لا توجد token مدير موحد.
- البوابة الحية 64/64 ناجحة.

## V1-303C تغطية جميع المسارات المصنفة — 2026-07-19

- axe على المسارات المصادَق عليها والحالات loading/empty/error/ready عند 375/768/1280 وzoom 200%.
- البنية كاملة (جرد 45 مسارًا × 4 حالات × 3 أبعاد + زوم)؛ أُصلحت كل الجذور المكتشفة (توكنات تباين، شفافية شارة، fallback ميت، cascade في topbar، تسميات aria).
- جولة الإثبات الكاملة 625/625 ناجحة (5 إخفاقات أولى كانت flake، أُعيد تشغيلها فنجحت 6/6).

## V1-732 تراجع/إعادة فعلي للتعديلات (مكدس حقيقي) — 2026-07-21

- وحدة عامة جديدة نقية `lib/undo-stack.ts` (`UndoStack<T>` غير قابلة للتحوّر، `push`/`undo`/`redo`/`canUndo`/`canRedo`).
- تُفرّغ سجل الإعادة عند دفع إجراء جديد (اصطلاح المحررات القياسي).
- طُبّقت على `/kanban` كأول استخدام حقيقي: زرّا "تراجع"/"إعادة" بعدد الخطوات بدل "تراجع عن آخر نقل" وحيد المستوى السابق.
- 5 اختبارات Vitest جديدة؛ مقصور على كانبان فقط (لم يُطبَّق على نماذج تحرير السجل بعد).
- **لم يُتحقق حيًا** — Docker متعطل؛ **متابعة مطلوبة عبر V1-732B..D.**

## V1-732B امتداد مكدس الحذف/الاستعادة الجماعي — 2026-07-21

- توست "تراجع" وحيد المستوى (V1-737) صار يدفع كل دفعة حذف على `deleteStack` جديد.
- زرّا "تراجع عن الحذف"/"إعادة الحذف" بعدد الدفعات — عدة عمليات حذف متتالية قابلة للتراجع كلٌّ على حدة.
- زر توست "تراجع" مُعاد توجيهه (مصدر حقيقة واحد).
- **إصلاح فخ closure قديم**: استُخدم `useRef` متزامن (نمط موثّق في `rules/react/hooks.md`).
- `tsc --noEmit` نظيف، 426 اختباراً خضراء عبر `pnpm test` دون انحدار؛ **لم يُتحقق حيًا** — Docker متعطل.

## V1-732C امتداد المكدس لنموذج تحرير السجل — 2026-07-21

- `RecordDescribeForm` مكوّن مستقل فعلاً (~100 سطر) لم يحتج تفكيكاً.
- تراجع/إعادة بنقاط تحقق عند `onBlur` لكل حقل — كل دفعة `{previous, next}` على `UndoStack<FormChange>`.
- **باگ حقيقي اكتُشف وأُصلح بالاختبار**: `handleUndo` كان يستدعي `commitCheckpoint()` ثم يقرأ `history` فورًا — القراءة كانت تحمل القيمة القديمة (فخ closure).
- أُصلح بـ`historyRef` متزامن يوازي `history`؛ صُدّرت `RecordDescribeForm`/`RecordDescribePatch` لاختبار مخصص.
- 6 اختبارات Vitest+RTL جديدة غطّت التراجع/الإعادة وتعديلاً غير محفوظ ومسح سجل الإعادة.
- `tsc --noEmit` نظيف، 432/432 اختباراً خضراء؛ **لم يُتحقق حيًا** — Docker متعطل.

## V1-732D امتداد المكدس للعمليات الجماعية الأخرى — 2026-07-21

- **المجموعات (`/collections`)**: `removeCollection` يدفع على `UndoStack<CreateCollectionPayload>`؛ التراجع يُعيد الإنشاء، والإعادة تبحث بالاسم وتحذف؛ 426 اختباراً خضراء.
- **المفردات (`/vocabulary`)**: `removeTerm` يدفع على `UndoStack<TermDeletion>`؛ التراجع يُعيد الإنشاء عبر `createVocabularyTerm` (بمعرّف جديد)؛ 426 اختباراً خضراء.
- **الوسوم (`/tags`)**: `updateParent` صارت تأخذ `recordUndo` اختيارياً وتدفع على `UndoStack<ParentChange>`؛ التراجع/الإعادة يعيدان استدعاء `updateParent` بـ`recordUndo=false`؛ 426 اختباراً خضراء.
- **سلة المحذوفات (`/trash`)**: `restore()` يدفع على `UndoStack<TrashEntry>`؛ **`purge()`عمداً خارج المكدس** — لا "تراجع" حقيقياً له؛ 426 اختباراً خضراء.
- **لم يُتحقق حيًا** — Docker متعطل.

## V1-733 فحص سلامة مجدول للملفات المخزنة (checksum دوري) — 2026-07-21

- أمر جديد `files:verify-integrity` يعيد قراءة كل مرفق من قرصه الفعلي (streaming عبر `hash_update_stream` بلا تحميل بالذاكرة).
- يقارن sha256 الناتج بـ`checksum_sha256` المسجَّل وقت الرفع.
- أي عدم تطابق أو ملف مفقود يُسجَّل في `audit_logs` ويُرجع كود خروج غير صفري.
- مجدول يوميًا في `routes/console.php`؛ 3 اختبارات Laravel جديدة.
- **لم يُتحقق حيًا محليًا** — Docker متعطل.

## V1-734 سلسلة تجزئة لسجل التدقيق (Tamper-evident Audit Log) — 2026-07-21

- عمودان جديدان `prev_hash`/`hash` (sha256) على `audit_logs`، تُحسب تلقائيًا عبر `AuditLog::booted()` (`creating`).
- يغطي أي مسار كتابة مستقبلي دون تكرار المنطق (المسار الوحيد حاليًا `AuditArchiveApiRequest`).
- أمر جديد `audit:verify-chain` يعيد حساب السلسلة صفًا صفًا ويكتشف أي صف مُعدَّل أو محذوف من المنتصف.
- مجدول يوميًا في `routes/console.php`؛ 3 اختبارات Laravel جديدة (سلسلة سليمة، تلاعب بصف، حذف صف).
- **لم يُتحقق حيًا محليًا** — Docker متعطل.

## V1-741 لوحة "يومي" شخصية لكل مستخدم — 2026-07-21

- صفحة جديدة `/daily` (مضافة لـ`primaryNav`) تجمّع أربع لوحات دون نموذج بيانات جديد.
- عناصر الوارد غير المفروزة، الإشعارات غير المقروءة، المفضلة والأخيرة (إعادة استخدام `api.inboxItems()` و`useNotifications()` و`lib/favorites.ts`/`lib/recent-items.ts`).
- إعادة استخدام أصناف CSS الحالية (`dashboard-recent__*`, `record-grid`) بلا CSS جديد.
- `pnpm tsc --noEmit` نظيف؛ **لم يُتحقق حيًا في المتصفح** — Docker متعطل.

## V1-744 اعتماد/رفض جماعي لاقتراحات الذكاء الاصطناعي — 2026-07-21

- `SuggestionsPanel.tsx` يضيف تحديدًا متعددًا (checkbox لكل اقتراح + "تحديد الكل").
- زرّا "اعتماد المحدد"/"رفض المحدد" يستدعيان نفس `onFeedback` الحالي لكل عنصر محدد (`Promise.all`).
- لا تغيير مطلوب في المستهلكين الثلاثة (`archive/[id]`, `search`, `discover`).
- 4 اختبارات Vitest جديدة؛ `pnpm tsc --noEmit` نظيف.

## V1-758 توسعة قواعد الأتمتة بشروط محتوى الملف — 2026-07-21

- **تصحيح نطاق**: لا محرّك تنفيذ حدثي موجود لقواعس الأتمتة؛ حقل `trigger` مجرد تصنيف مخزَّن، والتنفيذ الفعلي يوجد عبر `POST /automation/rules/{id}/run`.
- **"عند الاستيراد" تلقائيًا غير مبني** — يحتاج بنية تحتية جديدة (observers، ربط بخط ingest)، خارج النطاق.
- **المُنجز**: شرط جديد `fileExtension` (قائمة امتدادات مفصولة بفواصل، غير حساس لحالة الأحرف) في `AutomationRulesController`.
- يُطابق `record.fileName` (يُضبط بواسطة `UploadFinalizer`/`IngestScanner`).
- 4 اختبارات PHPUnit جديدة (تطابق، دمج مع شرط `type`، غياب الشرط، حساسية الحالة).
- **لم تُشغَّل الاختبارات محليًا** — لا PHP محلي ولا Docker؛ التحقق يعتمد على مراجعة الكود اليدوية.

## V1-765 جولة تفاعلية بالتلميحات لأول استخدام — 2026-07-21

- `components/FirstRunTour.tsx` (خمس خطوات: الأرشيف/البحث/الرفع/كانبان/الإعدادات) داخل `Dialog` موجود.
- زرّ تشغيل مضاف لصفحة `/first-run`.
- **تُبدَأ يدويًا وليست منبثقة تلقائيًا** — تجنبًا لتصادمها مع `WhatsNewDialog` المنبثق تلقائيًا (لم يُختبر تكديس الحوارين حيًا).
- 10 اختبارات (Vitest+RTL) خضراء عبر `pnpm test`.
- ليست جولة spotlight تُموضع فوق عناصر DOM حية — بديل أخف وأقل مخاطرة.

## V1-770 حركات الحوارات والتوست (Motion) — 2026-07-21

- الحوارات (`Dialog`/`ConfirmDialog`/`WhatsNewDialog`) لم تحمل أي حركة دخول/خروج؛ التوست كان له دخول فقط.
- أُضيفت حركات CSS مبنية على `data-state` الذي يضبطه Radix تلقائيًا.
- تعتيم الـoverlay دخولاً/خروجًا، وتحجيم/انزلاق محتوى الحوار (`.ui-dialog-content`/`.ui-drawer-content`/`.command-dialog`).
- Radix `Presence` يُطبّق حركة الخروج تلقائيًا دون أي تغيير JS.
- تحديث مظلة `prefers-reduced-motion` لتشمل الحالات الجديدة.
- `tsc --noEmit` نظيف، 13 اختباراً خضراء.

## V1-772 تحسين تجربة اللمس للوحة Kanban — 2026-07-21

- مقبض السحب الوحيد (`.kanban-card__handle`) كان السبب الجذري لضعف اللمس — بلا `touch-action: none` ومساحة ~22px.
- أُضيف `touch-action: none` ورُفعت المساحة إلى `min-block-size: 2.25rem`.
- قائمة "نقل" المنسدلة تبقى البديل الكامل القابل للوصول.
- صفحة المونتاج (`/projects`) لا تحتوي سحبًا باللمس أصلاً (لا `dnd-kit`/`draggable`).
- **لم يُتحقق حيًا** — Docker متعطل.

## V1-792 تصنيفات وقوالب افتراضية جاهزة — 2026-07-21

- تقدم 2026-07-19 (6cb8b5f): 8 أنواع افتراضية بحقول عربية جاهزة تُستورد من صفحة الأنواع، إضافيّ فقط.
- تقدم إضافي (59e9987): 14 وسمًا افتراضيًا عربيًا تُستورد من صفحة المفردات.
- **منجز 2026-07-21**: قسم "تصنيفات ووسوم افتراضية جاهزة" لمدير النظام في صفحة أول تشغيل يربط مباشرة لزري الاستيراد الموجودين في /types و/vocabulary (بلا تكرار منطق).
- `tsc --noEmit` نظيف.

## V1-745 تحديد متعدد بالسحب (drag-select) في عرض الشبكة — 2026-07-21

- دالة نقية جديدة `computeDragSelectedIds` (تقاطع مستطيلات) في `app/archive/page.tsx`؛ `mousedown` على مساحة فارغة من `.records-surface` (وليس على بطاقة، فيبقى النقر/shift+نقر/ctrl+نقر الحالي على البطاقات كما هو) يبدأ سحبًا مستطيلًا يُحدَّث مع كل `mousemove` عبر `getBoundingClientRect` لكل عنصر يحمل `data-record-id` الجديد على `ArchiveRecordCard`.
- shift/ctrl/cmd عند بدء السحب يضيف الحصيلة للتحديد القائم بدل استبداله؛ عتبة حركة 4px قبل التفعيل لتفادي التصادم مع نقرة عادية على الخلفية.
- مقصور على عروض البطاقات (شبكة/معرض/مضغوط/قائمة)؛ عرض "تفاصيل" جدول `DataTable` وله تحديده الخاص، غير متأثر.
- 12 اختبار Vitest جديد للدالة النقية (بلا DOM)، `pnpm typecheck` و`next build` نظيفان. **لم يُتحقق حيًا في المتصفح** — Docker Desktop متعطل في هذه الجلسة (نفس قيد V1-726 أدناه)، فتعذّر تشغيل الحزمة الكاملة لمعاينة السحب فعليًا؛ التحقق الحي متبقٍ.

## V1-726 تفويض وصول مؤقت لزميل (ينتهي تلقائياً) — 2026-07-20

- لا كيان "مشروع/مجموعة" في المخطط الحالي؛ النطاق مُعرَّف كقائمة `scope.itemIds` (معرّفات سجلات) على غرار `ShareLink` الموجود، بدل استحداث جدول عضوية جديد.
- جدول/نموذج `DelegatedAccess` جديد (`grantor_id`, `grantee_id`, `scope`, `permission='editor'`, `expires_at`, `revoked_at`) وضابط `DelegatedAccessController` (`POST/GET/DELETE /api/v1/delegated-access`) — إنشاء (editor/admin فقط)، قائمة مقسّمة ممنوحة/مستلمة، وإلغاء (المانح أو admin).
- الإنفاذ الفعلي عبر دالة جديدة `Controller::requireEditorOrDelegatedAccess()`، مستخدَمة حصراً في `RecordsController::bulk()`/`bulkDelete()` — يسمح لمستخدم Viewer بالمرور فقط إذا غطّى تفويض نشط كل معرّف مطلوب في الدفعة، دون أي تغيير على دوره العام أو على بقية الـ53 ضابطاً.
- انتهاء تلقائي بفحص سلبي (`expires_at > now()` عند كل استعلام)، بلا مجدول جديد — يكفي لمتطلب "التوقف عن منح الوصول بعد الانتهاء"، قياساً بنمط `ShareLink`/`InvitationsController` الحالي.
- صفحة `/delegations` (منح تفويض جديد + قائمة ممنوحة/مستلمة + إلغاء) تستهلك `GET /users/mentionable` الحالي لاختيار الزميل.
- 7 اختبارات Laravel جديدة (`DelegatedAccessTest`): منح ناجح، رفض viewer كمانح، منع التفويض الذاتي، نطاق جزئي (سجل داخل/خارج التفويض)، انتهاء فعلي بعد `travel()`، إلغاء يدوي، وفصل قائمتي الممنوح/المستلم.

## V1-722 ربط محادثة Copilot بسياق السجل المفتوح — 2026-07-20

- زر «اسأل المساعد عن هذا السجل» في صفحة تفاصيل السجل يفتح `/copilot?recordId=` — الصفحة تجلب السجل عبر `api.record()` وتبني نص سياق مختصر (`buildRecordContext`: العنوان/النوع/الفرعي/الوسوم/وصف مقتطع) وتعرضه في لافتة قابلة للإزالة قبل أي إرسال.
- السياق يُلحق بموجّه النظام (system prompt) في `/api/copilot/chat` فقط عند إرسال فعلي من المستخدم — لا يظهر كفقاعة محادثة منفصلة، ولا يُرفق تلقائيًا بصمت؛ يمكن إزالته أو إعادة إرفاقه من اللافتة في أي وقت.
- 16 اختبار Vitest جديد/موسّع لدوال `copilot-chat.ts` النقية (`validateChatMessages` مع سياق، `buildRecordContext`)؛ `tsc` و`next build` نظيفان، و348 اختباراً ناجحاً كليًا بلا انحدار.

## V1-721 الإشارة بـ @ في الملاحظات والتعليقات — 2026-07-20

- `MentionTextarea` (مكوّن جديد قابل لإعادة الاستخدام) يضيف منتقي `@الاسم` لحقلي الملاحظة والتعليق في صفحة تفاصيل السجل، مبنيًا على نقطة نهاية خفيفة `GET /users/mentionable` مفتوحة لأي دور مصادَق (لا تتطلب صلاحية admin كما في `/users`)، وتُعيد `{id, name}` فقط دون بريد أو دور.
- عند حفظ ملاحظة أو تعليق، يفحص `MentionNotifier` النص عن مطابقة حرفية لاسم مستخدم مسبوق بـ`@` وينشئ إشعار `mention` لكل مطابقة (باستثناء الإشارة الذاتية)، معروضًا في `/notifications` بأيقونة وتسمية عربية مخصصة.
- **قيد معروف:** التطابق بالاسم الحرفي الكامل فقط (المنتقي يُدرجه تلقائيًا)؛ لا معرّفات إشارة منفصلة، فتكرار الاسم بين مستخدمين يبقى غير مميّز — مقبول لحجم المؤسسة الحالي.
- 6 اختبارات Laravel جديدة (`MentionNotificationTest`) واختبارا Vitest لمكوّن `MentionTextarea`؛ `verify:api-contracts` و`RouteScopeTest` (تصنيف المسار الجديد v1/any-role) أخضران، و756 اختبار Laravel كليًا بلا فشل.
...
- يثبت workspace runtime محليًا بإصدار مطابق، لذلك تستخدم أوامر pnpm — ومنها `pnpm dev` — Node 26 تلقائيًا حتى عندما يبقى Node النظام أقدم.
- رُفعت صورة Next إلى `node:26.5.0-alpine` مع digest رسمي متعدد المنصات، وأضيف فحص reproducibility يمنع انحراف runtime المحلي عن عقد toolchain.

## V1-789 اكتمال سجل التدقيق للتعاون — 2026-07-18

- أصبحت عمليات حجز القفل وتجديده وتحريره وتحديث المستند التعاوني تمر عبر تدقيق مصادق، بأحداث منفصلة ومعرّفات موارد ونتيجة واضحة حتى عند تعارض 409.
- تستخدم metadata قائمة سماح تحفظ الغرفة والمورد والعملية وTTL أو أرقام الإصدارات وحجم المحتوى فقط؛ لا تحفظ محتوى المستند ولا snapshot التعارض.
- بقي heartbeat وقراءات الأقفال والمستندات خارج التدقيق لمنع الضجيج، وغُطي السلوك بأربعة اختبارات و30 assertion.

## V1-724 الحضور على السجلات — 2026-07-18

- ترسل صفحة تفاصيل السجل heartbeat إلى غرفة `record:{id}` عند الفتح وكل 30 ثانية عبر API الحضور الموجود.
- تعرض المشاركين الحاليين بأسمائهم وأحرفهم المختصرة، وتبقى الصفحة قابلة للاستخدام عند تعذر تحديث الحضور.
- يتوقف المؤقت عند مغادرة الصفحة، مع اختبارات لمفتاح الغرفة وتوليد الأحرف المختصرة.

## V1-723 العلامات الزمنية أثناء التشغيل — 2026-07-18

- يلتقط مشغل الوسائط الوقت الحالي ويحفظ علامة مشتركة كملاحظة زمنية على السجل عبر API الموجود.
- تظهر العلامات مرتبة زمنيًا مع الوصف، وينقل النقر المشغل إلى وقت العلامة مباشرة.
- لا يوجد عقد أو جدول جديد؛ غُطي الفرز وتنسيق الوقت باختبارات مستقلة.

## V1-743 الفلاتر المثبتة القابلة للترتيب — 2026-07-18

- اكتمل شريط العروض المحفوظة الموجود ليعمل كفلاتر مثبتة خاصة بالمستخدم، قابلة للتطبيق والحذف وإعادة الترتيب.
- يحفظ ترتيب معرفات الفلاتر محليًا بمفتاح مقيد بمعرف المستخدم، وتُلحق الفلاتر الجديدة دون فقدان الترتيب السابق.
- أضيفت أزرار صعود/هبوط مسماة لقارئ الشاشة واختبارات نقية للترتيب وحدود الحركة.

## V1-742 الفرز السريع للوارد — 2026-07-18

- أضيف وضع فرز صريح يستخدم J/K أو الأسهم للتنقل، والأرقام 1–4 لتغيير الحالة، وEnter لفتح الإجراء الأساسي.
- تتوقف الاختصارات داخل حقول الإدخال، ويظهر العنصر النشط بوضوح مع تعليمات مفاتيح عربية وحالة `aria-current`.
- غُطي mapper الاختصارات باختبارات مستقلة ونجح TypeScript.

## V1-735 تنبيه ارتفاع معدل الأعطال — 2026-07-18

- تحسب صفحة الأخطاء موجة الأعطال من مجموع مرات ظهور أخطاء `error` خلال نافذة متحركة مدتها خمس دقائق.
- يظهر تنبيه حي قابل للوصول عند بلوغ خمس حالات، مع العدد والنافذة وتوجيه مباشر إلى خطوات الاسترداد.
- يستبعد الحساب التحذيرات والمعلومات والأحداث القديمة أو ذات الوقت غير الصالح، ويعمل محليًا دون إرسال بيانات.

## V1-766 عدسات تجميع الرسم البياني — 2026-07-18

- أضيفت عدسة «كل الأنواع» وعدسات مشتقة من أنواع العقد الفعلية مع العدد، وتصفية فورية للعقد والصلات.
- يُحفظ اختيار المستخدم محليًا ويعود بأمان إلى العدسة العامة إذا اختفى النوع المحفوظ.
- تستخدم العدسات أزرارًا قابلة للوصول بحالة `aria-pressed` وأهداف لمس 44px، مع اختبارات للترتيب والاستعادة.

## V1-783 طبقة «ما الجديد» — 2026-07-18

- أضيفت نافذة عربية موجزة تعرض أبرز تحسينات الإصدار الحالي تلقائيًا داخل مسار العمل.
- يُحفظ إقرار المستخدم بالإصدار في المتصفح، فلا تتكرر النافذة إلا عند تغيير معرّف الإصدار.
- أعيد استخدام Dialog وتوكنات التصميم الحالية مع زر لمس 44px، وإغلاق بلوحة المفاتيح، واختبارات للسلوك والإقرار.

## V1-784 توحيد مصادر حقيقة المعمارية — 2026-07-18

- وُحّدت `AGENTS.md` ووثائق عقد API وLaravel على أن Laravel + Next.js هما المسار القانوني، وأن OpenAPI هو مصدر حقيقة العقد.
- أزيلت الإشارات إلى `archive-core` كطبقة قانونية وإلى خادم Node القديم كتنفيذ مرجعي.
- أضيف فحص دائم إلى بوابة نظافة المستودع يمنع عودة هذا الانحراف في المصادر الثلاثة.

## اعتماد Node.js 25 — 2026-07-18

- رُفع baseline القانوني للمستودع إلى Node.js `25.9.0` بنطاق `>=25.9.0 <26` في الحزم وعقد المنصة وCI وrelease workflows.
- ثُبتت صورة Next الإنتاجية على `node:25.9.0-alpine` مع digest رسمي، وأضيف `.node-version` لاختيار الإصدار تلقائياً عبر مديري الإصدارات.
- أصبحت بوابة runtime تقبل السلسلة الرئيسية 25 فقط وترفض Node 24 و26، مع اختبار reproducibility صريح لهذا العقد.

## V1-705 وتجربة البحث الموحدة — 2026-07-18

- يدعم البحث القانوني أوضاع الكلمات المفتاحية والاستدلال والتفريغ الزمني، مع fallback معلن عندما لا تتوفر بيانات الاستدلال أو التفريغ.
- تعيد نتائج التفريغ مقتطفًا وطابعًا زمنيًا، وينتقل إجراء النتيجة إلى مشغل الوسائط عند اللحظة المطابقة مع ترك التشغيل بقرار المستخدم.
- أضيف إكمال تلقائي قابل للوصول بلوحة المفاتيح يجمع السجلات والوسوم والأنواع وعمليات البحث الأخيرة المحلية، مع debounce وحد أقصى ثمانية اقتراحات.
- أضيف منشئ مرئي لفلاتر البحث المتقدم، وبحوث محفوظة خاصة بالمستخدم يمكن مشاركتها للقراءة فقط مع الفريق ونسخها إلى بحث خاص مستقل.
- وُثّقت عقود البحث والاقتراحات والمشاركة والنسخ في OpenAPI، وغُطيت باختبارات Laravel وVitest وفحوص TypeScript والبناء.
...
## V1-746 — تخصيص أعمدة الجدول لكل مستخدم — 2026-07-17

- خاصية اختيارية جديدة `columnVisibilityStorageKey` في `components/ui/DataTable.tsx` تُفعّل قائمة "الأعمدة" (زر + قائمة مربعات اختيار) باستخدام حالة `columnVisibility` المدمجة في TanStack Table، محفوظة عبر `persisted-view-state.ts` الموجود. تبقى الخاصية اختيارية فلا تتأثر استخدامات الجدول الأخرى (المستخدمون، الأخطاء، الملفات) إلا عند تمريرها صراحةً؛ طُبّقت على جدول الأرشيف كأول استخدام حي. إعادة ترتيب الأعمدة خارج النطاق؛ الإظهار/الإخفاء هو جوهر الطلب.

## V1-769 — حفظ تلقائي/مسودة محلية للنماذج الطويلة — 2026-07-17

- وحدة عامة جديدة `lib/local-draft.ts` (تخزين/تحميل/مسح مسودة واحدة لكل مفتاح في `localStorage` مع طابع زمني) قابلة لإعادة الاستخدام لنماذج أخرى غير `TypesEditor`. مُطبّقة عند إنشاء نوع جديد فقط؛ التحرير له مصدر حقيقة ثابت (`initialType`) فلا يعرض مسودة قد تخلط محتوى نوع آخر. حفظ تلقائي عند أي تغيير غير فارغ، وبانر "استعادة المسودة / تجاهل" عند وجود مسودة سابقة، ومسح تلقائي عند نجاح الحفظ.

## V1-773 — عرض العناصر الأخيرة والمفضّلة في الهيدر — 2026-07-17

- وحدة جديدة `lib/recent-items.ts` (لا توجد بنية تحتية سابقة لتتبع "الأخيرة") تُطابق شكل `lib/favorites.ts` تمامًا: تخزين في `localStorage`، أحدث عنصر أولاً، إزالة التكرار عند إعادة المشاهدة، حد أقصى 10 عناصر. تُسجَّل المشاهدة عند فتح صفحة تفاصيل السجل فقط (النطاق الأساسي)؛ صفحات عرض أخرى مرشحة لمتابعة لاحقة. زر جديد `RecentFavoritesMenu` في `AppHeader` يعرض المفضّلة والأخيرة معًا بنمط الإفصاح البسيط + الإغلاق بالنقر خارج القائمة، المطابق لِـ`NotificationsPanel` الموجود.

## V1-751 — إجراءات سريعة داخل Command Palette — 2026-07-17

- مجموعة "إجراءات سريعة" جديدة في `CommandPalette` تستخدم `useFocusMode`/`useDensity` الموجودين مسبقًا (بلا أي state جديد) لتبديل وضع التركيز والكثافة مباشرة من اللوحة. إجراءات على مستوى سجل مفتوح محدد (حذف/إعادة تسمية) تبقى خارج النطاق لأن اللوحة لا تحمل سياق "السجل الحالي" بعد.

## V1-737 — تراجع سريع من التوست (Toast Undo) — 2026-07-17

- `ToastItem`/`toastSuccess`/`toastError` تقبل الآن `action?: {label, onAction}` اختيارية؛ `ToastHub` يعرضها عبر `ToastAction` من Radix (كان مستوردًا في `ui/Toast.tsx` وغير مستخدم قط). حذف `/archive` الجماعي (ينقل السجلات إلى سلة المحذوفات فعليًا) يعرض توست "تراجع" يستدعي `restoreTrash` الموجود بنفس شكل الحمولة `{store, ids}` المستخدم في الحذف. صُححت رسالة تأكيد الحذف التي كانت تصف الإجراء خطأً بأنه "نهائي" و"لا يمكن التراجع عنه".
...
# 2026-07-18 — V1-717 سجلات وصفية ومرفقات دائمة متعددة

- أصبح `POST /api/v1/records` ينشئ سجلاً وصفياً صالحاً بلا ملف، مع عداد مرفقات صفري، صلاحية editor، ومعرّف UUID قابل للبحث والعرض عبر مخزن السجلات القانوني نفسه.
- أضيف جدول `record_attachments` المفهرس كعقد دائم قابل للتوسع بدلاً من JSON داخل السجل؛ يحفظ هوية السجل والمخزن والمسار والاسم الأصلي والنوع والحجم وSHA-256 وحالة المعالجة والمرفق الأساسي والمنشئ.
- أضيفت مسارات سرد ورفع متعدد وحذف مرفقات السجل، بحد 20 ملفاً و600MB للملف، وقائمة امتدادات مسموحة وفحص محتوى آمن قبل كتابة أي ملف في الدفعة، وتنظيف تعويضي إذا فشل حفظ البيانات.
...
# 2026-07-18 — V1-788 ملكية الكتابات المتبقية

- أصبحت كتابة الحقوق المؤسسية عبر `POST /rights` مقصورة على editor/admin، مع بقاء قراءات الحقوق والتنفيذ متاحة لكل مستخدم مصادق وتحديث توقع RouteScope إلى editor.
- بقي إنشاء ملاحظات السجل متاحاً لكل مستخدم مصادق مع تثبيت المؤلف من الجلسة؛ أصبح التعديل والحذف للمؤلف نفسه أو admin فقط، ويعود 404 لغير المؤلف حتى لو كان editor لمنع كشف الملاحظة الخاصة.
- حُميت الملاحظات القديمة بلا `author_id`: لا يستطيع نسبها أو تعديلها أو حذفها إلا admin.
- تحقق TDD: أثبت RED أن viewer يكتب الحقوق وeditor يعدل ملاحظة غيره، ثم نجحت 62 حالة مركزة و253 assertion في RoleMatrix/RecordNotes/Rights/RouteScope.
