...
- **إصلاح (V1-815، أُغلق فور اكتشافه)** — `/collections`: نموذج إنشاء مجموعة (بما فيه منتقي أيقونات ~30 خيارًا) كان يُعرض دائمًا، حتى قبل أي محاولة إنشاء وفوق قائمة فاضلة. صار مخفيًا خلف زر "+ مجموعة جديدة" (commit `da985cfd`).
