...
## إغلاق حواجز الإصدار — 2026-08-03

- **V1-501 game-day:** أُغلق. نُفِّذت 6 من 7 سيناريوهات تنفيذًا حقيقيًا
  (`--execute`) على مكدس Docker معزول: تعطّل PostgreSQL وRedis والعامل وReverb
  وعطل شبكي وامتلاء قرص — كل سيناريو بحقن عطل فعلي ثم كشف واستعادة وتعافٍ
  وتحقق من سلامة البيانات. 42 أمرًا، صفر أمر غير مُثبَت، وتنظيف مُثبَت بصفر
  أثر (`docker ps -a`/`volume ls` بعد التشغيل) ومكدس التطوير الحي سليم.
  `GD-CERT-07` يبقى `external-manual-required`. كُشف أثناء التنفيذ أن
  `scripts/game-day.mjs` يحذف `container_name` لكنه لا يُعيد تخصيص المنافذ
  المنشورة، فيفشل كل تدريب مع مكدس تطوير يعمل — سجّلت الأداة الفشل بصدق
  (`not-proven`) دون إنتاج نجاح زائف، ووُثِّق تجاوز المنافذ في
  `docs/ops/v1-501-game-day.md`. الأدلة في
  [`docs/evidence/v1-501/README.md`](docs/evidence/v1-501/README.md).

- **V1-210D قبول Windows (وضع Docker، online):** أُغلق. **حدود النطاق:** التحقق
  على منصة `windows-10-11-docker` (`conditional` في العقد) لا على
  `windows-native` التي تبقى `planned` ومسارها غير مُنفَّذ؛ يتبع سابقة إغلاق
  V1-211D عبر Docker (2026-07-22) وبمعيار أعلى منها. دورة كاملة
  `install --config=` (وصف إصدار موقَّع، صور Docker حقيقية موسومة
  `@sha256:` عبر سجل محلي فعلي `registry:2` — تقنية مطابقة لآلية
  `offline-bundle` المعتمدة، لا اختلاق) → `update` → `rollback` نجحت فعليًا
  على Windows 11 حقيقي، ضمن مكدس معزول تمامًا عن بيئة التطوير الحية
  (`git worktree`، صفر تصادم مؤكَّد). اكتُشفت وأُصلحت 5 عيوب حقيقية كانت
  تمنع مسار الإصدار الموقَّع من العمل إطلاقًا (لم يكن مُختبَرًا من قبل في
  تاريخ هذا المستودع): قبول `ARCHIVE_COMPOSE_PROFILES` فارغة لتثبيت
  core-only، مسار حجم Postgres غير المطابق لاصطلاح PG18+، متغيرات بيئة
  إدارية ناقصة عن `docker-compose.release.yml`، وأخطرها — إسقاط `options`
  في `buildUpdateAdapter.exec` الذي كان يُجهض كل تحديث موقَّع (commits
  `0c58b32d`، `c289e000`). آلية التراجع التلقائي عند فشل الصحة تحقّقت 3
  مرات مستقلة أثناء التصحيح. التفاصيل الكاملة في
  [`docs/evidence/v1-210d/README.md`](docs/evidence/v1-210d/README.md).
...
- **V1-303D الوصولية:** أُغلق. أضيف `e2e/screen-reader-sample.authed.spec.ts`
  ...
  الجلسة البشرية فعليًا. أُضيف إلى البوابة الحية في
  `scripts/verify-next-laravel-live.mjs`. التحقق: 7/7 اختبارات ناجحة على المكدس
  الحي (Docker: Laravel + Next على 3000)، و239 محطة تركيز مُلتقطة عبر ستة
  مسارات بصفر عنصر بلا اسم. الأدلة والانحراف الموثّق عن اشتراط عيّنة NVDA
  البشرية في [`docs/evidence/v1-303d/README.md`](docs/evidence/v1-303d/README.md).

## V1-786 وV1-790 — 2026-07-31

- **V1-877 توجيه الوارد حسب القسم:** أضيف توجيه صريح لعناصر صندوق الوارد إلى
  القسم المستهدف مع معاينة قبل التنفيذ، وسجل سلسلة التوجيه، وحاجز يمنع إعادة
  التوجيه إلى قسم شوهد سابقاً أو الحلقة أو تجاوز ثماني نقلات. رُبط إجراء
  الأتمتة `create-inbox-item` بصندوق الوارد القانوني بدلاً من مخزن منفصل، مع
  قسم مستهدف اختياري؛ تبقى معاينة الأتمتة بلا كتابة. التحقق: 22 اختبار Laravel
  و149 assertion، وتوليد/تحقق العقد وفحص أنواع Next.js ناجحة.

- **V1-878 صلاحيات المجموعات المحفوظة:** استُبدلت المشاركة العامة لقائمة
  البحوث المحفوظة بقائمة أعضاء صريحة، لا تظهر المادة للمالك أو للمستخدمين
  المضافين بدور `editor` أو `viewer` فقط. يتيح المحرر إدارة الأعضاء والقسم،
  بينما يبقى الحذف للمالك؛ وكل استبدال للعضوية يدخل سجل التدقيق المركزي. التحقق:
  15 اختبار Laravel/75 assertion، وعقد OpenAPI وفحص أنواع Next.js ناجحة.

- **V1-879 مالك الحقل حسب القسم:** أضيف سجل مالكية مركزي للحقول حسب القسم،
  مع مالك افتراضي `*` ومحرر إداري ضمن مكتبة القوالب. تستعمل طلبات المعلومات
  مالك الحقل كاقتراح/إسناد تلقائي، لكنها تحتفظ بالإسناد الصريح الذي يحدده
  المستخدم المخوّل. التحقق: 13 اختبار Laravel/58 assertion، وعقد OpenAPI
  وفحص أنواع Next.js ناجحة.

- **V1-880 مؤشرات استخدام القوالب:** لوحة مجمعة داخل مكتبة القوالب تعرض عدد
  القوالب والمنشور منها وقواعد الجودة والمواد والحقول الناقصة لكل قسم، دون
  تسريب محتوى المادة أو استخدام خدمة تحليلات خارجية. التحقق: 9 اختبارات
  Laravel/29 assertion، وعقد OpenAPI وفحص أنواع Next.js ناجحة.

- **V1-881 تسليم المادة بين الأقسام:** مسار تسليم محدود يسجل جهتي التسليم
  والاستلام وملخصًا آمنًا للحقول الناقصة والحقوق والتعليقات، بلا تغيير تلقائي
  لمالك السجل أو صلاحياته. التحقق: اختبار Laravel لمسار التسليم ونطاق المسار
  والعقد والعميل ناجحة.

- **V1-882 مفردات مقيدة بالقسم:** تفضيلات صريحة للوسوم والقيم المعتمدة حسب
  القسم، مع إرجاعها للواجهة وترتيبها أولاً في نتائج القاموس، وسجل تدقيق لاستبدال
  التفضيلات. تبقى المفردات العامة والمرادفات مرئية وقابلة للبحث ولا ينشأ قاموس
  مستقل. التحقق: 17 اختبار Laravel/72 assertion، وتوليد/فحص عقد OpenAPI،
  وفحص الأنواع وبناء Next.js ناجحة.

- **V1-876 قواعد الجودة:** قواعد قابلة للضبط لكل قسم ونوع، ومعاينة تعيد الحقول
  الناقصة وسبب عدم الجاهزية بلا كتابة أو فرض على قسم آخر. أضيفت إلى شاشة
  القوالب المركزية وعقد OpenAPI وسجل التدقيق. التحقق: 9 اختبارات Laravel/28
  assertion، وعقد API وفحص أنواع Next.js ناجحة.

- **V1-875 نشر القوالب:** فصلت المسودة عن اللقطة المنشورة؛ لا يستعمل العميل
  إلا الإصدار المنشور، ويمتلك `admin` اعتماد المسودة أو استعادة إصدار سابق.
  سجل التدقيق ونطاقات المسارات وعقد API والواجهة المركزية متصلة. التحقق: 11
  اختبار Laravel/59 assertion، وتوليد العقد وفحص Next.js ناجحة.

- **V1-874 قوالب البيانات حسب القسم:** مكتبة مركزية في Laravel وNext.js تفرض
  القسم المالك، وتدعم الإنشاء والتعديل والتعطيل وأدوار الاستخدام، وتحفظ لقطة
  لكل إصدار. صفحة `/metadata-templates` تعرض القيم قبل التطبيق وسجل الإصدارات
  من دون تغيير المواد السابقة. التحقق: 13 اختبار Laravel/67 assertion، عقد
  OpenAPI والعميل المولد وفحص الأنواع ووحدة المعاينة وبناء Next.js ناجحة.

- **V1-871 قاموس المصطلحات المعتمد:** يمكن ربط البديل بقيمة رئيسية يملكها
  المستخدم نفسه، وتظهر العلاقة في القاموس مع بقاء المرادفات قابلة للبحث.
  حالات دمج المصطلحات المستخدمة تمر عبر معاينة وإعادة ربط V1-850 المدققتين.
  التحقق: اختبار Laravel مركّز (2/9)، عقد OpenAPI والعميل المولّد وفحص أنواع
  Next.js ناجحة.

- **V1-870 معاينة أثر التغيير:** معاينة أثر محكومة تعرض العلاقات والمشاركات
  والمقاطع والتقارير قبل الحذف، وتحجب الحذف بوجود علاقات غير محلولة. الواجهة
  وعقد OpenAPI وسجل التدقيق ونطاق الصلاحيات متصلة؛ التحقق المركّز ناجح.

- **V1-865 استبدال مصدر المادة:** استبدال آمن يحفظ هوية السجل وعلاقاته، يسجل
  نسخة من المصدر السابق، ويعرضها للاستعادة الصريحة. يُعيد الملفات الإعلامية
  إلى طابور المشتقات القائم. التحقق: اختبار Laravel للاستبدال والاستعادة،
  نطاق المسارات، وعقد OpenAPI وعميل Next.js وفحص الأنواع ناجحة.

- **V1-855 قواعد فرز المجلد المراقَب:** تضيف قواعد المسار أو نمط الاسم قالب
  بيانات ووسوماً ووجهة تجهيز إلى معاينة الدفعة المحكومة. لا تنتقل البيانات إلى
  السجل إلا بعد اعتماد الدفعة؛ عندها يطبّق المسار القالب والوسوم ويخزن الملف في
  الوجهة المختارة. التحقق: 20 اختباراً/125 assertion في `IngestApiTest`،
  وعقد OpenAPI وعميل Next.js المولّد وفحص الأنواع ناجحة.

- **V1-854 وV1-856 المجلد المراقَب والحجر:** أضيف مسار Laravel + Next.js
  للمجلد المحلي المراقَب: معاينة دفعة قبل أي كتابة، اعتماد صريح، checksum
  متدفق متوافق مع أقراص التخزين، وعرض حالة وسبب كل ملف في واجهة الاستيراد.
  جُهّز Docker بمسار مضيف ضيق `WATCHED_INGEST_HOST_PATH` فقط. تُعزل الملفات
  المخالفة أو المتعارضة فعلياً، ولا تدخل السجل. ترتبط المعاينة والتطبيق بسجل
  التدقيق المركزي وبنطاق الصلاحية editor/admin. التحقق: 17 اختباراً/101
  assertion للاستيراد، 55/192 للصلاحيات، وتحققات OpenAPI/الأنواع وCompose
  ناجحة.

- **V1-786 فصل تنفيذ الوسائط:** يوجد عقد `MediaJobExecutor` بحدّ تنفيذ ضيق
  وموائم `LocalMediaJobExecutor` المتوافق، بينما يظل
  `ProcessMediaWorkflow` مالك الحالة وإعادة المحاولة والتنقية. تُحفظ
  `executor` و`contractVersion` في `media_jobs`، وتظهران عبر عقد OpenAPI
  وعميل Next.js المولّد. التحقق المركّز: 28 اختبارًا في
  `MediaJobsApiTest` و`MediaJobsReliabilityTest` ناجحة.
- **V1-790 طبقة الوصول الموحدة:** تستخدم مسارات السجل والبحث وإنهاء الرفع
  مستودع `StorageRowRepository`، وانضم إليها `TypesController` ومسار الحذف
  الجماعي للسجلات. يضمن `findManyByUidOrRecordId()` أن البحث عن معرّف سجل
  يظل مقيدًا بالمخزن المطلوب ويحتفظ بحالات المعرّفات القديمة. التحقق: 45
  اختبارًا و282 assertion في `StorageRowRepositoryTest` و`TypesControllerTest`
  و`RecordsApiTest` و`TrashApiTest` ناجحة؛ لا يبقى وصول مباشر إلى
  `storage_rows` في المالكين الأربعة المستهدفين.
- **V1-847 استيراد البيانات الوصفية من CSV:** تستقبل نقطة الاستيراد رؤوس
  الحقول المعتمدة وحدها، وتستخدم `dryRun` كمعاينة بلا كتابة مع نتيجة أو خطأ
  لكل صف، ولا تنشئ معرّفات أو حقولًا مخصصة. صار سجل التدقيق يلتقط نتيجة الدفعة
...
- **AS-02 التصنيف الهرمي والحقول المخصصة:** شجرة الوسوم والأنواع والحقول
  المخصصة تدعم الشروط والإلزام وصلاحيات الحقول وقوالب الأقسام المنشورة. صحح
  اختبار إدارة المسودات ليطلب وضع المحرر بدلاً من إظهار مسودة للعامة. التحقق:
  30 اختبار Laravel/156 assertion.

- **AS-03 قاموس المصطلحات المعتمد:** أضيفت فئات قاموس قابلة للضبط لكل مستخدم
  فوق الفئات الأساسية (الشخص، المكان، الحدث)، مع اسم ووصف وأيقونة وترتيب. تُقبل
  الفئات المضبوطة في الإدخال والاستيراد وتظهر في واجهة المفردات؛ تبقى المرادفات
  والقيمة الرئيسية وإعادة الربط المدققة محفوظة. التحقق: 17 اختبار Laravel/114
  assertion، و`api:generate` و`verify:api-contracts` و`typecheck` واختبارات العميل.

- **AS-04 الصلاحيات والتدقيق:** ثُبتت مصفوفة الصلاحيات عبر عمليات الأرشيف، مع
  جلسات وإعدادات أمنية محمية، تنقيح الحقول الحساسة في سجل التدقيق، وسلسلة hashes
  تكشف العبث أو الحذف. التحقق: 71 اختبار Laravel/252 assertion.

- **AS-05 إدارة دورة حياة المادة:** نُقلت المفضلة من localStorage إلى API
  موثق ومحفوظ للحساب مع عزل المستخدمين؛ تعرض صفحة المفضلة وسجل المادة الحالة
  نفسها عبر الجلسات. تكتمل الدورة بالحذف والاسترجاع واللقطات وسجل التغيير
  والتجميد الموجودين. التحقق: 28 اختبار Laravel/232 assertion، وعقد OpenAPI
  وأنواع Next.js.

- **AS-06 رفع واستيراد الوسائط:** أضيف استيراد SRT وWebVTT إلى المادة، مع
  طوابع زمنية ونص قابلين للتحرير وحفظ نمط العرض. أضيف محرر في صفحة التفريغ
  للفتح والتعديل والنسخ والتنزيل والحفظ؛ يعمل المحلل بوضع Unicode-safe فلا
  يفسد العربية. التحقق: 40 اختبار Laravel/248 assertion، وعقد OpenAPI وأنواع
  Next.js واختبار العميل.

- **AS-07 البحث والفلاتر المتقدمة:** أضيفت فلاتر تاريخ المادة واكتمال التوصيف
  إلى API وواجهة البحث، إلى جانب البحث النصي والدلالي وداخل التفريغ والفلاتر
  القائمة والبحوث المحفوظة. التحقق: 24 اختبار Laravel/165 assertion، وعقد
  OpenAPI وأنواع Next.js.

- **AS-08 صفحة تفصيل المادة والتعاون:** رُبطت صفحة المادة بالتوصيف والملاحظات
  الخاصة وتعليقات الفريق والحضور. أُضيفت واجهة طلب استكمال البيانات: حقل
  ورسالة ومكلّف وموعد متابعة، مع عرض الطلبات وحلّها؛ لا تُخفى هذه البيانات في
  مخزن محلي. التحقق: 23 اختبار Laravel/181 assertion، واختبار Vitest للوحة
  الإسناد و`typecheck`.

- **AS-09 خدمات الذكاء الاصطناعي المساعدة:** أضيفت خدمة مسودة مساعدة محلية
  وغير معدِّلة للسجل: ملخص ووسوم وكيانات مطابقة للقاموس وملاحظات تدقيق. تعيد
  دائماً `reviewRequired=true` و`changesApplied=[]`، وتعرضها صفحة المادة كمسودة
  للمحرر، بالتوازي مع تفريغ مهام الوسائط القائم. التحقق: 37 اختبار Laravel/182
  assertion، وعقد OpenAPI و`typecheck` واختبارات العميل.

- **AS-10 المجموعات والمشاريع:** امتدت مشاريع العمل إلى ملاحظات وترتيب دائم
  للمواد، مع تحديثٍ منضبط وإعادة ترتيب تتحقق من احتواء كل الروابط مرة واحدة.
  أضيفت صفحة `/project-groups` لإنشاء حقائب العمل وربط المواد وترتيبها؛ فصلت
...
- **AS-13 مشاركة المشاريع:** المشاركة والمراجعة تفرضان الصلاحية والمدة وكلمة
  المرور الاختيارية، وأُضيف إلغاء فوري موثق بعقد API؛ الرابط الملغى يعود 404.
  التحقق: 12 اختبار Laravel/100 assertion وعقد OpenAPI و`typecheck`.

- **AS-14–AS-18:** تدقيق مستقل لشبكة العلاقات، الاستكشاف المكاني/الزمني،
  التقارير والتصدير، النسخ والاستعادة، وتحصين السحابة. التحقق: 74 اختبار
  Laravel/346 assertion ناجحة؛ تخطي MySQL/PostgreSQL الحيين لأنهما غير مهيأين
  في بيئة الاختبار، ولا يمثلان فشلاً في التطبيق.
...
- **المشكلة:** كانت `CORE_ROUTES` في `archive-next/e2e/fixtures/visual-routes.ts`
  تضم `/` و`/help` و`/reports` و`/settings` و`/archive` و`/media/jobs`، وليس
  أيٌّ منها عامًّا: `proxy.ts` يحوّل كل مسار خارج `publicPathPrefixes` إلى
  `/login` عند غياب كعكة الجلسة. لذا كانت بوابتا axe والانحدار البصري تقيسان
  صفحة تسجيل الدخول ست مرات في ثلاث مقاسات، وتُبلغان نجاحًا لكل مسار — تغطية
  وهمية لا فشل ظاهر. المسارات الستة مغطّاة أصلًا بجلسة حقيقية عبر
  `ROUTE_COVERAGE` في `fixtures/route-inventory.ts`.
- **الإصلاح:** قُصرت `CORE_ROUTES` على `/login` و`/share/demo-token` (تحقُّق
  مباشر: هما وحدهما يجتازان `isPublicPath`). صُدِّرت `isPublicPath` من
  `proxy.ts`، وأُضيف فحص وقت-الاستيراد يرفض أي مسار غير عام في القائمة.
- **الاتجاه المعاكس:** أُضيفت `gotoPublicRoute` ورُبطت بـ `accessibility.spec.ts`
  و`visual-regression.spec.ts` بدل `page.goto` المباشر، فتفشل البوابة صراحةً لو
  توقّف مسار عام عن كونه كذلك (مصادقة أو راية ميزة أو إعادة كتابة).
- التحقق: `pnpm typecheck` نظيف، و517 اختبارًا في 97 ملفًا ناجحة.

## V1-793 دليل المستخدم داخل التطبيق — 2026-07-27

- **V1-793 إغلاق** — فصول Markdown حسب الدور مع بحث محلي وروابط «كيف تعمل هذه
  الصفحة؟» من كل مسار. محتوى الفصول يُحمَّل عبر BFF محمية تعيد محتوى دور
  المستخدم فقط، فلا يصل نص فصل غير مصرّح به إلى العميل. أُضيفت تغطية اختبار
  لمتصفح الدليل (`f8d906c9`) وتحسينات وصولية للدليل والملفات (`15342caf`).
  التحقق: `pnpm verify` كامل أخضر، و65/65 في
  `node --test scripts/acceptance/*.test.mjs tests/performance-contract.test.mjs`.
  الالتزامات: `2b13cc1b`, `599386ff`, `317eb088`, `f8d906c9`, `2fb2160e`,
  `b2e97e0e`, `15342caf`.

## مساحة إدارة الملفات متعددة المزودين — 2026-07-27

- أضيف كتالوج موحد لوحدات التخزين المحلية وS3 وDropbox مع قدرات صريحة وحالة
  خالية من الأسرار، ومتصفح مجلدات عربي متعدد المزودين.
- أضيفت عمليات ملفات دائمة بمعاينات موقعة ومفاتيح idempotency وحالات إلغاء
  واستئناف وفحص checksum، مع محولات نقل متدفق وحماية للمسارات وسجل تدقيق منقح.
- وُحد عقد OpenAPI وLaravel وعميل Next.js وصلاحيات مسارات الإدارة؛ التحقق
  الكامل `pnpm verify` ناجح. الالتزامات الأساسية: `dcd9b666`, `f800f218`,
  `d36f2522`, `619ec2f5`.
...
- **إصلاح (V1-816، أُغلق فور اكتشافه)** — أُعطي هذا البند الرقم `V1-815` عند إنشائه في 2026-07-26، ثم استُخدم الرقم نفسه لاحقًا في اليوم ذاته لبند مختلف («تغطية رموز أخطاء API»، ما زال مفتوحًا في TASKS.md)؛ رُقِّم هنا `V1-816` لفض التعارض. `/collections`: نموذج إنشاء مجموعة (بما فيه منتقي أيقونات ~30 خيارًا) كان يُعرض دائمًا، حتى قبل أي محاولة إنشاء وفوق قائمة فاضلة. صار مخفيًا خلف زر "+ مجموعة جديدة" (commit `da985cfd`).

## إصلاح AppShell مكرّر في /notifications — 2026-07-26

- **إصلاح** — `app/notifications/layout.tsx` كان يغلّف الأبناء بـ`<AppShell>` بينما `app/notifications/page.tsx` يغلّف عائده الخاص بـ`<AppShell>` أيضًا، فيركّب Next.js شِلّين متداخلين (رأس وقائمة تنقّل وskip-link مكررة) في كل تحميل لـ`/notifications` — مؤكَّد عبر DOM (`document.querySelectorAll('.app-shell').length` من 2 إلى 1) على عملية Next dev جديدة كليًا بعد تنقّل نظيف. أُزيل `app/notifications/layout.tsx` بالكامل؛ لا صفحة أخرى تستخدم هذا النمط. اكتُشف أثناء تدقيق V1-795/V1-791 (commit `9ba7eff6`).

## V1-801..805 منسق قبول Docker وSmoke — 2026-07-26

- **V1-801 عقد السيناريو والمنسق الأساسي** — أُنجز registry versioned والاختيار بالوسم/المعرّف، ميزانية login/refresh، المهل والإلغاء، الحالات الحاسمة، والتحقق المسبق من الاختيار قبل إنشاء provider أو ملف أسرار مؤقت. الأمر القانوني هو `node scripts/acceptance.mjs run --tag smoke`.
- **V1-802 مخزن الأدلة والفحص ضد التسرب** — تُكتب الأدلة خارج شجرة المصدر بصلاحيات المالك، وتُنقّى وتُفحص قبل finalization؛ تُرفض الملفات غير القابلة للفحص، وتُعطّل trace/video التلقائية في القبول. صار `generatedAt` يُلتقط عند الإنهاء الفعلي لا عند إنشاء المخزن.
- **V1-803 Docker provider آمن وقابل للتنظيف** — دورة `prepare/install/start/exec/collect/reset/destroy` مع مشروع Compose ومنافذ وأسرار مؤقتة معزولة، وفحص صريح لعدم بقاء container/network/volume، وجمع 8 بصمات صور حقيقية. عولجت قابلية البناء بإقصاء `.next` و`storage/app` المحليين، وتوافق ترحيل PostgreSQL، وتشغيل nginx غير المميز، وأصل CORS الديناميكي. readiness لا ينجح مع health متدهور، ويتحقق من worker/scheduler/Reverb ومن heartbeat حديث.
- **V1-804 خمسة سيناريوهات Smoke تفاعلية** — نُفذت ونجحت `V1-IA-PLAT-001` و`V1-IA-ARCH-001` و`V1-IA-ADMIN-001` و`V1-IA-ADMIN-002` و`V1-IA-MULTI-001` فوق Laravel+Next+PostgreSQL+Redis الحية، مع جلسات أدوار مستقلة وbackup+verify فعلي.
- **V1-805 دورة التشخيص والإصلاح وإعادة الفاشل** — التصنيف وإعادة flake الواحدة والاستهداف الدقيق و`--last-failed` مثبتة. تشغيل حي لـ`--last-failed` اختار `V1-IA-ADMIN-001` وحده ونجح في `run-2026-07-26t13-39-32-022z`. تشغيل smoke المشدد النهائي في `run-2026-07-26t13-41-21-025z` نجح 5/5 بمحاولة واحدة لكل سيناريو خلال 103096ms، مع `cleanup.proved=true` و8 image digests وhealth يعلن `ok=true`, `degraded=false`, `schedulerFresh=true`; `generatedAt` بعد `finishedAt`.
- **التحقق والمراجعة** — 72/72 اختبار قبول/إعادة إنتاج و`pnpm typecheck` أخضران قبل التشغيل الحي. المراجعة الشاملة الواحدة فحصت دورة الأسرار والتنظيف ودلالة الصحة والتوقيت وإعادة الفاشل، ثم أعادت المراجعة بعد `f4547877` وحكمت: **Ready to archive: Yes؛ لا ملاحظات متبقية**. أبرز commits: `90a93476`, `cc1b79b8`, `993dfb63`, `dbef1e1a`, `352d2c0d`, `ca48a84b`, `14505c70`, `22d2b739`, `bce78ea7`, `8c7f4297`, `4ac197c3`, `b57c18be`, `80440f8f`, `c2719002`, `327c4fb4`, `f7f048c7`, `4d73fcbb`, `2cb7a90d`, `f125ccff`, `41fb79e1`, `f4547877`.

## V1-307A مولد بيانات benchmark — 2026-07-23

- **V1-307A مولد بيانات benchmark** — 100 ألف سجل عربي و10 آلاف ملف وعينة رفع 1GB ببذرة ثابتة، دون إدراج بيانات حقيقية أو حساسة. — تقدم 2026-07-22: أمر `archive:generate-benchmark-dataset` (1cdca6e) ينشئ سجلات عربية Faker مزودة ببذرة ثابتة تحت `store=benchmark-synthetic` وملفات checksum محددة الحجم الإجمالي، وكلاهما deterministic لنفس البذرة؛ اختبار `GenerateBenchmarkDatasetCommandTest` يغطي التوليد والحتمية وملخص `--json`. — إغلاق 2026-07-22: `pnpm verify:laravel` كامل أخضر حيًّا عبر Docker (812 اختبار ناجح، 2 متجاوَز، صفر فشل)، ثم تشغيل فعلي بحجم الإنتاج عبر `node scripts/laravel-docker.mjs artisan archive:generate-benchmark-dataset --json` أنتج 100000 سجل و10000 ملف بإجمالي 1073741824 بايت (1GiB بالضبط) تحت `store=benchmark-synthetic`، seed=42، مؤكَّد بالعدّ المباشر من `storage_rows`/`record_attachments`؛ الملفات تحت `storage/app/private/benchmark-synthetic/` (متجاهَلة في git). جاهز كمدخل مباشر لـV1-307B..D وV1-812.

## V1-211D قبول Linux نظيف — 2026-07-23
...
## V1-747 مسجّل الإجراءات الجماعية — 2026-07-22

- أضيفت ماكروهات مملوكة للمستخدم ومخزنة في Laravel، بخطوات مرتبة من 1 إلى 10 لإضافة وسم أو تغيير حالة سير العمل أو الحذف القابل للاستعادة، وتستهدف تحديدًا صريحًا فريدًا حتى 1000 سجل.
- أصبحت المعاينة غير المعدِّلة إلزامية وتصدر رمزًا موقعًا لمدة 15 دقيقة مرتبطًا بالمستخدم والماكرو وإصداره وترتيب الأهداف نفسه؛ التنفيذ يرفض الرمز المنتهي أو القديم أو المختلف، ويوثق النتائج الجزئية والفاشلة والمتخطاة لكل هدف وخطوة في سجل تشغيل دائم.
- توحّدت محاكاة المعاينة والتنفيذ بعد الحذف، وأصبحت أخطاء الخطوات معزولة بلا تسرب لتعديلات فاشلة، مع فحص عدد الصفوف المتأثرة وإطلاق `RecordChanged` مرة واحدة بعد الحالة النهائية للسجل، والحذف عبر سلة قابلة للاستعادة.
- وُثقت نقاط CRUD والمعاينة والتنفيذ والسجل في OpenAPI بأنواع خطوات مميّزة صارمة وحالات فشل متطابقة، وأعيد توليد عميل TypeScript وربط عميل Next اليدوي.
- أضيف مسجّل عربي قابل للوصول في `/archive`: إنشاء الخطوات وترتيبها وحفظها، منع المعاينة عند وجود مسودة غير محفوظة، إبطال التأكيد عند تغير الأهداف أو انتهاء الصلاحية، حراسة الطلبات المكررة، وتفاصيل قبل/بعد والأسباب وقابلية التراجع في المعاينة والتنفيذ والسجل.
- التحقق النهائي: `pnpm verify:api-contracts` و`pnpm verify:api-generated` و`pnpm typecheck` ناجحة؛ 89 ملف Vitest و482 اختبارًا ناجحًا؛ build ناجح لـ57 صفحة؛ اختبارات Laravel المركزة 20 اختبارًا و188 assertion؛ وسيناريو Playwright مصادق حي Laravel+Next ناجح. المراجعة الشاملة الوحيدة أعادت الفحص بعد الإصلاحات وحكمت `Ready to merge: Yes`.
- الالتزامات الأساسية والإصلاحية: `db620b59`, `615e7eb0`, `51db5a8c`, `9427a4c1`, `1d3336ab`, `cee12c1c`, `5cd5908b`, `a33f40ba`, `aa4df570`, `10ab1214`, `c594b03f`, `3e882a78`, `c1b720fc`, `c3445a6c`, `10973cd0`, `ff971eee`, `c4fb4a59`, `f71150ef`, `f3720ef1`, `3c9bb632`.

## V1-736 بيئة معاينة اصطناعية للعمليات الحساسة — 2026-07-22

- أُضيفت نقطتا نهاية محميتان للمحرر/المدير لعرض سيناريوهات ثابتة وتشغيل حذف جماعي أو استعادة داخل حالة PHP اصطناعية محضة؛ لا تقرآن أو تكتبان `storage_rows` أو `trashed_records` أو الوسائط أو الطوابير أو سجل التدقيق أو أي خدمة خارجية.
- تحمل كل الاستجابات، بما فيها 401/403/422، العلامة `synthetic: true` عبر middleware محدود بهذه المسارات، مع بقاء المسارات خارج `archive.audit` وتغطية عدم تغيّر قواعد البيانات وسجل التدقيق باختبارات HTTP فعلية.
- وُثّق العقد في OpenAPI وطُبّقت bindings المولدة والعميل اليدوي بأنواع نجاح/خطأ متطابقة، مع envelope خاص بالمعاينة يحفظ علامة الخطأ الاصطناعية دون توسيع أخطاء بقية API.
- أُضيفت صفحة Next عربية `/safety-preview` تعرض أوصاف السيناريو من الخادم، العدادات قبل/بعد، التعارض/عدم الوجود بصياغة عربية، انتهاء المعاينة، ورسالة صريحة بأن لا بيانات إنتاجية تأثرت؛ لا تستدعي الصفحة مسارات الحذف أو الاستعادة الفعلية.
- التحقق النهائي: عقود API والتوليد وtypecheck خضراء؛ 87 ملف Vitest و471 اختبارًا؛ build ناجح لـ57 صفحة؛ 17 اختبار Laravel و89 assertion لبيئة المعاينة ونطاق المسارات؛ وPlaywright ناجح على Chromium وmobile-chrome (2/2). المراجعة الشاملة النهائية: Spec PASS وCode Quality PASS ولا ملاحظات Critical/Important/Minor (`1cc31e2`, `ff1a1df`, `4b109b4`, `ceaddb5`, `2373d09`, `974021a`).

## موجة تسليم مُدارة — تصحيح السجل وشرائح V1-794/V1-306C/V1-791 — 2026-07-22

- صُحح عدّ البنود المفتوحة في `TASKS.md` من 63 إلى 47 بعد عد ميكانيكي دون تغيير أي checkbox (`4f36a3c`).
...
- التحقق: `pnpm verify:api-contracts` و`pnpm verify:api-generated` و`pnpm typecheck` خضراء؛ نجح `pnpm test:next` في 467 اختبارًا، ونجح `pnpm build:next` في بناء 56 صفحة، ونجح `node scripts/laravel-docker.mjs test tests/Feature/TypesControllerTest.php` في 19 اختبارًا و73 assertion.
