...
# جولة فحص حيّة لكل الصفحات — ملاءمة الاستخدام اليومي (2026-07-10)

فُتحت **39 صفحة مصادَقة فعلياً** (iframe harness + فحص بصري لصفحات الإدخال والأرشيف على 1280 و930 و375). كل الصفحات تُصيَّر بنجاح داخل shell موحّد وبلا page-level overflow. زمن تحميل الصفحة الكامل 3–13 ثانية (وسيط ~5s، وضع dev) — كل تنقّل إعادة تحميل كاملة.

## مؤكد حيّاً — يكسر الاستخدام اليومي

1. **لا يوجد زر تسجيل خروج إطلاقاً على desktop.** على 1280px: صفر عناصر logout في DOM، «Integration User» نص غير قابل للنقر، زر التنبيهات المرئي بلا onClick بينما الزر الوظيفي (`فتح الإشعارات`) عرضه صفر. (يطابق P0#1 أعلاه — مؤكد الآن قياساً).
2. **ترويسة الهاتف مكسورة بنيوياً:** على 375px عمود `.brand` ينضغط إلى 45px فيتكسر «مركز السجلات» حرفاً-حرفاً عمودياً (`brand-subtitle` بعرض 0 وارتفاع 172px) والترويسة 245px. المستخدم يمرّر ~2.5 شاشة (ترويسة + شريط أوامر + لافتة أول تشغيل + hero) قبل أول محتوى فعلي.
3. **سطح العمل الفعلي تحت الطية في كل الصفحات التشغيلية.** النمط المتكرر على 1280×800: شريط أوامر (~150px) + لافتة «هل هذا أول تشغيل؟» (تظهر في كل صفحة وكل زيارة) + hero panel ضخم يكرر عنوان الصفحة وأزرار تنقّل مكررة من sidebar → منطقة الإفلات في `/uploads` وأول سجل في `/archive` غير مرئيين دون تمرير. **هذه أكبر مشكلة عملية للاستخدام اليومي.**
4. **تلميح الاختصار المعروض للمستخدم `Ctrl + Cmd + K`** في حقل البحث — الخلل البرمجي (P0#2) ظاهر حرفياً في الواجهة.
5. **على ~930px الترويسة تقصّ الأزرار منتصف الحرف** («الإعدادات» تظهر «دادات») بدل الطي.

## مؤكد حيّاً — P1

6. أزرار hero مكررة للتنقل (فتح الأرشيف / مهام الوسائط / Add Workspace) + صندوقا بحث على بعض الأعراض (topbar + شريط الأوامر).
7. تسريبات إنجليزية على أسطح يومية: `Add Workspace`، `Archive Workspace`، `Metadata`، placeholder «video»، أسماء الثيمات في الإعدادات (`Theme Presets`, `Cinematic Dark`, `Luxury Dark`)، و`/activity` يعرض اسم المسار الخام `post.api.v1.inbox` كتسمية نشاط.
8. `/media/jobs`: mojibake مؤكد في الواجهة («Ø§ÙØ®Ø§Ø¯Ù» بدل «الخادم») + حقل «Access token» معروض لمستخدم نهائي.
9. `/types`: تُصيَّر بلا أي inputs (أفقر صفحة في الجرد — يطابق P0#3).
10. أدوات ثانوية («قوالب الإدخال»، «روابط رفع خارجية») تجاور المعالج الرئيسي في `/uploads` بنفس الوزن البصري وتنافسه.

## ما ثبت أنه جيد للاستخدام اليومي

- Sidebar مجمّع فعلاً (الإدخال/المكتبة/…) — شكوى «38 رابطاً مسطحاً» القديمة معالجة على desktop.
- `/archive` أفضل شاشة عمل: فلاتر منظمة، 5 أوضاع عرض، 3 كثافات، عداد نتائج وفلاتر نشطة.
- نماذج الإدخال نفسها سليمة بنيوياً (wizard بخطوات واضحة في `/uploads`، حقول موسومة في `/ingest` و`/transcriber`) — المشكلة في الموضع والأولوية لا في النماذج.
- Empty states حاضرة في أغلب الصفحات، ولا overflow أفقي على 375px.

## أثر الجولة على ترتيب التنفيذ

يبقى الترتيب أدناه صالحاً مع إضافتين قبل البند 3: **(أ)** إصلاح CSS ترويسة الهاتف (دقائق — `min-width`/إخفاء subtitle)، **(ب)** تحويل لافتة أول التشغيل إلى قابلة للإغلاق بشكل دائم وتقليص hero panels إلى صف PageToolbar مضغوط حتى يصبح سطح العمل فوق الطية.
