...
- [ ] **1.5.1-REL-001 — تجهيز ونشر إصدار 1.5.1.** تحديث رقم الإصدار في
  المسارات المعتمدة، وإعداد ملاحظات الإصدار العربية والإنجليزية، وتجهيز أصول
  Docker وNative والحزمة دون اتصال، مع توثيق تنزيل وتجميع الحزمة المقسّمة
  وفك ضغطها والتحقق من بصماتها على Windows وLinux وmacOS. **الاعتماديات:**
  قرار نطاق 1.5.1 وإغلاق مهام الإصدار الحالية.

  ```text
  المالك: غير معيّن
  الحالة: جاهزة للتخطيط
  الملفات المتوقعة: package.json، archive-next/package.json،
    docs/release-notes/v1.5.1{,.ar}.md، infra/offline/README{,.ar}.md،
    مسار GitHub Release
  معيار القبول: أرقام الإصدار متطابقة؛ أصول الإصدار منشورة؛ كل أجزاء الحزمة
    المقسّمة قابلة لإعادة التجميع؛ ينجح التحقق عبر SHA256 قبل التثبيت وبعده؛
    وتشرح الوثائق خطوات Windows وLinux وmacOS بوضوح.
  ```

- [ ] **1.5.1-INSTALLER-001 — توفير مُثبّت ومدير نظام قابل للتنزيل من صفحة
  الإصدار.** بناء برنامج مُثبّت رسمي يكتشف نظام التشغيل والمعمارية، ويتحقق
  من سلامة الأصل قبل تشغيله، ويثبّت Archive Suite من أصل الإصدار المناسب
  (بما في ذلك الحزمة المقسّمة عند الحاجة)، ثم يوفّر إدارة دورة الحياة اليومية:
  الفحص المسبق، الإعداد الأولي، التشغيل والإيقاف وإعادة التشغيل، التحديث،
  النسخ الاحتياطي والاستعادة، عرض الحالة والسجلات، وإلغاء التثبيت دون حذف
  البيانات إلا بموافقة صريحة. يجب رفع المُثبّت وروابطه وبصماته إلى GitHub
  Release وتوثيق المسار المدعوم للمستخدم. **الاعتماديات:** 1.5.1-REL-001
  وتعريف المنصات المدعومة ونموذج الأذونات وإدارة الأسرار.

  ```text
  المالك: غير معيّن
  الحالة: جاهزة للتصميم
  الملفات المتوقعة: scripts/control-center/، infra/offline/،
    .github/workflows/release.yml، docs/control-center{,.ar}.md،
    docs/native-installation{,.ar}.md
  معيار القبول: مُثبّت قابل للتنزيل من صفحة الإصدار؛ فحص بصمة ناجح وفشل آمن
    عند عدم التطابق؛ تثبيت جديد وتحديث واستعادة مجرّبة؛ أوامر الإدارة تعرض
    حالة الخدمات والسجلات؛ لا تُحذف وحدات التخزين أو النسخ الاحتياطية تلقائيًا؛
  ويمر مسار Windows وLinux المدعوم باختبارات القبول والتوثيق العربي.
  ```

- [ ] **1.5.1-NATIVE-001 — استكمال نسخة Native Standalone بأدوات التثبيت
  والإدارة.** توفير حزمة مستقلة كاملة للمستخدم لا تعتمد على Docker، مع أدوات
  تثبيت وإعداد وفحص مسبق وإدارة الخدمات والتشغيل والإيقاف وإعادة التشغيل
  والتحديث والنسخ الاحتياطي والاستعادة وعرض الحالة والسجلات وإلغاء التثبيت.
  يجب أن تتضمن الحزمة ملفات التثبيت والتشغيل المطلوبة، وملفًا يوضح إصدار
  الحزمة وتاريخ بنائها، وسجلًا واضحًا لتاريخ الإصدارات والتغييرات، مع توثيق
  عربي وإنجليزي يشرح المتطلبات وخطوات التثبيت والإدارة والاستعادة لكل منصة
  مدعومة. **الاعتماديات:** 1.5.1-INSTALLER-001، وتعريف حدود Native مقارنةً
  بمسار Docker واعتماديات التشغيل لكل من Windows وLinux.

  ```text
  المالك: غير معيّن
  الحالة: جاهزة للتصميم
  الملفات المتوقعة: docs/native-installation{,.ar}.md،
    docs/versioning{,.ar}.md، scripts/control-center/،
    infra/platform/، أصول Native في مسار GitHub Release
  معيار القبول: حزمة Native Standalone كاملة قابلة للتنزيل من صفحة الإصدار؛
    تثبيت جديد وتشغيل وإدارة وتحديث واستعادة مجرّبة دون Docker؛ ملف إصدار
    وتاريخ بناء مضمّنان؛ تعليمات عربية وإنجليزية كاملة؛ وبصمات الأصول وفحوص
    Windows وLinux موثقة ومتحققة.
  ```
