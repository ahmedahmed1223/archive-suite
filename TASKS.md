...
- [x] **V1-208A توحيد عقد الخيارات** — مكتملة 2026-07-14: فصل runtime profiles (`core/media/edge`) عن capabilities (`ocr/ai/observability`) في العقد وschema، وضبط Setup ليبدأ بـ`core` فقط ويختار `media` (معالجة/OCR اختيارية وعبء موارد) و`edge` (وصول عام/TLS) صراحة، مع بوابة drift بين العقد وCompose وSetup. الدليل: `node --test scripts/platform-contract.test.mjs` (3/3) و`node --test scripts/control-center.test.mjs` (18/18)؛ `pnpm verify:infra` حُاول لكن حُجب ببيئة Docker/Node المحلية كما يوثقه تقرير V1-208A.
