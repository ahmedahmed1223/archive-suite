...
- [ ] `[P1]` ⏱️M **معالج «جولة الميزات» واحد قابل للتجاهل ولإعادة التشغيل** — دمج `UsageOnboarding` + الجولات الموزّعة + خطوات `ONBOARDING_STEPS` الثانوية (`interface`/`shortcuts`/`data`/`appearance`) في معالج واحد يعرض الميزات الأساسية في 4–5 شرائح، يظهر مرة بعد التثبيت، يحتوي على «تخطّى نهائياً»، ويُحفظ تجاهله في الإعدادات.
  - الملفات: `archive-app/src/features/onboarding/FeatureTour.jsx` (جديد، يدمج المحتوى الحالي)، `archive-app/src/features/onboarding/V1OnboardingWizard.jsx` (تقليم الخطوات المنقولة)، `archive-app/src/app/RuntimeShellApp.js` (`shouldShowV1Tour`).
...
- [ ] `[P1]` ⏱️M **تحسين `setup.bat`/`control-center.mjs` — أوامر مختصرة + مساعدة موسّعة + تشخيص أولي** — إضافة: (أ) `setup quick` (أمر مختصر: deploy + start + يفتح المتصفح على :8080). (ب) `setup help` بمساعدة مفصّلة لكل أمر مع أمثلة. (ج) تشخيص أولي تلقائي عند الإطلاق التفاعلي يكشف Node/pnpm/Docker/المنافذ المشغولة قبل عرض القائمة. (د) رسائل خطأ أوضح مع خطوة العلاج المقترحة. (هـ) أمر `setup doctor` يجمع كل الفحوصات في تقرير واحد.
  - الملفات: `scripts/control-center.mjs`، `setup.bat` (تحديث التعليقات).
...
- [ ] `[P1]` ⏱️M **تحسين تصميم صفحات تسجيل الدخول والخروج** — مراجعة بصرية شاملة لـ `LoginScreen` (و/أو `LoginPage`) وشاشة الخروج/قفل الجلسة: استخدام نفس tokens الحالية (`var(--va-text)`, `va-accent-*`, `border-[var(--va-border-soft)]`) مع heading تركيبي، حقل اسم مستخدم/كلمة مرور بقياس لمسي ≥44px، رسائل خطأ inline a11y-safe (`aria-live="polite"`)، بطاقة "البدء السريع" أوضح، وزر "تشغيل معالج البداية" بدور ثانوي. شاشة الخروج/القفل: hero بسيط + خياران (إعادة الدخول / قفل التطبيق) مع رسائل واضحة دون مفاجآت بصرية.
  - الملفات المتوقعة: `archive-app/src/components/auth/LoginScreen.jsx` (أو ما يقابلها)، `archive-app/src/pages/LoginPage.jsx` (إن وُجدت)، ملفات شاشة القفل/الخروج في `archive-app/src/features/auth/`.
  - القبول: تباين 4.5:1+ على كل النصوص، تركيز لوحة مفاتيح واضح، تخطيط يعمل من 320px وحتى الديسكتوب، لا يكسر اختبارات a11y في `__tests__/a11y/`.
  - المصدر: طلب المستخدم 2026-06-21.
