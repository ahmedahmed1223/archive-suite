...
- [x] **V1-208B تفكيك Control Center إلى وحدات** — مكتملة 2026-07-14: بقي `scripts/control-center.mjs` نقطة الدخول المتوافقة، وصار يركب وحدات `scripts/control-center/cli.mjs` (تحليل CLI)، و`configuration.mjs` (قراءة/كتابة `.env` والنسخ الاحتياطي وإخفاء الأسرار)، و`docker-compose.mjs` (Compose والتحقق من contract/profile)، و`operations.mjs` (الخادم والهجرة والنسخ الاحتياطية والتشخيص/التحديث)، و`runtime-adapter.mjs`. العقد الموحد للـadapter يعلن `install/start/stop/restart/status/health/logs/exec/update/rollback/uninstall`؛ Docker ينفذ دورة الخادم الحالية، ويعيد لـ`update`/`rollback`/`uninstall` غير المنفذة نتيجة structured صريحة بلا تنفيذ وهمي. حُفظ default `core` وخيارا `media/edge` الصريحان وعدم اعتبار capabilities profiles. دليل TDD: RED لغياب adapter ثم GREEN، وRED لتكوين entry point ثم GREEN. التحقق: Control Center + adapter ‏23/23، contract ‏4/4، و`git diff --check`; بوابة `pnpm verify:infra` محجوبة محلياً بسبب Node/Docker كما يوثق التقرير `v1-208b-report.md`.
