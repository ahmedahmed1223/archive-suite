...
- [ ] `[P1]` ⏱️M **تحويل نصوص معالج الإعداد (bash/terminal) إلى الإنجليزية كلغة افتراضية، مع العربية كخيار صريح** — حالياً يطبع المعالج العربية مباشرةً (`scripts/deploy-wizard.mjs` ~75 سطراً عربياً، `scripts/setup.mjs` ~50 سطراً)، وهذا يسبّب تشوّه العرض في كثير من الطرفيات (RTL + ترميز/خطوط في Windows Console/PowerShell وبعض طرفيات SSH)، ما يربك المستخدم أثناء التثبيت.
