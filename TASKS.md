...
> js-yaml/next)، واختبار tar في verify:reproducibility يفشل بيئيًا على Windows
> (bsdtar يعامل C: كمضيف بعيد) — كلاهما سابق لتغييرات 1.4 ولا يخص المنتج.
...
- [ ] **V2-MEDIA-002a — عقد تمثيل الوسيط.** تعريف `MediaRepresentation`
  وهوية الإصدار وأنواع `source|preservation|mezzanine|proxy|thumbnail|text|output`
  وحالاتها، مع منع كشف مفاتيح التخزين ومساراته للمتصفح.
- [ ] **V2-MEDIA-002b — سجل النسخ في Laravel.** هجرة ونموذج وعلاقة بالسجل
  وبالمرفق/المشتق، مع حفظ بصمة المصدر التي أنتجت منها النسخة.
- [ ] **V2-MEDIA-002c — تزامن الاستبدال.** عند تغيير المصدر تصبح كل نسخة ذات
  بصمة قديمة غير حالية ولا يمكن عرضها أو تشغيلها في مسار السجل أو الاستوديو.
- [ ] **V2-MEDIA-002d — واجهة السجل والاستوديو.** عرض صريح لما هو موجود فقط:
  لا تظهر نسخة حفظ أو mezzanine بوصفها جاهزة ما لم تسجلها الخدمة فعلًا.
- [ ] **V2-MEDIA-002e — اختبار القبول.** اختبار API وواجهة حي يثبتان المصدر
  والمشتقات الحالية، ثم استبدال المصدر، ثم اختفاء النتائج القديمة.
...
- [ ] **V2-MEDIA-004 — توصيف زمني وبحث داخل الفيديو.**
...
- [!] **V2-DESIGN-001 — توليد `Video Asset Workspace` المتخصصة في Stitch.**
  العائق: `gcloud 581.0.0` مثبت، لكن بيئة SDK لا تملك صلاحية إنشاء سجل داخل
  مجلد حساب المستخدم؛ لا تتوفر أدوات Stitch MCP في الجلسة الحالية.
  التالي: إصلاح إعداد SDK أو تسجيل الدخول إلى Stitch، ثم استخدام المشروع
  `12079300562270009833` ونظام التصميم الموجود لتوليد شاشة متخصصة واحدة
  ومراجعتها قبل إدخالها في الكود.
