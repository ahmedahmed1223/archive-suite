...
- [~] `[P2]` ⏱️XL **تعاون حي (Live Collaboration)** — حضور فوري + تحرير متزامن + إشعارات WebSocket حقيقية (`PresenceIndicator` موجود).
  ...
  - نطاق مؤجَّل عن قصد: (1) OT/CRDT كامل على مستوى العمليات/الأحرف بدلاً من optimistic shared draft؛ (2) ربط إشعارات فورية بصفحات تشغيلية أخرى غير `/collaboration` (مثل `/media/review`, `/files`). البند يبقى `[~]` لحين تغطية هاتين النقطتين أو تجزئتهما إلى بند مستقل.
