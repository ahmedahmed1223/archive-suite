...
> **المصدر:** 7 تقارير (HTML) في `<redacted-local-path>\Reports\P4`:
...
- [x] `[P1]` ⏱️XXL **ترحيل تدريجي إلى TypeScript** — frontend + server (الحالة: مكتمل 2026-06-29).
  ...
  - ✅ شريحة 12 — إغلاق خطة leaf tests + بوابة الإصدار (2026-06-28): حُوّلت اختبارات `hijriDate`, `subtitleParser`, و`transcriptToSrt` إلى `.test.ts` مع إبقاء جسور `.js` للاستيرادات الحالية، ووُسّع نوع `hijriDate` ليقبل `null/undefined` كما يفعل السلوك الحالي، وأُضيف `pnpm run typecheck` إلى بداية `release:verify`. العد الحالي خارج المخرجات: 796 JS/JSX و76 TS/TSX. التحقق الموحد لهذه الشريحة: `pnpm run verify:typescript-migration`.
  - ✅ شريحة 13 — pure feature model batches عبر وكلاء mini (2026-06-28): حُوّلت 13 وحدة pure و12 اختبارًا مرافقًا إلى TypeScript مع جسور `.js`: collections (`smartCollectionRules`, `collectionSources`)، errors (`recoveryQueue`, `errorReportBuilder`, `errorLogStore`)، views/dnd/timeline/dashboard (`kanbanModel`, `dndController`, `timelineSelectors`, `dashboardLayoutModel`)، workflow/users/activityLog (`recentDefaults`, `users/viewModel`, `users/permissions`, `activityLog/viewModel`). العد الحالي خارج المخرجات: 785 JS/JSX و101 TS/TSX. مرّ `pnpm run typecheck:app`، ومرّ `pnpm run verify:typescript-migration` بنتيجة 15 ملف اختبار و144 اختبار ناجح، ثم مرّ `pnpm run release:verify` بالكامل بعد ضبط fixture الأمان في `.env.example`.
  - ✅ شريحة 14 — media/montage pure modules عبر وكلاء mini (2026-06-28): حُوّلت 8 وحدات pure و7 اختبارات مرافقة إلى TypeScript مع جسور `.js`: media (`viewModel`, `scrubberPreview`, `mediaClient`) وmontage (`waveform`, `timelineModel`, `renderGraph`, `multiTrackModel`, `filterRegistry`). العد الحالي خارج المخرجات: 778 JS/JSX و116 TS/TSX. أُضيفت اختبارات الشريحة إلى `verify:typescript-migration` وصار `release:verify` يستدعي بوابة TypeScript الموحّدة قبل فحوص الإصدار. مرّ `pnpm run release:verify` كاملًا بنتيجة 22 ملف اختبار TypeScript و216 اختبار ناجح ضمن بوابة التحويل.
  - ✅ شريحة 15 — bootstrap/hooks/i18n/utils/data-portability عبر وكلاء mini (2026-06-28): حُوّلت خدمات data-portability، طبقة bootstrap، hooks المشتركة، أدوات utils، theme، وi18n locales إلى TypeScript مع جسور `.js` للاستيرادات الحالية، وحُوّلت اختبارات الدفعة إلى `.test.ts`. أُضيفت أنواع React إلى `@archive/app` حتى يغطي `tsc` hooks/theme بدون فجوات. العد الحالي خارج المخرجات: 764 JS/JSX و187 TS/TSX. مرّ `pnpm run typecheck:app`، ومرّت suite الواجهة كاملة عبر اختبار الدفعة بنتيجة 142 ملف اختبار و1237 اختبار ناجح.
  - ✅ شريحة 16 — storage adapters/services/stores الصغيرة عبر وكلاء mini (2026-06-28): حُوّلت storage adapters العامة وFirebase، خدمات health/router/storage/storageAccess/push/sentry، store helpers، وشرائح stores الصغيرة إلى TypeScript مع جسور `.js` للاستيرادات الحالية. حُذفت جسور `.test.js` المحوّلة حتى لا يلتقط Vitest ملفات اختبار فارغة أو مكررة. العد الحالي خارج المخرجات: 756 JS/JSX و256 TS/TSX. مرّ `pnpm run typecheck:app`، ومرّت suite الواجهة كاملة عبر اختبار الدفعة بنتيجة 142 ملف اختبار و1237 اختبار ناجح.
  - ✅ شريحة 17 — pure feature modules عبر وكلاء mini (2026-06-28): حُوّلت معظم وحدات features غير JSX إلى TypeScript مع جسور `.js`، بما في ذلك archive helpers، analytics/AI/automation/autosave، guide/navigation/notifications/offline، onboarding/projects/settings/share/sync/theme/types وغيرها، إضافة إلى ملفات runtime الصغيرة (`main`, `test-setup`, `ProgressContext`, `vendor`, `errorReporting`). اختبارات features المحوّلة أصبحت `.test.ts` بدون جسور `.test.js`. العد الحالي خارج المخرجات: 715 JS/JSX و404 TS/TSX. تم تبسيط `verify:typescript-migration` ليشغّل typecheck لكل الحزم ثم suite الواجهة كاملة بدل قائمة يدوية طويلة.
  - ✅ شريحة 18 — component helper modules محليًا بعد نفاد الوكلاء (2026-06-28): حُوّلت helper/view-model hooks في components (`commandPaletteViewModel`, `shortcutDialogViewModel`, `undoManager`, `useFocusOnMount`, `useFocusTrap`, `useFormSaveState`, `navigation/viewModel`, `ui/motion`) إلى TypeScript مع جسور `.js`. التنفيذات الفعلية المتبقية داخل `archive-app/src` انخفضت إلى 295، والعد الكلي خارج المخرجات صار 715 JS/JSX و412 TS/TSX.
  - ✅ شريحة 19 — component barrels + hooks الصغيرة عبر وكيل mini ومحليًا (2026-06-28): حُوّلت barrels في `components/**/index` إلى TypeScript مع جسور `.js`، بما فيها `components/icons/index` مع أنواع icon spec، وحُوّلت hooks الصغيرة `useCanPerform`, `useAiAssist`, `useServerStatusMonitor`, و`useBackgroundNotificationBridge` إلى TypeScript. التنفيذات الفعلية المتبقية داخل `archive-app/src` انخفضت إلى 279، والعد الكلي خارج المخرجات صار 715 JS/JSX و428 TS/TSX. مرّ `pnpm --filter @archive/app run typecheck`.
  - ✅ شريحة 20 — feature widgets/hooks عبر وكيل mini ومحليًا (2026-06-28): حُوّلت widgets/hooks صغيرة في features إلى TypeScript/TSX مع جسور `.js/.jsx`: `AiAssistBar`, `ServerStatusBadge`, `TagCloud`, `useTypeToJump`, `useReorderDnd`، ومكوّنات settings الصغيرة `LocalStorageEngineSettings`, `FirebaseBackendSettings`, `DatabaseSettings`, و`CloudControlTab`. عولجت أخطاء دمج الوكيل حول imports وtypes عند حدود `dbConfigClient`. التنفيذات الفعلية المتبقية داخل `archive-app/src` انخفضت إلى 258، والعد الكلي خارج المخرجات صار 715 JS/JSX و437 TS/TSX. مرّ `pnpm --filter @archive/app run typecheck`.
  - ✅ شريحة 21 — app shell الصغيرة + store slices عبر وكلاء mini ومحليًا (2026-06-28): حُوّلت `App`, `AppSync`, `AppNotifications`, و`AppRouter` إلى TypeScript/TSX مع جسور `.jsx`، وحُوّلت store slices الصغيرة (`ui`, `settings`, `folders`, `dataTransfer`, `autosave`, `automation`, `auth`) إلى TypeScript مع جسور `.js`. حُوّل اختبار `authSlice.remember` إلى `.test.ts` بدون جسر اختبار. التنفيذات الفعلية المتبقية داخل `archive-app/src` انخفضت إلى 246، والعد الكلي خارج المخرجات صار 714 JS/JSX و449 TS/TSX. مرّ `pnpm --filter @archive/app run typecheck` ومرّ `node .\node_modules\vitest\vitest.mjs run src/stores/slices/authSlice.remember.test.ts`.
  - ✅ شريحة 22 — components الدفعة الأولى عبر وكلاء mini ومحليًا (2026-06-28): حُوّلت 38 مكوّنًا/ملفًا من components إلى TSX مع جسور `.jsx`، شملت `activity`, `dnd`, `layout`, `offline`, `sync`, `theme`, `lists`, `favorites`, `recommendations`, `cards`, `collaboration`, `errors`, `suggestions`, `shortcuts`، ومجموعة UI V2 (`Button/Input/Card/Dialog/Badge/Switch/Tabs/Tooltip/Toast/AnimatedNumber/MobileActionBar`). حُوّل اختبار `OfflineBanner` إلى `.test.tsx` بدون جسر اختبار؛ اختبارات UI V2 بقيت JSX لشريحة اختبارات لاحقة. التنفيذات الفعلية المتبقية داخل `archive-app/src` انخفضت إلى 208، والعد الكلي خارج المخرجات صار 713 JS/JSX و487 TS/TSX. مرّ `pnpm --filter @archive/app run typecheck` ومرّ `node .\node_modules\vitest\vitest.mjs run src/components/offline/OfflineBanner.test.tsx`.
  - ✅ شريحة 23 — اختبارات UI V2 إلى TSX محليًا (2026-06-28): حُوّلت اختبارات UI V2 التسعة (`Badge/Button/Card/Dialog/Input/Switch/Tabs/Toast/Tooltip`) إلى `.test.tsx` بدون جسور اختبار، مع ضبط refs وDOM nullability وmock framer-motion. التنفيذات الفعلية المتبقية داخل `archive-app/src` انخفضت إلى 199، والعد الكلي خارج المخرجات صار 704 JS/JSX و496 TS/TSX. مرّ `pnpm --filter @archive/app run typecheck` ومرّت اختبارات UI V2 بنتيجة 9 ملفات/96 اختبار.
  - ✅ شريحة 24-28 — إغلاق تحويل `archive-app/src` بالكامل (2026-06-29): حُوّلت الدفعات الكبيرة المتبقية في components/pages/features/stores/app إلى TS/TSX مع جسور توافق لملفات التنفيذ، ثم حُوّلت آخر 34 ملفًا فعليًا بدون جسور اختبار: 33 اختبارًا (`components`, `features`, `pages`, `app/shell`, `__tests__/a11y`, `styles`) وbarrel `pages/index.ts`. أُضيف type augmentation لـ `toHaveNoViolations`، وضُبطت mockات/DOM nullability في الاختبارات، وأُضيف حارس unmount في `DrAlertsPanel` لمنع تحديث state بعد teardown. التنفيذات الفعلية المتبقية داخل `archive-app/src` أصبحت 0؛ العد الكلي خارج المخرجات صار 670 JS/JSX (جسور توافق فقط داخل app) و697 TS/TSX. مرّ `pnpm run verify:typescript-migration` كاملًا: typecheck لكل الحزم + 143 ملف اختبار و1245 اختبار ناجح.
  - ✅ شريحة 29 — تحويل خدمات `archive-server` (Task 10) عبر 8 وكلاء Haiku ومحليًا (2026-06-29): حُوّل 94 ملف خدمة إلى TypeScript مع جسور `.js` على مجموعات كتابة منفصلة — auth، export، ai، media، backup/enterprise، config+share+rights، mos+ingest، ومجموعة متنوعة (logger/cache/conversion/files/monitoring/notifications/retention/versions/webhooks/workflow…). حُوّلت 6 اختبارات مرافقة إلى `.test.ts` (بقيت اختبارات `__tests__/*.mjs` التكاملية كما هي). صُحّح خطأ جسور ذاتية المرجع في mos/ingest، وعولجت 173 خطأ أنواع عبر 5 وكلاء build-error-resolver (أساسًا بتوسعة أنواع المدخلات «record اعتباطي» المفرطة الصرامة) + إعلانات `.d.ts` لـ `ws`/`web-push`/`qrcode`/`nodemailer`. التنفيذات الفعلية في `archive-server/src` انخفضت 142→48 (المتبقي كله سطح Task 11: api/routes/adapters/index). العد الكلي خارج المخرجات: 663 JS/JSX و773 TS/TSX. مرّ `pnpm run typecheck` (0 أخطاء) و`pnpm --filter archive-server test` (21 ملف/228 اختبار).
  - ✅ شريحة 30 — إغلاق سطح `archive-server` (Task 11) عبر 4 وكلاء Haiku + 3 وكلاء إصلاح ومحليًا (2026-06-29): حُوّل 47 ملفًا (adapters التخزين كلها، api/server+handlers+routes، routes/*، وentrypoint `index.ts` مع جسر `index.js`) إلى TypeScript. عولجت 120 خطأ أنواع. ‼️ ضُبط أيضًا خطأ زمن-تشغيل لا يكشفه typecheck: 9 جسور backup (Task 10) + 12 جسر api كانت **ذاتية المرجع** (`export * from "./self.js"`) و33 جسرًا فيها `export { default }` لوحدة بلا default — أُعيد توليد كل الـ136 جسر server حتميًا من المصدر (default فقط عند وجوده فعلاً في `.ts`). أُضيف `ssh2-sftp-client.d.ts`. التنفيذات الفعلية في `archive-server/src` أصبحت 0. العد الكلي خارج المخرجات: 663 JS/JSX (جسور فقط) و821 TS/TSX. مرّ `pnpm run typecheck` (0) و`pnpm --filter archive-server test` (29 ملف/287 اختبار). المتبقي: Task 9 (استراتيجية build/export لـ core + حذف الجسور) وTask 12 (تشديد/تنظيف نهائي).
  - ✅ شريحة 31 — إغلاق خطة TypeScript النهائية (2026-06-29): أُغلقت Tasks 9 و12، وثُبتت حالة الجسور كقرار نهائي: `archive-app/src` و`archive-server/src` بلا تنفيذات JavaScript فعلية، و`archive-core/src` يحتفظ بجسور التوافق إلى أن يحتاج `dist` للنشر الخارجي. المسح النهائي عبر `archive-app/src`, `archive-core/src`, و`archive-server/src`: 663 ملف JS/JSX/MJS، منها 659 جسر توافق فوق TS/TSX و4 اختبارات تكامل server `node:test` بصيغة `.mjs` مقصودة، مقابل 845 ملف TS/TSX. أُزيل استثناء `src/services/storage/schema.js` من `archive-app/tsconfig.json`. صُححت جسور app التي كانت تعيد تصدير `default` غير موجود، وصار إشعار اكتمال الرفع يحل بريد المستخدم من Prisma بدل تمرير user id كبريد، وصار `createAiProvider` يمرر `transcribe.fetchImpl` للفحوص/المزودات المعزولة، وضُبط فحص readiness ليفحص `index.ts` خلف جسر `index.js`. مرّ `pnpm run release:verify` كاملًا: typecheck، و143 ملف اختبار app / 1246 اختبار، وapp/core/server verify، و`build:spa`، و`build:cloud`، و`security:baseline`، وrelease readiness.
