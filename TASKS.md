...
- [ ] **V2-UX-002 — مراجعة العائلات التسع بصريًا ووظيفيًا.**
  ...
  **المتبقي:** «لا صلاحية» ليست حالة مسمّاة في جرد الحالات. العمل عليها جارٍ
  في جلسة أخرى على الشجرة نفسها (`e2e/no-permission-surface.authed.spec.ts`
  وصفحات `/reports` و`/settings/users` و`/status` و`/system/control`
  و`/data-center`)، ولم يُودَع بعد؛ تُغلق المهمة عند إيداعه.
