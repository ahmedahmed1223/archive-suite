...
- [ ] **V1-756 (B13)** تنبؤ نمو التخزين في صفحة التقارير — *جزئي (2026-07-16): محرك التنبؤ جاهز ومختبَر في `lib/storage-forecast.ts` (انحدار أصغر المربعات، R² كثقة، تاريخ نفاد السعة، ورفض التنبؤ عند عيّنة واحدة/لحظة واحدة/بيانات فاسدة). **المتبقي: مصدر البيانات** — `SystemMetrics.disk` لقطة لحظية بلا تاريخ، فربط الواجهة يحتاج عقد metrics-history في Laravel + OpenAPI (مؤجل: قاعدة الدفعة 1 تمنع تغييرين متداخلين للعقد).*
- [x] **V1-757 (B21)** كشف التكرار شبه المتطابق للفيديو (perceptual hash) *(2026-07-16: `lib/perceptual-hash.ts` — dHash 64-bit يتجاوز تغيّر التعريض والضغط، مقارنة توقيعات best-match تتحمّل القص وإعادة الترتيب، وتجميع single-linkage يربط سلاسل إعادة الترميز ويُبلغ بأضعف رابط لا بمتوسط مُجمِّل. استخراج الإطارات يبقى بذرة محقونة. 20 اختباراً.)*
