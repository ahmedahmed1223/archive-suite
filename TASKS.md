...
- [ ] `[P2]` ⏱️L **موجة 2 — يحتاج backend جديد في Laravel أولاً:** المستخدمون والأدوار (CRUD+دعوات) · رفع الملفات (Uploader/AddVideo — endpoint upload) · سجل النشاط/التاريخ (activity + undo/diffs فوق audit log الموجود) · المجموعات Collections (يدوي + smart rules).
  - ✅ أُنجِز رفع الملفات (`550e1725`): `POST /api/v1/uploads` في Laravel (تحقق حجم/امتداد، تخزين على قرص ingest، إنشاء سجل `storage_rows`، وتفعيل media job للوسائط) + صفحة `/uploads` في Next (AppShell/PageToolbar) + `uploadFile()` في `archive-api.ts` + تحديث العقد. المتبقي من الموجة: المستخدمون/الأدوار، سجل النشاط، Collections.
