...
- [x] **V1-208G بوابة Setup الأساسية** — مكتملة 2026-07-14: صار `--json` يعيد غلافًا ثابتًا وآمنًا (`ok/code/message/details/nextActions`) لكل أوامر البوابة الأساسية، مع exit code صحيح ودون إدراج transcript أو أسرار في `details`. بقيت `plan/import/export/install/repair/wizard` بعقودها المتخصصة، وتستعمل دورة lifecycle إصدار Docker فقط. يمنع `migrate` و`rotate-secrets` في JSON التنفيذ الصامت: يلزمان `--yes`، ثم ينفذان الأمر الحقيقي؛ وتبقى update/rollback/uninstall غير مدعومة في adapter بلا تنفيذ وهمي. يصبح diagnostics صامتًا في JSON حتى لا يخلط مخرجات `pnpm verify` بالعقد. اختبار CLI يمرر URL/password/token حقيقية ويثبت تعقيم stdout/stderr وsupport bundle، كما يثبت أن فشل ACL في Windows يحذف الحزمة ولا يكشف السر. التحقق: 78 اختبار Node مركزًا (Control Center/manifest/release/runtime/observability/platform) ناجح؛ `pnpm verify:infra` محجوب محليًا بإصدار Node/Docker المعروفين.
...
- [x] **V1-208N أوضاع الوصول والشهادات** — مكتملة 2026-07-14: أضيفت وحدة وصول مستقلة تتحقق من سياسة V1-208F (`public` يتطلب `edge` و`edge` لا يعمل خارج public)، وتنفذ port preflight قبل أي كتابة، ثم DNS/certificate الصريحين لـpublic فقط. تحفظ حالة `.env` داخل الذاكرة وتطبق selectors غير السرية بكتابة ذرية، ثم تستعيدها عند health فاشل بلا عرض أسرار. دليل TDD: 7 اختبارات للوصول المحلي/الشبكي/العام، policy، port/DNS/cert failure، rollback وredaction؛ `node --test scripts/control-center/access-mode.test.mjs` ‏7/7 و`git diff --check`. بوابة `pnpm verify:infra` محجوبة ببيئة Node/Docker المحلية المعروفة.
