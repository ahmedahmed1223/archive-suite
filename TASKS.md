...
- [~] `[P2]` ⏱️L **مشغّل وسائط قانوني في Next + بثّ HTTP Range (حلّ التشغيل المحلي + كل أماكن التخزين)** — يستبدل مشغّل archive-app المجمّد ويحلّ منع المتصفح لتشغيل `file://`.
  ...
  - المتبقّي: بثّ byte-range للـ disks البعيدة (s3/ftp/sftp) عبر تقسيم طلبات seek — المحلي والـ sequential منجز؛ waveform/transcript-sync القانوني (نُقل من legacy عند الحاجة).
