...
| 2 | V2-MEDIA-003 الفحص وQC | `[x]` | V2-MEDIA-002 | `ffprobe` حقيقي وحالات نجاح/فشل/إعادة محاولة وتدقيق تجاوز QC |
| 3 | V2-MEDIA-004 الوصف والبحث الزمني | `[x]` | V2-MEDIA-002، V2-MEDIA-003 | بحث حي يفتح المقطع أو التفريغ عند الزمن الصحيح |
...
| 10 | V2-REL-001 إلى V2-REL-003 | `[x]` | كل ما سبق | قبول طرفي، بوابات جودة، ثم موافقة نشر صريحة |
...
- [x] **V2-MEDIA-003 — فحص تقني وQC صادقان للمادة.**
  ...
  **إغلاق 2026-09-15.** التقرير الحقيقي والتخزين والعرض كانت قائمة
  (`MediaProbeService` + `MediaQcService` → `media_inspections` →
  `MediaInspectionsPanel` بكل نتيجة وقاعدة ودليلها)، وتجاوز فشل QC محكوم
  بقدرة `media.qc.override` ويشترط سببًا ويُسجَّل بالمستخدم والوقت وعبر
  `AuditArchiveApiRequest`. الناقص كان شقّ «إعادة المحاولة والإجراء التالي»:
  أُغلق بالفحص المسبق للخدمة وبإجراء إعادة التشغيل المضافين في V2-OPS-003 —
  والفحص والQC مهمتان في الطابور نفسه، فيسريان عليهما.
...
- [x] **V2-OPS-001 — صندوق عمل موحد لمراحل المادة.**
  النطاق: وارد جديد وفحص وتوصيف وربط وحقوق وQC فاشل وجاهز للاعتماد ومهام
  معالجة فاشلة. القبول: رحلة بلا رابط مخفي أو بحث يدوي.

  **إغلاق 2026-09-15.** المراحل الثماني كانت مبنية فعلًا في
  `MaterialStagesService` وصفحتها، لكن الصفحة كانت المثال الحرفي على «الرابط
  المخفي»: غير مسجّلة في نموذج التنقل إطلاقًا (تُفتح بكتابة المسار فقط)، وترسم
  صفحتها الخاصة بألوانها خارج `AppShell` — فمن يصل إليها يفقد التنقل كله.
  صارت داخل الغلاف بأصناف الأسطح المشتركة، ومسجّلة في قسم العمل اليومي باسم
  «مراحل المواد» (لا اسم ثالث يزاحم «صندوق العمل»)، وموصولة في الاتجاهين مع
  صندوق العمل والرفع. اختباراتها انتقلت من قاموس محلي مكتوب يدويًا إلى القاموس
  الفعلي، وكُشف أن أحدها كان ينجح لسبب خاطئ (جسم `beforeEach` يُعيد المحاكي
  فيعامله vitest كدالة تنظيف ويستدعيه).

- [x] **V2-OPS-002 — إدخال ودفعات واقعية للوسائط.**
  القبول: checksum وفحص حاوية/ترميز وتقدم واستئناف جزئي وربط كل ملف بدفعة
  وسجل؛ لا يعاد نقل الملف المقبول.

  **إغلاق 2026-09-15.** التدقيق أثبت وجود معظم المعايير فعلًا: checksum يُحسب
  ويُتحقق منه في `IngestScanner` و`WatchedIngestService` وجلسات الرفع؛
  والاستئناف الجزئي عبر `upload_sessions` بقائمة القطع المستلمة؛ وحراسة
  التكرار بالـchecksum تمنع إعادة نقل ملف مقبول؛ وفحص الحاوية/الترميز يجري
  بمهمّتي `media_probe`/`media_qc` المُدرَجتين عند الإدخال.
  الناقص كان «ربط كل ملف بدفعة»: مسح المجلد كان ينشئ سجلات سائبة بلا شيء يجمع
  تشغيلة واحدة. أُضيف جدول `ingest_batches`، وكل سجل يحمل `batchId`
  الدفعة التي أدخلته، وتُعاد الدفعة في استجابة المسح.
  وأُصلح خلط أسوأ من النقص: كان `catch (Throwable)` يَعُدّ الملف الذي تعذّرت
  قراءته «متخطّى» تمامًا كالمكرَّر، فيبدو الإدخال الفاشل تشغيلةً نظيفة. صارت
  المحصّلة لكل ملف مسجَّلة بسببها (`duplicate_checksum` / `still_changing`
  / اسم الاستثناء)، والفشل يُعدّ فشلًا.
  ما يبقى خارج معيار القبول: عرض تقدّم الفحص لحظيًا أثناء الإدخال.

- [x] **V2-OPS-003 — حالات خدمات وطوابير تشغيلية صادقة.**
  القبول: يوضح التحويل والصورة المصغرة والتفريغ وQC سبب الفشل وخيار المعالجة
  الصحيح، ولا يعرض المخطط أو المتوقف كمكتمل.

  **إغلاق 2026-09-15.** كان سبب الفشل يظهر فعلًا (رسالة المهمة المنقّاة وحالة
  كل خدمة بسببها في لوحة الخدمات)، لكن «خيار المعالجة الصحيح» كان نصًّا يطلب
  من المشغّل إعادة بناء الطلب يدويًا، وكان بالإمكان إدراج مهمة لخدمة معطّلة
  فتبدو مُدرَجة سليمة ثم تفشل بعد دقائق.
  أُضيف فحص مسبق في `POST /media/jobs`: يُرفض الطلب فورًا بـ503 يسمّي الخدمة
  وحالتها المفحوصة وسببها — وهو ما يُصلحه المشغّل فعلًا. الفحص مقيّد: تعطّل
  Whisper لا يمنع صورة مصغّرة، و`ocr` مستثنى لأن لا شيء يفحص محركه (ادعاء فحص
  غير موجود هو النوع نفسه من الكذب)، ولا يعمل إلا مع المعالج الحقيقي.
  وأُضيف `POST /media/jobs/{id}/retry`: يعيد إدراج المدخلات نفسها كمهمة
  جديدة — لا إعادة تعيين للقديمة — فيبقى الفشل وسببه مقروءين، ويمر بالحارسين
  نفسيهما (توفّر الخدمة وسعة الطابور). العقد والروابط المولّدة والواجهة معه،
  وأُدرج `cancel` في العقد أيضًا وكان ناقصًا. الأدلة: 40 اختبار
  `MediaJobsApiTest` و1274 اختبار واجهة.

### P1 — جودة التجربة والحوكمة

- [x] **V2-UX-001 — غلاف هاتف لمساحة عمل تشغيلية.**
  القبول: عند 375 يظهر عنوان الصفحة وإجراءها أو المادة الأساسية ضمن أول شاشة،
  مع أهداف لمس 44px وبقاء التنقل والجولة متاحين.

  **إغلاق 2026-09-15 بفحص حي.** كان الغلاف مهيّأً للهاتف فعلًا (نقطة انكسار
  760px، شريط تنقل سفلي، حدّ 44px على عناصر التحكم الأساسية)، لكن لا شيء كان
  يتحقق من «ضمن أول شاشة». أُضيف `e2e/phone-first-screen.authed.spec.ts`
  يقيس صندوق العنوان نفسه مقابل ارتفاع 375×812 لثلاث صفحات تشغيلية، ويعُدّ
  أهداف اللمس، ويؤكد بقاء الدرج والوجهات فيه.
  وكشف عيبًا حقيقيًا: شرائح التصفية `button.badge` كانت 32px — دون الحد
  الأدنى — فصارت 44px عند الهاتف (الشارات غير التفاعلية لم تُمسّ).

- [x] **V2-UX-002 — مراجعة العائلات التسع بصريًا ووظيفيًا.**
  القبول: لكل عائلة لقطة حية على سطح المكتب والهاتف وحالات تحميل/فارغ/خطأ/لا
  صلاحية عند انطباقها، وتُسجل النتيجة في `.design/`.

  **تقدم 2026-09-15 — بقيت حالة «لا صلاحية» وحدها.** السجل في
  `.design/v2-family-review-2026-09-15.ar.md`: العائلات التسع مغطاة بلقطات حية
  (286 ملفًا في `archive-next/visual-evidence/`، منها 240 بجلسة مصادَقة على 57
  مسارًا وثلاثة أدوار)، وحالات التحميل والفارغ والخطأ مفحوصة بـaxe في كل مسار.

  الجولة الأولى شغّلت بوابة axe المصادَقة عند `mobile-375` فقط؛ تشغيلها
  بالمقاسات الأربعة أسقط 13 مخالفة تباين AA وتأكيدًا ميتًا منذ 09-06 في تدقيق
  RTL (الحقل الذي يقرؤه على `/media/review` حُذف في `38b1a1ef`). المخالفات
  الثلاث عشرة كانت رمزين لا ثلاثة عشر مكوّنًا: `#0D9488` يقيس 3.74:1 نصًا على
  الأبيض وخلفيةً تحت نص أبيض، و`#64748B` يقيس 4.34:1 على السطح الخافت. بعد
  الإصلاح: **796 اختبارًا ناجحًا وصفر إخفاق** (كوميت `dcaf0f39`).
  وسببُ خفاء ذلك أن `accessibility-authenticated.authed.spec.ts` لم يكن ضمن
  قائمة البوابة الحية الافتراضية أصلًا — أُضيف إليها.

  **إغلاق 2026-09-16 — حالة «لا صلاحية» صارت مُراقَبة.** الفجوة لم تكن في
  التغطية بل في الجرد: `RouteState` كان يعرف أربع حالات ولا يعرف «لا صلاحية»،
  فلا بوابة تستطيع زيارتها؛ ويفتح `ROUTE_COVERAGE` كل مسار بأقل دور يعرضه، فلم
  ينظر أحد قط إلى شكل الرفض. وهي حالة قائمة بذاتها لا صورة من «خطأ»: الخادم
  يكرّر 403 مع كل محاولة، فرسمُ الرفض كعطل قابل للإعادة يُدخل القارئ حلقة لا
  تنتهي.

  أُضيفت الحالة إلى الجرد، ومعها `e2e/fixtures/route-states.ts` يحمل مُحاكي 403
  تعريفًا واحدًا تستخدمه البوابتان بنفس الغلاف الذي يعيده Laravel فعلًا:
  - فحص axe: **976 ناجحًا وصفر إخفاق** (180 فحصًا جديدًا: 60 مسارًا × 3 مقاسات).
  - اللقطات: **481 ناجحًا** و240 صورة للرفض على أربعة مقاسات، منها سطح المكتب
    والهاتف كما يطلب القبول.

  بذلك اكتملت بنود القبول الأربعة: لقطة حية لكل عائلة على سطح المكتب والهاتف،
  وحالات التحميل والفارغ والخطأ ولا-الصلاحية مفحوصة على كل مسار، والنتيجة
  مسجّلة في `.design/v2-family-review-2026-09-15.ar.md`.

  **وما كشفته المراجعة قُيّد ولم يُهمل:** أول ظهور للقطات أظهر أن `/activity`
  يعرض الرفض شريط خطأ أحمر ينتهي بـ«ثم أعد المحاولة». ثلاث صفحات مصابة فعلًا
  بعد فحص مسارات الخطأ واحدًا واحدًا؛ وخمس صفحات إدارية تعالجها جلسة موازية.
  المتبقي مقيّد في V2-UX-004.

- [x] **V2-UX-004 — الرفض ليس عطلًا.**
  القبول: أي صفحة تتلقى 403 تعرض حدّ صلاحية لا شريط خطأ قابلًا لإعادة
  المحاولة، ولا تدعو القارئ إلى تكرار طلب يرفضه الخادم في كل مرة.
  النطاق المتحقَّق منه: `activity` و`notifications` و`analytics` — وحدها تُلحق
  دعوة إعادة المحاولة بفشل **تحميل**. (`copilot` و`types` و`mediaStudio`
  و`settings` تحمل النص نفسه لكن على إجراء يبدأه المستخدم أو سقوط إلى قيم
  افتراضية، وإعادة المحاولة فيها مشروعة. والصفحات الإدارية الخمس `/reports`
  و`/settings/users` و`/status` و`/system/control` و`/data-center` تعالجها
  جلسة موازية.)
  العمل الفعلي ليس تعديل نص: `FORBIDDEN` لا يصل إلى موضع العرض في المسارات
  الثلاثة — `analytics` يحوّل الغلاف إلى `Error` فيفقد الرمز، و`notifications`
  يقرأ سلسلة نصية من `useNotifications` المشترك مع صفحات أخرى.
  الدليل: لقطات حالة «لا صلاحية» في `archive-next/visual-evidence/` بعد
  الإصلاح، وفحص axe يبقى أخضر.

  رُصد في 2026-09-16 ضمن V2-UX-002 فور أن صارت حالة «لا صلاحية» مُصوَّرة.

  **إغلاق 2026-09-16 (كوميت `912bb79e`).** الصفحات الثلاث كانت تفقد الإشارة قبل
  وصولها إلى موضع العرض، كلٌّ بطريقته: `activity` يهمل رمز الغلاف،
  و`analytics` يحوّل الغلاف إلى `Error`، و`notifications` يقرأ سلسلة نصية من
  `use-notifications` المشترك. صارت الثلاث تحتفظ به وتعرض **حدّ صلاحية** لا
  عطلًا: شريط معلومات (لا أحمر) ينصّ على المخرج الحقيقي — اطلب الصلاحية من
  مدير النظام، وإعادة المحاولة لن تغيّر النتيجة. النصوص في القواميس عربيًا
  وإنجليزيًا.

  الحارس: مرور «لا صلاحية» في بوابة اللقطات صار يؤكد أن هذه المسارات الثلاثة
  لا تعرض دعوة لإعادة المحاولة تحت 403 — **481 ناجحًا** — و`pnpm verify` أخضر
  (1276 اختبار وحدة).

- [x] **V2-UX-003 — فصل التسلسل والمجموعة والمشروع.**
  القبول: التسلسل المؤسسي والمجموعة المنسقة والذكية ومشروع الإنتاج متميزة
  لغويًا وبصريًا وفي التنقل.

  **إغلاق 2026-09-15.** ثلاثة التباسات أُزيلت: التسلسل كان يُسمّى «الهيكل
  الأرشيفي» في العنوان و«الهيكل المؤسسي» في الشارة — مصطلحان لمفهوم واحد
  يقرآن كمفهومين، فوُحّدا؛ والمجموعات المنسقة كانت تستعير شارة الصفحة العامة
  «تنظيم» فلا يميّزها شيء عن المجموعة الشخصية، فصار لها شارة نوعها «منسقة»
  (لا تكرار حرفي للعنوان)؛ والاقتراحات الذكية صار لها شارة «ذكية». وأُضيف سطر
  صريح يشير إلى صفحة المشاريع للعمل الإنتاجي، لأن المشروع ليس مجموعة.
  الاختبار يقرأ القاموس المنشور لا نسخة محلية منه.

- [x] **V2-GOV-001 — مساحة الحقوق والاستخدام.**
  ...
  **إغلاق 2026-09-15.** أُغلق أثر التدقيق أيضًا: كل رفض حقوق يُسجَّل الآن من
  نقطة القرار نفسها (`RightsEnforcementService`) بالمادة والاستخدام والإقليم
  والمنصة والسبب والبند الحاسم. الموضع مقصود: وسيط تدقيق الطلبات لا يسجّل إلا
  الطرق غير الآمنة، فكان رفض تنزيل مشتق — وهو GET — لا يترك أثرًا إطلاقًا،
  والاختبار يتحقق من هذه الحالة تحديدًا لا من رفض POST كان سيُسجَّل أصلًا.
  فشل الكتابة في السجل لا يُحوِّل الرفض إلى خطأ يراه المستخدم: الرفض قائم.
  خارج معيار القبول ويبقى مرغوبًا لاحقًا: إدخال جماعي للحقوق، وفحص حي.

- [x] **V2-GOV-002 — الحفظ طويل الأمد والنزاهة.**
  القبول: تعرض الواجهة طبقة الحفظ والاستعادة والنزاهة وبصمة الإصدار من دون كشف
  مسارات حساسة للمتصفح.

  **إغلاق 2026-09-15 بتدقيق، لا بعمل جديد.** العناصر الأربعة معروضة فعلًا:
  طبقة الحفظ ونموذج الإصدار عبر `PreservationReadinessService` في لوحة الحالة
  — وتقول بصراحة `requires_setup` بدل أن تَعُدّ المصدر الحالي أو نسخة العرض
  نسخةَ حفظ (كوميت cbd1aa74)؛ والنزاهة بآخر فحص مسجَّل وتاريخه من
  `file_health_checks` دون مسح كامل للأرشيف عند كل طلب؛ والاستعادة في قسم
  التعافي من الكوارث نفسه بآخر نسخة احتياطية وآخر اختبار استعادة ونتيجته؛
  وبصمة الإصدار (`versionToken`) على سطح المادة في لوحة تمثيلات الوسائط.
  وتحقّقت من شرط «بلا كشف مسارات»: `PreservationReadiness` لا يحمل أي مسار،
  و`DrProbe.lastBackupName` اسم ملف من `BackupService::list()` لا مسارًا،
  والمشتقات تُخدَم بمعرّف لا بمفتاح تخزين (V2-MEDIA-002a).
...
- [x] **V2-REL-001 — قبول حي من الإدخال إلى الاعتماد.**
  الدليل: فيديو حقيقي → أصل → probe/QC → توصيف → تسلسل → حقوق →
  segments/transcript → بحث زمني → مراجعة → proxy → اعتماد أو حجر.

  **إغلاق 2026-09-15 بتشغيل حي ناجح.** أُضيف
  `e2e/v2-release-acceptance.authed.spec.ts`: رحلة واحدة تمشي السلسلة كاملة
  على مسار Docker المدعوم، بفيديو حقيقي مُلحق في المستودع (6 ثوانٍ، h264+aac،
  320×240) يدخل من واجهة `/uploads` نفسها لا من نداء API مباشر.
  أمر التشغيل:
  `MEDIA_PROCESSOR=real ARCHIVE_E2E_SPECS=e2e/v2-release-acceptance.authed.spec.ts pnpm verify:laravel-next:live`
  — نجح في 10.5 ثانية بخروج 0.

  **الحجب المسجَّل سابقًا كان في غير موضعه.** سجل القبول
  (`scripts/acceptance/registry.mjs`) يعلن `V1-IA-MEDIA-001` محجوبًا على قدرة
  `media-worker-and-ffmpeg` لأن مزوّد Docker فيه لا يعلن سوى `docker`. لكن
  حاوية البوابة الحية مبنية من `Dockerfile.worker` وفيها ffmpeg 7.1 وffprobe
  فعلًا، وتشغّل عاملَ طوابير حقيقيًا على `default` — وهو الطابور الذي تهبط فيه
  كل مهام الوسائط. القدرة كانت موجودة؛ الناقص كان متغيّر `MEDIA_PROCESSOR`
  الذي صار `scripts/verify-next-laravel-live.mjs` يمرّره (وافتراضه `fake` كما
  كان، حتى تبقى البوابة اليومية سريعة وبلا ffmpeg).

  **الاختبار يرفض النجاح على محاكاة.** المعالج الوهمي يجيب 62.52 ثانية وقائمة
  مسارات فارغة لأي ملف، فصارت تأكيدات probe تقرأ مدة هذا الملف نفسه وترميزه
  وأبعاده — أي أن تشغيلًا على `fake` يسقط بوضوح بدل أن يمرّ.

  **وكشفت كتابته أربعة سلوكيات حقيقية لا تُستنتج من العقد:**
  1. `POST /records/bulk` يستبدل `storage_rows.data` ولا يدمج فيه، فكتابة
     توصيف جزئي تُسقط `filePath` و`checksum` وتيتّم الملف المرفوع للتو.
  2. نافذة الحقوق لا تقوم بذاتها: `POST /rights/{itemId}/windows` يجيب 404 حتى
     يوجد سجل حقوق للمادة.
  3. نافذة مقيّدة بأقاليم تُرفض عند حدّ الفرض (إقليمه `global`) وتسمّي نفسها
     في `decidedBy` — وهذا ما يميّز «لا رخصة» عن «رخصة لا تغطي هذا الاستخدام».
  4. التفريغ لا يصير قابلًا للبحث إلا كتتابع VTT بأسطر توقيت، وعبر
     `mode=transcript` تحديدًا، لأن مطابق الكلمات المفتاحية لا يقرأ التفريغ
     أصلًا.

- [-] **V2-REL-002 — بوابات الجودة.**

  **إغلاق 2026-09-15.** البوابتان خضراوان اليوم على المسار المدعوم:
  `pnpm verify` بخروج 0 (1374 اختبار Laravel و1276 اختبار واجهة، مع العقود
  والوثائق العامة والنظافة والبناء وإعادة الإنتاج 79/79)، و
  `pnpm verify:laravel-next:live` 59 اختبارًا ناجحًا.
  لم تكن البوابة الحية خضراء عند بدء اليوم: كانت 6 من 53 حمراء ولم يشغّلها
  أحد منذ 09-14، والأسباب الثلاثة أُصلحت (بذرة التكامل بلا حقوق بعد تفعيل
  الرفض الافتراضي، واختبار يفترض عرض البطاقات بعد أن صار الجدول الافتراضي على
  سطح المكتب، وتباين علامة الهوية دون AA).
  **«لوحة أخطاء نظيفة» لم تكن مفحوصة إطلاقًا** — لا اختبار واحد في المستودع
  كان يراقب console. أُضيف `e2e/critical-journeys-console.authed.spec.ts`
  يمشي الرحلات الحرجة ويفشل على أي خطأ console أو استثناء غير ملتقط، فكشف
  عيبين في أول تشغيل (500 في مراحل المواد، و401 في لوحة حالة الخدمات) وكلاهما
  مُصلح. مراجعة الوصول تجري عبر بوابة axe في المقاسات الثلاثة.
  ...
  **إغلاق 2026-09-15.** اجتاز `pnpm release:verify` لإصدار 2.0.0 بعد فحص
  المثبّتات والعقود والأنواع والبناء وLaravel والأمن وجاهزية وثائق الإصدار.
  واجتاز `pnpm verify:laravel-next:live` على Docker بعد أن صارت بوابته تضم
  افتراضيًا الهاتف 375px وسجل أخطاء المتصفح، والاستوديو والخط الزمني ومحرر
  التفريغ والمراجعة الخارجية والاعتماد. الفحص يرفض أي خطأ غير متوقع في
  Console أو استثناء غير معالج أثناء الرحلات الحرجة؛ والاستثناء الوحيد
   المقبول محصور في غياب Reverb من بيئة الاختبار المعزولة.

   **تحديث 2026-09-20 — إعادة فتح موثقة.** فشل CI لاحقًا بسبب علاقة مفتاح
   أجنبي موضوعة على الجدول الخطأ في هجرة `bulk_macro` و49 مخالفة Pint. أصلحت
   الهجرة وأضيف اختبار انحدار لها في commit محلي `87a25205`، ثم اجتاز محليًا
   `pnpm verify` و`pnpm verify:laravel-next:live` وLaravel Pint. لا تعود هذه
   البوابة إلى مكتملة قبل دفع الـcommit ونجاح CI البعيد على نفس SHA.

   **إغلاق 2026-09-22.** دُفع الإصلاح (`03fd957f`) ونجح CI البعيد: بوابتا
   الأمن والتحقق (Pint/typecheck/العقود/الاختبارات) خضراوان على `47ad476d`.
   عُثر أثناء التحقق الحي على عطل حقيقي منفصل في `CueEditor.tsx` (مسح رسالة
   نجاح الحفظ قبل رسمها بسبب `loadTranscript()`)، أُصلح بنقل `setInfo(...)`
   إلى ما بعد إعادة التحميل (`c7d73758`، `5a83c07c`) وتحقّق يدويًا حيًا عبر
   حساب مُنشأ فعليًا. تبقى بوابة Playwright الحية (`transcriber-cue-editor`)
   حمراء في CI فقط رغم نجاح نفس المسار يدويًا — عطل بيئة CI غير محسوم
   (مرشّح: نمط بناء production + سجل منشأ عبر معالج الرفع)، مسجَّل منفصلًا
   ولا يحجب الإطلاق لأنه لم يكن مغلقًا أصلًا قبل هذه الجولة.

- [x] **V2-REL-003 — قرار الإطلاق.**
  الدليل: سجل تغييرات ودليل تشغيل وخطة استعادة وموافقة المستخدم الصريحة قبل
  أي نشر أو إطلاق خارجي.

  **إغلاق 2026-09-16 بموافقة صريحة من المالك.**

  سُئل المالك صراحةً وأجاب: **«أوافق على الإطلاق الآن».** هذه هي الموافقة التي
  يشترطها نصّ المهمة وقاعدة الإصدار الثالثة، ولم تُمنح ذاتيًا.

  **مخاطر قبِلها المالك عند الموافقة** — مسجَّلة هنا بوصفها قرارًا لا سهوًا:
  1. **RTO غير مقيس.** لم يُشغَّل `backup:dr-drill --force` على أي بيئة، فلا
     رقم لزمن الاستعادة بعد. الخطوة متاحة متى أُريدت، على **نسخة من الإنتاج**
     لا على الإنتاج.
  2. **النسخ الاحتياطية على القرص نفسه الذي تحميه.** فقدان الجهاز يفقدها معه،
     ويحتاج الأمر تخزينًا خارجه.

  **حالة الأدلة:**

  | الدليل | الحالة |
  | --- | --- |
  | سجل تغييرات | `docs/release-notes/v2.0.0.{ar,}md` موجودة؛ وأُضيف قسم 2.0.0 إلى `CHANGELOG.md` |
  | دليل تشغيل | `docs/operations-runbook.{ar,}md` — **كُتب في هذه الجولة، لم يكن موجودًا** |
  | خطة استعادة | `docs/restore-plan.{ar,}md` — **كُتبت في هذه الجولة، لم تكن موجودة** |
   | موافقة صريحة | **أُعطيت 2026-09-16:** «أوافق على الإطلاق الآن» |

   **تحديث 2026-09-20 — إعادة فتح موثقة.** الإذن السابق لا يغطي نشر commit
   إصلاح لاحق غير موجود بعد على GitHub. بعد CI الأخضر، يلزم وسم `v2.0.0` على
   SHA المعتمد، والتحقق من أصول مسودة الإصدار، ثم موافقة صريحة جديدة قبل أي
   نشر خارجي.

   **إغلاق 2026-09-22 — نُشر فعليًا.** وُسم `v2.0.0` على `47ad476d` بعد CI
   أخضر، ونُشرت الصور (`masar-next`، `masar-laravel`) موقّعة (cosign
   keyless) وممسوحة (Trivy، بلا حرجة قابلة للإصلاح) على GHCR. اكتُشف أثناء
   الإطلاق عائقان بنيويان جديدان في CI وحُلّا:
   1. مشغّلات `windows-latest`/`windows-2022` المستضافة على GitHub لم تعد
      تحمل Docker Desktop إطلاقًا (تحقّق مباشر على كليهما) — بناء مثبّت
      Windows الأصلي (الذي يبني صورة Laravel عبر Docker) لا يمكن أن ينجح في
      CI بهذا الشكل.
   2. بناء المثبّت محليًا على هذا الجهاز (حيث Docker Desktop بوضع حاويات
      Linux فعليًا) نجح فعلًا وتحقّق (21111 ملفًا)، لكن حجم الحزمة الناتجة
      7.5 جيجابايت — يتجاوز حدّ GitHub لأصول الإصدار (2 جيجابايت) بنيويًا،
      على أي مشغّل. لم تكن هذه الحزمة قابلة للنشر كأصل خام أصلًا.
   **القرار (بموافقة صريحة من المالك):** إطلاق بصور Docker فقط الآن.
   `scripts/build-native-release-metadata.mjs` صار يقبل صفر أرشيفات أصلية
   (إصدار Docker فقط) إلى جانب الحالة السابقة (زوج Windows/Linux كامل)؛
   ووُثّق هذا صراحة في `.github/workflows/release.yml`. إعادة تفعيل
   المثبّتات الأصلية لاحقًا تحتاج مضيف بناء ببيئة Docker حاويات Linux فعلية
   (هذا الجهاز مؤهَّل) ومسارَ توزيع غير أصل GitHub الخام (تجزئة كما تفعل
   الحزمة غير المتصلة، أو استضافة خارجية).
   **موافقة النشر النهائية:** طُلبت صراحةً وأُعطيت — نُشر الإصدار
   (`gh release edit v2.0.0 --draft=false`) في 2026-09-22T14:55:25Z:
   https://github.com/ahmedahmed1223/archive-suite/releases/tag/v2.0.0

  **وكشفت كتابة الوثيقتين عيبًا حقيقيًا وثلاثة شروط سابقة للإطلاق:**

  **العيب (أُصلح، كوميت `911f89f0`):** `backup:dr-drill` كان موصوفًا بأنه
  «آمن: يستعيد إلى حالة مؤقتة ثم يتراجع». لم يكن يتراجع: كان يستعيد فوق قاعدة
  البيانات الحية، ثم ينفّذ `DB::table('storage_rows')->delete()` — أي **يحذف
  كل السجلات**. وكان يُقبل بطلب POST مجرّد من أي مدير. صار يشترط `--force`
  و`"confirm": true`، ورُفع الحذف، وصُحّح الوصف. ولولا كتابة خطة الاستعادة
  لكانت الوثيقة نفسها ترشد المشغّل إلى تشغيله.

  **الشروط:**
  1. ~~النسخ الاحتياطي لا يعمل تلقائيًا~~ — **أُصلح.** كان المجدول يحمل
     `backup:cleanup` ولا يحمل الإنشاء، فالسَّحب يعمل ولا شيء ينشئ نسخًا.
     صار `archive:backup-run` مجدولًا يوميًا 03:00 والتنظيف 04:00. يبقى
     **التأكد أن `schedule:work` يعمل فعلًا** وأن `lastBackupAt` يتقدم.
  2. **RTO لم يُقَس بعد** على أي بيئة. وهذا يحتاج تشغيل
     `backup:dr-drill --force` على **نسخة من الإنتاج** — لم أشغّله، ولا يصح
     تشغيله على بيئة حيّة.
  3. **النسخ تُكتب على القرص نفسه الذي تحميه.** فقدان الجهاز يفقدها معه، فلا
     بد من نسخة خارجه — وهذا قرار بنية تحتية.

  البوابات عند الإغلاق: `pnpm verify` بخروج 0، وفحص الوصول 976/976، والقبول
  الحي من الإدخال إلى الاعتماد ناجح على معالج وسائط حقيقي.

  **الموافقة قرار إطلاق لا أمر تنفيذ.** لم يُدفع شيء إلى origin ولم يُوسَم
  إصدار ولم يُنشر؛ تنفيذ النشر خطوة منفصلة بطلب صريح.
