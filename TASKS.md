...
- [x] **V1-208I update ذري من artifacts** — مكتملة 2026-07-14: `setup update` (Docker release فقط) ينفذ preflight للـdescriptor غير القابل للتغيير ثم backup قانونيًا وpull/load متحققًا و`archive:migrate-safe` وswitch وhealth وrole-boundary smoke. فشل switch/health/smoke يوقف حالة target غير الآمنة ويستعيد تلقائيًا الإصدار/config المثبتين السابقين؛ manifest يسجل target أثناء التنفيذ وprevious release reference بعد النجاح ولا يحذف images. أخطاء manifest I/O تتحول إلى envelope JSON أحمر `UPDATE_MANIFEST_IO_FAILED` بلا raw rejection أو تسريب URL. التحقق: 139/139 Node، `node --check` و`git diff --check` ناجحة.
