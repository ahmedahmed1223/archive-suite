...
- [ ] `[P1]` ⏱️XXL **ترحيل معماري إلى Laravel API + Next.js TypeScript** — اعتماد Laravel كخادم نطاق وAPI، وNext.js كواجهة TypeScript تدريجية، دون إدخال Astro 5.
  ...
  - **خطة التنفيذ القادمة (شرائح عمودية، Laravel يقود عند نقص endpoint، Next يتبع، ثم بوابة قطع):**
    - [ ] شريحة 5a — صفحة Next لقائمة الأرشيف + البحث فوق `search` الموجود في Laravel: مسار `/archive` بـ App Router، حقل بحث، شبكة سجلات typed عبر `createArchiveApiClient().search()`، حالات loading/error/empty، RTL. بوابة: `typecheck:next` + `build:next` + smoke في `e2e:next`.
