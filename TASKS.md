...
- [-] **V2-UI-004 — مساحة عمل أصل الفيديو الاحترافية.**
  ...
  768 و375. شجرة المشتقات وStudioRecordTasks لا تظهران عمدًا تحت 900px، ولا
  توجد لهما نقطة وصول بديلة من الاستوديو؛ هذه فجوة UX يجب حلها بتبويبات أو
  ورقة سفلية قبل الإغلاق. "المقاطع" (`/records/{id}/clips`، موجودة في الخلفية
  وفي واجهة `/media/compare`) وحالات الفحص/QC (`media_inspections`،
  `media_qc_overrides`) غير مُسطَّحة داخل الاستوديو نفسه بعد.
