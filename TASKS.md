...
- [x] `[P1]` ⏱️L **إعادة تصميم Masar لاستعادة قدرات الواجهة القديمة داخل Next** — اعتماد التصور في `docs/design/masar-ui-redesign-vision.md`: AppShell موحد، toolbar كثيف، أوضاع عرض الأرشيف، saved views، bulk actions، details rail، Settings Hub، ومراقبة موحدة.
  ...
  - ✅ موجة المكتبات وتجربة UI الغنية أُغلقت في `archive-next`: `next-themes`, Radix UI, TanStack Query/Table/Virtual, React Hook Form/Zod, dnd-kit, motion, Recharts, cmdk, وLucide كلها موصولة بطبقة Masar المحلية. راجع وثيقة الإغلاق: `docs/design/masar-rich-ui-completion-audit.md`.
  - المتبقي الحقيقي بعد إغلاق خطة UI ليس واجهة ناقصة، بل Backend/تحقق خارجي موثق أدناه: تخزين دائم لكيانات الموجات 2/3، undo/diffs، OT/CRDT، وبيئات GPU/K8s/ODBC/Sentry الحية.
- [~] `[P2]` ⏱️L **موجة 2 — يحتاج backend جديد في Laravel أولاً:** المستخدمون والأدوار (CRUD+دعوات) · رفع الملفات (Uploader/AddVideo — endpoint upload) · سجل النشاط/التاريخ (activity + undo/diffs فوق audit log الموجود) · المجموعات Collections (يدوي + smart rules).
  ...
  - ✅ شريحة UI تشغيلية منجزة (2026-07-03): أُضيفت `/collections` كمجموعات محلية/ذكية فوق السجلات الحالية، و`/activity` كسجل نشاط يومي يجمع تغييرات السجلات ومهام الوسائط. المتبقي: endpoint audit دائم مع undo/diffs، ومخزن Collections دائم في Laravel بدلاً من local preferences.
- [~] `[P2]` ⏱️L **موجة 3 — مخازن مساندة:** بحوث محفوظة · Vocabulary · وسوم هرمية · Inbox/التقاط · مكررات (كشف hash) · Kanban · أتمتة (rules engine).
  - ✅ شريحة UI تشغيلية منجزة (2026-07-03): البحث المحفوظ موجود داخل `/search`، وأُضيفت صفحات Next فعلية للمساندات: `/vocabulary` (قاموس ومرادفات محلية من الأنواع/الوسوم)، `/tags` (وسوم هرمية محلية + كشف تشابه)، `/inbox` (التقاط وفرز سريع محلي)، `/duplicates` (كشف checksum/title)، `/kanban` (لوحة workflow مع نقل الحالة عبر `records/bulk`)، و`/automation` (مصمم قواعد + dry-run محلي). المتبقي: تخزين Laravel دائم لهذه الكيانات، rules engine تنفيذي، وواجهات merge/undo العميقة.
- 🔁 **تحديث قرار النقل (2026-07-03):** أُلغي قرار الاستبعاد السابق. Discover وSyncLog وSystemControl وFirstRun وProjects/مونتاج وبقية صفحات/ميزات legacy تُراجع الآن عبر مصفوفة المطابقة الرسمية `docs/design/masar-legacy-parity-audit.md`، وكل عنصر غير منقول يجب أن يكون إما منفذاً في `archive-next`/`archive-laravel` أو موثقاً كمدمج في سطح Masar أحدث.
- ملاحظة ترقية جزئية: `/archive` لديه الآن saved views وإجراءات bulk أساسية وموسعة؛ المتبقي لاحقاً هو workflows أعمق عند توفر Backend دائم للـ collections/activity/relations، و`/archive/[id]` يحتاج علاقات/نشاط تفصيلي عند اكتمال endpoints الخاصة بها.
...
- [~] `[P3]` ⏱️XL **محرّك قواعد مرئي (Visual Rules Engine)** — سحب/إفلات trigger+conditions+actions فوق `automationSlice`.
  - ✅ شريحة UI منجزة (2026-07-03): صفحة `/automation` توفر بناء قاعدة من trigger + شروط + action مع dry-run على سجلات الأرشيف الحالية وتخزين محلي. المتبقي: backend rules engine للتنفيذ الفعلي، سجل تشغيل، وصلاحيات.
