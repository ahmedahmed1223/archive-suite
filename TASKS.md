...
- [ ] `[P1]` ⏱️XL **تبنّي DaisyUI كنظام تصميم موحّد فوق Tailwind v4 وترحيل المكوّنات تدريجياً** — المكوّنات الحالية مكتوبة بـ CSS مخصص غير موحّد، ما ينتج تبايناً بصرياً ويصعّب الصيانة والتخصيص ودعم السمات.
  ...
  - 🔄 **مرحلة 12 (2026-06-16):** ترحيل أسطح صفحتين كبيرتين (Dashboard + Users) منخفضتي الخطورة إلى أصناف DaisyUI مع الحفاظ على السلوك وRTL والسمات وإمكانية الوصول: في `pages/DashboardPage.jsx` — زر التخصيص وأزرار شريط التحرير (حفظ/إلغاء/استعادة) ورقائق إظهار اللوحات المخفية → `btn` (+`btn-sm`/`btn-xs`/`btn-ghost`)، أزرار البطل (إضافة فيديو/بحث/في الأرشيف) وزر الحالة الفارغة → `btn`، حقل بحث البطل → `input input-bordered`، أزرار شرائط التنبيه (فتح الأرشيف/الأمان/إخفاء) → `btn` (+`btn-xs`/`btn-square`)، `RecentSearchButton` → `btn btn-sm btn-ghost`؛ في `pages/UsersPage.jsx` — زر «مستخدم جديد» وأزرار الأدوار/التصفية وأزرار البطاقة (تعطيل/تعديل/حذف) وأزرار راديو الدور في النموذج → `btn` (+`btn-sm`/`btn-ghost`/`btn-square`)، بطاقات الملخص → `card`، شارات الدور/الحالة (معطل/دعوة معلّقة/الحالي) وعدّادات التصفية → `badge badge-sm`، حقول البحث/النموذج → `input input-bordered`، قائمة تصفية الأدوار → `select select-bordered`. لم تُمسّ آليات `DashboardGrid` (السحب/التحجيم) ولا المكوّنات المشتركة المُرحَّلة سابقاً (`ReportStrip`/`CommandPanel`/`QuickActionGrid`). أُبقيت تدرّجات الألوان وطبقة `va-*` والأنماط السطرية فوق أساس DaisyUI لحفظ المظهر، وحُفظت جميع المعالجات والتوجيه وأدوار ARIA. **709 اختبار تمرّ، build:spa أخضر.** المتبقي للمراحل القادمة: `pages/DataCenterPage.jsx` وبقية الصفحات الكبيرة، و`drawer` للجوال، وحاوية رموز الاسترداد في `TwoFactorSettings`، و`ActivityEntry`، وإزالة طبقة `va-*` (مرتبط بـ§1982 عالي الخطورة).
