...
- [~] `[P2]` ⏱️XL **تعاون حي (Live Collaboration)** — حضور فوري + تحرير متزامن + إشعارات WebSocket حقيقية (`PresenceIndicator` موجود).
  ...
  - نطاق مؤجَّل عن قصد (لم يُنفَّذ في هذه الشريحة): (1) تحرير متزامن حقيقي (OT/CRDT) لمحتوى الموارد — الأقفال (شريحة 2) لا تزال آلية منع التعارض الوحيدة؛ (2) ربط إشعارات فورية بصفحات تشغيلية أخرى غير `/collaboration` (مثل `/media/review`, `/files`) — البث الحالي يقتصر على تحديثات presence داخل غرفة التعاون نفسها؛ توسيعه لصفحات أخرى يتطلب تحديد الأحداث المطلوب بثها لكل صفحة، وهو خارج نطاق "استبدال/تعزيز polling" المطلوب هنا. البند يبقى `[~]` لحين تغطية هاتين النقطتين أو تجزئتهما إلى بند مستقل.
