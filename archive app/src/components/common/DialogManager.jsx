...
export function DialogProvider({ children }) {
  ...
  const open = useCallback((id, component, props = {}) => {
    setStack((prev) => {
      if (prev.some((d) => d.id === id)) return prev; // prevent duplicates
...
function DialogBackdrop({ children, zIndex, isTop, onBackdropClick }) {
  return (
    <div
      style={{ zIndex }}
      className={[
...
function FocusTrap({ children }) {
  ...
  useEffect(() => {
    ...
    if (focusable.length > 0) {
      ...
      const rafId = requestAnimationFrame(() => focusable[0]?.focus());
