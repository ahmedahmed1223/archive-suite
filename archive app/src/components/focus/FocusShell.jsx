...
export function FocusShell() {
  ...
  const doNotDisturb = useAppStore((s) => s.focusDoNotDisturb);
  const setFocusMode = useAppStore((s) => s.setFocusMode);
  const setDoNotDisturb = useAppStore((s) => s.setFocusDoNotDisturb);
  ...
  React.useEffect(() => {
    ...
    const id = setInterval(() => setPomodoro((prev) => tickPomodoro(prev, 1)), 1000);
  ...
  React.useEffect(() => {
    if (focusMode) return;
    setControlsVisible(true);
    setPomodoro((prev) => (prev.running ? resetPomodoro(prev) : prev));
  }, [focusMode]);
    ...
    <AnimatePresence>
      {controlsVisible && (
        ...
        >
          ...
          <button
            type="button"
            onClick={() => setPomodoro((prev) => togglePomodoro(prev))}
          ...
          <button
            type="button"
            onClick={() => setPomodoro((prev) => resetPomodoro(prev))}
            className="flex h-9 w-9 items-center justify-center rounded-full bg-white/5 transition-colors hover:bg-white/15"
