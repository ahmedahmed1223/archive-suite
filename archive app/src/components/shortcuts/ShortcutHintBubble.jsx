...
import {
  ...
  markHintShown,
  markLearned,
} from "../../features/shortcuts/shortcutLearningState.js";
 ...
 *
 * @param {object} props
 * @param {string} props.actionId - SHORTCUT_ACTIONS id (e.g. "deleteSelected")
 * @param {object} [props.settings] - user settings (for custom shortcut values)
 * @param {() => void} [props.onDismiss] - called when bubble disappears
 * @param {"bottom-right"|"bottom-left"|"top-right"|"top-left"} [props.position]
 */
export function ShortcutHintBubble({ actionId, settings = {}, onDismiss, position = "bottom-right" }) {
  ...
  const shortcutKey = getShortcutValue(settings, actionId);
  ...
  return jsx("div", {
    ...
    "aria-atomic": "true",
    dir: "rtl",
    className: `fixed z-[9999] ${positionClasses} pointer-events-auto`,
    children: jsxs("div", {
      className: [
      ...
      children: [
        jsx(Keyboard, { className: "h-4 w-4 shrink-0 text-indigo-400", "aria-hidden": "true" }),

        jsxs("span", {
          className: "text-xs text-gray-300",
          children: [
            jsx("span", { className: "text-gray-500", children: "نصيحة: " }),
            label,
            " ← ",
            jsx("kbd", {
              className: [
              ...
              children: formatShortcut(shortcutKey),
            }),
          ],
        }),
