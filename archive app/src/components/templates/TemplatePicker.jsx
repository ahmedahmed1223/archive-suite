 ...
 */
export function TemplatePicker({ isOpen, onClose, onApply, context = {} }) {
  const userTemplates = useAppStore((state) => state.templates || []);
  ...
  const allTemplates = React.useMemo(() => {
    const custom = (userTemplates || []).filter((t) => !t.isBuiltIn);
    return [...custom, ...BUILT_IN_TEMPLATES];
  }, [userTemplates]);

  const filtered = React.useMemo(() => filterTemplates(allTemplates, query), [allTemplates, query]);
