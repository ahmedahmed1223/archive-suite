...
export function ServerStatusBadge({ compact = false }) {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      markRpcFailure?.({ error: error?.message || "فشل فحص الخادم", status: error?.status });
    } finally {
      setBusy(false);
    }
  };
        ...
        children: [
          ...
          jsx("div", { className: "mt-3 grid gap-2 text-xs sm:grid-cols-2", children: [
            ...
            children: [
              ...
              jsx("p", { className: "mt-1 truncate font-semibold text-gray-200", dir, title: value, children: value })
            ]
          }, name)) }),
