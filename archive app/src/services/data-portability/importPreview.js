...
export const IMPORT_PREVIEW_ENTITIES = [
...
export function comparableImportRecord(record, ignoredKeys = []) {
...
export function summarizeImportEntity(config, importedRecords, currentRecords) {
  ...
  const currentById = new Map(current.map((record) => [record.id, record]).filter(([id]) => id));
  const currentTitles = config.titleKey
    ? new Set(current.map((record) => normalizeArabicSearchText(record?.[config.titleKey] || "")).filter(Boolean))
    : null;
  ...
  imported.forEach((record) => {
    ...
    if (!existing) {
      newCount += 1;
      const titleKey = currentTitles ? normalizeArabicSearchText(record?.[config.titleKey] || "") : "";
...
export function createImportPreviewSummary(payload, currentState = {}, options = {}) {
  const normalizePayload = typeof options.normalizePayload === "function" ? options.normalizePayload : (value) => value || {};
...
export function formatImportPreviewSummary(summary, options = {}, helpers = {}) {
  const formatFileSize = typeof helpers.formatFileSize === "function" ? helpers.formatFileSize : (value) => `${value} بايت`;
