...
import { DashboardSkeleton as DashboardSkeletonBase } from "./shell/ShellParts.jsx";
...
interface UnknownPageScreenProps {
  onBackToDashboard: () => void;
}

interface AppRouterSettings {
  accentColor?: string;
  ui?: {
    daisyTheme?: string;
    customDaisyTheme?: unknown;
...
const DashboardSkeleton = DashboardSkeletonBase as React.ComponentType<{ compact?: boolean }>;

function PageLoadingFallback({ title }: PageLoadingFallbackProps) {
  return (
    <div
      dir="rtl"
      role="status"
 ...
 */
function UnknownPageScreen({ onBackToDashboard }: UnknownPageScreenProps) {
  return jsx("div", {
    dir: "rtl",
    role: "alert",
    className:
...
export function AppRouter() {
  const currentPage = useAppStore((s: any) => s.currentPage) as string;
  const settings = useAppStore((s: any) => s.settings) as AppRouterSettings;
  const copilotOpen = useAppStore((s: any) => s.copilotOpen) as boolean;
  const toggleCopilot = useAppStore((s: any) => s.toggleCopilot) as (() => void) | undefined;
  const setCurrentPage = useAppStore((s: any) => s.setCurrentPage) as ((page: string) => void) | undefined;
