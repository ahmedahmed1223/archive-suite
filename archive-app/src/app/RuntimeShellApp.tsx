...
export function App() {
  ...
  const [startupRecovery, setStartupRecovery] = React.useState<any>(null);
  const [onboardingWizardMode, setOnboardingWizardMode] = React.useState<any>(null);
  ...
  const [serverSetupComplete, setServerSetupComplete] = React.useState<any>(null);
  React.useEffect(() => {
    let cancelled = false;
    fetch("/api/setup/status")
      .then((res: any) => (res.ok ? res.json() : null))
      .then((data: any) => { if (!cancelled && data && typeof data.needsSetup === "boolean") setServerSetupComplete(!data.needsSetup); })
      ...
      const init = async () => {
        try {
          ...
          setStartupRecovery({
            ...
            warnings: [{ id: "startup", message: (normalized as any).userMessage || normalized.message, severity: "error", at: new Date().toISOString() }]
          });
        } finally {
  ...
  React.useEffect(() => {
    const applyPageFromLocation = () => {
      ...
      if (!PAGE_COMPONENTS[page as any]) return;
      (window as any).__videoArchiveApplyingHistory = true;
      setSelectedItemId(page === "detail" ? route.selectedItemId : null);
      setCurrentPage(page as any);
      (window as any).__videoArchiveApplyingHistory = false;
    };
  ...
  React.useEffect(() => {
    ...
    if (!isLocked) {
      ...
      const hasUsers = appStore.users.some((user: any) => user.isActive);
      ...
      if (!isPasswordSet && hasUsers && settings.ui?.onboardingSecurityMode === "quick" && settings.ui?.v1OnboardingCompleted) {
        const activeUser = appStore.users.find((user: any) => user.username === "admin" && user.isActive) || appStore.users.find((user: any) => user.isActive);
  ...
  React.useEffect(() => {
    const openOnboarding = (event: any) => {
  ...
  React.useEffect(() => {
    if (shouldShowStartupOnboarding({ authState, settings })) {
      setOnboardingWizardMode((mode: any) => mode || "startup");
  ...
  React.useEffect(() => {
    ...
    const firstTaskChoice = settings.ui?.firstTaskChoice as any;
    const firstTaskRoute = ({
      "import-backup": "backup",
      "add-video": "add",
      "create-type": "types",
      dashboard: "dashboard"
    } as any)[(firstTaskChoice || "dashboard")];
  ...
  const openStartupDiagnostics = React.useCallback(async () => {
    ...
    setShowSplash(false);
    setSelectedItemId(null);
    setCurrentPage("settings" as any);
    try {
  ...
  const handleKeyDown = React.useCallback((event: any) => {
    ...
    if (shortcutAction === "undo") {
      ...
      if (action) showToast(`تراجع: ${(action as any).description}`, "info");
      return;
    }
    if (shortcutAction === "redo") {
      event.preventDefault();
      const action = undoRedoManager.redo();
      if (action) showToast(`إعادة: ${(action as any).description}`, "info");
      return;
    }
    if (shortcutAction === "viewGrid") {
      event.preventDefault();
      setViewMode("grid" as any);
      return;
    }
    if (shortcutAction === "viewList") {
      event.preventDefault();
      setViewMode("list" as any);
      return;
    }
    if (shortcutAction === "viewTable") {
      event.preventDefault();
      setViewMode("details" as any);
      return;
    }
    if (shortcutAction === "viewGallery") {
      event.preventDefault();
      setViewMode("gallery" as any);
      return;
    }
    if (shortcutAction === "viewKanban") {
      event.preventDefault();
      setSelectedItemId(null);
      setCurrentPage("kanban" as any);
      return;
    }
        ...
        jsxs("div", {
          ...
          children: [
            ...
            jsx("div", {
              ...
              children: Array.from({ length: 8 }).map((_: any, index: any) => jsx("div", {
