...
export const PAGE_GROUPS = PAGE_MANIFEST.reduce<Record<string, string[]>>((groups, page) => {
  groups[page.group] = groups[page.group] || [];
  groups[page.group].push(page.id);
  return groups;
}, {});

export const HEAVY_PAGE_IDS: string[] = PAGE_MANIFEST.filter((page) => page.heavy).map((page) => page.id);
