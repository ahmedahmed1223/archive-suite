...
interface ActivityEntrySettings {
  numberSystem?: string;
}

interface ActivityEntryProps {
  entry: ActivityLogEntry;
  settings?: ActivityEntrySettings;
...
function toneClasses(action: string): string {
 ...
 */
export function ActivityEntry({ entry, settings = {}, onUndo, onRedo }: ActivityEntryProps) {
  const [expanded, setExpanded] = React.useState(false);
  const Icon = ACTION_ICON[entry.action] || PenLine;
  const hasDiff = Boolean(entry.snapshot?.diff && Object.keys(entry.snapshot.diff).length > 0);
