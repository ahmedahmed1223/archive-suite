...
export function ActivityFilterBar({ value = {}, onChange }: ActivityFilterBarProps) {
  ...
  const update = (patch: Partial<ActivityFilters>) => onChange?.(patch);
  const clearAll = () =>
    ...
    children: [
      jsxs("div", {
        ...
        children: [
          ...
          jsx("select", {
            value: value.action || "",
            onChange: (event: ChangeEvent<HTMLSelectElement>) => update({ action: event.target.value }),
          ...
          jsx("select", {
            ...
            onChange: (event: ChangeEvent<HTMLSelectElement>) => update({ targetType: event.target.value }),
          ...
          jsx("input", {
            ...
            onChange: (event: ChangeEvent<HTMLInputElement>) => update({ dateFrom: event.target.value }),
            "aria-label": "من تاريخ",
            title: "من تاريخ",
            className: dateClass
          }),
          jsx("input", {
            type: "date",
            value: value.dateTo || "",
            onChange: (event: ChangeEvent<HTMLInputElement>) => update({ dateTo: event.target.value }),
