...
export function SummaryPanel({ item, showToast }: any) {
  const summary = useAppStore((state: any) => selectSummaryForItem(state, item?.id));
  const setSummary = useAppStore((state: any) => state.setSummary);
  ...
  async function handleSummarize() {
    ...
    const shortSummary =
      typeof result === "string"
        ? result
        : (result as any)?.summary || (result as any)?.text || "";

    const keyPoints = Array.isArray((result as any)?.keyPoints) ? (result as any).keyPoints : [];
    const detailedSummary =
      typeof result === "object" ? (result as any)?.detailedSummary || "" : "";
        ...
        <div className="mb-3 space-y-3">
          ...
          {summary.keyPoints.length > 0 && (
            <ul className="space-y-1 text-sm text-gray-600 dark:text-gray-400">
              {summary.keyPoints.map((point: any, i: any) => (
          ...
          {summary.detailedSummary && (
            <div>
              <button
                type="button"
                onClick={() => setShowDetails((v: any) => !v)}
