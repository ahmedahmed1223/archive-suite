 ...
 */
async function readBlobWithProgress(res: any, onProgress: any) {
...
export function ExportButton({ selectedIds = [] }: any) {
  const [open, setOpen] = useState(false);
  const { anyLoading, isLoading, withLoading } = useLoading();
  const showNotification = useAppStore((s: any) => s.showNotification);
  const updateNotificationProgress = useAppStore((s: any) => s.updateNotificationProgress);
  const finalizeNotification = useAppStore((s: any) => s.finalizeNotification);

  async function handleExport(format: any) {
    setOpen(false);
    ...
    const operation = startOperation(store, {
      title: "تصدير السجلات",
      message: `جارٍ تصدير ${count} بصيغة ${(FORMAT_LABELS as any)[format] || format}…`,
      category: "export",
    });
    await withLoading("export", async () => {
      const body = { format, store: "videoItems" };
      if (selectedIds.length > 0) (body as any).ids = selectedIds;
      ...
      const blob = await readBlobWithProgress(res, (percent: any) => operation.setProgress(percent));
      ...
      operation.succeed({ message: `تم تصدير ${count} بصيغة ${(FORMAT_LABELS as any)[format] || format} بنجاح.` });
    });
  }
