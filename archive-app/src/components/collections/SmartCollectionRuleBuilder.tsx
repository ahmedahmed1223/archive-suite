...
function toRuleset(match: any, conditions: any) {
 ...
 */
export function SmartCollectionRuleBuilder({ collection, videoItems = [], folders = [], onCancel, onSave }: any) {
  ...
  const [conditions, setConditions] = React.useState(
    existingRules?.conditions?.length
      ? existingRules.conditions.map((condition: any) => ({
          field: condition.field,
          operator: condition.operator,
  ...
  const updateCondition = (index: any, patch: any) => {
    setConditions((prev: any) => prev.map((condition: any, idx: any) => {
  ...
  const addCondition = () => setConditions((prev: any) => [...prev, blankCondition()]);
  const removeCondition = (index: any) => setConditions((prev: any) =>
    prev.length <= 1 ? prev : prev.filter((_: any, idx: any) => idx !== index)
  );
      ...
      children: [
        jsxs("div", { className: "grid gap-3 md:grid-cols-[5rem_minmax(0,1fr)]", children: [
          jsxs("div", { className: "space-y-1 text-sm text-gray-300", children: [
            ...
            jsx("input", { id: iconId, value: icon, onChange: (event: any) => setIcon(event.target.value.slice(0, 4)), className: "input input-bordered min-h-11 w-full va-surface-deep rounded-xl border px-3 text-center text-xl text-white outline-none focus:border-cyan-500/40" })
          ] }),
          jsxs("div", { className: "space-y-1 text-sm text-gray-300", children: [
            jsx("label", { htmlFor: nameId, className: "block", children: "اسم المجموعة الذكية" }),
            jsx("input", { id: nameId, "data-autofocus": true, value: name, onChange: (event: any) => setName(event.target.value), className: "input input-bordered min-h-11 w-full va-surface-deep rounded-xl border px-3 text-sm text-white outline-none focus:border-cyan-500/40", placeholder: "مثال: مقاطع العمل الحديثة" })
          ] })
        ] }),
        ...
        jsx("div", {
          className: "space-y-2",
          children: conditions.map((condition: any, index: any) => {
            ...
            return jsxs("div", {
              ...
              children: [
                jsx("select", {
                  ...
                  "aria-label": "الحقل",
                  onChange: (event: any) => updateCondition(index, { field: event.target.value }),
                  ...
                  children: fieldOptions.map(([key, spec]: any) => jsx("option", { value: key, children: spec.label }, key))
                }),
                jsx("select", {
                  value: condition.operator,
                  "aria-label": "العملية",
                  onChange: (event: any) => updateCondition(index, { operator: event.target.value }),
                  className: "select select-bordered select-sm min-h-10 w-full va-surface-deep rounded-lg border px-2 text-sm text-white outline-none focus:border-cyan-500/40",
                ...
                showValue ? jsx("input", {
                  ...
                  "aria-label": "القيمة",
                  onChange: (event: any) => updateCondition(index, { value: event.target.value }),
