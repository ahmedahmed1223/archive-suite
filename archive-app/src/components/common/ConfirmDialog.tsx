...
const LEVEL_STYLES: Record<number, any> = {
  1: {
...
function TieredDialogModal({ config, onResolve }: { config: any; onResolve: any }) {
  ...
  React.useEffect(() => {
    ...
    const id = window.setTimeout(() => setSecondsLeft((s: number) => s - 1), 1000);
  ...
  return jsx("div", {
    ...
    dir: "rtl",
    onClick: (event: any) => { if (event.target === event.currentTarget) onResolve(false); },
 ...
 */
export function showConfirm(config: any): Promise<boolean> {
...
function normalizeDialogInput(message: any, options: any = {}) {
...
function fallbackDialog(request: any, mode: any) {
...
function DialogModal({ request, mode, onResolve }: { request: any; mode: any; onResolve: any }) {
...
function openReactDialog(message: any, options: any = {}, mode: any = "alert") {
