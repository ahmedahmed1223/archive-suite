 ...
 *   { type: "separator" }
 */
export function ContextMenu({ menu, onClose }: any) {
  const open = !!menu;
  ...
  React.useEffect(() => {
    ...
    const rect = (panel as any).getBoundingClientRect();
    const margin = 8;
        ...
        children: [
          ...
          jsxs(motion.div, {
            ...
            children: [
              ...
              (menu.items || []).map((entry: any, index: any) => {
                ...
                const handleClick = () => {
                  if (disabled) return;
                  try { entry.onSelect?.(); } catch (error: any) { console.warn("[ContextMenu] onSelect threw", error); }
