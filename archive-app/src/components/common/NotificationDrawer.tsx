...
function getActionLabel(item: any) {
...
export function NotificationDrawer() {
  const open = useAppStore((state: any) => state.notificationCenterOpen);
  const toggle = useAppStore((state: any) => state.toggleNotificationCenter);
  const history = useAppStore((state: any) => state.notificationHistory || []);
  const clearHistory = useAppStore((state: any) => state.clearNotificationHistory);
  const markAllRead = useAppStore((state: any) => state.markAllNotificationsRead);
  const markRead = useAppStore((state: any) => state.markNotificationsRead);
  const archiveNotification = useAppStore((state: any) => state.archiveNotification);
  ...
  React.useEffect(() => {
    if (open) {
      window.requestAnimationFrame(() => (closeButtonRef.current as any)?.focus());
    }
  }, [open]);
  ...
  const runNotificationAction = (item: any) => {
  ...
  const copyTechnicalDetails = async (item: any) => {
    try {
            ...
            children: [
              ...
              jsxs("div", { className: "space-y-2 border-b border-[var(--va-border-soft)] px-3 py-3", children: [
                ...
                jsx("div", {
                  ...
                  children: FILTER_TABS.map((tab: any) => {
                ...
                jsx("div", { className: "grid grid-cols-3 gap-1 rounded-[var(--va-radius-md)] border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-1", children: READ_FILTERS.map((item: any) => jsx("button", {
                  type: "button",
              ...
              jsx("div", {
                ...
                children: filtered.length === 0
                  ...
                  : jsx("div", {
                    className: "space-y-4",
                    children: groups.map((group: any) => jsxs("section", {
                      children: [
                        jsx("h3", { className: "px-1 pb-2 text-xs font-bold text-[var(--va-text-muted)]", children: group.label }),
                        jsx("ul", { className: "space-y-2", children: group.items.map((item: any) => {
                          const meta = (TYPE_ICON as any)[item.type] || TYPE_ICON.info;
