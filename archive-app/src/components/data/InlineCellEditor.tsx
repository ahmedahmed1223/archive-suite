...
function tagsToText(value: any) {
  if (Array.isArray(value)) return value.join("، ");
  if (typeof value === "string") return value;
  return "";
}

function textToTags(text: any) {
  return String(text || "")
    .split(/[،,]/)
...
function displayText(value: any, fieldType: any, options: any, placeholder: any) {
  if (fieldType === "tags") {
    const tags = Array.isArray(value) ? value : textToTags(value);
    return tags.length ? tags.join("، ") : placeholder || "—";
  }
  if (fieldType === "select") {
    const match = options.find((option: any) => String(option.value) === String(value));
...
function readValue(fieldType: any, raw: any) {
...
function DisplayMode({ text, isEmpty, ariaLabel, disabled, onStartEdit }: any) {
  return jsxs("button", {
    type: "button",
    disabled,
...
function EditField({ fieldType, draft, setDraft, options, placeholder, maxLength, inputRef, onKeyDown, onBlur, ariaLabel }: any) {
  const shared = {
    ref: inputRef,
    dir: "rtl",
    "aria-label": ariaLabel,
  ...
  return jsx("input", {
    ...shared,
    type: (typeMap as any)[fieldType] || "text",
...
export function InlineCellEditor({
  ...
  React.useEffect(() => {
    ...
    if (node) {
      (node as any).focus();
      if (typeof (node as any).select === "function") (node as any).select();
    }
