import { Star, X } from "lucide-react";

import { useAppStore } from "../../stores/appStore.js";
import { FAVORITE_ENTITY_TYPES } from "../../stores/slices/favoritesSlice.js";

const ENTITY_ICON = {
  [FAVORITE_ENTITY_TYPES.ITEM]: "🎬",
  [FAVORITE_ENTITY_TYPES.COLLECTION]: "📁",
  [FAVORITE_ENTITY_TYPES.FOLDER]: "🗂️",
  [FAVORITE_ENTITY_TYPES.SEARCH]: "🔍"
} as const;

export interface FavoriteEntry {
  key: string;
  entityType: string;
  entityId: string;
  label?: string;
}

export interface FavoritesSidebarSectionProps {
  onNavigate?: () => void;
  collapsed?: boolean;
}

export function FavoritesSidebarSection({ onNavigate, collapsed = false }: FavoritesSidebarSectionProps) {
  const { favorites, removeFavorite, setCurrentPage, setSelectedItemId } = useAppStore();
  const favoriteList = favorites as FavoriteEntry[] | undefined;

  if (!favoriteList || favoriteList.length === 0) return null;

  function handleFavoriteClick(fav: FavoriteEntry) {
    if (fav.entityType === FAVORITE_ENTITY_TYPES.ITEM) {
      setSelectedItemId?.(fav.entityId);
      setCurrentPage?.("detail");
    } else if (fav.entityType === FAVORITE_ENTITY_TYPES.COLLECTION) {
      setCurrentPage?.("collections");
    } else if (fav.entityType === FAVORITE_ENTITY_TYPES.FOLDER) {
      setCurrentPage?.("archive");
    } else if (fav.entityType === FAVORITE_ENTITY_TYPES.SEARCH) {
      setCurrentPage?.("search");
    }
    onNavigate?.();
  }

  if (collapsed) {
    return (
      <div className="mb-2 flex flex-col items-center gap-1">
        <div
          title="المفضلة"
          className="flex h-8 w-8 items-center justify-center rounded-lg border border-white/10 bg-white/[0.03] text-yellow-400"
        >
          <Star className="h-4 w-4" fill="currentColor" />
        </div>
      </div>
    );
  }

  return (
    <div className="mb-3">
      <div className="mb-1 flex items-center gap-2 px-3 text-[11px] font-medium uppercase tracking-wider text-gray-500">
        <Star className="h-3 w-3 text-yellow-400" fill="currentColor" />
        <span>المفضلة</span>
      </div>
      <div className="flex flex-col gap-0.5">
        {favoriteList.slice(0, 8).map((fav) => (
          <div
            key={fav.key}
            className="group flex cursor-pointer items-center gap-2 rounded-lg px-3 py-1.5 text-sm text-gray-300 hover:bg-white/5 hover:text-white"
          >
            <span className="shrink-0 text-base">{ENTITY_ICON[fav.entityType as keyof typeof ENTITY_ICON] || "⭐"}</span>
            <span className="flex-1 truncate" onClick={() => handleFavoriteClick(fav)}>
              {fav.label || fav.entityId}
            </span>
            <button
              type="button"
              title="إزالة من المفضلة"
              onClick={(e) => {
                e.stopPropagation();
                removeFavorite({ entityType: fav.entityType, entityId: fav.entityId });
              }}
              className="invisible shrink-0 rounded p-0.5 text-gray-600 hover:text-red-400 group-hover:visible"
            >
              <X className="h-3 w-3" />
            </button>
          </div>
        ))}
      </div>
    </div>
  );
}
