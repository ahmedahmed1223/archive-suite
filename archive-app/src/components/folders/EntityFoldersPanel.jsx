...
function defaultLabelFor(entity = {}) {
...
export function EntityFoldersPanel({
  ...
  const scopedFolders = React.useMemo(
    () => folders.filter((folder) => (folder.scope || "archive") === scope),
    [folders, scope]
  );
  ...
  const selectedFolder = selectedFolderId ? tree.byId[selectedFolderId] : null;
  ...
  const available = React.useMemo(() => {
    const list = entities.filter((entity) => entity?.id && !assignedIds.has(entity.id));
    if (!normalizedQuery) return list;
    return list.filter((entity) => getEntityLabel(entity).toLowerCase().includes(normalizedQuery));
  ...
  React.useEffect(() => {
    if (!selectedFolderId || tree.byId[selectedFolderId]) return;
    setSelectedFolderId(null);
  }, [selectedFolderId, tree.byId]);

  const createScopedFolder = async (event) => {
      ...
      <header className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <h2 className="flex items-center gap-2 text-base font-bold text-[var(--va-text)]">
            ...
            {title || `مجلدات ${FOLDER_SCOPE_LABELS[scope] || "القسم"}`}
          </h2>
        ...
        <aside className="space-y-3 rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-3">
          <form onSubmit={createScopedFolder} className="flex gap-2">
            <input
              ...
              placeholder={selectedFolder ? `مجلد داخل ${selectedFolder.name}` : "مجلد جديد"}
            ...
            <>
              <div className="rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <h3 className="text-sm font-bold text-[var(--va-text)]">{selectedFolder.name || "مجلد"}</h3>
              ...
              <div className="grid gap-3 lg:grid-cols-2">
                <div className="rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-3">
                  ...
                  <div className="max-h-80 space-y-2 overflow-y-auto">
                    {assigned.length ? assigned.map((entity) => (
                ...
                <div className="rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-3">
                  ...
                  <div className="max-h-80 space-y-2 overflow-y-auto">
                    {available.length ? available.slice(0, 200).map((entity) => (
                      <button
                        key={entity.id}
                        type="button"
