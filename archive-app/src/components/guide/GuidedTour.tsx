...
export function GuidedTourController() {
  ...
  const itemCount = React.useMemo(
    () => videoItems.filter((item: any) => !item?.isDeleted).length,
    [videoItems]
  );
  ...
  const handleStepChange = React.useCallback((stepId: any) => {
    setCurrentStepId(stepId);
    updateSettings?.(getMarkStepSeenPatch(stepId, useAppStore.getState().settings || {}));
  }, [updateSettings]);

  const handleNavigate = React.useCallback((page: any) => {
    if (page) setCurrentPage?.(page);
  }, [setCurrentPage]);

  const endTour = React.useCallback((complete: any) => {
    setOpen(false);
    autoStartedRef.current = true;
