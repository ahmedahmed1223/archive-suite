...
function NoteRow({ note, canDelete, onSeek, onRemove }: any) {
...
export function ItemNotesPanel({
  ...
  return jsxs("section", {
    ...
    children: [
          ...
          canAnchorTime
            ? jsxs("label", { className: "inline-flex items-center gap-2 text-xs text-gray-300", children: [
                jsx("input", {
                  ...
                  onChange: (event: any) => setAnchorAtTime(event.target.checked),
                  className: "h-4 w-4"
                }),
      ...
      itemNotes.length
        ? jsx("ul", { className: "space-y-2", role: "list", children: itemNotes.map((note: any) => NoteRow({
            note,
