...
function sortedByTime(bookmarks: any) {
  return [...(bookmarks || [])].sort((a: any, b: any) => a.time - b.time);
}

/** Export bookmarks as a Markdown string. */
function toMarkdown(bookmarks: any, itemTitle: any) {
...
function downloadText(text: any, filename: any, mimeType: any) {
  const blob = new Blob([text], { type: mimeType });
  triggerDownload(blob, filename);
}

export function bookmarkTimeToSeconds(value: any) {
  const parts = String(value || "").trim().split(":").map(Number);
  if (!parts.length || parts.some((part: any) => !Number.isFinite(part) || part < 0)) return null;
...
function BookmarkEditor({ bookmark, onSave, onCancel }: any) {
  ...
  return (
    <div className="col-span-full w-full space-y-2" aria-label={`تحرير العلامة الزمنية: ${bookmark.title}`}>
      <div className="grid gap-2 sm:grid-cols-[110px_minmax(0,1fr)]">
        <label className="space-y-1 text-xs text-gray-400"><span>الوقت</span><input value={time} onChange={(event: any) => setTime(event.target.value)} className="input input-bordered input-sm w-full font-mono" dir="ltr" aria-label="وقت العلامة الزمنية" /></label>
        <label className="space-y-1 text-xs text-gray-400"><span>العنوان</span><input value={title} onChange={(event: any) => setTitle(event.target.value)} className="input input-bordered input-sm w-full" aria-label="عنوان العلامة الزمنية" /></label>
      </div>
      <label className="block space-y-1 text-xs text-gray-400"><span>الملاحظة</span><textarea value={note} onChange={(event: any) => setNote(event.target.value)} rows={3} className="textarea textarea-bordered w-full" aria-label="ملاحظة العلامة الزمنية" /></label>
 ...
 */
export function TimeBookmarkButton({ getTime, getSuggestion, onSave }: any) {
  ...
  const openForm = () => {
    ...
    window.requestAnimationFrame(() => (titleRef.current as any)?.focus());
  };
 ...
 */
export function TimeBookmarkList({ bookmarks = [], onSeek, onDelete, onUpdate, itemTitle = "" }: any) {
          ...
          jsxs("li", {
            ...
            children: [
              editingId === bm.id ? jsx(BookmarkEditor, {
                ...
                onSave: (changes: any) => { onUpdate?.(bm.id, changes); setEditingId(null); }
...
export function TimeBookmarkTimelineMarkers({ bookmarks = [], duration = 0, currentTime = 0, onSeek }: any) {
    ...
    children: [
      jsxs("div", {
        ...
        children: [
          ...
          markers.map((marker: any) => jsx("button", {
            type: "button",
