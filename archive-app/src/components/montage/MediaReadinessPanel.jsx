 ...
 */
export function MediaReadinessPanel({ project, items, onExport, onCancel }) {
  const projectItems = React.useMemo(() => {
    const ids = new Set([
      ...(project?.itemIds || []),
      ...(project?.roughCuts || []).map((c) => c.itemId).filter(Boolean)
    ]);
    return items.filter((item) => ids.has(item.id));
  }, [project, items]);

  const readinessResults = React.useMemo(
    () => projectItems.map((item) => ({ item, readiness: buildMediaReadiness(item) })),
    [projectItems]
  );
  ...
  const blockedItems = readinessResults.filter((r) => r.readiness.status === "blocked");
  const warningItems = readinessResults.filter((r) => r.readiness.status === "warning");
  const readyItems = readinessResults.filter((r) => r.readiness.status === "ready");
    ...
    >
      ...
      {(blockedItems.length > 0 || warningItems.length > 0) && (
        <ul className="max-h-48 space-y-2 overflow-y-auto" aria-label="قائمة مشاكل الوسائط">
          {[...blockedItems, ...warningItems].map(({ item, readiness }) => (
            <li
              key={item.id}
              ...
              <div className="min-w-0 flex-1">
                ...
                {readiness.missing.length > 0 && (
                  <p className="mt-0.5 text-[var(--va-text-muted)]">
                    ناقص: {readiness.missing.map((m) => m.label).join("، ")}
                  </p>
                )}
              </div>
            </li>
