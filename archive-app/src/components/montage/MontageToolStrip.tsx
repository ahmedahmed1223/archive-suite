...
function ToolButton({ label, icon: Icon, pressed, disabled, onClick }: any) {
  return (
...
export function MontageToolStrip({
  ...
  const updatePreferences = (patch: any) => onPreferencesChange?.({ ...preferences, ...patch });
  const chooseTool = (tool: any) => {
    setActiveTool(tool);
      ...
      <div className="montage-toolstrip__group" aria-label="سلوك الخط الزمني">
        ...
        <label className="montage-toolstrip__select-label">
          ...
          <select
            ...
            onChange={(event: any) => updatePreferences({ rippleMode: event.target.value })}
          >
