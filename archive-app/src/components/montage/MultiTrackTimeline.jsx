...
function DeleteTrackDialog({ track, tracks, clips, onClose, onCommand }) {
  const destinations = tracks.filter((candidate) => candidate.id !== track.id && candidate.type === track.type && !candidate.locked);
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label={`حذف مسار ${track.name}`} className="multitrack-delete-dialog">
      ...
      {destinations.length ? (
        <select className="select select-bordered select-xs" aria-label="المسار البديل" value={destinationTrackId} onChange={(event) => setDestinationTrackId(event.target.value)}>
...
export function MultiTrackTimeline({
  ...
  const duration = Math.max(10, ...clips.map((clip) => (Number(clip.timelineStartSec) || 0) + clipDuration(clip)), ...markers.map((marker) => Number(marker.atSec) || 0));
  ...
  const invalidClipIds = React.useMemo(() => new Set(collisions.flatMap((collision) => [collision.firstId, collision.secondId])), [collisions]);
  ...
  const requestDelete = (track) => {
    if (!clips.some((clip) => clip.trackId === track.id)) {
  ...
  return (
    <div className="multitrack-timeline" style={{ "--timeline-surface-width": `${surfaceWidth}px` }}>
      <div className="multitrack-timeline__toolbar" aria-label="إضافة مسارات">
        {TRACK_ACTIONS.map(({ type, label, icon: Icon }) => (
      ...
      <DndContext
        sensors={sensors}
        onDragStart={({ active }) => setActiveClip(clips.find((clip) => clip.id === active.data.current?.clipId) || null)}
      ...
      >
        <div className="multitrack-timeline__scroll">
          <div className="multitrack-timeline__matrix" style={{ minWidth: `${surfaceWidth + 188}px` }}>
            ...
            <div className="multitrack-ruler" style={{ width: `${surfaceWidth}px` }}>
              {ticks.map((tick) => <span key={tick} style={{ insetInlineStart: `${tick * scale}px` }}>{tick}s</span>)}
              {markers.map((marker) => <i key={marker.id} title={marker.label} style={{ insetInlineStart: `${marker.atSec * scale}px` }} />)}
            </div>
        ...
        <DragOverlay>{activeClip ? <div className="multitrack-drag-overlay">{activeClip.label || activeClip.itemId}</div> : null}</DragOverlay>
      </DndContext>
