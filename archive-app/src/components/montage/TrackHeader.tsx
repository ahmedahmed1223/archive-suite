...
function HeaderButton({ label, icon: Icon, active, disabled, onClick }: any) {
  return (
...
export function TrackHeader({ track, index, total, onCommand, onRequestDelete }: any) {
  const [name, setName] = React.useState(track.name);
  React.useEffect(() => setName(track.name), [track.name]);

  const patch = (nextPatch: any) => onCommand?.({ type: "track.patch", trackId: track.id, patch: nextPatch });
  const commitName = () => {
