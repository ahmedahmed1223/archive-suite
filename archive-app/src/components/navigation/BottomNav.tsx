...
export function BottomNav() {
  ...
  const setSelectedItemId = useAppStore((s: any) => s.setSelectedItemId);
  const setSidebarOpen     = useAppStore((s: any) => s.setSidebarOpen);
  ...
  const unreadInbox        = inboxItems.filter((i: any) => !i.readAt).length;
  const activeNotifs       = notifications.filter((n: any) => !n.dismissed).length;
  const badgeCounts        = { inbox: unreadInbox + activeNotifs };

  function navigate(pageId: any) {
  ...
  return jsx("nav", {
    ...
    children: [
      ...TABS.map(({ id, labelAr, Icon, badgeKey }: any) => {
      const active     = currentPage === id;
      const badgeCount = badgeKey ? ((badgeCounts as any)[badgeKey] || 0) : 0;
