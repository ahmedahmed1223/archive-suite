...
function getRoleMeta(roleId: any) {
...
function SidebarButton({ page, active, onClick, badge, collapsed }: any) {
  const Icon = (iconMap as any)[page.id] || Database;
  return jsxs("button", {
    type: "button",
    onClick,
...
export function Sidebar() {
  ...
  const activeCount = videoItems.filter((item: any) => !item.isDeleted).length;
  ...
  const permittedGroups = getSidebarNavigationGroups()
    .map((group: any) => ({
      ...group,
      pages: group.pages.filter((page: any) => {
        if (page.id === "detail") return false;
        const requiredAction = (SIDEBAR_PAGE_PERMISSIONS as any)[page.id];
        if (!requiredAction) return true;
        return canPerform(currentUser, requiredAction);
      })
    }))
    .filter((group: any) => group.pages.length > 0);
  const guidedGroups = React.useMemo(() => {
    const ids = permittedGroups.flatMap((group: any) => group.pages.map((page: any) => page.id));
    const _orderedIds: any[] = orderPageIdsForRoleProfile(ids as never[], roleProfileId);
    const order = new Map<any, number>(_orderedIds.map((id, index) => [id, index] as [any, number]));
    return permittedGroups
      .map((group: any) => ({
        ...group,
        pages: [...group.pages].sort((first: any, second: any) => (order.get(first.id) ?? 999) - (order.get(second.id) ?? 999))
      }))
      .sort((first: any, second: any) => {
        const firstRank = Math.min(...first.pages.map((page: any) => order.get(page.id) ?? 999));
        const secondRank = Math.min(...second.pages.map((page: any) => order.get(page.id) ?? 999));
  ...
  const availableIds = React.useMemo(
    () => guidedGroups.flatMap((group: any) => group.pages.map((page: any) => page.id)),
    [guidedGroups]
  );
  ...
  const setDrawerOpen = React.useCallback((open: any) => {
    if (setSidebarOpen) {
      setSidebarOpen(open);
      return;
    }
  ...
  const renderItem = (page: any, listIds: any) => {
          ...
          children: [
            ...
            jsxs("div", {
              ...
              children: [
                jsx("button", {
                  type: "button",
                  onClick: () => setDraftLayout((l: any) => setSidebarItemPinned(l, page.id, !page._pinned)),
                ...
                jsx("button", {
                  type: "button",
                  onClick: () => setDraftLayout((l: any) => setSidebarItemHidden(l, page.id, !page._hidden)),
                ...
                jsx("button", {
                  type: "button",
                  disabled: index <= 0,
                  onClick: () => setDraftLayout((l: any) => reorderSidebarItem(l, listIds, page.id, "up")),
                  title: "تحريك لأعلى",
                ...
                jsx("button", {
                  ...
                  onClick: () => setDraftLayout((l: any) => reorderSidebarItem(l, listIds, page.id, "down")),
                  title: "تحريك لأسفل",
        ...
        children: jsxs(Fragment, {
          children: [
            !editing && jsx(ContextualQuickActions, {
              currentPage,
              collapsed,
              onNavigate: (action: any) => goToPage(action.targetPage)
            }),
            ...
            shaped.pinned.length > 0 && jsx("section", {
              children: jsxs("ul", {
                ...
                children: [
                  ...
                  ...shaped.pinned.map((page: any) => renderItem(page, shaped.pinned.map((p: any) => p.id)))
                ]
              })
            }, "pinned"),
            ...shaped.groups.map((group: any) => jsx("section", {
              children: jsxs("ul", {
                className: "menu menu-sm p-0 w-full gap-0.5",
                children: [
                  !collapsed && jsx("li", {
                  ...
                  ...group.pages.map((page: any) => renderItem(page, group.pages.map((p: any) => p.id)))
                ]
              })
            }, group.id))
          ]
