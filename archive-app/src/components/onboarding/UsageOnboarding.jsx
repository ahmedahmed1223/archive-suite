...
function countDistinctTags(videoItems = []) {
  const seen = new Set();
...
export function UsageOnboarding({
  ...
  const steps = React.useMemo(
    () => computeUsageSteps({ itemCount, folderCount, tagCount }),
    [itemCount, folderCount, tagCount]
  );
  const progress = computeUsageProgress(steps);
  ...
  return jsxs("section", {
    ...
    children: [
      ...
      jsx("ul", {
        ...
        children: steps.map((step, index) => jsx(StepRow, { step, index, onNavigate }, step.id))
      })
    ]
  });
}
...
export function UsageOnboardingPanel() {
  ...
  const itemCount = videoItems.filter((item) => !item?.isDeleted).length;
  ...
  const handleNavigate = React.useCallback((pageId) => {
