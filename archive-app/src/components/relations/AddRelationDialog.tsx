...
export function AddRelationDialog({
  ...
  const existingTargetIds = React.useMemo(
    () => new Set(existingRelations.map((relation: any) => relation.targetId)),
    [existingRelations]
  );
  ...
  const candidates = React.useMemo(() => {
    ...
    return allItems
      .filter((item: any) => item && item.id !== sourceItem?.id && !item.isDeleted)
      .filter((item: any) => {
  ...
  const selectedTarget = allItems.find((item: any) => item.id === targetId) || null;
  ...
  const handleSubmit = (event: any) => {
    ...
    const duplicate = existingRelations.some(
      (relation: any) => relation.targetId === targetId && relation.type === type
    );
    if (duplicate) {
      setError("هذه العلاقة موجودة بالفعل");
      return;
    }
    const bidirectional = Boolean(RELATION_OPTIONS.find((opt: any) => opt.key === type)?.bidirectional);
      ...
      >
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4">
          ...
          <label className="block">
            ...
            >
              {RELATION_OPTIONS.map((option: any) => (
          ...
          {error ? (
            <p role="alert" className="alert alert-error block rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-300">
              {error}
            </p>
          ) : null}
        </div>
