...
function toElements(nodes = [], edges = [], centerId = null) {
  const elements = [];
 ...
 */
export function RelationsGraph({ graph, centerId, onNodeClick, className = "" }) {
    ...
    (async () => {
      try {
        ...
        const cy = cytoscape({
          ...
          elements: toElements(nodes, edges, centerId),
  ...
  const fitAll = () => cyRef.current?.fit(undefined, 20);
