...
function RelationRow({ relation, otherItem, fromSourcePerspective, onRemove }: any) {
...
function RelationSection({ title, icon, relations, resolveOther, fromSourcePerspective, onRemove, emptyHint }: any) {
  return (
...
export function RelationsPanel({
  ...
  const itemsById = React.useMemo(
    () => new Map(allItems.map((item: any) => [item.id, item])),
    [allItems]
  );
  ...
  const graph = React.useMemo(
    () => buildRelationsGraph(itemId, allRelations as never[], allItems as never[], 1),
      ...
      {isEmpty ? (
        ...
        <div className="space-y-5">
          <RelationsGraph
            graph={graph}
            centerId={itemId}
            onNodeClick={(id: any) => { if (id !== itemId) onNavigateToItem?.(id); }}
            className="w-full"
          />
          <RelationSection
            title="علاقات صادرة"
            ...
            relations={outgoing}
            resolveOther={(relation: any) => itemsById.get(relation.targetId)}
          ...
          <RelationSection
            ...
            relations={incoming}
            resolveOther={(relation: any) => itemsById.get(relation.sourceId)}
