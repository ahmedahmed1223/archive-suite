...
export function SaveSearchButton({ query = "", filters = {}, className = "" }) {
  ...
  const hasContent = normalizedQuery || Object.keys(filters || {}).some((k) => {
    const v = filters[k];
    return v !== undefined && v !== null && v !== "" && !(Array.isArray(v) && v.length === 0);
  });

  const existing = React.useMemo(
    () => savedSearches.find((s) => s.query === normalizedQuery),
