...
export function ApiKeysSettings({ baseUrl = "", fetchImpl }: any) {
  ...
  const [createdKey, setCreatedKey] = useState<any>(null); // one-time plaintext reveal
  ...
  const toggleScope = (id: any, checked: any) => {
    setScopes((prev: any) => (checked ? [...new Set([...prev, id])] : prev.filter((s: any) => s !== id)));
  };
  ...
  const handleRevoke = useCallback(async (id: any) => {
    try {
