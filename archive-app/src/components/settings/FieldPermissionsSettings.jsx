...
function isRestricted(acl, key) {
  const allowed = acl?.[key];
  return Array.isArray(allowed) && allowed.length > 0;
}

function hasRole(acl, key, role) {
  const allowed = acl?.[key];
  return Array.isArray(allowed) && allowed.includes(role);
}

function toggleRole(acl = {}, key, role) {
...
function removeRestriction(acl = {}, key) {
  const next = { ...acl };
  delete next[key];
  return next;
}
...
export function FieldPermissionsSettings() {
  ...
  const getDraft = useCallback(
    (type) => drafts[type.id] ?? type.fieldAcl ?? {},
    [drafts]
  );

  const patch = useCallback((typeId, updater) => {
    setDrafts((prev) => ({
      ...prev,
      [typeId]: updater(prev[typeId] ?? contentTypes.find((t) => t.id === typeId)?.fieldAcl ?? {}),
    }));
  }, [contentTypes]);

  const save = useCallback(async (type) => {
    const acl = drafts[type.id] ?? type.fieldAcl ?? {};
    setSaving((p) => ({ ...p, [type.id]: true }));
    try {
      await updateContentType({ ...type, fieldAcl: acl });
      setSaved((p) => ({ ...p, [type.id]: true }));
      setTimeout(() => setSaved((p) => ({ ...p, [type.id]: false })), 2000);
    } finally {
      setSaving((p) => ({ ...p, [type.id]: false }));
    }
  }, [updateContentType, drafts]);

  const activeTypes = contentTypes.filter(
    (t) => t.status !== "archived" && Array.isArray(t.fields) && t.fields.length > 0
  );
        ...
        return (
          <section key={type.id} className="rounded-xl border border-white/10 bg-gray-950/30">
            ...
            <div className="flex items-center justify-between gap-3 border-b border-white/5 px-4 py-3">
              ...
              <button
                type="button"
                disabled={!isDirty || saving[type.id]}
                onClick={() => save(type)}
              ...
              >
                {saving[type.id] ? "جاري الحفظ…" : saved[type.id] ? "✓ تم الحفظ" : "حفظ"}
              </button>
            </div>
                    ...
                    {ROLES.map((role) => (
                      ...
                      >
                        <input
                          ...
                          onChange={() => patch(type.id, (a) => toggleRole(a, key, role.id))}
