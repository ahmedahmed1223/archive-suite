...
async function apiFetch(path: any, options: any = {}) {
...
export function TwoFactorSettings() {
  ...
  const isEnabled = !!(profile as any)?.totpEnabled;
        ...
        <div>
          ...
          {isEnabled && (
            <p className="mt-0.5 text-xs text-gray-400">
              رموز الاسترداد المتبقية: {(profile as any).totpRecoveryCodesRemaining ?? 0}
            </p>
          )}
        </div>
      </div>
        ...
        <div className="space-y-4 rounded-xl border border-white/10 bg-white/[0.03] p-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <input
              ...
              placeholder="000000"
              value={verifyCode}
              onChange={(e: any) => setVerifyCode(e.target.value.replace(/\D/g, ""))}
