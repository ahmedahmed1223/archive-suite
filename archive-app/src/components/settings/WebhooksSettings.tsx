...
export function WebhooksSettings() {
  ...
  const [events, setEvents] = useState(ALL_EVENTS.map((e: any) => e.id));
  ...
  async function handleCreate(e: any) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        setShowForm(false);
        setUrl("");
        setEvents(ALL_EVENTS.map((ev: any) => ev.id));
        fetchHooks();
      } else {
  ...
  function toggleEvent(eventId: any, checked: any) {
    setEvents((prev: any) =>
      checked ? [...prev, eventId] : prev.filter((x: any) => x !== eventId)
    );
  }
            ...
            {ALL_EVENTS.map((ev: any) => (
              <label key={ev.id} className="flex items-center gap-1.5 text-xs cursor-pointer">
                <input
                  ...
                  onChange={(e: any) => toggleEvent(ev.id, e.target.checked)}
                />
                {ev.label}
              </label>
            ))}
