...
type SyncStatus = keyof typeof BADGE_CONFIG;

export default function PerItemSyncBadge({ status }: { status?: string }) {
  const config = BADGE_CONFIG[status as SyncStatus] ?? BADGE_CONFIG.local;
  return (
