...
function buildContext(counter: any, lastItem: any) {
  return {
    counter,
    lastValues: lastItem
      ? {
 ...
 */
export function QuickAddBar({ initialTemplate = null, contentTypes = [], defaultTypeId = "", onDone, onClose }: any) {
  const addVideoItem = useAppStore((state: any) => state.addVideoItem);
  ...
  const allTags = useAppStore((state: any) => {
  ...
  const [recentItems, setRecentItems] = React.useState([] as any[]);
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      const tags = parseVideoTags(tagsText as any);
        ...
        children: [
          ...
          jsx(AnimatePresence, {
            children: showRecent && jsx(motion.ul, {
              ...
              children: recentItems.map((item: any, i: any) =>
                jsxs("li", {
