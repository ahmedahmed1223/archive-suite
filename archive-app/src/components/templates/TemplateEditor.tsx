 ...
 */
export function TemplateEditor({ isOpen, template, onClose, onSaved }: any) {
  const createTemplate = useAppStore((state: any) => state.createTemplate);
  const updateTemplate = useAppStore((state: any) => state.updateTemplate);
  const deleteTemplate = useAppStore((state: any) => state.deleteTemplate);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const tags = tagsRaw.split(/[,،]/).map((t: any) => t.trim()).filter(Boolean);
      const fields = {};
      if (typeField.trim()) (fields as any).type = typeField.trim();
      if (tags.length) (fields as any).tags = tags;
      if (notesField.trim()) (fields as any).notes = notesField.trim();

      const dynamicFields = {};
      if (titleFormula.trim()) (dynamicFields as any).title = titleFormula.trim();
              ...
              <div className="space-y-3 rounded-xl border border-white/5 bg-gray-800/30 p-4">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="space-y-1.5">
                    ...
                    <input id="template-type" type="text" value={typeField}
                      onChange={(e: any) => setTypeField(e.target.value)}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input id="template-tags" type="text" value={tagsRaw}
                      onChange={(e: any) => setTagsRaw(e.target.value)}
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea id="template-notes" value={notesField}
                    onChange={(e: any) => setNotesField(e.target.value)}
              ...
              <div className="space-y-3 rounded-xl border border-white/5 bg-gray-800/30 p-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input id="template-title-formula" type="text" value={titleFormula}
                    onChange={(e: any) => setTitleFormula(e.target.value)}
