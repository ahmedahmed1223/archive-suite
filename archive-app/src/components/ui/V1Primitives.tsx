...
export function SaveIndicator({ state = "idle", message, onRetry, className = "" }: any) {
...
function renderWorkflowIcon(icon: any) {
  if (!icon) return null;
...
export function StatusBadge({ tone = "slate", children, className = "" }: any) {
  return (
    <span className={cx("badge badge-sm badge-soft va-status-badge inline-flex min-h-7 items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium", (toneClasses as any)[tone] || toneClasses.slate, className)}>
      {children}
    </span>
  );
}

export function PageShell({ children, className = "", as: Component = "div", ...props }: any) {
  return (
...
export function MotionPage({ children, className = "", ...props }: any) {
...
export function PageHero({ icon, title, description, actions, children, className = "", compact = false }: any) {
  return (
...
export function Stepper({ steps, activeStepId, className = "" }: any) {
  const activeIndex = Math.max(0, steps.findIndex((step: any) => step.id === activeStepId));
  return (
...
export function MetricCard({ label, value, hint, icon, tone = "accent", className = "" }: any) {
  return (
    <section className={cx("card card-border va-metric-card rounded-xl border p-4 text-right", className)} dir="rtl">
      <div className="flex items-start justify-between gap-3">
        ...
        {icon && (
          <span className={cx("va-icon-tile flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border", (toneClasses as any)[tone] || toneClasses.accent)}>
            {icon}
          </span>
        )}
      </div>
...
export function ActionCard({ label, detail, icon, onClick, tone = "accent", className = "" }: any) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      {icon && (
        <span className={cx("va-icon-tile flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border", (toneClasses as any)[tone] || toneClasses.accent)}>
          {icon}
        </span>
      )}
...
export function FormSection({ title, description, icon, actions, children, className = "" }: any) {
  return (
...
export function SectionToolbar({ title, description, icon, actions, children, className = "" }: any) {
  return (
...
export function CommandPanel({ title, description, icon, actions, children, highlight = false, className = "" }: any) {
  return (
    <section
      className={cx(
...
export function ReportStrip({ items = [], className = "" }: any) {
    ...
    >
      {items.map((item: any) => (
        <div key={item.id || item.label} className="stat px-4 py-3">
          {item.icon && (
            <div className={cx("stat-figure va-icon-tile flex h-9 w-9 items-center justify-center rounded-xl border", (toneClasses as any)[item.tone] || toneClasses.accent)}>
              {item.icon}
            </div>
          )}
...
export function QuickActionGrid({ actions = [], className = "" }: any) {
  return (
    <div className={cx("grid gap-2 sm:grid-cols-2", className)} dir="rtl">
      {actions.map((action: any) => (
        ...
        >
          {action.icon && (
            <span className={cx("va-icon-tile flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border", (toneClasses as any)[action.tone] || toneClasses.accent)}>
              {action.icon}
            </span>
          )}
          <span className="min-w-0">
...
export function ResultPreview({ title, meta, icon, actions, children, onClick, className = "" }: any) {
...
export function RiskActionPanel({ title, description, icon, actions, children, className = "" }: any) {
  return (
...
export function SkeletonBlock({ className = "" }: any) {
...
export function FloatingActionBar({ children, className = "", label = "إجراءات سريعة" }: any) {
  return (
...
export function InsightPanel({ icon, title, description, actions, children, tone = "accent", className = "" }: any) {
  return (
    <section className={cx("card card-border va-insight-panel rounded-2xl border p-4 text-right", `va-insight-panel-${tone}`, className)} dir="rtl">
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-3">
          {icon && <span className={cx("va-icon-tile flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border", (toneClasses as any)[tone] || toneClasses.accent)}>{icon}</span>}
          <div className="min-w-0">
...
export function UXEmptyState({ icon, title, description, actions, className = "" }: any) {
  return (
...
export function ResponsiveTabs({ tabs, activeTab, onChange, ariaLabel = "تبويبات", className = "" }: any) {
  return (
...
export function EntityCard({ title, description, icon, meta, actions, selected = false, onClick, children, className = "" }: any) {
  const Wrapper = onClick ? "button" : "article";
  return (
    <Wrapper
      type={onClick ? "button" : undefined}
