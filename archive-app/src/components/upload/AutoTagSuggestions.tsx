...
export function AutoTagSuggestions({ name = "", summary = "", categories = [], onAccept, authToken }: any) {
  ...
  const accept = (tag: any) => {
    onAccept?.(tag);
    setTags((prev: any) => prev.filter((t: any) => t !== tag));
  };

  const dismiss = (tag: any) => setTags((prev: any) => prev.filter((t: any) => t !== tag));
  ...
  return (
    <div className="mt-2 p-3 rounded-lg bg-gray-800/50 border border-gray-700/60">
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        {tags.length > 1 && (
          <button
            onClick={() => { tags.forEach((t: any) => onAccept?.(t)); setTags([]); }}
