...
function UploadRow({ item, onRemove, onRetry }: any) {
  const tone = (STATUS_TONE as any)[item.status] || "text-[var(--va-text-2)]";
          ...
          jsxs("div", {
            ...
            children: [
              ...
              jsxs("p", {
                ...
                children: [(STATUS_LABEL as any)[item.status] || item.status, " · ", formatBytes(item.size)]
              })
            ]
          }),
          jsxs("div", {
...
export function UploadQueue() {
  ...
  const removeUpload = useAppStore((state: any) => state.removeUpload);
  const retryUpload = useAppStore((state: any) => state.retryUpload);
  const clearFinishedUploads = useAppStore((state: any) => state.clearFinishedUploads);

  if (!uploads || uploads.length === 0) return null;

  const overall = selectUploadProgress(uploads);
  const active = uploads.filter((u: any) => u.status !== "done" && u.status !== "duplicate").length;
