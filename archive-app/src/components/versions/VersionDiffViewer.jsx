...
export function VersionDiffViewer({
  ...
  const visible = showUnchanged ? rows : rows.filter((r) => r.type !== "same");
    ...
    children: [
      ...
      visible.length > 0 && jsx("div", {
        ...
        children: jsx("table", {
          ...
          children: jsxs("tbody", {
            children: visible.map(({ key, bStr, aStr, type }) => {
              const Icon  = TYPE_ICONS[type];
              const color = TYPE_COLORS[type];
              return jsxs("tr", {
                className: `border-b border-white/5 last:border-0 ${ROW_STYLES[type]}`,
                children: [
                  jsx("td", {
