 ...
 */
export function StatusTransitionMenu({ item, role = "viewer", store = "video_items", baseUrl = "", onChanged, fetchImpl }: any) {
  ...
  const pendingOption = pendingTo ? options.find((o: any) => o.to === pendingTo) : null;

  useEffect(() => {
    if (!open) return undefined;
    const onOutside = (event: any) => {
      if (menuRef.current && !(menuRef.current as any).contains(event.target)) setOpen(false);
    };
  ...
  const transition = useCallback(async (to: any, opts: any = {}) => {
    setBusy(true);
    setError("");
    ...
    try {
      ...
      if (opts.note?.trim()) (body as any).note = opts.note.trim();
      if (opts.dueDate) (body as any).dueDate = opts.dueDate;
