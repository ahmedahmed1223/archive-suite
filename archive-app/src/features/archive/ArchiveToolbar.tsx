...
export function ArchiveSortMenu({ sortField = "updatedAt", sortDirection = "desc", onChange }: any) {
  ...
  React.useEffect(() => {
    ...
    const handler = (event: any) => {
      if (!(menuRef.current as any)?.contains(event.target) && !(buttonRef.current as any)?.contains(event.target)) {
        setOpen(false);
      }
    };
  ...
  const activeOption = ARCHIVE_SORT_OPTIONS.find((option: any) => option.value === sortField) || ARCHIVE_SORT_OPTIONS[0];
    ...
    children: [
      ...
      open && jsxs("div", {
        ...
        children: [
          ...
          ...ARCHIVE_SORT_OPTIONS.map((option: any) => jsxs("button", {
            type: "button",
            role: "menuitemradio",
...
function FilterChip({ icon, label, onRemove }: any) {
  return jsxs("span", {
