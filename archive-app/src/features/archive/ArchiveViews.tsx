...
export function ViewModeSwitch({ value = "grid", onChange, className = "" }: any) {
  return jsx("div", {
    ...
    children: ARCHIVE_VIEW_MODES.map(({ id, label, Icon }: any) => jsx("button", {
      type: "button",
      onClick: () => onChange?.(id),
 ...
 */
export function ItemSizeSlider({ value = "comfortable", onChange, className = "" }: any) {
  const found = ARCHIVE_ITEM_SIZE_OPTIONS.findIndex((option: any) => option.value === value);
  ...
  return jsxs("label", {
    ...
    children: [
      ...
      jsx("input", {
        ...
        step: 1,
        value: idx,
        onChange: (event: any) => onChange?.(ARCHIVE_ITEM_SIZE_OPTIONS[Number(event.target.value)]?.value || "comfortable"),
 ...
 */
export function GridDensitySlider({ gridColumns, gridColumnCount, onChange, className = "" }: any) {
...
function ItemBadges({ item, compact = false, completeness = null }: any) {
  const reviewStatus = item.metadata?.reviewStatus;
  const rating = Number(item.metadata?.rating) || 0;
  const statusClass = reviewStatus ? (REVIEW_STATUS_CLASS as any)[reviewStatus] : null;
  const tier = completeness ? (COMPLETENESS_TIERS as any)[completeness.tier] : null;
  return jsxs("div", {
...
function BulkCheckbox({ checked, onToggle, label }: any) {
  return jsx("button", {
    type: "button",
    role: "checkbox",
...
export function getGridStyleForColumns(gridColumns: any) {
...
function getArchivePaginationSlots(currentPage: any, totalPages: any) {
  if (totalPages <= 7) return Array.from({ length: totalPages }, (_: any, index: any) => index + 1);
...
export function ToolbarButton({ children, onClick, active = false, danger = false, icon }: any) {
  return jsxs("button", {
    type: "button",
    onClick,
    "aria-pressed": active,
...
export function ArchiveMetric({ label, value, hint }: any) {
  return jsxs("div", {
...
export function ArchivePagination({ currentPage, totalPages, onPageChange }: any) {
  ...
  return jsxs("nav", {
    ...
    children: [
      ...
      jsx("div", {
        ...
        children: pageSlots.map((slot: any) => typeof slot === "number" ? jsx("button", {
          type: "button",
...
function getContentKind(item: any) {
...
function getArchiveFileMeta(item: any = {}) {
...
function FileMetaStrip({ item, compact = false }: any) {
  const file = getArchiveFileMeta(item);
...
function encodePreviewPathSegments(path: any) {
  return path
    .split("/")
...
function getBrowserPreviewSource(path: any = "", runtimeProtocol: any = "") {
...
function getDocumentMimeType(item: any = {}, file: any = getArchiveFileMeta(item)) {
  ...
  return (DOCUMENT_MIME_BY_EXTENSION as any)[file.extension] || "application/octet-stream";
}

function renderPreviewUnavailable({ descriptor, compact = false }: any) {
  return jsxs("div", {
    ...
    children: [
      ...
      jsx("span", {
        ...
        children: (PREVIEW_STATUS_LABELS as any)[descriptor?.status] || "لا توجد معاينة HTML5 لهذا المسار"
      })
    ]
  });
}

function renderPreviewMedia({ item, compact = false, allowDocuments = false }: any) {
  const file = getArchiveFileMeta(item);
  const runtimeProtocol = typeof window !== "undefined" ? window.location.protocol : "";
...
export function SegmentedControl({ label, value, options, onChange }: any) {
  return jsxs("div", {
    ...
    children: [
      ...
      jsx("div", {
        ...
        role: "group",
        "aria-label": label,
        children: options.map((option: any) => jsxs("button", {
          type: "button",
...
export function VideoCard({ item, typeLabel, subtypeLabel, selected, onPreview, onOpen, onFavorite, onDelete, onRestore, showDeleted, itemSize = "comfortable", bulkMode = false, bulkSelected = false, onBulkToggle, onContextMenu, completeness = null }: any) {
  const size = (ARCHIVE_CARD_SIZE as any)[itemSize] || ARCHIVE_CARD_SIZE.comfortable;
          ...
          jsxs("div", {
            ...
            children: [
              ...
              item.tags?.length > 0 && jsxs("div", {
                ...
                children: item.tags.slice(0, size.tags).map((tag: any) => jsx("span", {
...
export function AnimatedItem({ index, children, as = "div", className = "", itemId, disableMotion = false, ...rest }: any) {
  const shouldDisableMotion = disableMotion || index >= 24;
  const Component = shouldDisableMotion ? as : (motion as any)[as] || motion.div;
 ...
 */
export function VideoTileItem({ item, typeLabel, subtypeLabel, selected, onPreview, onOpen, onFavorite, onDelete, onRestore, showDeleted, itemSize = "comfortable", bulkMode = false, bulkSelected = false, onBulkToggle, onContextMenu, completeness = null }: any) {
  const highlight = bulkMode ? bulkSelected : selected;
  // §19.8 — single click previews (desktop), double-click opens; touch tap opens.
  const activation = buildCardActivationHandlers({ bulkMode, onSelect: onPreview, onOpen, onToggle: onBulkToggle });
      ...
      jsxs("button", {
        ...
        children: [
          ...
          item.tags?.length > 0 && jsx("div", {
            ...
            children: item.tags.slice(0, tagLimit).map((tag: any) => jsx("span", {
...
export function VideoListItem({ item, typeLabel, subtypeLabel, selected, onPreview, onOpen, onFavorite, onDelete, onRestore, showDeleted, itemSize = "comfortable", bulkMode = false, bulkSelected = false, onBulkToggle, onContextMenu, completeness = null }: any) {
  const size = (ARCHIVE_LIST_SIZE as any)[itemSize] || ARCHIVE_LIST_SIZE.comfortable;
  const highlight = bulkMode ? bulkSelected : selected;
  // §19.8 — single click previews (desktop), double-click opens; touch tap opens.
  const activation = buildCardActivationHandlers({ bulkMode, onSelect: onPreview, onOpen, onToggle: onBulkToggle });
...
function getColumnLabel(column: any) {
  return column?.label || (COLUMN_LABELS as any)[column?.id] || column?.id || "";
}

function getMetadataColumnKey(column: any) {
...
function normalizeInlineFieldType(fieldType: any) {
  const value = String(fieldType || "text");
  return ["text", "tags", "number", "date", "select"].includes(value) ? value : "text";
}

function normalizeInlineOptions(options: any = []) {
  return options.map((option: any) => typeof option === "string" ? { value: option, label: option } : option);
}

function isInlineEditableTableColumn(column: any, typeOptions: any = []) {
...
function getAdjacentInlineEditableColumnId(columns: any, typeOptions: any, currentColumnId: any, direction: any) {
  const editableColumnIds = columns
    .filter((column: any) => isInlineEditableTableColumn(column, typeOptions))
...
function getFileSizeForItem(item: any) {
  const file = normalizeLocalFileValue(item?.localFile);
  return (file as any)?.size > 0 ? formatFileSize((file as any).size) : null;
}
...
function valuesEqual(a: any, b: any) {
  if (Array.isArray(a) || Array.isArray(b)) {
    ...
    return left.length === right.length && left.every((value: any, index: any) => value === right[index]);
  }
  return a === b;
}

function renderTableCell({ column, item, size, showDeleted, bulkMode, onPreview, onOpen, onFavorite, onDelete, onRestore, onBulkToggle, typeLabel, subtypeLabel, typeOptions, editingCell, onStartCellEdit, onCommitCellEdit, onCancelCellEdit }: any) {
  const isEditingCell = (columnId: any) => editingCell?.itemId === item.id && editingCell?.columnId === columnId;
      ...
      if (isEditingCell("tags")) {
        return jsx("td", {
          ...
          children: jsx(InlineCellEditor, {
            ...
            onSave: (nextTags: any, meta: any = {}) => {
              const current = item.tags || [];
              const unchanged = nextTags.length === current.length && nextTags.every((tag: any, index: any) => tag === current[index]);
...
function ResizableHeader({ column, label, cellClass, onResize }: any) {
  ...
  const handleMouseDown = React.useCallback((event: any) => {
    ...
    const handleMove = (moveEvent: any) => {
      ...
      const delta = (state as any).startX - moveEvent.clientX;
      const next = Math.max(60, (state as any).startWidth + delta);
...
export function VideoTableView({ items, previewItem, typeLabel, subtypeLabel, typeOptions, showDeleted, onPreview, onOpen, onFavorite, onDelete, onRestore, itemSize = "comfortable", bulkMode = false, isSelected, onBulkToggle, allSelected, onSelectAll, columns, onColumnResize, disableRowMotion = false, onCellSave }: any) {
  const size = (ARCHIVE_TABLE_SIZE as any)[itemSize] || ARCHIVE_TABLE_SIZE.comfortable;
  const visibleColumns = React.useMemo(
    () => (columns && columns.length ? columns : DEFAULT_TABLE_COLUMNS).filter((column: any) => column.visible !== false),
    [columns]
  );
  // §13.3 inline cell editing — one cell at a time ({ itemId, columnId }).
  const [editingCell, setEditingCell] = React.useState<any>(null);
  const handleStartCellEdit = React.useCallback((itemId: any, columnId: any) => {
    setEditingCell({ itemId, columnId });
  }, []);
  const handleCancelCellEdit = React.useCallback(() => setEditingCell(null), []);
  const handleCommitCellEdit = React.useCallback((item: any, patch: any, meta: any = {}) => {
      ...
      children: jsxs("table", {
        ...
        children: [
          jsx("thead", {
            ...
            children: jsxs("tr", {
              children: [
                ...
                ...visibleColumns.map((column: any) => jsx(ResizableHeader, {
                  column,
          ...
          jsx("tbody", {
            ...
            children: items.map((item: any, index: any) => {
              ...
              return jsxs(RowComponent, {
                ...
                children: [
                  ...
                  ...visibleColumns.map((column: any) => renderTableCell({
...
export function PreviewPanel({
  ...
  React.useEffect(() => {
    ...
    const focusTimer = window.setTimeout(() => (closeFullscreenButtonRef.current as any)?.focus(), 0);

    const handlePreviewKeyDown = (event: any) => {
      if (event.key === "Escape") {
        event.preventDefault();
  ...
  const availableCollections = collections.filter((collection: any) => !collection.itemIds?.includes(item.id));
  const projectCount = projects.filter((project: any) => project.status !== "archived").length;

  const actionButton = (props: any) => jsxs("button", {
    type: "button",
    ...
    children: [
      ...
      jsxs("div", {
        ...
        children: [
          ...
          item.tags?.length > 0 ? jsx("div", {
            className: "mt-2 flex flex-wrap gap-1.5",
            children: item.tags.slice(0, 8).map((tag: any) => jsx("span", {
              className: "va-chip rounded-full border border-white/5 bg-gray-950/45 px-2 py-0.5 text-xs text-gray-400",
              children: tag
            }, tag))
            ...
            children: [
              ...
              jsxs("select", {
                ...
                children: [
                  ...
                  ...availableCollections.map((collection: any) => jsx("option", { value: collection.id, children: collection.name || collection.id }, collection.id))
                ]
              })
            ]
          }),
