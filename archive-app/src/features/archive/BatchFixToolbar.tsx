...
function FixDropdown({ label, value, onChange, options, placeholder }: any) {
...
export function BatchFixToolbar({
  ...
  const statusOptions = WORKFLOW_STATES.map((s: any) => ({
    value: s,
    label: (STATE_META as any)[s]?.label || s,
  }));

  const typeOptions = contentTypes.map((ct: any) => ({
    value: ct.id,
    label: ct.name,
  }));
  ...
  const handleApply = async () => {
    ...
    const patch = {};
    if (batchStatus) (patch as any).workflowStatus = batchStatus;
    if (batchType) { (patch as any).type = batchType; (patch as any).subtype = ""; }
    if (batchBranch) (patch as any).branch = batchBranch;

    const selected = videoItems.filter((item: any) => selectedItems.includes(item.id));
