...
export function BulkActionBar({
  ...
  const [openMenu, setOpenMenu] = React.useState<string | null>(null); // "tags" | "collection" | "type" | "project" | null
  ...
  const handleTagsClick = async () => {
    ...
    if (raw === null) return;
    const tags = String(raw).split(/[،,]+/).map((value: any) => value.trim()).filter(Boolean);
  ...
  const handleCollectionClick = () => {
    ...
    setOpenMenu((current: any) => (current === "collection" ? null : "collection"));
  };
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {typeof onSetType === "function" && contentTypes.length > 0 && (
              <div className="relative">
                <button
                  type="button"
                  onClick={() => setOpenMenu((current: any) => (current === "type" ? null : "type"))}
                  disabled={busy}
                  className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs font-semibold text-gray-200 hover:bg-white/[0.08] disabled:opacity-50"
                >
            ...
            {typeof onSetProject === "function" && projects.length > 0 && (
              <div className="relative">
                <button
                  type="button"
                  onClick={() => setOpenMenu((current: any) => (current === "project" ? null : "project"))}
                  disabled={busy}
                  className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs font-semibold text-gray-200 hover:bg-white/[0.08] disabled:opacity-50"
                >
