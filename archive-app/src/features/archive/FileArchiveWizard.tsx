...
function getWizardButtonClass(tone: any = "neutral", disabled: any = false) {
...
function WizardButton({ children, onClick, disabled = false, tone = "neutral", icon }: any) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function FileChoiceLabel({ children, htmlFor, tone = "neutral", icon }: any) {
  return (
...
export function FileArchiveWizard({
  ...
  const firstType = contentTypes.find((type: any) => type.status !== "archived") || contentTypes[0] || null;
  ...
  React.useEffect(() => {
    ...
    if (stored.length) {
      setRows(stored.map((entry: any, index: any) => {
  ...
  const duplicateCount = rows.filter((row: any) => row.duplicate).length;
  const totalSize = selectedRows.reduce((sum: any, row: any) => sum + (row.file?.size || 0), 0);

  const toggleRow = (rowId: any) => {
    setRows((current: any) => current.map((row: any) => row.id === rowId ? { ...row, selected: !row.selected } : row));
  };
  ...
  const createItems = async () => {
    ...
    try {
      for (const row of selectedRows) {
        const item: any = createImportedVideoItem(row, { typeId, subtypeId, notes });
        addedIds.push(item.id);
        const savedItem = await addVideoItem?.(item);
        if (!(row as any).stored) {
          useAppStore.getState().enqueueUploads?.([(row as any).file], {
        ...
        <div className="grid gap-5 p-5 md:grid-cols-[1fr_320px]">
          <main className="space-y-4">
            ...
            >
              ...
              <input id={folderInputId} type="file" aria-label="اختيار مجلد فيديو للاستيراد" multiple {...{ webkitdirectory: "", directory: "" } as any} onChange={handleFileInputChange} style={{ position: "absolute", width: 1, height: 1, opacity: 0, overflow: "hidden" }} />
            </div>
          ...
          <aside className="h-fit space-y-4 rounded-3xl border border-white/10 bg-white/[0.03] p-4 lg:sticky lg:top-5">
            ...
            <label className="block text-sm text-slate-300">
              النوع
              <select value={typeId} onChange={(event: any) => setTypeId(event.target.value)} className="select select-bordered w-full">
                <option value="">بدون نوع</option>
                {contentTypes.filter((type: any) => type.status !== "archived").map((type: any) => (
            ...
            <label className="block text-sm text-slate-300">
              الفرع
              <select value={subtypeId} onChange={(event: any) => setSubtypeId(event.target.value)} disabled={!subtypes.length} className="select select-bordered w-full">
            ...
            <label className="block text-sm text-slate-300">
              ملاحظة مشتركة
              <textarea value={notes} onChange={(event: any) => setNotes(event.target.value)} rows={4} className="textarea textarea-bordered w-full" placeholder="اختياري" />
            </label>
