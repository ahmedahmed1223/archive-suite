const HTML5_VIDEO_EXTENSIONS = new Set<string>([
  ".mp4",
  ".webm",
  ".ogg",
  ".ogv",
...
function isHttpRuntime(protocol = ""): boolean {
  return /^https?:$/i.test(String(protocol || ""));
}

export function isLocalFilePath(path = ""): boolean {
  const value = String(path || "").trim();
  if (!value) return false;
  return /^(file:|[a-z]:[\\/]|\\\\|\/)/i.test(value);
}

function encodeFilePathSegments(path: string): string {
  return path
    .split("/")
...
function encodeWindowsFilePath(path: string): string {
  const drive = path.slice(0, 2);
  const rest = path.slice(3);
  return rest ? `${drive}/${encodeFilePathSegments(rest)}` : `${drive}/`;
}

export function isHtml5PreviewableVideo(path = ""): boolean {
  return HTML5_VIDEO_EXTENSIONS.has(getExtension(path));
}

export function getHtml5VideoPreviewSource(path = ""): string | null {
  const value = String(path || "").trim();
...
export function getMediaPreviewDescriptor(path = "", options: { runtimeProtocol?: string } = {}): MediaPreviewDescriptor {
  const value = String(path || "").trim();
