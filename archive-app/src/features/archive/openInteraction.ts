...
export interface ActivationHandlers {
  onClick: () => void;
  onDoubleClick: () => void;
  onPointerUp: (event?: { pointerType?: string | null }) => void;
}
 ...
 */
export function resolveTapAction({ pointerType, bulkMode = false }: { pointerType?: string; bulkMode?: boolean } = {}): "open" | "select" | "toggle" {
 ...
 * @returns {boolean}
 */
export function shouldOpenOnDoubleClick({ pointerType, bulkMode = false }: { pointerType?: string; bulkMode?: boolean } = {}): boolean {
...
export function buildCardActivationHandlers({ bulkMode = false, onSelect, onOpen, onToggle }: {
  ...
  const onPointerUp = (event?: { pointerType?: string | null }) => {
