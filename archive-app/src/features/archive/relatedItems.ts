 ...
 */
export function getRelatedItems(item: RelatedItemLike | null | undefined, allItems: RelatedItemLike[] = [], { limit = 6 }: { limit?: number } = {}): RelatedItemResult[] {
  if (!item) return [];
  const focusTags = new Map<string, unknown>();
  for (const tag of item.tags || []) {
    const key = normalizeTag(tag);
    if (key) focusTags.set(key, tag);
  }

  const scored: Array<Omit<RelatedItemResult, "percent">> = [];
  for (const other of allItems) {
    if (!other || other.id === item.id || other.isDeleted) continue;

    const sharedTags: string[] = [];
    for (const tag of other.tags || []) {
      if (focusTags.has(normalizeTag(tag))) sharedTags.push(String(tag));
    }
...
function hasSource(item: RelatedItemLike = {}): boolean {
  const metadata = item.metadata || {};
  const media = metadata.media as { sourceKey?: unknown } | undefined;
  return Boolean(
    item.path ||
    item.filePath ||
    item.url ||
...
function collectPeerTags(item: RelatedItemLike, relatedItems: RelatedItemResult[] = [], limit = 4): string[] {
  ...
  for (const related of relatedItems) {
    for (const tag of related.item?.tags || []) {
      ...
      const current = counts.get(key) || { tag: String(tag), count: 0, score: 0 };
      current.count += 1;
...
export function getItemImprovementSuggestions(item: RelatedItemLike | null | undefined, allItems: RelatedItemLike[] = [], {
  relatedItems = getRelatedItems(item, allItems, { limit: 6 }),
  explicitRelationIds = [],
  limit = 4
}: {
  relatedItems?: RelatedItemResult[];
  explicitRelationIds?: Iterable<string> | Array<string>;
  limit?: number;
...
export function getArchiveImprovementSuggestions(items: RelatedItemLike[] = [], { contentTypes = [], limit = 4 }: { contentTypes?: Array<{ status?: string }>; limit?: number } = {}): ImprovementSuggestion[] {
