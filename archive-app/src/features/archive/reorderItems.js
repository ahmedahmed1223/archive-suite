 ...
 * @returns {string[]}
 */
export function buildCustomOrderIds(items, fromId, toId) {
 ...
 * @returns {T[] | ReadonlyArray<T>}
 */
export function applyCustomOrder(items, orderIds) {
  ...
  return items
    ...
    .sort((a, b) => {
      const rankA = rank.has(getItemId(a.item)) ? rank.get(getItemId(a.item)) : fallback;
      const rankB = rank.has(getItemId(b.item)) ? rank.get(getItemId(b.item)) : fallback;
