...
/** @typedef {"open" | "edit"} OpenAction */
/** @typedef {"detail" | "sidePanel"} OpenTarget */

const VALID_ACTIONS = new Set(["open", "edit"]);
 ...
 */
export function resolveOpenTarget({ action = "open", editInSidePanel = false } = {}) {
  const normalizedAction = VALID_ACTIONS.has(action) ? action : "open";
 ...
 * @returns {boolean}
 */
export function isEditInSidePanelEnabled(settings) {
