...
describe("resolveOpenTarget (§19.9)", () => {
  ...
  test("only a literal true enables the side panel (no truthy coercion)", () => {
    ...
    expect(resolveOpenTarget({ action: "edit", editInSidePanel: "yes" as never })).toBe("detail");
    expect(resolveOpenTarget({ action: "edit", editInSidePanel: 1 as never })).toBe("detail");
  });
});
