 ...
 */
export function timecodeToSeconds(a: unknown, b: unknown, c: unknown): number | null {
  const n = (x: unknown): number | null => (x === undefined || x === null || x === "" ? null : Number(x));
 ...
 */
export function parseTimestampSegments(text: unknown): TimestampSegment[] {
...
export function hasTimestamps(text: unknown): boolean {
