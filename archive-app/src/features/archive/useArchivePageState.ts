...
export function useArchivePageState() {
  ...
  const isItemSelected = React.useCallback((id: any) => selectedIdSet.has(id), [selectedIdSet]);
  ...
  React.useEffect(() => {
    ...
    const observer = new ResizeObserver((entries: any) => {
      const width = entries[0]?.contentRect?.width || (element as any).clientWidth || 0;
      setGridColumnCount(getGridColumnCount(width, activeItemSize));
    });
    observer.observe(element);
    setGridColumnCount(getGridColumnCount((element as any).clientWidth || 0, activeItemSize));
  ...
  const typeById = React.useMemo(
    () => new Map(contentTypes.map((type: any) => [type.id, type])),
    [contentTypes]
  );
  const typeCounts = React.useMemo(() => new Map(contentTypes.map((type: any) => [
    type.id,
    videoItems.filter((item: any) => item.type === type.id && !item.isDeleted).length
  ])), [contentTypes, videoItems]);
  const activeType = typeById.get(filterType) as any;
  const subtypes = (activeType?.subtypes as any) || [];
  const previewItem = filteredItems.find((item: any) => item.id === previewId) || visibleItems[0] || null;
  ...
  React.useEffect(() => {
    if (filterSubtype !== "all" && !subtypes.some((subtype: any) => subtype.id === filterSubtype)) {
      setFilterSubtype?.("all");
    }
  }, [filterSubtype, setFilterSubtype, subtypes]);

  const visibleIds = React.useMemo(() => visibleItems.map((item: any) => item.id), [visibleItems]);
  const allVisibleSelected = visibleIds.length > 0 && visibleIds.every((id: any) => selectedIdSet.has(id));
  const toggleSelectAllVisible = React.useCallback(() => {
    if (allVisibleSelected) {
      clearSelection?.();
    } else {
      visibleIds.forEach((id: any) => {
        if (!selectedIdSet.has(id)) toggleBulkSelect?.(id);
      });
    }
  }, [allVisibleSelected, clearSelection, selectedIdSet, toggleBulkSelect, visibleIds]);

  const goToPage = React.useCallback((nextPage: any) => {
    const normalized = normalizeArchivePage(nextPage);
    setPage(Math.min(Math.max(normalized, 1), totalPages));
  }, [totalPages]);

  const updateArchiveUiPreference = React.useCallback((patch: any) => {
    updateSettings?.({ ui: { ...(settings.ui || {}), ...patch } });
  }, [settings.ui, updateSettings]);

  const changePageSize = React.useCallback((nextPageSize: any) => {
  ...
  const changeTopMode = React.useCallback((nextMode: any) => {
  ...
  const changeViewMode = React.useCallback((nextViewMode: any) => {
    const normalized = normalizeArchiveViewMode(nextViewMode);
    setViewMode?.(normalized);
    setPage(1);
    const patch: any = { archiveViewMode: normalized };
  ...
  const changeItemSize = React.useCallback((nextSize: any) => {
    const normalized = normalizeArchiveItemSize(nextSize);
    setItemSize(normalized);
    setPage(1);
    const patch: any = { archiveItemSize: normalized };
    if (normalized === "xs") {
  ...
  const changeGridRows = React.useCallback((nextRows: any) => {
  ...
  const changeGridColumns = React.useCallback((nextColumns: any) => {
  ...
  const changeTableColumns = React.useCallback(async (next: any) => {
  ...
  const resizeFlushRef = React.useRef<any>(null);
  const handleColumnResize = React.useCallback((columnId: any, width: any) => {
    setTableColumns((current: any) => {
  ...
  const applySavedView = React.useCallback((view: any) => {
  ...
  const saveCurrentView = React.useCallback(async (name: any, filters: any) => {
  ...
  const removeView = React.useCallback(async (viewId: any) => {
  ...
  const openItem = React.useCallback((item: any) => {
    ...
    setNavItemIds?.(filteredItems.map((entry: any) => entry.id));
    setSelectedItemId?.(item.id);
    setCurrentPage?.("detail" as any);
  ...
  const reorderArchiveItems = React.useCallback(async (fromId: any, toId: any) => {
  ...
  const openProjects = React.useCallback(() => {
    setCurrentPage?.("projects" as any);
  }, [setCurrentPage]);

  const confirmDelete = React.useCallback(async (item: any) => {
  ...
  const confirmEmptyTrash = React.useCallback(async (deletedCount: any) => {
  ...
  const typeLabel = React.useCallback(
    (item: any) => (typeById.get(item.type) as any)?.name || item.type || "",
    [typeById]
  );
  const subtypeLabel = React.useCallback(
    (item: any) => (typeById.get(item.type) as any)?.subtypes?.find((subtype: any) => subtype.id === item.subtype)?.name || item.subtype || "",
    [typeById]
  );
