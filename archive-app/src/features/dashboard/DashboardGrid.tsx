...
import { Responsive as ResponsiveGridLayoutBase } from "react-grid-layout";
...
const ROW_HEIGHT = 8;
const MARGIN: [number, number] = [12, 8];
...
function pxToRows(px: any) {
  return Math.max(2, Math.ceil((px + MARGIN[1]) / (ROW_HEIGHT + MARGIN[1])));
}

function stopGridDragStart(event: any) {
 ...
 */
function GridPanel({ id, title, editing, autoHeight, onMeasure, onToggleHidden, onToggleAuto, children }: any) {
  ...
  React.useEffect(() => {
    ...
    const report = () => onMeasure(id, pxToRows((el as any).scrollHeight + (editing ? 40 : 0)));
 ...
 */
export function DashboardGrid({ children, titles = {}, layout, editing = false, onChange, onToggleHidden, onToggleAuto, prefersReducedMotion = false }: any) {
  const nodes = (Array.isArray(children) ? children : [children]).filter(Boolean);
  const nodeById = {};
  for (const node of nodes) { if (node && node.key != null) (nodeById as any)[node.key] = node; }
  ...
  const gridLayout = baseGrid.map((n: any) => {
    const it = layout.items[n.i];
    return (it && it.autoHeight !== false && (measured as any)[n.i]) ? { ...n, h: (measured as any)[n.i] } : n;
  });
  ...
  React.useEffect(() => {
    ...
    if (!el || typeof ResizeObserver === "undefined") {
      if (el) setWidth((el as any).clientWidth);
      return undefined;
    }
    const ro = new ResizeObserver(() => setWidth((el as any).clientWidth));
    ro.observe(el);
    setWidth((el as any).clientWidth);
    return () => ro.disconnect();
  }, []);

  // Auto-height: update a panel's row count only when it actually changed,
  // which prevents an onLayoutChange feedback loop.
  const handleMeasure = React.useCallback((id: any, rows: any) => {
    setMeasured((prev: any) => (prev[id] === rows ? prev : { ...prev, [id]: rows }));
  }, []);

  const handleLayoutChange = React.useCallback((nextRgl: any) => {
    if (!editing) return;
    ...
    const a = JSON.stringify(toGridLayout(layoutRef.current, availableIds).map((n: any) => [n.i, n.x, n.y, n.w]));
    const b = JSON.stringify(nextRgl.map((n: any) => [n.i, n.x, n.y, n.w]));
