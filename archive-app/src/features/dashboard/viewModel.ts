...
export function getDashboardDemoItemIds(videoItems: AnyRecord[] = []) {
  return videoItems.filter((item) => item.id?.startsWith("demo-")).map((item) => item.id);
}

function getElapsedDays(value: unknown, now: number) {
...
export function hasDashboardLayoutDraftChanges({
  ...
  draftHiddenWidgets?: unknown[];
  currentHiddenWidgets?: unknown[];
  draftActivePresetId?: string;
