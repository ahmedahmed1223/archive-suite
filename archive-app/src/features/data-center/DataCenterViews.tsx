...
export function PageCard({ children, className = "" }: any) {
  return jsx("section", {
...
export function DataMetric({ label, value, hint, icon }: any) {
...
export function TaskChoiceCard({ task, active, onClick, count }: any) {
  const Icon = (tabIconMap as any)[task.id] || Database;
  return jsxs("button", {
    type: "button",
    onClick,
    "aria-pressed": active,
...
export function TaskStageRail({ stages = [] }: any) {
  return jsx("ol", {
    className: "grid gap-2 md:grid-cols-4",
    children: stages.map((stage: any, index: any) => jsxs("li", {
...
export function CollapsibleSummary({ title, description, open, onToggle, children }: any) {
  return jsxs("section", {
...
export function TabButton({ tab, active, onClick }: any) {
  const Icon = (tabIconMap as any)[tab.id] || Database;
  return jsxs("button", {
    type: "button",
    role: "tab",
...
export function SegmentedButton({ active, children, onClick, danger = false }: any) {
...
export function ActionButton({ children, icon, onClick, disabled = false, tone = "emerald" }: any) {
...
export function SummaryGrid({ rows }: any) {
  return jsx("div", {
    className: "grid gap-3 sm:grid-cols-2 xl:grid-cols-3",
    children: rows.map((item: any) => jsxs("div", {
...
export function PreviewSummary({ preview }: any) {
  if (!preview) return null;
  const visibleEntities = preview.summary.entities.filter((entity: any) => entity.total > 0 || entity.conflictCount > 0 || entity.potentialDuplicateCount > 0);
  return jsxs("div", {
    className: "space-y-3",
    children: [
      jsxs("div", {
        ...
        children: [
          ...
          jsxs("p", {
            ...
            children: [
              preview.fileName,
              " - ",
              (sourceTypeLabels as any)[preview.sourceType] || "ملف بيانات",
      ...
      visibleEntities.length > 0 && jsx("div", {
        ...
        children: visibleEntities.map((entity: any) => jsxs("div", {
