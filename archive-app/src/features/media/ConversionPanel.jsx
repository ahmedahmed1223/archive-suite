...
function ConversionOption({ recipe, onStart, busy, disabled }) {
 ...
 * }} props
 */
export function ConversionPanel({ sourceKey, mediaClient, disabled = false, onJobCreated, onError }) {
  const [busy, setBusy] = React.useState("");
  const [lastJob, setLastJob] = React.useState(null);
  const [error, setError] = React.useState("");

  const fileType = fileTypeFromKey(sourceKey);
  const recipes = CONVERSIONS_BY_TYPE[fileType] || [];
  ...
  async function handleStart(recipe) {
  ...
  return jsxs("div", {
    ...
    children: [
      ...
      lastJob && jsxs("div", {
        ...
        children: [
          "✓ المهمة بدأت — معرّف: ",
          jsx("span", { dir: "ltr", className: "font-mono", children: String(lastJob.id || lastJob.jobId || "—") })
        ]
      })
    ]
  });
