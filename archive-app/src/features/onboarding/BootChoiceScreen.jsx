 ...
 */
export function BootChoiceScreen({ onAdvanced, onComplete }) {
  ...
  const handleQuick = async () => {
    ...
    try {
      ...
      const adminUser = (useAppStore.getState().users || [])
        .find((user) => user.username === "admin" && user.isActive !== false)
        || (useAppStore.getState().users || []).find((user) => user.isActive !== false);
      if (adminUser) {
