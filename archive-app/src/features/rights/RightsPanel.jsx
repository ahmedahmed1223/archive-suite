...
function isUnderEmbargo(start, end) {
...
function isExpiringWithin(dateStr, days = 30) {
  if (!dateStr) return false;
...
function licenseConfig(type) {
  return LICENSE_OPTIONS.find((o) => o.value === type) ?? LICENSE_OPTIONS[LICENSE_OPTIONS.length - 1];
}
...
function TextInput({ id, value, onChange, placeholder = "", dir = "rtl" }) {
  return (
    <input
      id={id}
      type="text"
...
export function RightsPanel({ itemId, baseUrl, getToken }) {
...
function RightsPanelInner({ itemId, baseUrl, getToken }) {
  ...
  React.useEffect(() => {
    ...
    setLoading(true);
    setError("");
    fetchItemRights({ base: resolvedBase, getToken: resolvedToken, itemId })
      .then((data) => {
        if (cancelled) return;
        const r = data?.record;
        if (r) {
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <BadgeV2 variant={cfg.badge}>{cfg.label}</BadgeV2>
        </div>
