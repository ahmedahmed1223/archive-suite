...
type CloudState = keyof typeof STATE_STYLES;
...
interface ConnectionStatus {
  ...
  health?: {
    db?: string | boolean | null;
    storage?: string | boolean | null;
  };
}

function StatusPill({ state = "local", children }: StatusPillProps) {
...
function MetricRow({ icon, label, value, pill }: MetricRowProps) {
  return jsxs("div", {
...
function HealthDashboard({ connectionStatus }: { connectionStatus: ConnectionStatus }) {
  const applyServerHealth = useAppStore((s: any) => s.applyServerHealth);
  const markRpcFailure = useAppStore((s: any) => s.markRpcFailure);
  const [refreshing, setRefreshing] = React.useState(false);

  const { state = "local", engine, lastLatencyMs, lastCheckedAt, health } = connectionStatus;
  ...
  async function handleRefresh() {
    ...
    try {
      ...
      markRpcFailure?.({ error: (err as { message?: string })?.message });
    } finally {
      setRefreshing(false);
    }
  }
      ...
      children: jsxs("div", {
        ...
        children: [
          ...
          jsx(MetricRow, {
            ...
            pill: jsx(StatusPill, { state: dbState as CloudState })
          }),
          jsx(MetricRow, {
            icon: jsx(HardDrive, { className: "h-4 w-4" }),
            label: "التخزين",
            pill: jsx(StatusPill, { state: storageState as CloudState })
          })
        ]
      })
    })
...
export function CloudControlTab() {
  const connectionStatus = useAppStore((s: any) => s.connectionStatus) as ConnectionStatus | undefined;
  ...
  return jsxs("div", {
    className: "space-y-4",
    children: [
      jsx(HealthDashboard, { connectionStatus: connectionStatus || {} }),
