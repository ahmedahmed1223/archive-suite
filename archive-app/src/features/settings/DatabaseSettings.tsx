...
const DEFAULT_PORT_BY_ENGINE: Record<string, string> = { postgresql: "5432", mysql: "3306", sqlserver: "1433" };
const databaseEngines = DATABASE_ENGINES as readonly string[];
const databaseEngineLabels = DATABASE_ENGINE_LABELS as Record<string, string>;
const buildDatabaseUrlAny = buildDatabaseUrl as unknown as (parts: Omit<DbParts, "port"> & { engine: string; port?: number | string }) => string;
const testDbConnectionAny = testDbConnection as unknown as (options: Record<string, unknown>) => Promise<{ ok: boolean; error?: string }>;
const saveDbConfigAny = saveDbConfig as unknown as (options: Record<string, unknown>) => Promise<{ database: DbConnectionInfo }>;
...
export function DatabaseSettings() {
  ...
  const role = (getCloudUser() as { role?: string } | null)?.role;
  const enabled = canManageDb({ backend, token: getCloudToken(), role });

  const [active, setActive] = React.useState<DbConnectionInfo | null>(null);
  const [loadErr, setLoadErr] = React.useState("");
  const [engine, setEngine] = React.useState("postgresql");
  const [mode, setMode] = React.useState<"url" | "parts">("url");
  const [url, setUrl] = React.useState("");
  const [parts, setParts] = React.useState<DbParts>({ host: "", port: "5432", database: "archive", user: "archive", password: "", file: "./archive.sqlite" });
  const [testing, setTesting] = React.useState(false);
  const [saving, setSaving] = React.useState(false);
  const [testMsg, setTestMsg] = React.useState<TestMessage | null>(null);
  ...
  React.useEffect(() => {
    ...
    let alive = true;
    fetchDbConfig(deps)
      .then((cfg: DbConfigPayload) => {
        if (!alive) return;
        setActive(cfg.database || null);
        if (cfg?.database?.engine) setEngine(normalizeDatabaseEngine(cfg.database.engine));
      })
      .catch((e: unknown) => { if (alive) setLoadErr((e as { message?: string })?.message || "تعذّر قراءة الإعداد."); });
    return () => { alive = false; };
  }, [enabled, deps]);

  if (!enabled) return null;

  const candidateUrl = () => (mode === "url" ? url.trim() : buildDatabaseUrlAny({ ...parts, engine, port: Number(parts.port) || Number(DEFAULT_PORT_BY_ENGINE[engine] || 0) }));
  ...
  const runTest = async () => {
    ...
    setTestMsg(null);
    try {
      const r = await testDbConnectionAny({ engine, url: u, ...deps });
  ...
  const runSave = async () => {
    ...
    setSaving(true);
    try {
      const r = await saveDbConfigAny({ engine, url: u, ...deps });
  ...
  const setPart = <K extends keyof DbParts>(k: K, v: DbParts[K]) => setParts((p) => ({ ...p, [k]: v }));
  const changeEngine = (next: string) => {
  ...
  return jsx(SettingsCard, {
    ...
    children: jsxs("div", { className: "space-y-4", dir: "rtl", children: [
      ...
      active && jsxs("div", { className: "flex flex-wrap items-center gap-2 rounded-xl va-surface-subtle border p-3 text-sm text-gray-300", children: [
        ...
        jsx("span", { className: "rounded border border-white/10 px-2 py-0.5 text-xs va-accent-text-on-soft", children: databaseEngineLabels[active.engine || ""] || active.engine || "SQL" }),
      ...
      jsxs("label", { className: "block space-y-1 text-sm text-gray-300", children: [
        ...
        jsx("select", {
          value: engine,
          onChange: (event: React.ChangeEvent<HTMLSelectElement>) => changeEngine(event.target.value),
          className: SELECT_FIELD,
          children: databaseEngines.map((id) => jsx("option", { value: id, children: databaseEngineLabels[id] || id }, id))
        })
      ] }),
      ...
      mode === "url"
        ? jsxs("label", { className: "block space-y-1 text-sm text-gray-300", children: [
            ...
            jsx("input", { value: url, onChange: (e: React.ChangeEvent<HTMLInputElement>) => setUrl(e.target.value), dir: "ltr", placeholder: engine === "sqlite" ? "file:./archive.sqlite" : `${engine}://user:pass@host:${DEFAULT_PORT_BY_ENGINE[engine] || ""}/db`, className: `${FIELD} text-start` })
          ] })
        : engine === "sqlite"
          ? jsxs("label", { className: "block space-y-1 text-sm text-gray-300", children: [
              jsx("span", { children: "ملف SQLite على الخادم" }),
              jsx("input", { value: parts.file, onChange: (e: React.ChangeEvent<HTMLInputElement>) => setPart("file", e.target.value), dir: "ltr", placeholder: "./archive.sqlite", className: `${FIELD} text-start` })
            ] })
          : jsxs("div", { className: "grid gap-2 sm:grid-cols-2", children: [
              jsxs("label", { className: "space-y-1 text-sm text-gray-300", children: [jsx("span", { children: "المضيف (host)" }), jsx("input", { value: parts.host, onChange: (e: React.ChangeEvent<HTMLInputElement>) => setPart("host", e.target.value), dir: "ltr", placeholder: "db.example.com", className: `${FIELD} text-start` })] }),
              jsxs("label", { className: "space-y-1 text-sm text-gray-300", children: [jsx("span", { children: "المنفذ (port)" }), jsx("input", { value: parts.port, onChange: (e: React.ChangeEvent<HTMLInputElement>) => setPart("port", e.target.value), dir: "ltr", className: `${FIELD} text-start` })] }),
              jsxs("label", { className: "space-y-1 text-sm text-gray-300", children: [jsx("span", { children: "قاعدة البيانات" }), jsx("input", { value: parts.database, onChange: (e: React.ChangeEvent<HTMLInputElement>) => setPart("database", e.target.value), dir: "ltr", className: `${FIELD} text-start` })] }),
              jsxs("label", { className: "space-y-1 text-sm text-gray-300", children: [jsx("span", { children: "المستخدم" }), jsx("input", { value: parts.user, onChange: (e: React.ChangeEvent<HTMLInputElement>) => setPart("user", e.target.value), dir: "ltr", className: `${FIELD} text-start` })] }),
              jsxs("label", { className: "space-y-1 text-sm text-gray-300 sm:col-span-2", children: [jsx("span", { children: "كلمة المرور" }), jsx("input", { type: "password", value: parts.password, onChange: (e: React.ChangeEvent<HTMLInputElement>) => setPart("password", e.target.value), dir: "ltr", className: `${FIELD} text-start` })] })
            ] }),
