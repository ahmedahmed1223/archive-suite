...
type ParseResult = ReturnType<typeof parseFirebaseConfigText>;

export function FirebaseBackendSettings() {
  const { showToast } = useAppStore();
  const [text, setText] = React.useState<string>(() => stringifyFirebaseConfig(getFirebaseConfig() || {}));
  const [migrate, setMigrate] = React.useState(false);
  const [saving, setSaving] = React.useState(false);
  const parsed: ParseResult = React.useMemo(() => parseFirebaseConfigText(text), [text]);
    ...
    children: jsxs("div", { className: "space-y-3", dir: "rtl", children: [
      ...
      jsxs("label", { className: "flex items-center gap-2 text-sm text-gray-300", children: [
        jsx("input", {
          type: "checkbox",
          checked: migrate,
          onChange: (event: React.ChangeEvent<HTMLInputElement>) => setMigrate(event.target.checked),
