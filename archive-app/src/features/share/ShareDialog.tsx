...
function PermissionPicker({ permission, onChange, disabled }: any) {
  return jsx("div", {
    className: "grid grid-cols-2 gap-2 sm:grid-cols-4",
    role: "radiogroup",
    "aria-label": "مستوى الصلاحية",
    children: SHARE_PERMISSIONS.map((level: any) => {
...
export function ShareDialog({
  ...
  const [state, setState] = useState<any>({ status: "idle", url: "", jti: "", error: "" });
  ...
  let grantError = "";
  try {
    grant = createShareGrant({ scopeType, scopeIds, permission, expiresInDays, label } as any);
  } catch (err: any) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setState({ status: "ready", url, jti: jti || "", error: "", invitedEmail: (result as any).invitation?.email || cleanEmail });
  ...
  const handleRevoke = async () => {
    ...
    try {
      await revokeShareLink({ jti: state.jti, baseUrl, getToken } as any);
        ...
        jsxs("div", { className: "space-y-4 px-5 py-4", children: [
          ...
          jsxs("div", { className: "space-y-2", children: [
            ...
            jsx("select", {
              ...
              children: EXPIRY_OPTIONS.map((opt: any) => jsx("option", { value: opt.value, children: opt.label }, opt.value))
            })
          ] }),
          ...
          jsxs("div", { className: "rounded-xl border border-[var(--va-border-soft)] bg-white/[0.03] p-3 space-y-3", children: [
            ...
            jsxs("label", { className: "block space-y-2", children: [
              ...
              jsx("textarea", {
                value: inviteMessage,
                disabled: isBusy,
                onChange: (e: any) => setInviteMessage(e.target.value),
                rows: 2,
          ...
          state.status === "ready" && jsxs("div", { className: "space-y-2", children: [
            (state as any).invitedEmail && jsx("div", {
              role: "status",
              className: "alert alert-info rounded-xl px-3 py-2 text-xs",
              children: `تم إرسال الدعوة إلى ${(state as any).invitedEmail}`
            }),
            jsxs("div", {
              role: "alert",
