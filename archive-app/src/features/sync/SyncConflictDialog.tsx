...
function renderValuePreview(value: any, kind: any) {
  ...
  if (kind === "stringArray") {
    ...
    return jsx("div", {
      ...
      children: list.map((tag: any, index: any) => jsx("span", { className: "rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[11px] text-gray-200", children: tag }, `${tag}-${index}`))
    });
  }
  if (kind === "object") {
    ...
    return jsx("ul", {
      ...
      children: entries.slice(0, 5).map(([key, val]: any) => jsxs("li", {
        children: [
...
function FieldChoiceButton({ active, onClick, icon, label, accent }: any) {
...
function FieldRow({ row, choice, onChange }: any) {
  ...
  return jsxs("div", {
    className: `rounded-xl border p-3 ${(STATUS_CLASS as any)[row.status] || STATUS_CLASS.identical}`,
    children: [
      jsxs("div", { className: "flex flex-wrap items-center justify-between gap-2", children: [
        ...
        jsx("span", { className: "rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[11px]", children: (STATUS_LABEL as any)[row.status] || row.status })
      ] }),
...
function ConflictItem({ item, choice, onChange }: any) {
  const diff = React.useMemo(() => buildFieldDiff({
    local: item.local,
    incoming: item.incoming,
    base: null
  } as any), [item.local, item.incoming]);
  const conflictRows = diff.filter((row: any) => row.status !== "identical");
  const identicalCount = diff.length - conflictRows.length;

  const setFieldChoice = (key: any, value: any) => onChange({ ...choice, [key]: value });
    ...
    children: [
      jsxs("header", { className: "flex flex-wrap items-start justify-between gap-3", children: [
        ...
        jsxs("div", { className: "flex flex-wrap gap-2", children: [
          jsx("button", {
            type: "button",
            onClick: () => onChange(Object.fromEntries(DIFFABLE_FIELDS.map((field: any) => [field.key, "local"]))),
          ...
          jsx("button", {
            type: "button",
            onClick: () => onChange(Object.fromEntries(DIFFABLE_FIELDS.map((field: any) => [field.key, "incoming"]))),
      ...
      jsx("div", { className: "space-y-2", children: diff.map((row: any) => jsx(FieldRow, {
        row,
        choice: choice[row.key],
        onChange: (next: any) => setFieldChoice(row.key, next)
      }, row.key)) })
    ]
  });
}
 ...
 */
export function SyncConflictDialog({ open, conflicts = [], onApply, onCancel }: any) {
  ...
  const itemChoices = (choices as any)[active.id] || {};

  const setActiveItemChoices = (next: any) => setChoices((current: any) => ({ ...current, [active.id]: next }));

  const handleApply = () => {
    const resolved = {};
    for (const item of conflicts) {
      const picks = (choices as any)[item.id] || {};
      (resolved as any)[item.id] = applyFieldChoices({ local: item.local, incoming: item.incoming, choices: picks });
    }
    onApply?.(resolved);
  };
  ...
  return jsx(AnimatePresence, {
    children: jsx(motion.div, {
      ...
      onClick: (event: any) => { if (event.target === event.currentTarget) onCancel?.(); },
        ...
        children: [
          ...
          conflicts.length > 1 && jsxs("nav", {
            ...
            children: [
              jsxs("button", {
                type: "button",
                onClick: () => setActiveIndex((value: any) => Math.max(0, value - 1)),
              ...
              jsxs("button", {
                type: "button",
                onClick: () => setActiveIndex((value: any) => Math.min(conflicts.length - 1, value + 1)),
