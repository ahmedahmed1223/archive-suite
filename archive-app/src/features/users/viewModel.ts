import { normalizeArabicSearchText } from "../../utils/formatting.js";

export type UserRoleId = "admin" | "editor" | "viewer";

export interface UserRole {
  id: UserRoleId;
  label: string;
  color: string;
  description: string;
}

export interface UserValue {
  id: string;
  username: string;
  displayName: string;
  passwordHash: string;
  role: UserRoleId;
  roleId?: string;
  avatar?: unknown;
  email?: string;
  inviteStatus?: string;
  invitedAt?: string;
  invitedBy?: string | null;
  customPermissions?: unknown;
  isActive: boolean;
  lastLoginAt?: string;
  mustChangePassword: boolean;
  createdAt: string;
  updatedAt: string;
}

export interface CreateUserPartial {
  id?: string;
  username?: string;
  displayName?: string;
  passwordHash?: string;
  role?: string;
  roleId?: string;
  role_id?: string;
  avatar?: unknown;
  email?: string;
  inviteStatus?: string;
  invitedAt?: string;
  invitedBy?: string | null;
  customPermissions?: unknown;
  isActive?: boolean;
  lastLoginAt?: string;
  mustChangePassword?: boolean;
  createdAt?: string;
}

export interface InvitationMetadata {
  email: string;
  inviteStatus: "pending";
  invitedAt: string;
  invitedBy: string | null;
  mustChangePassword: true;
}

export interface UserSummary {
  total: number;
  active: number;
  inactive: number;
  activeAdmins: number;
  byRole: Record<UserRoleId, number>;
}

export const USER_ROLES: UserRole[] = [
  { id: "admin", label: "مدير", color: "#ef4444", description: "إدارة كاملة للتطبيق والبيانات." },
  { id: "editor", label: "محرر", color: "#10b981", description: "إضافة وتعديل العناصر بدون إدارة الأمان." },
  { id: "viewer", label: "مشاهد", color: "#3b82f6", description: "تصفح وبحث وقراءة فقط." }
];

const USER_ROLE_IDS = new Set<UserRoleId>(USER_ROLES.map((role) => role.id));
const TEMP_PASSWORD_ALPHABET = "ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789!@#$%";

export function normalizeUserRole(role: string = "viewer"): UserRoleId {
  return USER_ROLE_IDS.has(role as UserRoleId) ? (role as UserRoleId) : "viewer";
}

export function isValidInviteEmail(email = ""): boolean {
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(email).trim());
}

export function createTemporaryPassword(length = 16, randomValues: null | ((bytes: Uint8Array) => void) = null): string {
  const size = Math.max(12, Number(length) || 16);
  const bytes = new Uint8Array(size);
  if (typeof randomValues === "function") {
    randomValues(bytes);
  } else if (globalThis.crypto?.getRandomValues) {
    globalThis.crypto.getRandomValues(bytes);
  } else {
    for (let i = 0; i < bytes.length; i += 1) bytes[i] = Math.floor(Math.random() * 256);
  }
  return [...bytes].map((byte) => TEMP_PASSWORD_ALPHABET[byte % TEMP_PASSWORD_ALPHABET.length]).join("");
}

export function createInvitationMetadata({ email, invitedBy = null, now = () => new Date().toISOString() }: {
  email?: string;
  invitedBy?: string | null;
  now?: () => string;
} = {}): InvitationMetadata {
  const normalizedEmail = String(email || "").trim().toLowerCase();
  if (!isValidInviteEmail(normalizedEmail)) {
    throw new Error("بريد الدعوة غير صالح");
  }
  return {
    email: normalizedEmail,
    inviteStatus: "pending",
    invitedAt: now(),
    invitedBy,
    mustChangePassword: true
  };
}

export function createUserValue(partial: CreateUserPartial = {}): UserValue {
  const now = new Date().toISOString();
  return {
    id: partial.id || `user_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`,
    username: String(partial.username || "").trim(),
    displayName: String(partial.displayName || "").trim(),
    passwordHash: partial.passwordHash || "",
    role: normalizeUserRole(partial.role),
    roleId: partial.roleId || partial.role_id,
    avatar: partial.avatar,
    email: partial.email,
    inviteStatus: partial.inviteStatus,
    invitedAt: partial.invitedAt,
    invitedBy: partial.invitedBy,
    customPermissions: partial.customPermissions,
    isActive: partial.isActive ?? true,
    lastLoginAt: partial.lastLoginAt,
    mustChangePassword: partial.mustChangePassword || false,
    createdAt: partial.createdAt || now,
    updatedAt: now
  };
}

export function getFilteredUsers(users: UserValue[] = [], query = "", role: UserRoleId | "all" = "all"): UserValue[] {
  const normalizedQuery = normalizeArabicSearchText(query);
  const normalizedRole = normalizeUserRole(role === "all" ? "viewer" : role);
  const useRoleFilter = role !== "all";
  return [...users]
    .filter((user) => !useRoleFilter || user.role === normalizedRole)
    .filter((user) => {
      if (!normalizedQuery) return true;
      return [
        user.username,
        user.displayName,
        user.email,
        USER_ROLES.find((item) => item.id === user.role)?.label
      ].some((value) => normalizeArabicSearchText(value).includes(normalizedQuery));
    })
    .sort((a, b) => String(a.displayName || a.username || "").localeCompare(String(b.displayName || b.username || ""), "ar"));
}

export function getUserSummary(users: UserValue[] = []): UserSummary {
  const active = users.filter((user) => user.isActive).length;
  const inactive = users.length - active;
  const byRole = Object.fromEntries(USER_ROLES.map((role) => [role.id, users.filter((user) => user.role === role.id).length])) as Record<UserRoleId, number>;
  const activeAdmins = users.filter((user) => user.role === "admin" && user.isActive).length;
  return {
    total: users.length,
    active,
    inactive,
    activeAdmins,
    byRole
  };
}

export function canDeactivateUser(user: Pick<UserValue, "isActive" | "role" | "id"> | null | undefined, users: Pick<UserValue, "role" | "isActive" | "id">[] = []): boolean {
  if (!user?.isActive) return true;
  if (user.role !== "admin") return true;
  return users.filter((item) => item.role === "admin" && item.isActive && item.id !== user.id).length > 0;
}
