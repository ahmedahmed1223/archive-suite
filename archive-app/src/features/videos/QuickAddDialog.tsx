...
function writeDraft(draft: any) {
  try {
    const hasContent = [draft.name, draft.description, draft.tagsText].some((value: any) => String(value || "").trim());
...
function getLatestItem(videoItems: any = [], selectedItemId: any = "") {
  const selected = videoItems.find((item: any) => item.id === selectedItemId && !item.isDeleted);
...
function getLatestProject(projects: any = []) {
  return [...projects]
    .filter((project: any) => project.status !== "archived")
    .sort((a: any, b: any) => new Date(b.updatedAt || b.createdAt || 0).getTime() - new Date(a.updatedAt || a.createdAt || 0).getTime())[0] || null;
}

function defaultModeForPage(page: any) {
...
export function QuickAddDialog({ open, onOpenChange }: any) {
  const {
    currentPage,
    selectedItemId,
    currentUser,
  ...
  const activeTypes = React.useMemo(() => contentTypes.filter((type: any) => type.status !== "archived"), [contentTypes]);
  ...
  const canUseAction = React.useCallback((permissionAction: any) => {
  ...
  const availableModes = React.useMemo(() => Object.entries(MODE_META)
    ...
    .map(([id, meta]: any) => ({ id, ...meta })), [activeTypes.length, canUseAction, canWorkflow, latestItem, latestProject]);
  ...
  React.useEffect(() => {
    ...
    const nextMode = availableModes.some((item: any) => item.id === saved?.mode)
      ? saved.mode
      : availableModes.some((item: any) => item.id === fallback)
        ? fallback
  ...
  React.useEffect(() => {
    if (!open) return;
    if (!availableModes.some((item: any) => item.id === mode) && availableModes[0]) setMode(availableModes[0].id);
  ...
  const meta = (MODE_META as any)[mode] || MODE_META.video;
  const Icon = meta.icon || Sparkles;
  const isComment = mode === "comment";
  const canSubmit = Boolean((isComment ? description || name : name).trim()) && !action.busy && availableModes.length > 0;
  const activeType = activeTypes.find((type: any) => type.id === targetTypeId) || activeTypes[0];
  ...
  const createEntity = async () => {
    ...
    if (mode === "video") {
      return addVideoItem?.(createVideoItemValue({ title, type: typeId, tags: parseVideoTags(tagsText as any), notes: detail }));
    }
    if (mode === "type") {
  ...
  const submit = async (next: any = "close") => action.run(async () => {
    try {
  ...
  return jsx(EntityFormModal, {
    ...
    children: jsxs("div", { className: "space-y-4", children: [
      availableModes.length ? jsx("div", { className: "flex flex-wrap gap-2", role: "tablist", "aria-label": "نوع العنصر المراد إنشاؤه", children: availableModes.map((item: any) => {
      ...
      jsxs("section", { className: "rounded-2xl va-surface-subtle border p-4", children: [
        ...
        mode === "field" && activeTypes.length ? jsxs("label", { className: "mb-3 block space-y-1 text-sm text-[var(--va-text-2)]", children: [
          ...
          jsx("select", { value: targetTypeId, onChange: (event: any) => setTargetTypeId(event.target.value), className: "min-h-11 w-full va-surface-deep rounded-xl border px-3 text-sm text-[var(--va-text)] outline-none", children: activeTypes.map((type: any) => jsx("option", { value: type.id, children: type.name }, type.id)) })
        ] }) : null,
        mode === "video" && activeTypes.length ? jsxs("label", { className: "mb-3 block space-y-1 text-sm text-[var(--va-text-2)]", children: [
          ...
          jsxs("select", { value: typeId, onChange: (event: any) => setTypeId(event.target.value), className: "min-h-11 w-full va-surface-deep rounded-xl border px-3 text-sm text-[var(--va-text)] outline-none", children: [
            jsx("option", { value: "", children: "بدون نوع" }),
            ...activeTypes.map((type: any) => jsx("option", { value: type.id, children: type.name }, type.id))
          ] })
        ] }) : null,
        !isComment ? jsxs("label", { className: "block space-y-1 text-sm text-[var(--va-text-2)]", children: [
          ...
          jsx("input", { "data-autofocus": true, value: name, onChange: (event: any) => setName(event.target.value), className: "min-h-11 w-full va-surface-deep rounded-xl border px-3 text-sm text-[var(--va-text)] outline-none", placeholder: mode === "video" ? "عنوان المادة" : "اسم واضح وقصير" })
        ] }) : null,
        isComment ? jsxs("label", { className: "block space-y-1 text-sm text-[var(--va-text-2)]", children: [
          jsx("span", { children: "نص التعليق" }),
          jsx("textarea", { "data-autofocus": true, value: description, onChange: (event: any) => setDescription(event.target.value), rows: 3, className: "w-full va-surface-deep rounded-xl border p-3 text-sm text-[var(--va-text)] outline-none", placeholder: "اكتب التعليق..." })
          ...
          jsx("textarea", { value: description, onChange: (event: any) => setDescription(event.target.value), rows: 2, className: "w-full va-surface-deep rounded-xl border p-3 text-sm text-[var(--va-text)] outline-none", placeholder: "اختياري" })
        ] }) : null,
        ...
        mode === "video"
          ? jsx(AutoTagSuggestions, {
              ...
              onAccept: (tag: any) => setTagsText((prev: any) => {
