...
function withKeyedChildren(props: any) {
  ...
  const children = React.Children.toArray(props.children).map((child: any, index: any) => (
...
const jsx = (type: any, props: any, key?: any) => runtimeJsx(type, withKeyedChildren(props), key);
const jsxs = (type: any, props: any, key?: any) => runtimeJsx(type, withKeyedChildren(props), key);
...
function persistDraft(draft: any) {
  try {
...
function getStepErrors(stepId: any, snapshot: any) {
  const errors = [];
  if (stepId === "basic") {
  ...
  if (stepId === "fields") {
    const missing = snapshot.fields.filter((field: any) => {
      if (!field.required) return false;
      const value = snapshot.metadata[fieldKey(field)];
      return value === undefined || value === "" || (Array.isArray(value) && value.length === 0);
    });
    if (missing.length) {
      errors.push(`أكمل الحقول المطلوبة: ${missing.map((field: any) => field.label).join("، ")}`);
    }
  }
  return errors;
}
 ...
 */
function FieldsStep({ fields, metadata, onChange }: any) {
  ...
  if (!visibleFields.length) {
    return jsx("p", { className: "rounded-xl border border-dashed border-[var(--va-border-soft)] bg-[var(--va-surface)] p-6 text-center text-sm text-[var(--va-text-2)]", children: "لا توجد حقول مخصصة لهذا النوع." });
  }
  ...
  return jsxs("div", {
    ...
    children: [
      ...
      jsx("div", { className: "grid gap-4 md:grid-cols-2", children: visible.map((field: any) => jsx(FieldRow, { field, value: metadata[fieldKey(field)], onChange }, field.id)) })
    ]
  });
}

function LocalFilePicker({ value, onFileSelect, inputId }: any) {
  const file = normalizeLocalFileValue(value);
  const inputRef = React.useRef(null);
  const [dragActive, setDragActive] = React.useState(false);
  const readFile = (nextFile: any) => {
    if (nextFile) onFileSelect(nextFile);
  };
  return jsxs(motion.div, {
    whileHover: { y: -1 },
    className: `rounded-xl border border-dashed p-3 transition-colors ${dragActive ? "va-accent-border va-accent-bg-soft" : "border-[var(--va-border-soft)] bg-[var(--va-surface)]"}`,
    ...
    !file && jsx("p", { className: "mt-2 text-xs leading-5 text-[var(--va-text-muted)]", children: "يمكنك سحب ملف فيديو هنا وسيتم ملء الاسم والمسار تلقائيًا قدر الإمكان." }),
...
function FieldInput({ field, value, onChange, inputId }: any) {
  ...
  if (field.type === "checkbox") {
    return jsxs("label", { className: "inline-flex min-h-11 items-center gap-2 va-surface-muted rounded-xl border px-3 text-sm text-[var(--va-text-2)]", children: [
      jsx("input", { id: inputId, type: "checkbox", checked: !!value, onChange: (event: any) => onChange(key, event.target.checked) }),
      "نعم"
    ] });
  }
  ...
  if (field.type === "tags") {
    return jsx("input", { id: inputId, value: Array.isArray(value) ? value.join("، ") : value || "", onChange: (event: any) => onChange(key, parseVideoTags(event.target.value)), className: commonClass, placeholder: "قيم مفصولة بفاصلة" });
  }
  if (field.type === "localFile") {
    return jsx(LocalFilePicker, { value, onFileSelect: (file: any) => onChange(key, createLocalFileValue(file)) });
  }
...
function FieldRow({ field, value, onChange }: any) {
  const inputId = React.useId();
  const isWide = field.type === "textarea" || field.type === "localFile";
  return jsxs("div", {
    className: `space-y-1 text-sm text-[var(--va-text-2)] ${isWide ? "md:col-span-2" : ""}`,
    children: [
      ...
      field.description && jsx("span", { className: "text-xs text-[var(--va-text-muted)]", children: field.description })
    ]
  }, field.id);
}

function RequiredFieldSummary({ checks = [] }: any) {
  const missing = checks.filter((check: any) => !check.ok);
  return jsxs("div", {
    ...
    children: [
        ...
        children: [
          jsxs("div", {
            ...
            children: [
              ...
              jsx("h3", { className: "text-sm font-bold text-[var(--va-text)]", children: missing.length ? "قبل الحفظ، أكمل المطلوب" : "جاهز للحفظ" })
            ]
          }),
          jsx("span", {
      ...
      jsx("div", {
        ...
        children: checks.map((check: any) => jsxs("div", {
          className: `flex items-start gap-2 rounded-lg border px-3 py-2 text-xs leading-5 ${check.ok ? "va-accent-border va-accent-bg-soft va-accent-text-on-soft" : "border-[var(--va-border-soft)] bg-[var(--va-surface)] text-[var(--va-text-2)]"}`,
          children: [
...
export function AddVideoPage() {
  ...
  const [stepError, setStepError] = React.useState<any>(null);
  ...
  const aiSuggestTags = async () => {
    ...
    if (!result) return;
    setTags((current: any) => mergeTagText(current, (result as any).tags));
  ...
  const aiSummarize = async () => {
    ...
    if (!result) return;
    setNotes((current: any) => applySummaryToNotes(current, (result as any).summary));
    setTags((current: any) => mergeTagText(current, (result as any).tags));
  ...
  const aiProofread = async () => {
    ...
    if (!result) return;
    setNotes((current: any) => applyProofread(current, result));
  ...
  const missingRequiredFields = React.useMemo(() => getMissingRequiredFields(fields as any, metadata), [fields, metadata]);
  const stepsWithDetail = React.useMemo(() => STEPS.map((step: any) => {
    if (step.id !== "fields" || !fields.length) return step;
    const reqCount = fields.filter((f: any) => f.required).length;
  ...
  const requiredChecks = React.useMemo(() => [
    ...
    ...fields.filter((field: any) => field.required).map((field: any) => {
  ...
  const readyCount = readyChecks.filter((check: any) => check.ok).length;
  ...
  React.useEffect(() => {
    if (subtypeId && !subtypes.some((subtype: any) => subtype.id === subtypeId)) setSubtypeId("");
  }, [subtypeId, subtypes]);
  ...
  const updateMetadata = (key: any, value: any) => setMetadata((current: any) => ({ ...current, [key]: value }));
  const applyPrimaryLocalFile = (file: any) => {
    ...
    setPath(patch.path);
    setMetadata((current: any) => ({ ...current, ...patch.metadata }));
  };

  const save = async (addAnother: any = false) => {
    if (!canSave) return;
    const missingRequired = getMissingRequiredFields(fields as any, metadata);
    if (missingRequired.length) {
      setStepIndex(STEPS.findIndex((step: any) => step.id === "fields"));
      setStepError(`حقول مطلوبة فارغة: ${missingRequired.map((field: any) => field.label).join("، ")}`);
      showToast?.("أكمل الحقول المطلوبة قبل الحفظ.", "error");
      return;
    }
    const uploadId = metadata?.localFile?.uploadId;
    const linkedUpload = uploadId ? uploads.find((upload: any) => upload.id === uploadId) : null;
  ...
  const tryAdvance = () => {
    ...
    setStepError(null);
    setStepIndex((value: any) => Math.min(STEPS.length - 1, value + 1));
  };
  ...
  const incrementTemplateUsage = useAppStore((state: any) => state.incrementTemplateUsage);

  const applyTemplate = (template: any, resolved: any) => {
    ...
    if (resolved.type) {
      const matched = contentTypes.find(
        (t: any) => t.id === resolved.type || t.name?.toLowerCase() === String(resolved.type).toLowerCase()
      );
  ...
  const [showPanel, setShowPanel] = React.useState(() => {
    try { return localStorage.getItem("videoArchive:addVideoSidePanel") !== "0"; } catch (error: any) { return true; }
  });
  const toggleSidePanel = () => setShowPanel((value: any) => {
    const next = !value;
    try { localStorage.setItem("videoArchive:addVideoSidePanel", next ? "1" : "0"); } catch (error: any) { /* ignore */ }
    return next;
  });

  const renderActionBar = (className: any, key: any) => jsxs("div", { className, children: [
    jsxs("div", { className: "flex items-center gap-3", children: [
      jsxs("button", { type: "button", disabled: stepIndex <= 0, onClick: () => setStepIndex((value: any) => Math.max(0, value - 1)), className: "btn btn-ghost gap-2", children: [jsx(ChevronRight, { className: "h-4 w-4" }, "icon"), "السابق"] }, "previous"),
      jsx(SaveIndicator, {
    ...
    jsxs("div", { className: "flex flex-wrap gap-2", children: [
      jsx("button", { type: "button", onClick: cancelEntry, className: "rounded-xl border border-[var(--va-border-soft)] px-3 py-2 text-xs text-[var(--va-text-2)] hover:bg-[var(--va-surface-2)]", children: "إلغاء" }, "cancel"),
      jsx("button", { type: "button", onClick: resetDraft, className: "rounded-xl border border-[var(--va-border-soft)] px-3 py-2 text-xs text-[var(--va-text-2)] hover:bg-[var(--va-surface-2)]", title: "مسح المسودة المحفوظة محليًا", children: "إعادة تعيين" }, "reset"),
  ...
  return jsxs(React.Fragment, {
    ...
    children: [
      jsx(PageHero, {
        ...
        children: jsxs("div", {
          ...
          children: [
            jsxs("div", {
              className: "rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface)] p-3 md:hidden",
              children: [
                jsxs("div", {
                  ...
                  children: [
                    jsxs("div", {
                      className: "min-w-0",
                      children: [
                        jsx("p", { className: "text-[11px] text-[var(--va-text-muted)]", children: `الخطوة ${stepIndex + 1} من ${STEPS.length}` }),
                        jsx("h2", { className: "mt-1 truncate text-base font-bold text-[var(--va-text)]", children: currentStep.label })
                      ]
                    }),
                    jsx(currentStep.icon, { className: "h-5 w-5 shrink-0 va-accent-text" })
                  ]
                }),
                jsx("p", { className: "mt-2 text-xs leading-6 text-[var(--va-text-muted)]", children: (stepsWithDetail.find((s: any) => s.id === currentStep.id) || currentStep).detail }),
                jsx("div", {
                  className: "mt-3 grid grid-cols-4 gap-1",
                  "aria-hidden": true,
                  children: STEPS.map((step: any, index: any) => jsx("span", {
            ...
            jsx("div", {
              ...
              children: jsx(WorkflowStepper, {
                ...
                completedStepIds: stepsWithDetail.slice(0, stepIndex).map((step: any) => step.id),
                onStepClick: (stepId: any) => setStepIndex(Math.max(0, STEPS.findIndex((step: any) => step.id === stepId))),
      ...
      jsxs("section", { className: "va-card rounded-2xl va-surface-muted border p-5 text-right", children: [
        jsxs("div", { className: "mb-4 flex items-center justify-between gap-3", children: [
          jsx("h2", { className: "text-lg font-bold text-[var(--va-text)]", children: currentStep.label }),
          jsxs("button", {
            type: "button",
            onClick: toggleSidePanel,
            "aria-pressed": showPanel,
            title: showPanel ? "إخفاء اللوحة الجانبية" : "إظهار اللوحة الجانبية",
            className: "inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-[var(--va-border-soft)] px-2.5 py-1.5 text-xs text-[var(--va-text-2)] transition-colors hover:bg-[var(--va-surface-2)] hover:text-[var(--va-text)]",
        ...
        currentStep.id === "basic" && jsxs("div", { className: "grid gap-4 md:grid-cols-2", children: [
          jsx("div", { className: "md:col-span-2 flex flex-wrap gap-2", children: [
            jsx("button", { type: "button", onClick: () => setShowTemplatePicker(true), className: "inline-flex items-center gap-2 rounded-xl border border-dashed border-[var(--va-border-strong)] px-4 py-2 text-sm text-[var(--va-text-2)] transition-colors hover:border-emerald-500/25 hover:text-[var(--va-text)]", children: [jsx(Sparkles, { className: "h-4 w-4 va-accent-text", "aria-hidden": "true" }, "icon"), "تطبيق قالب"] }),
            jsx("button", { type: "button", onClick: () => setShowQuickAdd((v: any) => !v), className: "inline-flex items-center gap-2 rounded-xl border border-dashed border-emerald-500/20 px-4 py-2 text-sm text-emerald-400 transition-colors hover:border-emerald-500/40 hover:text-emerald-300", children: [jsx("span", { className: "text-base leading-none", children: "⚡" }, "icon"), "إضافة سريعة"] })
          ] }),
          jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)] md:col-span-2", children: [jsx("label", { htmlFor: titleId, className: "block", children: "العنوان" }), jsx("input", { id: titleId, value: title, onChange: (event: any) => setTitle(event.target.value), className: "input input-bordered w-full", placeholder: "عنوان الفيديو" })] }),
          jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [jsx("label", { htmlFor: pathId, className: "block", children: "الرابط أو المسار" }), jsx("input", { id: pathId, value: path, onChange: (event: any) => setPath(event.target.value), dir: "ltr", className: "input input-bordered w-full", placeholder: "https:// أو D:\\..." })] }),
          jsxs("div", { className: "space-y-2 text-sm text-[var(--va-text-2)] md:col-span-2", children: [
            ...
            jsxs("div", { className: "flex flex-wrap gap-2", role: "radiogroup", "aria-label": "وضع الرفع", children: [
              jsxs("label", { className: `inline-flex cursor-pointer items-center gap-2 rounded-xl border px-3 py-1.5 ${uploadMode === "real" ? "border-emerald-500/40 text-[var(--va-text)]" : "border-[var(--va-border-soft)] text-[var(--va-text-2)]"}`, children: [
              ...
              jsxs("label", { className: `inline-flex cursor-pointer items-center gap-2 rounded-xl border px-3 py-1.5 ${uploadMode === "metadata" ? "border-emerald-500/40 text-[var(--va-text)]" : "border-[var(--va-border-soft)] text-[var(--va-text-2)]"}`, children: [
            ...
            jsxs("label", { className: "inline-flex cursor-pointer items-center gap-2 text-[var(--va-text-2)]", children: [
              jsx("input", { type: "checkbox", checked: archiveOnSave, onChange: (event: any) => setArchiveOnSave(event.target.checked), className: "accent-emerald-500" }),
              "أرشفة المادة المرفوعة مباشرة"
            ] })
          ] }),
          jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [jsx("label", { htmlFor: thumbnailId, className: "block", children: "الصورة المصغرة" }), jsx("input", { id: thumbnailId, value: thumbnail, onChange: (event: any) => setThumbnail(event.target.value), dir: "ltr", className: "input input-bordered w-full", placeholder: "رابط صورة اختياري" })] }),
          jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)] md:col-span-2", children: [jsx("label", { htmlFor: notesId, className: "block", children: "ملاحظات" }), jsx("textarea", { id: notesId, value: notes, onChange: (event: any) => setNotes(event.target.value), className: "textarea textarea-bordered w-full", placeholder: "ملخص أو ملاحظات أرشيفية" })] }),
        ...
        currentStep.id === "classify" && jsxs("div", { className: "grid gap-4 md:grid-cols-2", children: [
          jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [jsx("label", { htmlFor: typeSelectId, className: "block", children: "نوع المحتوى" }), jsxs("select", { id: typeSelectId, value: typeId, onChange: (event: any) => setTypeId(event.target.value), className: "min-h-11 w-full va-surface-deep rounded-xl border px-3 text-sm text-[var(--va-text)] outline-none", children: contentTypes.filter((type: any) => type.status !== "archived").map((type: any) => jsx("option", { value: type.id, children: type.name }, type.id)) })] }),
          jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [jsx("label", { htmlFor: subtypeSelectId, className: "block", children: "الفرع" }), jsxs("select", { id: subtypeSelectId, value: subtypeId, onChange: (event: any) => setSubtypeId(event.target.value), className: "min-h-11 w-full va-surface-deep rounded-xl border px-3 text-sm text-[var(--va-text)] outline-none", children: [jsx("option", { value: "", children: "بدون فرع" }, "empty"), ...subtypes.map((subtype: any) => jsx("option", { value: subtype.id, children: subtype.name }, subtype.id))] })] }),
          jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)] md:col-span-2", children: [jsx("label", { htmlFor: tagsId, className: "block", children: "الوسوم" }), jsx(TagAutocomplete, { id: tagsId, value: tags, onChange: setTags, placeholder: "وسوم مفصولة بفاصلة، ويمكن استخدام مسارات الوسوم الهرمية", allowed: ["tags", "vocabulary"] })] })
        ] }, "classify-step"),
        ...
        currentStep.id === "review" && jsxs("div", {
          ...
          children: [
            ...
            jsx("div", { className: "grid gap-3 md:grid-cols-2", children: [
              ...
              ["التصنيف", [selectedType?.name, subtypes.find((s: any) => s.id === subtypeId)?.name].filter(Boolean).join(" / ") || "غير محدد", 1],
              ["المصدر", path || metadata?.localFile?.name || "غير محدد", 0],
              ["الوسوم", parsedTags.length ? `${parsedTags.length} وسم` : "لا توجد", 1],
              ["الملاحظات", notes || "—", 0],
              ["حقول مخصصة", Object.keys(metadata).filter((k: any) => k !== "localFile").length ? `${Object.keys(metadata).filter((k: any) => k !== "localFile").length} حقل` : "لا توجد", 2]
            ].map(([label, value, toStep]: any) => jsxs("div", { className: "va-surface-muted rounded-xl border p-3", children: [
            ...
            children: [
              jsxs("div", { className: "va-card rounded-2xl va-surface-muted border p-4 text-right", children: [
                jsxs("div", { className: "flex items-center justify-between gap-3", children: [
                  jsxs("div", { className: "flex items-center gap-2", children: [
                    ...
                    jsx("h2", { className: "text-sm font-bold text-[var(--va-text)]", children: "جاهزية الحفظ" })
                  ] }),
                ...
                jsx("div", { className: "mt-3 grid gap-2 sm:grid-cols-2", children: readyChecks.map((check: any) => jsxs("span", { className: `inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-xs ${check.ok ? "va-accent-border va-accent-bg-soft va-accent-text-on-soft" : "border-[var(--va-border-soft)] bg-[var(--va-surface)] text-[var(--va-text-muted)]"}`, children: [
              ...
              jsxs("div", { className: "va-card rounded-2xl va-surface-muted border p-4 text-right", children: [
                jsxs("div", { className: "flex items-center gap-2", children: [
                  ...
                  jsx("h2", { className: "text-sm font-bold text-[var(--va-text)]", children: "ملخص مباشر" })
                ] }),
                jsx("div", { className: "mt-3 grid gap-2 text-sm text-[var(--va-text-2)]", children: [
                ...
                ].map(([label, value]: any) => jsxs("div", { className: "flex items-center justify-between gap-3 rounded-xl va-surface-subtle border px-3 py-2", children: [
                  jsx("span", { className: "text-[var(--va-text-muted)]", children: label }, "label"),
                  jsx("span", { className: "min-w-0 truncate text-[var(--va-text)]", children: value }, "value")
                ] }, label)) })
              ] }, "summary-card"),
              jsxs("div", { className: "va-card rounded-2xl va-surface-muted border p-4 text-right", children: [
                jsxs("div", { className: "flex items-center gap-2", children: [
                  jsx(Eye, { className: "h-5 w-5 text-cyan-300" }),
                  jsx("h2", { className: "text-sm font-bold text-[var(--va-text)]", children: "الخطوة الحالية" })
                ] }),
                jsx("p", { className: "mt-3 text-lg font-bold text-[var(--va-text)]", children: currentStep.label }),
                jsx("p", { className: "mt-1 text-sm leading-7 text-[var(--va-text-muted)]", children: currentStep.detail }),
                jsx("p", { className: "mt-3 rounded-xl va-surface-subtle border p-3 text-xs leading-6 text-[var(--va-text-muted)]", children: stepIndex === STEPS.length - 1 ? "راجع الملخص ثم احفظ، أو استخدم حفظ وإضافة آخر للعمل المتكرر." : "يمكنك الانتقال بين الخطوات بحرية؛ لن يتم الحفظ إلا من خطوة المراجعة." })
              ] }, "current-step-card")
            ]
          }, "side-panel")
        ]
    ...
    showQuickAdd && jsx("div", {
      ...
      children: jsx(QuickAddBar, {
        ...
        onDone: (count: any) => { setShowQuickAdd(false); showToast?.(`تمت إضافة ${count} عنصر`, "success"); },
