...
function parseConditionValue(condition: any) {
  const { operator, value } = condition;
  if (VALUELESS_OPERATORS.has(operator)) return null;
  if (operator === "between") {
    const [min, max] = String(value).split(/[,،-]/).map((part: any) => Number(part.trim()));
    return [Number.isFinite(min) ? min : 0, Number.isFinite(max) ? max : 0];
  }
  if (ARRAY_OPERATORS.has(operator)) {
    return String(value).split(/[,،\n]/).map((part: any) => part.trim()).filter(Boolean);
  }
  if (RULE_FIELDS[condition.field]?.type === "number") return Number(value);
  return value;
}

function buildRuleDraft(name: any, trigger: any, match: any, conditions: any, actions: any) {
  return createAutomationRule({
    ...
    actions: actions.map((action: any) => ({ type: action.type, value: action.value }))
  });
}

function ConditionRow({ condition, index, onChange, onRemove, disableRemove }: any) {
  ...
  return jsxs("div", {
    ...
    children: [
      jsx("select", {
        ...
        "aria-label": "الحقل",
        onChange: (event: any) => onChange(index, { field: event.target.value }),
        ...
        children: Object.entries(RULE_FIELDS).map(([key, spec]: any) => jsx("option", { value: key, children: spec.label }, key))
      }),
      jsx("select", {
        value: condition.operator,
        "aria-label": "العملية",
        onChange: (event: any) => onChange(index, { operator: event.target.value }),
        className: "min-h-10 w-full va-surface-deep rounded-lg border px-2 text-sm text-[var(--va-text)] outline-none focus:border-cyan-500/40",
        children: fieldSpec.operators.map((op: any) => jsx("option", { value: op, children: OPERATOR_LABELS[op] || op }, op))
      }),
      showValue ? jsx("input", {
        value: condition.value,
        "aria-label": "قيمة الشرط",
        onChange: (event: any) => onChange(index, { value: event.target.value }),
...
function actionValueControl(action: any, index: any, onChange: any, collections: any) {
  ...
  if (action.type === AUTOMATION_ACTIONS.ADD_TO_COLLECTION) {
    return jsx("select", {
      ...
      children: [
        ...
        ...collections.map((col: any) => jsx("option", { value: col.id, children: col.name || col.id }, col.id))
      ]
    });
  }
  return jsx("input", {
...
function ActionRow({ action, index, onChange, onRemove, disableRemove, collections }: any) {
  return jsxs("div", {
    className:
    ...
    children: [
      jsx("select", {
        ...
        onChange: (event: any) => onChange(index, { type: event.target.value, value: "" }),
        className: "min-h-10 w-full va-surface-deep rounded-lg border px-2 text-sm text-[var(--va-text)] outline-none focus:border-cyan-500/40",
        children: Object.values(AUTOMATION_ACTIONS).map((type: any) => jsx("option", { value: type, children: ACTION_LABELS[type] || type }, type))
      }),
...
function RuleEditorModal({ rule, videoItems, folders, collections, onCancel, onSave }: any) {
  const form = useRuleForm(rule);
  const nameId = React.useId();

  const updateCondition = (index: any, patch: any) => form.setConditions((prev: any) => prev.map((condition: any, idx: any) => {
  ...
  const updateAction = (index: any, patch: any) => form.setActions((prev: any) =>
    prev.map((action: any, idx: any) => (idx === index ? { ...action, ...patch } : action)));
  ...
  const matchCount = React.useMemo(() => {
    ...
    return videoItems.filter((item: any) => !item?.isDeleted && evaluateRule({ ...draft, actions: [{} as any] }, item, context).length > 0).length;
  ...
  return jsx(EntityFormModal, {
    ...
    children: jsxs("div", { className: "space-y-4", children: [
      jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [
        ...
        jsx("input", { id: nameId, "data-autofocus": true, value: form.name, onChange: (e: any) => form.setName(e.target.value), className: "min-h-11 w-full va-surface-deep rounded-xl border px-3 text-sm text-[var(--va-text)] outline-none focus:border-cyan-500/40", placeholder: "مثال: وسم مقاطع العمل تلقائياً" })
      ] }),
      jsxs("div", { className: "flex items-center gap-2 rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-2", children: [
        ...
        ...Object.values(AUTOMATION_TRIGGERS).map((value: any) => jsx("button", {
          type: "button",
      ...
      jsxs("div", { className: "flex items-center gap-2 rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-2", children: [
        ...
        ...["all", "any"].map((mode: any) => jsx("button", {
          type: "button",
      ...
      jsx("div", { className: "space-y-2", children: form.conditions.map((condition: any, index: any) => jsx(ConditionRow, {
        condition, index, onChange: updateCondition,
        onRemove: (i: any) => form.setConditions((prev: any) => (prev.length <= 1 ? prev : prev.filter((_: any, idx: any) => idx !== i))),
        disableRemove: form.conditions.length <= 1
      }, index)) }),
      jsxs("button", { type: "button", onClick: () => form.setConditions((prev: any) => [...prev, blankCondition()]), className: "inline-flex items-center gap-1.5 rounded-lg border border-cyan-500/20 bg-cyan-500/5 px-3 py-1.5 text-sm font-medium text-cyan-300 hover:bg-cyan-500/15", children: [jsx(Plus, { className: "h-3.5 w-3.5" }), "إضافة شرط"] }),
      jsx("div", { className: "border-t border-[var(--va-border-soft)] pt-3 text-sm font-semibold text-[var(--va-text-2)]", children: "عندئذٍ نفّذ" }),
      jsx("div", { className: "space-y-2", children: form.actions.map((action: any, index: any) => jsx(ActionRow, {
        action, index, onChange: updateAction, collections,
        onRemove: (i: any) => form.setActions((prev: any) => (prev.length <= 1 ? prev : prev.filter((_: any, idx: any) => idx !== i))),
        disableRemove: form.actions.length <= 1
      }, index)) }),
      jsxs("div", { className: "flex flex-wrap items-center justify-between gap-2", children: [
        jsxs("button", { type: "button", onClick: () => form.setActions((prev: any) => [...prev, blankAction()]), className: "inline-flex items-center gap-1.5 rounded-lg border border-cyan-500/20 bg-cyan-500/5 px-3 py-1.5 text-sm font-medium text-cyan-300 hover:bg-cyan-500/15", children: [jsx(Plus, { className: "h-3.5 w-3.5" }), "إضافة إجراء"] }),
...
function RuleCard({ rule, onToggle, onEdit, onRemove }: any) {
  return jsxs("div", {
...
export function AutomationPage() {
  ...
  const [editing, setEditing] = React.useState<any>(null); // null | "new" | rule
  ...
  const saveRule = async (value: any) => {
    if (value.id && automationRules.some((rule: any) => rule.id === value.id)) {
  ...
  return jsxs(motion.div, {
    ...
    children: [
      ...
      automationRules.length === 0 ? jsx("div", {
        ...
        children: automationRules.map((rule: any) => jsx(RuleCard, {
          rule,
          onToggle: (id: any) => toggleAutomationRule?.(id),
          onEdit: (target: any) => setEditing(target),
          onRemove: (id: any) => removeAutomationRule?.(id)
        }, rule.id))
      }),
