...
function CollectionForm({ collection, onCancel, onSave }: any) {
    ...
    children: jsxs("div", {
      ...
      children: [
        jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [
          ...
          jsx("input", { id: iconId, value: icon, onChange: (event: any) => setIcon(event.target.value.slice(0, 4)), className: "input input-bordered w-full text-center text-xl" })
        ] }),
        jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [
          jsx("label", { htmlFor: nameId, className: "block", children: "اسم المجموعة" }),
          jsx("input", { id: nameId, ref: nameRef, "data-autofocus": true, value: name, onChange: (event: any) => setName(event.target.value), className: "input input-bordered w-full", placeholder: "مثال: مقابلات مهمة" })
        ] }),
        jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)] md:col-span-2", children: [
          ...
          jsx("textarea", { id: descriptionId, value: description, onChange: (event: any) => setDescription(event.target.value), className: "textarea textarea-bordered w-full", placeholder: "ملاحظة قصيرة عن استخدام المجموعة" })
        ] }),
...
function CollectionCard({ collection, itemCount, active, index, onOpen, onEdit, onDelete, onDrop }: any) {
  ...
  const handleDragLeave = React.useCallback((event: any) => {
...
function CollectionAddPicker({ availableItems, typeLabel, onAddItems }: any) {
  ...
  const filteredAvailable = normalized
    ? availableItems.filter((item: any) => (item.title || "").toLowerCase().includes(normalized))
    : availableItems;
  const filteredIds = filteredAvailable.map((item: any) => item.id);
  const allFilteredSelected = filteredIds.length > 0 && filteredIds.every((id: any) => selectedIds.includes(id));

  const toggle = (id: any) => setSelectedIds((prev: any) => prev.includes(id) ? prev.filter((x: any) => x !== id) : [...prev, id]);
  const toggleSelectAll = () => setSelectedIds(allFilteredSelected
    ? selectedIds.filter((id: any) => !filteredIds.includes(id))
  ...
  return jsxs("div", {
    ...
    children: [
      ...
      jsxs("div", { className: "relative", children: [
        ...
        jsx("input", { value: addQuery, onChange: (event: any) => setAddQuery(event.target.value), placeholder: "ابحث عن عنصر لإضافته...", "aria-label": "بحث عن عنصر لإضافته", className: "input input-bordered w-full" })
      ] }),
      jsx("div", {
        role: "listbox",
        ...
        children: filteredAvailable.length ? filteredAvailable.slice(0, 250).map((item: any) => {
...
function CollectionItemRow({ item, typeLabel, canManageItems, onOpen, onRemove }: any) {
  const label = typeLabel?.(item);
  return jsxs("div", {
...
function CollectionDetails({ collection, items, availableItems, typeLabel, onAddItems, onRemoveItem, onOpenItem, onShare, shareEnabled, onCopyTitles, onBack }: any) {
  ...
  const filteredItems = normalizedItemQuery
    ? items.filter((item: any) => (item.title || "").toLowerCase().includes(normalizedItemQuery))
    : items;
    ...
    children: [
      ...
      jsxs("div", {
        ...
        children: [
          ...
          jsxs("div", { className: "flex items-center gap-2", children: [
            items.length > 6 && jsx("input", { value: itemQuery, onChange: (event: any) => setItemQuery(event.target.value), placeholder: "تصفية...", "aria-label": "تصفية عناصر المجموعة", className: "input input-bordered w-full" }),
      ...
      items.length ? (filteredItems.length ? jsx("div", {
        className: "space-y-2",
        children: filteredItems.slice(0, 200).map((item: any) => jsx(CollectionItemRow, {
          item,
          typeLabel,
          canManageItems,
...
function SaveFilterModal({ onSave, onCancel, initialQuery }: any) {
        ...
        jsxs("label", {
          ...
          children: [
            jsx("input", {
              ...
              type: "checkbox",
              checked: isLive,
              onChange: (e: any) => setIsLive(e.target.checked),
...
export function CollectionsPage() {
  ...
  const isRuleCollection = (collection: any) => collection?.type === "smart" && collection?.filterRules?.kind === "rules";
  ...
  const saveSmartCollection = async (value: any) => {
    try {
      if (editingSmart) {
  ...
  const handleSaveFilter = async (name: any, isLive: any) => {
    try {
  ...
  const handleOpenSavedFilter = (filter: any) => {
  ...
  const typeById = React.useMemo(() => new Map(contentTypes.map((type: any) => [type.id, type])), [contentTypes]);
  const typeLabel = React.useCallback((item: any) => (typeById.get(item?.type) as any)?.name || "", [typeById]);

  const filteredCollections = React.useMemo(() => getFilteredCollections(virtualCollections, query), [query, virtualCollections]);
  const selectedCollection = virtualCollections.find((collection: any) => collection.id === selectedCollectionId) || filteredCollections[0] || null;
  const openedCollection = openedCollectionId
    ? virtualCollections.find((collection: any) => collection.id === openedCollectionId) || null
    : null;
  ...
  React.useEffect(() => {
    if (selectedCollectionId && virtualCollections.some((collection: any) => collection.id === selectedCollectionId)) return;
  ...
  React.useEffect(() => {
    if (openedCollectionId && !virtualCollections.some((collection: any) => collection.id === openedCollectionId)) {
  ...
  const saveCollection = async (draft: any, opts: any = {}) => {
    try {
      if (editingCollection) {
        const updated = createVirtualCollectionValue({ ...editingCollection, ...draft, createdAt: (editingCollection as any).createdAt });
  ...
  const copyTitles = async (list: any = []) => {
    const text = list.map((item: any) => item.title || "بدون عنوان").join("\n");
    if (!text) return;
    try {
  ...
  const handleSharedCollection = async ({ url, permission, jti, expiresAt, passwordProtected }: any = {}) => {
    const targetLabel = (sharingCollection as any)?.name || "مجموعة";
  ...
  return jsxs(motion.div, {
    ...
    children: [
      ...
      sharingCollection && jsx(ShareDialog, {
        ...
        scopeIds: [(sharingCollection as any).id],
        label: (sharingCollection as any).name || "مجموعة",
        title: (sharingCollection as any).name ? `مراجعة: ${(sharingCollection as any).name}` : "مراجعة أرشيف مشتركة",
      ...
      virtualCollections.length > 0 && jsx(EntityFoldersPanel, {
        ...
        getEntityLabel: (collection: any) => collection.name || collection.id,
        getEntityMeta: (collection: any) => collection.type === "smart" ? "ذكية" : "يدوية"
      }),
      virtualCollections.length === 0 ? jsx("div", {
        ...
        jsxs("label", { className: "va-filter-surface relative block rounded-2xl va-surface-muted border p-3", children: [
          jsx(Search, { className: "pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--va-text-muted)]" }),
          jsx("input", { value: query, onChange: (event: any) => setQuery(event.target.value), placeholder: "بحث في المجموعات...", className: "input input-bordered w-full" })
        ] }),
        filteredCollections.length ? jsx("div", { className: "grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4", children: filteredCollections.map((collection: any, index: any) => jsx(CollectionCard, {
          collection,
          index,
          ...
          onDrop: collection.type !== "smart" ? (ids: any) => addItemsToCollection?.(collection.id, ids) : undefined
      ...
      jsxs("section", {
        ...
        children: [
          ...
          jsx("div", {
            ...
            children: savedFilters.map((filter: any) => jsx(SavedFilterCard, {
              filter,
