...
const SAFETY_STEP_ORDER = DATA_SAFETY_STEPS.map((step: any) => step.id);

function getCompletedSafetySteps(activeStepId: any) {
...
export function DataCenterPage() {
  ...
  const syncPeerList = React.useMemo(
    () => Object.values(syncPeers).sort((a: any, b: any) => (new Date(b.lastSentAt || 0).getTime()) - (new Date(a.lastSentAt || 0).getTime())),
    [syncPeers]
  );
  ...
  const itemsById = React.useMemo(() => new Map(videoItems.map((item: any) => [item.id, item])), [videoItems]);
  const projectTimelines = React.useMemo(() => projects
    .filter((project: any) => project.status !== "archived" && (project.roughCuts || []).length)
    .map((project: any) => ({ project, timeline: buildProjectTimeline(project, itemsById) }))
    .filter((row: any) => row.timeline.clips.length > 0), [itemsById, projects]);
  ...
  const healthCards = React.useMemo(() => {
    const lastError = auditLogs.find((log: any) => /error|fail|denied|conflict/i.test(String(log.eventType || "")));
    return [
      {
        id: "storage",
        label: "التخزين",
  ...
  const activeSafetyStep = isWorking
    ...
    : activeTab === "backup" || activeTab === "transfer"
      ...
      : activeTab === "import" && importPreview
        ? "preview"
        : "preflight";
  const activeTask = DATA_CENTER_TABS.find((tab: any) => tab.id === activeTab) || DATA_CENTER_TABS[0];
  const taskCounts = {
    export: exportPayload.videoItems?.length || 0,
    import: (importPreview as any)?.summary?.totals?.records || importErrors.length || 0,
  ...
  const loadBackups = React.useCallback(async () => {
    try {
      ...
      setBackups(rows.slice().sort((a: any, b: any) => new Date(b.timestamp || 0).getTime() - new Date(a.timestamp || 0).getTime()));
    } catch (error: any) {
  ...
  React.useEffect(() => {
    const handleDataTab = (event: any) => {
      const tab = event?.detail?.tab;
      if (DATA_CENTER_TABS.some((item: any) => item.id === tab)) setActiveTabState(tab);
    };
  ...
  const runPreflight = async (kind: any, summary: any = {}) => {
  ...
  const makeFileName = (prefix: any, extension: any) => `${prefix}-${new Date().toISOString().slice(0, 10)}.${extension}`;

  const handleExport = async (kind: any = selectedFormat) => {
    setIsWorking(true);
    setOperationMessage("");
    ...
    try {
      ...
      if (kind === "json") {
        ...
        const payload = {
          ...
          projects: projectTimelines.map((row: any) => row.timeline)
        };
        ...
        projectTimelines.forEach(({ project }: any) => {
  ...
  const handleApplyImport = async () => {
    if (!(importPreview as any)?.payload) return;
    ...
    setOperationMessage("");
    try {
      const ok = await runPreflight("import", (importPreview as any).summary.totals);
      if (!ok) return;
      await createBackup?.(`قبل استيراد ${(importPreview as any).fileName}`);
      const result = await importNormalizedPayload((importPreview as any).payload, importMode);
  ...
  const handleSmartMerge = async () => {
    if (!(importPreview as any)?.payload || !planIncomingDelta) return;
    setIsWorking(true);
    setOperationMessage("");
    try {
      const plan = planIncomingDelta((importPreview as any).payload, {
        baseSyncFloor: (importPreview as any).payload.baseSyncFloor || {}
      });
      if (!plan) {
      ...
      if (!plan.summary.needsReview) {
        ...
        // ride along.
        await createBackup?.(`قبل دمج ${(importPreview as any).fileName}`);
        await applyResolvedDelta?.({ autoApply: plan.autoApply, resolved: {} });
        const result = await importNormalizedPayload((importPreview as any).payload, "merge");
  ...
  const handleConflictResolve = async (resolved: any) => {
    if (!conflictPlan || !(importPreview as any)?.payload) return;
    setIsWorking(true);
    setOperationMessage("");
    try {
      await createBackup?.(`قبل حل تعارضات ${(importPreview as any).fileName}`);
      await applyResolvedDelta?.({ autoApply: (conflictPlan as any).autoApply, resolved });
      ...
      await loadAllData?.();
      await loadBackups();
      const counts = (conflictPlan as any).summary;
  ...
  const renderExportPanel = () => jsxs(Fragment, {
    children: [
      ...
      jsx("div", {
        ...
        ].map((option: any) => jsx(SegmentedButton, {
          ...
          jsxs("label", {
            ...
            children: [
              ...
              jsxs("select", {
                ...
                onChange: (event: any) => setCollectionFilter(event.target.value),
                className: "select select-bordered w-full",
                children: [
                  jsx("option", { value: "all", children: "كل المجموعات" }),
                  ...virtualCollections.map((collection: any) => jsx("option", { value: collection.id, children: collection.name || collection.id }, collection.id))
                ]
              })
            ]
          }),
          jsxs("label", {
            ...
            children: [
              jsx("input", {
                ...
                onChange: (event: any) => setFavoritesOnly(event.target.checked),
    ...
    children: [
      jsxs("div", {
        ...
        children: [
          ...
          jsx(ActionButton, {
            onClick: () => (importInputRef.current as any)?.click(),
            disabled: isWorking,
      ...
      importErrors.length > 0 && jsx("div", {
        ...
        role: "alert",
        children: importErrors.map((error: any) => jsx("p", { children: error }, error))
      }),
      jsx("div", {
        className: "mt-5",
      ...
      jsx(SyncConflictDialog, {
        ...
        conflicts: (conflictPlan as any)?.needsReview || [],
          ...
          jsxs("div", {
            ...
            children: [
              ...
              jsx("input", {
                ...
                onChange: (event: any) => setSourceDeviceName(event.target.value),
          ...
          jsxs("div", {
            ...
            children: [
              ...
              jsx("ul", {
                ...
                ].map((item: any) => jsx("li", { children: item }, item))
              })
            ]
          })
        ]
          ...
          jsxs("div", {
            ...
            children: [
              jsxs("label", { className: "block text-xs text-[var(--va-text-muted)]", children: [
                ...
                jsxs("select", {
                  value: deltaTargetId,
                  onChange: (event: any) => setDeltaTargetId(event.target.value),
                  className: "select select-bordered w-full",
                  children: [
                    jsx("option", { value: "", children: "— جديد —" }),
                    ...syncPeerList.map((peer: any) => jsx("option", { value: peer.deviceId, children: `${peer.deviceName}${peer.lastSentAt ? ` (آخر مزامنة: ${formatDateTime(peer.lastSentAt)})` : ""}` }, peer.deviceId))
                  ]
                })
              ] }),
              !deltaTargetId && jsxs("label", { className: "block text-xs text-[var(--va-text-muted)]", children: [
                ...
                jsx("input", {
                  ...
                  onChange: (event: any) => setDeltaNewDeviceName(event.target.value),
    ...
    children: [
      ...
      jsx("div", {
        ...
        children: backups.length
          ? backups.map((backup: any) => jsxs("div", {
    ...
    children: [
      ...
      jsxs("section", {
        ...
        children: [
          ...
          jsx("div", { className: "grid gap-3 md:grid-cols-2 xl:grid-cols-3", children: healthCards.map((card: any) => {
      ...
      jsxs("section", {
        ...
        children: [
          ...
          jsx("div", {
            ...
            children: DATA_CENTER_TABS.map((task: any) => jsx(TaskChoiceCard, {
              task,
              active: activeTab === task.id,
              count: (taskCounts as any)[task.id],
      ...
      jsx(CollapsibleSummary, {
        ...
        open: summaryOpen,
        onToggle: () => setSummaryOpen((value: any) => !value),
        children: jsxs("section", {
          className: "grid gap-4 sm:grid-cols-2 xl:grid-cols-4",
          children: [
            jsx(DataMetric, { label: "العناصر", value: formatNumber(videoItems.length), hint: `${formatNumber(videoItems.filter((item: any) => !item.isDeleted).length)} عنصر نشط`, icon: jsx(FileText, { className: "h-5 w-5" }) }),
      ...
      jsx(MobileActionBar, {
        ...
        actions: [
          ...
          { id: "import", label: "استيراد", icon: Upload, active: activeTab === "import", disabled: isWorking, onClick: () => { setActiveTab("import"); (importInputRef.current as any)?.click(); } },
