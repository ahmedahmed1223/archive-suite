...
function LocalFilePicker({ value, onFileSelect }: any) {
  ...
  return jsxs("div", { className: "rounded-xl border border-dashed border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-3", children: [
    jsxs("div", { className: "flex flex-wrap items-center justify-between gap-3", children: [
      ...
      jsx("button", { type: "button", onClick: () => (inputRef.current as any)?.click(), className: "btn btn-primary btn-sm", children: "استعراض" })
    ] }),
 ...
 */
function GroupedFields({ fields, renderField, gap = "gap-4" }: any) {
  ...
  return jsxs("div", {
    className: "space-y-3",
    children: [
      tabbed ? jsx("div", { role: "tablist", className: "flex flex-wrap gap-1 overflow-x-auto rounded-xl va-surface-muted border p-1", children: groups.map((group: any, index: any) => jsxs("button", {
        type: "button",
        role: "tab",
...
function EditableField({ field, value, onChange }: any) {
  const key = fieldKey(field);
  const commonClass = "input input-bordered w-full";
  if (field.type === "textarea" || field.type === "transcript") return jsx("textarea", { value: value || "", onChange: (event: any) => onChange(key, event.target.value), rows: 3, className: `${commonClass} p-3` });
  if (field.type === "checkbox") return jsx("input", { type: "checkbox", checked: !!value, onChange: (event: any) => onChange(key, event.target.checked), className: "h-5 w-5" });
  if (field.type === "select" || field.type === "radio") return jsxs("select", { value: value || "", onChange: (event: any) => onChange(key, event.target.value), className: commonClass, children: [
    jsx("option", { value: "", children: "اختر..." }),
    ...(field.options || []).map((option: any) => jsx("option", { value: option, children: option }, option))
  ] });
  if (field.type === "multiselect") {
    ...
    return jsx("select", {
      multiple: true,
      value: selected,
      onChange: (event: any) => onChange(key, Array.from(event.target.selectedOptions, (option: any) => option.value)),
      className: `${commonClass} min-h-28 py-2`,
      children: (field.options || []).map((option: any) => jsx("option", { value: option, children: option }, option))
    });
  }
  if (field.type === "tags") return jsx("input", { value: Array.isArray(value) ? value.join("، ") : value || "", onChange: (event: any) => onChange(key, parseVideoTags(event.target.value)), className: commonClass });
  if (field.type === "localFile") return jsx(LocalFilePicker, { value, onFileSelect: (file: any) => onChange(key, createLocalFileValue(file)) });
  if (field.type === "rating") return jsx("div", { className: "flex min-h-11 items-center", children: jsx(StarRating, { value: Number(value) || 0, onChange: (val: any) => onChange(key, val) }) });
  return jsx("input", { type: field.type === "number" ? "number" : field.type === "date" ? "date" : field.type === "url" ? "url" : "text", value: value || "", onChange: (event: any) => onChange(key, event.target.value), className: commonClass });
}

function ReadonlyField({ field, value }: any) {
...
function FieldAclControls({ fieldKey, acl, onChange }: any) {
  const selected = acl?.[fieldKey] || [];
  const unrestricted = !selected.length;
  const toggle = (role: any) => {
    const next = selected.includes(role) ? selected.filter((item: any) => item !== role) : [...selected, role];
  ...
  return jsxs("div", { className: "mt-2 flex flex-wrap items-center gap-1.5", children: [
    ...
    ...FIELD_ACL_ROLES.map((role: any) => jsx("button", {
      type: "button",
      onClick: () => toggle(role),
      ...
      children: (FIELD_ROLE_LABELS as any)[role] || role
    }, role))
  ] });
}

function TranscriptSyncWorkbench({ segments = [], currentTime = 0, onSeek }: any) {
  ...
  const visibleSegments = React.useMemo(() => {
    ...
    return segments.filter((segment: any) => normalizeArabicSearchText(segment.text).includes(normalized));
  }, [query, segments]);
  ...
  return jsxs("section", { className: "rounded-2xl va-surface-subtle border p-4 text-right", dir: "rtl", children: [
    jsxs("div", { className: "flex flex-wrap items-start justify-between gap-3", children: [
      ...
      jsx("span", { dir: "ltr", className: "rounded-full border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] px-3 py-1 font-[family-name:var(--va-font-mono)] text-xs text-[var(--va-text-2)]", children: segments.find((segment: any) => segment.index === activeIndex)?.timecode || "0:00" })
    ] }),
    ...
    jsx("div", { className: "mt-3 max-h-[360px] space-y-2 overflow-y-auto pe-1", children: visibleSegments.map((segment: any) => {
...
function PreviewAction({ onClick, children, icon, primary = false, disabled = false }: any) {
  return jsxs("button", {
    type: "button",
    onClick,
    disabled,
...
function MediaPreviewFallback({ state, descriptor, onCopyPath, onEditPath, onMetadataOnly }: any) {
  const content = (PREVIEW_STATE_COPY as any)[state] || PREVIEW_STATE_COPY[MEDIA_PREVIEW_STATUS.TIMED_OUT];
...
function MediaToolButton({ icon, label, busy, disabled, onClick }: any) {
  return jsxs("button", {
    type: "button",
    onClick,
    disabled: disabled || busy,
    className: "inline-flex min-h-9 items-center justify-center gap-1.5 rounded-lg border border-[var(--va-border-soft)] bg-white/[0.035] px-3 py-1.5 text-xs font-semibold text-[var(--va-text-2)] transition-colors hover:bg-white/[0.07] disabled:cursor-not-allowed disabled:opacity-45",
    children: [busy ? jsx(Loader2, { className: "h-3.5 w-3.5 animate-spin" }) : icon, label]
  });
}

function MediaJobsMiniList({ jobs, onRetry }: any) {
  if (!jobs.length) return jsx("p", { className: "text-xs text-[var(--va-text-muted)]", children: "لا توجد مهام وسائط حديثة." });
  return jsx("ul", { className: "space-y-2", children: jobs.slice(0, 4).map((job: any) => {
    ...
    return jsxs("li", { className: "rounded-xl va-surface-subtle border p-3", children: [
      jsxs("div", { className: "flex items-center justify-between gap-2", children: [
        jsxs("div", { className: "min-w-0", children: [
          jsx("p", { className: "truncate text-xs font-semibold text-[var(--va-text-2)]", children: job.type === "montage" ? "مونتاج" : "تحويل نسخة ويب" }),
          jsx("p", { className: `mt-0.5 text-[11px] ${status.tone}`, children: status.label })
        ] }),
        jsx("span", { dir: "ltr", className: "font-mono text-xs text-[var(--va-text-muted)]", children: `${Math.round(status.progress)}%` })
      ] }),
      jsx("div", { className: "mt-2 h-1.5 overflow-hidden rounded-full bg-[var(--va-surface-2)]", children: jsx("div", { className: "h-full rounded-full bg-[var(--va-action)]", style: { width: `${status.progress}%` } }) }),
      job.outputKey && jsx("p", { dir: "ltr", className: "mt-2 truncate text-left font-mono text-[11px] text-[var(--va-text-muted)]", children: job.outputKey }),
...
export function DetailPage() {
  ...
  const navOrder = React.useMemo(
    () => (navItemIds.length ? navItemIds : videoItems.map((video: any) => video.id)),
    [navItemIds, videoItems]
  );
  const navPosition = getItemPosition(selectedItemId, navOrder);
  const goToAdjacent = React.useCallback((direction: any) => {
  ...
  const [importedCues, setImportedCues] = React.useState<any[]>([]);
  ...
  const transcriptCues = React.useMemo(() => {
    const timed = transcriptSegments.filter((segment: any) => Number.isFinite(segment.seconds));
    return segmentsToCues(timed.map((segment: any, index: any) => ({
      start: segment.seconds,
      end: Number.isFinite(timed[index + 1]?.seconds) ? (timed[index + 1] as any).seconds : segment.seconds + 4,
      text: segment.text
    })));
  }, [transcriptSegments]);
  ...
  const autosaveEngineRef = React.useRef<any>(null);
  const draftRef = React.useRef(null);
  const [autosaveStatus, setAutosaveStatus] = React.useState<any>("idle");
  ...
  React.useEffect(() => {
    ...
    const storage = {
      get: (k: any) => { try { return Promise.resolve(JSON.parse(localStorage.getItem(k) || "null")); } catch { return Promise.resolve(null); } },
      put: (k: any, v: any) => { try { localStorage.setItem(k, JSON.stringify(v)); } catch { /* quota/private */ } return Promise.resolve(); },
      delete: (k: any) => { try { localStorage.removeItem(k); } catch { /* ignore */ } return Promise.resolve(); }
    };
  ...
  React.useEffect(() => {
    ...
    if (!engine) return;
    if (editing) {
      (engine as any).start(() => draftRef.current);
      setAutosaveStatus((engine as any).getStatus());
      const pollId = setInterval(() => setAutosaveStatus((engine as any).getStatus()), 2000);
  ...
  const aiSuggestTags = async () => {
    const result = await ai.suggestTags(buildSuggestPayload({ title: (draft as any)?.title, notes: (draft as any)?.notes, contentTypes }));
    if (!result) return;
    setEditing(true);
    updateDraft({ tagsText: mergeTagText((draft as any)?.tagsText, (result as any).tags) });
    showToast?.((result as any).tags?.length ? `أُضيفت ${(result as any).tags.length} وسوم مقترحة` : "لا وسوم جديدة مقترحة", "success");
  };
  const aiSummarize = async () => {
    if (!hasSourceText({ title: (draft as any)?.title, notes: (draft as any)?.notes })) { showToast?.("أضف ملاحظات أو عنوانًا أولاً للتلخيص.", "warning"); return; }
    const result = await ai.summarize((draft as any)?.notes?.trim() || (draft as any)?.title || "");
    if (!result) return;
    setEditing(true);
    updateDraft({ notes: applySummaryToNotes((draft as any)?.notes, (result as any).summary), tagsText: mergeTagText((draft as any)?.tagsText, (result as any).tags) });
    showToast?.("أُضيف ملخّص إلى الملاحظات", "success");
  };
  const aiProofread = async () => {
    if (!String((draft as any)?.notes || "").trim()) { showToast?.("لا توجد ملاحظات لتدقيقها.", "warning"); return; }
  ...
  const fields = React.useMemo(() => item ? getFieldsForSelection(contentTypes, (draft as any)?.type || item.type, (draft as any)?.subtype || item.subtype) : [] as any[], [contentTypes, (draft as any)?.subtype, (draft as any)?.type, item]);
  const canManageFieldAcl = currentUser?.role === "admin";
  const canEditTags = canViewField("tags", (draft as any)?.fieldAcl || item?.fieldAcl, currentUser);
  const canEditNotes = canViewField("notes", (draft as any)?.fieldAcl || item?.fieldAcl, currentUser);
  ...
  const editFields = React.useMemo(() => getVisibleFields(fields as any, (draft as any)?.metadata || {}).filter((field: any) => canViewField(fieldKey(field), (draft as any)?.fieldAcl || item?.fieldAcl, currentUser)), [currentUser, (draft as any)?.fieldAcl, (draft as any)?.metadata, fields, item?.fieldAcl]);
  const allReadFields = React.useMemo(() => item ? getVisibleFields(fields as any, item.metadata || {}) : [], [fields, item]);
  const readFields = React.useMemo(() => allReadFields.filter((field: any) => canViewField(fieldKey(field), item?.fieldAcl, currentUser)), [allReadFields, currentUser, item?.fieldAcl]);
  ...
  const selectedType = contentTypes.find((type: any) => type.id === ((draft as any)?.type || item?.type));
  ...
  const explicitRelationIds = React.useMemo(() => [
    ...(explicitItemRelations.outgoing || []).map((relation: any) => relation.targetId),
    ...(explicitItemRelations.incoming || []).map((relation: any) => relation.sourceId)
  ...
  const itemHistory = React.useMemo(
    () => (changeHistory || []).filter((record: any) => record.itemId === item?.id && Array.isArray(record.changes) && record.changes.length > 0),
    [changeHistory, item?.id]
  );
  const subtypes = selectedType?.subtypes || [];
  const history = React.useMemo(() => item ? changeHistory.filter((record: any) => record.itemId === item.id).sort((a: any, b: any) => new Date(b.timestamp || 0).getTime() - new Date(a.timestamp || 0).getTime()).slice(0, 10) : [], [changeHistory, item]);
  ...
  const deleteDerivedFile = React.useCallback(async (file: any) => {
  ...
  const refreshMediaJobs = React.useCallback(async () => {
    ...
    try {
      ...
      setMediaJobs(mergeMediaJobs(jobs as any) as any);
    } catch {
      setMediaJobs([]);
    }
  ...
  React.useEffect(() => {
    ...
    const timeoutId = timeoutApi.setTimeout(() => {
      setPreviewRuntimeState((current: any) => (
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      window.requestAnimationFrame(() => {
        ...
        if (!video) return;
        (video as any).currentTime = nextTime;
  ...
  React.useEffect(() => {
    ...
    Promise.resolve(updateVideoItem?.(updated))
      ...
      .catch((error: any) => reportError(showNotification, error, { context: "ربط ملف مشتق" }));
  ...
  const updateDraft = (patch: any) => setDraft((current: any) => ({ ...current, ...patch }));
  const updateMetadata = (key: any, value: any) => setDraft((current: any) => ({ ...current, metadata: { ...(current.metadata || {}), [key]: value } }));
  const updateFieldAcl = (key: any, roles: any) => setDraft((current: any) => {
    const acl = normalizeFieldAcl(current?.fieldAcl || {});
    if (!roles?.length) delete acl[key];
    else acl[key] = roles;
    return { ...current, fieldAcl: acl };
  });
  const handleImprovementSuggestion = (suggestion: any) => {
  ...
  const handleRecommendationFeedback = (suggestion: any, value: any) => {
  ...
  const applyPrimaryLocalFile = (file: any) => {
    const patch = createVideoLocalFilePatch(file, { currentTitle: (draft as any)?.title || item.title });
    if (!patch) return;
    setDraft((current: any) => ({
      ...current,
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      const mentionedUsers = users.filter((user: any) => mentionUsernames.some((username: any) => username.toLowerCase() === String(user.username || "").toLowerCase()));
      showNotification?.("تمت إضافة تعليق على هذه المادة.", {
        ...
        targetLabel: mentionUsernames.length
          ? `${item.title || "مادة"} · ${mentionedUsers.map((user: any) => user.displayName || user.username).join("، ") || mentionUsernames.join("، ")}`
          : item.title || "مادة",
  ...
  const handleRemoveRelation = async (relationId: any) => {
    try {
  ...
  const seekToBookmark = (timestamp: any) => {
    const video = videoRef.current;
  ...
  const addTimeBookmark = async (bookmark: any) => {
    try {
  ...
  const editTimeBookmark = async (id: any, changes: any) => {
    try {
  ...
  const renderTextWithTimestamps = (text: any) => {
    ...
    return parseTimestampSegments(text).map((seg: any, index: any) => {
  ...
  const restoreVersion = async (record: any) => {
  ...
  const save = async () => {
    const missingRequired = getMissingRequiredFields(fields as any, draft.metadata || {});
    if (missingRequired.length) {
      showToast?.(`حقول مطلوبة فارغة: ${missingRequired.map((field: any) => field.label).join("، ")}`, "error");
      return;
    }
    const updated = createVideoItemValue({
      ...item,
    ...
    try {
      ...
      (autosaveEngineRef.current as any)?.stop();
      setAutosaveStatus("idle");
    } catch (error: any) {
  ...
  const handleSharedItem = async ({ url, permission, jti, expiresAt, passwordProtected }: any = {}) => {
  ...
  const startPathEdit = () => {
    ...
    const focusPathInput = () => {
      (pathInputRef.current as any)?.focus();
      (pathInputRef.current as any)?.select?.();
    };
  ...
  const showMetadataOnly = () => {
    (metadataSectionRef.current as any)?.scrollIntoView({ block: "start", behavior: "smooth" });
  };

  const saveMediaPatch = async (patch: any, message: any) => {
    const metadata = {
      ...(item.metadata || {}),
      ...(patch.metadata || {}),
      media: {
  ...
  const runMediaAction = async (id: any, action: any) => {
    if (mediaUnavailable) {
  ...
  const runAudio = () => runMediaAction("audio", async (client: any) => {
  ...
  const runPreviewGif = () => runMediaAction("preview", async (client: any) => {
  ...
  const runTranscode = () => runMediaAction("transcode", async (client: any) => {
    ...
    if (result?.job) setMediaJobs((current: any) => mergeMediaJobs([result.job, ...current]));
  });

  const handleConversionJobCreated = React.useCallback((result: any, recipe: any) => {
    if (result?.job) setMediaJobs((current: any) => mergeMediaJobs([result.job, ...current]));
  ...
  const retryMediaJob = async (jobId: any) => {
    await runMediaAction(`retry-${jobId}`, async (client: any) => {
      await client.retryJob(jobId);
      showToast?.("أُعيدت المهمة إلى الطابور", "info");
    });
  };

  const handleWorkflowChanged = async (result: any) => {
  ...
  return jsxs(MotionPage, {
    ...
    children: [
      ...
      jsxs("div", { className: "flex flex-wrap items-center justify-between gap-3", children: [
        jsxs("nav", { className: "flex items-center gap-2 text-sm text-[var(--va-text-muted)]", "aria-label": "مسار التنقل", children: [
          jsxs("button", { type: "button", onClick: () => setCurrentPage?.("archive"), className: "inline-flex items-center gap-1.5 rounded-lg px-2 py-1 text-[var(--va-text-muted)] transition-colors hover:bg-[var(--va-surface-2)] hover:text-[var(--va-text-2)]", children: [jsx(ArrowRight, { className: "h-3.5 w-3.5" }), "الأرشيف"] }),
          jsx("span", { className: "text-[var(--va-text-muted)]", children: "/" }),
          jsx("span", { className: "max-w-[200px] truncate text-[var(--va-text-muted)]", children: item.title || "التفاصيل" })
        ] }),
      ...
      jsxs("section", { className: "va-page-hero overflow-hidden rounded-2xl border border-[var(--va-border-soft)] bg-gradient-to-l from-gray-900 via-gray-900/95 to-gray-950 text-right shadow-2xl shadow-black/10", children: [
        previewSource && previewState !== MEDIA_PREVIEW_STATUS.TIMED_OUT ? jsx(VideoPlayer, {
          videoRef,
          src: previewSource,
          cues: subtitleCues,
          subtitlesOn,
          onToggleSubtitles: () => setSubtitlesOn((value: any) => !value),
          captionSize,
          captionColor,
        ...
        previewState === MEDIA_PREVIEW_STATUS.PLAYABLE && jsxs("div", { className: "border-t border-[var(--va-border-soft)] bg-[var(--va-surface)] p-4 space-y-3", dir: "rtl", children: [
          jsxs("h3", { className: "flex items-center gap-2 text-sm font-bold text-[var(--va-text)]", children: [
            ...
            itemBookmarks.length ? jsx("span", { className: "rounded-full border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] px-2 py-0.5 text-[10px] text-[var(--va-text-2)]", children: `${itemBookmarks.length}` }) : null
          ] }),
          jsx(TimeBookmarkButton, {
            getTime: () => (videoRef.current as any)?.currentTime ?? 0,
            getSuggestion: (time: any) => createTranscriptBookmarkDraft({ time, segments: transcriptSegments } as any),
          ...
          jsx(TimeBookmarkList, {
            bookmarks: itemBookmarks.map((bookmark: any) => ({
              id: bookmark.id,
          ...
          jsxs("div", { className: "flex flex-wrap items-start justify-between gap-4", children: [
            jsxs("div", { className: "min-w-0 flex-1", children: [
              jsxs("div", { className: "flex flex-wrap items-center gap-2", children: [
                jsx("h2", { className: "text-2xl font-bold text-[var(--va-text)]", children: item.title || "بدون عنوان" }),
              ...
              jsx("p", { className: "mt-2 text-sm text-[var(--va-text-muted)]", children: [getTypeLabel(contentTypes, item.type), getSubtypeLabel(contentTypes, item.type, item.subtype)].filter(Boolean).join(" / ") || "غير مصنف" }),
              mediaPath && jsxs("div", { className: "mt-2 flex flex-wrap items-center gap-2", children: [
                jsx("p", { className: "max-w-2xl break-all text-left text-xs text-[var(--va-text-muted)]", dir: "ltr", children: mediaPath }),
                jsxs("button", { type: "button", onClick: copyPath, className: "inline-flex min-h-8 items-center gap-1.5 rounded-lg border border-[var(--va-border-soft)] px-2.5 py-1 text-xs text-[var(--va-text-muted)] hover:bg-[var(--va-surface-2)] hover:text-[var(--va-text)] transition-colors", children: [jsx(Copy, { className: "h-3.5 w-3.5" }), "نسخ"] })
              ] }),
            ...
            jsxs("div", { className: "flex flex-wrap gap-2", children: [
              shareEnabled && jsxs("button", { type: "button", onClick: () => setShareDialogOpen(true), className: "btn btn-ghost btn-sm gap-2 text-[var(--va-text-2)] hover:bg-[var(--va-surface-2)]", children: [jsx(Share2, { className: "h-4 w-4" }), "مشاركة"] }),
              jsxs("button", { type: "button", onClick: () => toggleFavorite?.(item.id), className: `inline-flex items-center gap-1.5 rounded-xl border px-3 py-2 text-sm transition-colors ${item.isFavorite ? "border-amber-500/30 bg-amber-500/10 text-amber-200 hover:bg-amber-500/15" : "border-[var(--va-border-soft)] text-[var(--va-text-muted)] hover:bg-[var(--va-surface-2)] hover:text-amber-200"}`, children: [jsx(Star, { className: `h-4 w-4 ${item.isFavorite ? "fill-current" : ""}` }), item.isFavorite ? "إزالة المفضلة" : "مفضلة"] }),
              jsxs("button", { type: "button", onClick: () => { setActiveDetailTab("data"); setEditing((value: any) => !value); }, className: `btn btn-ghost btn-sm gap-2 ${editing ? "va-accent-border va-accent-bg-soft va-accent-text-on-soft" : "text-[var(--va-text-2)] hover:bg-[var(--va-surface-2)]"}`, children: [jsx(PenLine, { className: "h-4 w-4" }), editing ? "إغلاق التحرير" : "تحرير"] }),
      ...
      jsx("section", { className: "grid gap-3 sm:grid-cols-2 xl:grid-cols-4", children: itemStats.map((stat: any, index: any) => {
          ...
          children: [
            jsxs("div", { className: "flex items-start justify-between gap-3", children: [
              jsxs("div", { className: "min-w-0", children: [
                ...
                jsx("p", { className: "mt-2 truncate text-lg font-bold text-[var(--va-text)]", children: stat.value })
              ] }),
      ...
      jsxs("section", { className: `va-card space-y-4 rounded-2xl va-surface-muted border p-5 text-right ${activeDetailTab === "media" ? "" : "hidden"}`, dir: "rtl", children: [
        jsxs("div", { className: "flex flex-wrap items-center gap-2", children: [
          jsxs("label", {
            className: "inline-flex min-h-9 cursor-pointer items-center gap-1.5 rounded-xl border border-[var(--va-border-soft)] px-3 py-1.5 text-xs font-semibold text-[var(--va-text-2)] transition-colors hover:bg-[var(--va-surface-2)] hover:text-[var(--va-text)]",
            children: [
          ...
          importedCues.length > 0 && jsx("button", {
            ...
            className: "inline-flex min-h-9 items-center gap-1.5 rounded-xl border border-[var(--va-border-soft)] px-3 py-1.5 text-xs font-semibold text-[var(--va-text-muted)] transition-colors hover:bg-[var(--va-surface-2)] hover:text-[var(--va-text)]",
          ...
          subtitleCues.length > 0 && jsxs("button", {
            ...
            className: "inline-flex min-h-9 items-center gap-1.5 rounded-xl border border-[var(--va-border-soft)] px-3 py-1.5 text-xs font-semibold text-[var(--va-text-2)] transition-colors hover:bg-[var(--va-surface-2)] hover:text-[var(--va-text)]",
            children: [jsx(Download, { className: "h-3.5 w-3.5" }), "تنزيل الترجمة (SRT)"]
          }),
          subtitleCues.length > 0 && jsxs("select", {
            value: captionSize,
            onChange: (event: any) => setCaptionSize(event.target.value),
            "aria-label": "حجم الترجمة",
            className: "min-h-9 rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface)] px-2 text-xs text-[var(--va-text-2)]",
            children: [
          ...
          subtitleCues.length > 0 && jsxs("label", {
            className: "inline-flex items-center gap-1.5 text-xs text-[var(--va-text-muted)]",
            children: [
              "لون",
              jsx("input", {
                type: "color",
                value: captionColor,
                onChange: (event: any) => setCaptionColor(event.target.value),
                "aria-label": "لون الترجمة",
                className: "h-7 w-9 cursor-pointer rounded border border-[var(--va-border-soft)] bg-transparent"
              })
            ]
          })
        ] }),
        ...
        jsxs("div", { className: "flex flex-wrap items-start justify-between gap-3", children: [
          jsxs("div", { className: "min-w-0", children: [
            jsxs("h2", { className: "flex items-center gap-2 text-lg font-bold text-[var(--va-text)]", children: [jsx(Sparkles, { className: "h-5 w-5 va-accent-text" }), "أدوات ffmpeg"] }),
            mediaSourceKey ? jsx("p", { dir: "ltr", className: "mt-1 max-w-xl truncate text-left font-mono text-xs text-[var(--va-text-muted)]", title: mediaSourceKey, children: mediaSourceKey }) : null
          ] }),
        ...
        !mediaUnavailable && mediaSourceKey ? jsxs("div", { className: "rounded-xl va-surface-subtle border p-3 space-y-2", children: [
          jsx("h3", { className: "text-sm font-bold text-[var(--va-text)]", children: "تحويل الصيغ" }),
          jsx(ConversionPanel, {
            sourceKey: mediaSourceKey,
            mediaClient: stableMediaClient,
            disabled: false,
            onJobCreated: handleConversionJobCreated,
            onError: (err: any) => reportError(showNotification, err, { context: "تحويل الصيغ" })
          })
        ] }) : null,
        mediaRows.length ? jsx("div", { className: "grid gap-2 sm:grid-cols-2 xl:grid-cols-5", children: mediaRows.map((row: any) => jsxs("div", { className: "rounded-xl va-surface-subtle border p-3", children: [
          jsx("p", { className: "text-[11px] text-[var(--va-text-muted)]", children: row.label }),
          jsx("p", { dir: row.dir || "auto", className: "mt-1 truncate text-sm font-semibold text-[var(--va-text-2)]", children: row.value })
        ] }, row.id)) }) : null,
        displayDerivedFiles.length ? jsxs("div", { className: "rounded-xl va-surface-subtle border p-3", children: [
          jsxs("div", { className: "flex items-center justify-between gap-2", children: [
            jsx("h3", { className: "text-sm font-bold text-[var(--va-text)]", children: "الملفات المشتقة" }),
            jsx("span", { className: "badge badge-sm border-[var(--va-border-soft)] bg-[var(--va-surface-2)] text-[var(--va-text-2)]", children: displayDerivedFiles.length })
          ] }),
          jsx("ul", { className: "mt-3 space-y-2", children: displayDerivedFiles.map((file: any) => jsxs("li", { className: "flex items-center justify-between gap-2 rounded-lg border border-[var(--va-border-soft)] bg-[var(--va-surface)] px-3 py-2", children: [
            jsxs("div", { className: "min-w-0", children: [
              jsx("p", { className: "text-xs font-semibold text-[var(--va-text-2)]", children: file.label || "ملف مشتق" }),
              jsx("p", { dir: "ltr", className: "mt-0.5 truncate text-left font-mono text-[11px] text-[var(--va-text-muted)]", children: file.key })
            ] }),
        ...
        jsxs("div", { className: "space-y-2", children: [
          jsxs("div", { className: "flex items-center justify-between gap-2", children: [
            jsx("h3", { className: "text-sm font-bold text-[var(--va-text)]", children: "مهام الوسائط" }),
            jsx("button", { type: "button", onClick: refreshMediaJobs, disabled: !mediaToolsEnabled, className: "rounded-lg border border-[var(--va-border-soft)] px-2 py-1 text-[11px] text-[var(--va-text-2)] hover:bg-[var(--va-surface-2)] disabled:opacity-40", children: "تحديث" })
          ] }),
      ...
      activeDetailTab === "data" && editing && draft && jsxs("section", { className: "va-card space-y-4 rounded-2xl border va-accent-border va-accent-bg-soft p-5 text-right", children: [
        jsx("h2", { className: "text-lg font-bold text-[var(--va-text)]", children: "تحرير التفاصيل" }),
        jsxs("div", { className: "grid gap-4 md:grid-cols-2", children: [
          jsxs("label", { className: "space-y-1 text-sm text-[var(--va-text-2)] md:col-span-2", children: [jsx("span", { children: "العنوان" }), jsx("input", { value: (draft as any).title || "", onChange: (event: any) => updateDraft({ title: event.target.value }), className: "min-h-11 w-full va-surface-deep rounded-xl border px-3 text-sm text-[var(--va-text)] outline-none" })] }),
          jsxs("label", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [jsx("span", { children: "النوع" }), jsx("select", { value: (draft as any).type || "", onChange: (event: any) => updateDraft({ type: event.target.value, subtype: "" }), className: "min-h-11 w-full va-surface-deep rounded-xl border px-3 text-sm text-[var(--va-text)] outline-none", children: contentTypes.filter((type: any) => type.status !== "archived").map((type: any) => jsx("option", { value: type.id, children: type.name }, type.id)) })] }),
          jsxs("label", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [jsx("span", { children: "الفرع" }), jsxs("select", { value: (draft as any).subtype || "", onChange: (event: any) => updateDraft({ subtype: event.target.value }), className: "min-h-11 w-full va-surface-deep rounded-xl border px-3 text-sm text-[var(--va-text)] outline-none", children: [jsx("option", { value: "", children: "بدون فرع" }), ...subtypes.map((subtype: any) => jsx("option", { value: subtype.id, children: subtype.name }, subtype.id))] })] }),
          jsxs("label", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [jsx("span", { children: "المسار" }), jsx("input", { ref: pathInputRef, value: (draft as any).path || "", onChange: (event: any) => updateDraft({ path: event.target.value }), dir: "ltr", className: "min-h-11 w-full va-surface-deep rounded-xl border px-3 text-left text-sm text-[var(--va-text)] outline-none" })] }),
          jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)] md:col-span-2", children: [
            jsx("span", { children: "ملف محلي من الجهاز" }),
            jsx(LocalFilePicker, { value: (draft as any).metadata?.localFile, onFileSelect: applyPrimaryLocalFile })
          ] }),
          jsxs("label", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [jsx("span", { children: "الصورة المصغرة" }), jsx("input", { value: (draft as any).thumbnail || "", onChange: (event: any) => updateDraft({ thumbnail: event.target.value }), dir: "ltr", className: "min-h-11 w-full va-surface-deep rounded-xl border px-3 text-sm text-[var(--va-text)] outline-none" })] }),
          canEditTags && jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)] md:col-span-2", children: [
            jsx("label", { className: "block", children: "الوسوم" }),
            jsx("input", { value: (draft as any).tagsText || "", onChange: (event: any) => updateDraft({ tagsText: event.target.value }), className: "min-h-11 w-full va-surface-deep rounded-xl border px-3 text-sm text-[var(--va-text)] outline-none" }),
            canManageFieldAcl && jsx(FieldAclControls, { fieldKey: "tags", acl: (draft as any).fieldAcl || {}, onChange: updateFieldAcl })
          ] }),
          canEditNotes && jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)] md:col-span-2", children: [
            jsx("label", { className: "block", children: "ملاحظات" }),
            jsx("textarea", { value: (draft as any).notes || "", onChange: (event: any) => updateDraft({ notes: event.target.value }), className: "min-h-[90px] w-full va-surface-deep rounded-xl border p-3 text-sm text-[var(--va-text)] outline-none" }),
            canManageFieldAcl && jsx(FieldAclControls, { fieldKey: "notes", acl: (draft as any).fieldAcl || {}, onChange: updateFieldAcl })
          ] }),
        ...
        editFields.length > 0 && jsx(GroupedFields, { fields: editFields, gap: "gap-4", renderField: (field: any) => jsxs("div", { className: `space-y-1 text-sm text-[var(--va-text-2)] ${field.type === "textarea" || field.type === "localFile" ? "md:col-span-2" : ""}`, children: [
          jsx("label", { className: "block", children: field.label }),
          jsx(EditableField, { field, value: (draft as any).metadata?.[fieldKey(field)], onChange: updateMetadata }),
          canManageFieldAcl && jsx(FieldAclControls, { fieldKey: fieldKey(field), acl: (draft as any).fieldAcl || {}, onChange: updateFieldAcl })
        ] }, field.id) }),
        jsxs("div", { className: "flex justify-end gap-2", children: [
          jsx("button", { type: "button", onClick: () => setEditing(false), className: "rounded-xl border border-[var(--va-border-soft)] px-4 py-2 text-sm text-[var(--va-text-2)] hover:bg-[var(--va-surface-2)]", children: "إلغاء" }),
      ...
      jsxs("section", { className: "space-y-6", children: [
        jsxs("div", { ref: metadataSectionRef, className: `va-card space-y-4 rounded-2xl va-surface-muted border p-5 text-right ${activeDetailTab === "data" ? "" : "hidden"}`, children: [
          jsxs("h2", { className: "flex items-center gap-2 text-lg font-bold text-[var(--va-text)]", children: [jsx(FileText, { className: "h-5 w-5 va-accent-text" }), "البيانات"] }),
          completeness && jsxs("div", { className: "rounded-xl va-surface-muted border p-3", children: [
            jsxs("div", { className: "flex items-center justify-between gap-3", children: [
              ...
              jsx("span", { dir: "ltr", className: "font-mono text-sm text-[var(--va-text-2)]", children: `${completeness.percent}%` })
            ] }),
            jsx("div", { className: "mt-2 h-1.5 overflow-hidden rounded-full bg-[var(--va-surface-2)]", children: jsx("div", { className: "h-full rounded-full", style: { width: `${completeness.percent}%`, background: COMPLETENESS_TIERS[completeness.tier].color } }) }),
            completeness.missing.length ? jsxs("p", { className: "mt-2 text-xs leading-6 text-[var(--va-text-muted)]", children: ["ينقص: ", completeness.missing.join("، ")] }) : jsx("p", { className: "mt-2 text-xs text-[var(--va-text-muted)]", children: "كل الحقول الأساسية والمطلوبة مكتملة." })
          ] }),
          canViewNotes && item.notes && jsx("p", { className: "rounded-xl va-surface-muted border p-3 text-sm leading-relaxed text-[var(--va-text-muted)]", dir: "auto", children: renderTextWithTimestamps(item.notes) }),
          canViewPrimaryLocalFile && item.metadata?.localFile && jsxs("div", { className: "rounded-xl va-surface-muted border p-3", children: [
            ...
            jsx("div", { className: "mt-2 text-sm text-[var(--va-text-2)]", children: jsx(ReadonlyField, { field: { type: "localFile" }, value: item.metadata.localFile }) })
          ] }),
          readFields.length ? jsx(GroupedFields, { fields: readFields, gap: "gap-3", renderField: (field: any) => jsxs("div", { className: "rounded-xl va-surface-muted border p-3", children: [
            jsx("p", { className: "text-xs text-[var(--va-text-muted)]", children: field.label }),
            jsx("div", { className: "mt-1 text-sm text-[var(--va-text-2)]", children: jsx(ReadonlyField, { field, value: item.metadata?.[fieldKey(field)] }) })
          ] }, field.id) }) : jsx("p", { className: "text-sm text-[var(--va-text-muted)]", children: "لا توجد حقول مخصصة لهذا العنصر." }),
        ...
        jsxs("aside", { className: "va-preview-panel space-y-5 rounded-2xl va-surface-muted border p-5 text-right", children: [
          jsxs("section", { className: activeDetailTab === "data" ? "" : "hidden", children: [
            jsxs("h2", { className: "flex items-center gap-2 text-base font-bold text-[var(--va-text)]", children: [jsx(Tags, { className: "h-4 w-4 va-accent-text" }), "الوسوم", canViewTags && item.tags?.length ? jsx("span", { className: "mr-auto rounded-full bg-[var(--va-surface-2)] px-2 py-0.5 text-xs text-[var(--va-text-muted)]", children: item.tags.length }) : null] }),
            canViewTags ? (item.tags?.length ? jsx("div", { className: "mt-3 flex flex-wrap gap-1.5", children: item.tags.map((tag: any) => jsx("span", { className: "va-tag-chip inline-flex items-center rounded-full border border-[var(--va-border-soft)] bg-[var(--va-surface)] px-2.5 py-1 text-xs text-[var(--va-text-2)] transition-colors hover:border-emerald-500/25 hover:text-emerald-200", children: tag }, tag)) }) : jsx("p", { className: "mt-3 text-sm text-[var(--va-text-muted)]", children: "لا توجد وسوم." })) : jsx("p", { className: "mt-3 rounded-xl border border-amber-500/20 bg-amber-500/10 p-3 text-sm text-amber-100", children: "الوسوم محجوبة حسب صلاحيات العرض." })
          ] }),
          jsx(CommentThread, {
          ...
          activeDetailTab === "ai" && jsxs("section", { className: "rounded-xl va-surface-subtle border p-4", children: [
            jsxs("h2", { className: "flex items-center gap-2 text-base font-bold text-[var(--va-text)]", children: [jsx(Sparkles, { className: "h-4 w-4 va-accent-text" }), "مساعد AI للمادة"] }),
            jsx("p", { className: "mt-2 text-sm leading-6 text-[var(--va-text-muted)]", children: ai.available ? "اختر إجراءً وسيضاف الناتج إلى مسودة التحرير. راجع التغييرات ثم احفظها من تبويب البيانات." : "مزود AI غير متاح حالياً. افتح إعدادات AI أو استخدم الملاحظات اليدوية." }),
          ...
          activeDetailTab === "notes" ? jsx("section", { children: jsx(ItemNotesPanel, {
            ...
            onAdd: (note: any) => addItemNote?.(note),
            onRemove: (id: any) => removeItemNote?.(id),
            onSeek: seekToBookmark
          }) }) : null,
          activeDetailTab === "relations" ? jsxs("section", { className: "space-y-4", children: [
            ...
            jsx(RelatedContentPanel, { items: relatedItems, onOpenItem: (related: any) => setSelectedItemId?.(related.id) }),
          ...
          activeDetailTab === "history" && itemHistory.length ? jsxs("section", { children: [
            ...
            jsx("ul", { className: "mt-3 space-y-2", children: itemHistory.slice(0, 12).map((record: any) => jsxs("li", { className: "rounded-xl va-surface-subtle border p-3", children: [
              jsxs("div", { className: "flex items-center justify-between gap-2", children: [
                jsx("span", { className: "text-xs text-[var(--va-text-muted)]", children: formatDateTime(record.timestamp) }),
                jsx("button", { type: "button", onClick: () => restoreVersion(record), className: "shrink-0 rounded-lg border border-[var(--va-border-soft)] px-2 py-1 text-[11px] text-[var(--va-text-2)] transition-colors hover:border-emerald-500/30 hover:text-emerald-200", children: "استرجاع" })
              ] }),
              jsx("ul", { className: "mt-1.5 space-y-0.5", children: record.changes.slice(0, 5).map((change: any, index: any) => jsxs("li", { className: "truncate text-[11px] text-[var(--va-text-muted)]", children: [
                jsxs("span", { className: "text-[var(--va-text-muted)]", children: [change.label, ": "] }),
                jsx("span", { children: describeFieldValue(change.oldValue) }),
                " ← ",
                jsx("span", { className: "text-[var(--va-text-2)]", children: describeFieldValue(change.newValue) })
              ] }, `${record.id}-${index}`)) }),
              record.changes.length > 5 ? jsx("p", { className: "mt-1 text-[10px] text-[var(--va-text-muted)]", children: `+${record.changes.length - 5} تغييرات أخرى` }) : null
            ] }, record.id)) })
          ] }) : null,
          jsxs("section", { className: `rounded-xl va-surface-subtle border p-3 space-y-2 ${activeDetailTab === "history" ? "" : "hidden"}`, children: [
            jsx("h2", { className: "text-xs font-semibold uppercase tracking-wide text-[var(--va-text-muted)]", children: "معلومات العنصر" }),
            jsxs("div", { className: "space-y-1.5", children: [
              jsxs("div", { className: "flex items-center justify-between gap-2", children: [
                jsx("span", { className: "text-xs text-[var(--va-text-muted)]", children: "المعرّف" }),
                jsx("span", { className: "min-w-0 max-w-[12rem] truncate text-left font-mono text-xs text-[var(--va-text-muted)]", dir: "ltr", title: item.id, children: item.id || "—" })
              ] }),
              (item.metadata?.checksum || item.checksum || item.hash) && jsxs("div", { className: "flex items-center justify-between gap-2", children: [
                jsx("span", { className: "text-xs text-[var(--va-text-muted)]", children: "Checksum" }),
                jsx("span", { className: "min-w-0 max-w-[12rem] truncate text-left font-mono text-xs text-[var(--va-text-muted)]", dir: "ltr", title: item.metadata?.checksum || item.checksum || item.hash, children: item.metadata?.checksum || item.checksum || item.hash })
              ] }),
              jsxs("div", { className: "flex items-center justify-between gap-2", children: [
                jsx("span", { className: "text-xs text-[var(--va-text-muted)]", children: "أنشئ" }),
                jsx("span", { className: "text-xs text-[var(--va-text-muted)]", children: item.createdAt ? formatDateTime(item.createdAt) : "—" })
              ] }),
              jsxs("div", { className: "flex items-center justify-between gap-2", children: [
                jsx("span", { className: "text-xs text-[var(--va-text-muted)]", children: "آخر تحديث" }),
                jsx("span", { className: "text-xs text-[var(--va-text-muted)]", children: item.updatedAt ? formatDateTime(item.updatedAt) : "—" })
              ] }),
              jsxs("div", { className: "flex items-center justify-between gap-2", children: [
                jsx("span", { className: "text-xs text-[var(--va-text-muted)]", children: "الإصدار" }),
                jsx("span", { className: "text-xs text-[var(--va-text-muted)]", children: `v${formatNumber(item.version || 1)}` })
              ] })
            ] })
          ] }),
          activeDetailTab === "history" && history.length > 0 && jsxs("section", { children: [
            jsx("h2", { className: "text-base font-bold text-[var(--va-text)]", children: "آخر التغييرات" }),
            jsx("div", { className: "mt-3 space-y-2", children: history.slice(0, 5).map((record: any) => jsxs("div", { className: "rounded-xl va-surface-subtle border p-3", children: [
              jsx("p", { className: "text-xs font-semibold text-[var(--va-text-2)]", children: record.action === "create" ? "إنشاء" : record.action === "update" ? "تعديل" : record.action === "delete" ? "حذف" : record.action === "restore" ? "استعادة" : record.action || "نشاط" }),
              jsx("p", { className: "mt-0.5 text-xs text-[var(--va-text-muted)]", children: record.timestamp ? formatDateTime(record.timestamp) : "" })
            ] }, record.id)) })
          ] })
        ] })
      ] })
      ...
      jsx(DraftRecoveryDialog, {
        ...
        draft: recoveryDraft,
        onRestore: (data: any) => {
          setDraft((current: any) => ({ ...current, ...data }));
          setEditing(true);
      ...
      jsx(MobileActionBar, {
        ...
        actions: [
          ...
          shareEnabled
            ...
            : { id: "play", label: "تشغيل", icon: Play, disabled: !previewSource, onClick: () => (videoRef.current as any)?.play?.().catch(() => {}) },
          { id: "save", label: "حفظ", icon: Save, primary: true, disabled: !editing || !draft, onClick: save },
          { id: "comment", label: "تعليق", icon: MessageSquare, onClick: () => { setActiveDetailTab("comments"); window.requestAnimationFrame?.(() => (commentsSectionRef.current as any)?.scrollIntoView({ block: "center", behavior: "smooth" })); } },
          { id: "edit", label: editing ? "إغلاق" : "تحرير", icon: PenLine, active: editing, onClick: () => { setActiveDetailTab("data"); setEditing((value: any) => !value); } }
        ]
      })
    ]
  });
