...
function fileName(key: any = "") {
  return key.slice(key.lastIndexOf("/") + 1);
}

export function FileManagerPage({ api = defaultApi, queueUpload = queueUploadedFile, storageProvider, onArchive }: any) {
  ...
  const load = React.useCallback(async ({ append = false, cursor = "" }: any = {}) => {
    setLoading(true);
    setError("");
    try {
      const result = await api.browseFiles({ path, query, cursor, ...deps });
      setEntries((current: any) => append ? mergeBrowserEntries(current, result.entries) : result.entries || []);
  ...
  React.useEffect(() => {
    const openUpload = () => (uploadInput.current as any)?.click();
  ...
  const navigate = (nextPath: any) => {
    setPath(nextPath);
    setSelection(new Set());
    setActiveKey("");
  };

  const changeView = (mode: any) => setView(saveViewMode(mode));
  const activeEntry = entries.find((entry: any) => entry.key === activeKey) || null;
  const selectedKeys = [...selection];

  const runAction = async (action: any, extra: any = {}) => {
  ...
  const archiveSelected = async () => {
    ...
    const entry = entries.find((item: any) => item.key === selectedKeys[0]) || { key: selectedKeys[0] };
    ...
    try {
      ...
      queueRecord = records.find((record: any) => record.fileKey === entry.key) || null;
  ...
  const entryButton = (entry: any) => {
    ...
    return (
      <button
        ...
        onClick={() => { setActiveKey(entry.key); setSelection((current: any) => toggleSelection(current, entry.key)); }}
    ...
    <main className="min-h-full bg-base-200/40" dir="rtl">
      <div className="mx-auto max-w-[1600px] space-y-3 p-3 sm:p-5">
        <section className="flex flex-wrap items-center gap-2 border-b border-base-300 pb-3">
          <button type="button" className="btn btn-primary btn-sm gap-2" onClick={() => (uploadInput.current as any)?.click()} disabled={busy}><Upload className="h-4 w-4" />رفع ملفات</button>
          <input ref={uploadInput} className="hidden" type="file" multiple onChange={uploadFiles} aria-label="اختيار ملفات للرفع" />
          <label className="flex items-center gap-2 px-1 text-xs text-base-content/70"><input type="checkbox" className="checkbox checkbox-xs checkbox-primary" checked={queueThisUpload} onChange={(event: any) => setQueueOverride(event.target.checked)} />تجهيز هذه الدفعة للأرشفة</label>
          ...
          <label className="input input-bordered input-sm flex min-w-[220px] flex-1 items-center gap-2 sm:max-w-md">
            ...
            <input value={query} onChange={(event: any) => setQuery(event.target.value)} className="grow" placeholder="ابحث داخل هذا المجلد" aria-label="البحث في الملفات" />
          </label>
        ...
        <nav className="breadcrumbs overflow-x-auto text-sm" aria-label="مسار الملفات">
          <ul>{buildBreadcrumbs(path).map((crumb: any) => <li key={crumb.path || "root"}><button type="button" onClick={() => navigate(crumb.path)}>{crumb.label}</button></li>)}</ul>
        </nav>
        ...
        <section className="grid min-h-[520px] gap-3 lg:grid-cols-[220px_minmax(0,1fr)_280px]">
          <aside className="hidden border-l border-base-300 pl-3 lg:block">
            ...
            {entries.filter((entry: any) => entry.kind === "folder").map((entry: any) => <button type="button" key={entry.key} className="btn btn-ghost btn-sm mt-1 w-full justify-start gap-2" onClick={() => navigate(entry.key)}><Folder className="h-4 w-4" /><span className="truncate">{entry.name}</span></button>)}
          </aside>
          ...
          <aside className="border-r border-base-300 pr-3">
            ...
            {activeEntry ? <div className="space-y-4"><div className="flex aspect-video items-center justify-center bg-base-300/50">{(activeEntry as any).kind === "folder" ? <Folder className="h-14 w-14 text-warning" /> : <File className="h-14 w-14 text-info" />}</div><div><p className="break-all font-semibold">{(activeEntry as any).name}</p><p className="mt-1 break-all text-xs text-base-content/55" dir="ltr">{(activeEntry as any).key}</p></div><dl className="grid grid-cols-2 gap-2 text-sm"><dt className="text-base-content/55">النوع</dt><dd>{(activeEntry as any).kind === "folder" ? "مجلد" : "ملف"}</dd><dt className="text-base-content/55">الحجم</dt><dd dir="ltr">{formatBytes((activeEntry as any).size)}</dd></dl></div> : <p className="text-sm leading-7 text-base-content/55">حدد ملفًا لعرض معلوماته. لا تُنشأ مادة في الأرشيف إلا عند اختيار «بدء الأرشفة».</p>}
            <label className="mt-6 flex items-start gap-3 border-t border-base-300 pt-4 text-sm"><input type="checkbox" className="toggle toggle-primary toggle-sm mt-0.5" checked={autoQueue} onChange={(event: any) => updateSettings?.({ fileManager: { ...(settings?.fileManager || {}), autoQueueUploads: event.target.checked } })} /><span><b className="block">إضافة الرفع لصندوق التجهيز</b><span className="text-xs text-base-content/55">افتراضيًا نعم، ويمكن تغييرها قبل كل رفع.</span></span></label>
          </aside>
        </section>
      </div>

      {folderDialog && <div className="modal modal-open" role="dialog" aria-modal="true" aria-label="إنشاء مجلد"><div className="modal-box max-w-md"><h3 className="text-lg font-bold">مجلد جديد</h3><label className="form-control mt-4"><span className="label-text mb-1">اسم المجلد</span><input autoFocus value={folderName} onChange={(event: any) => setFolderName(event.target.value)} className="input input-bordered" /></label><div className="modal-action"><button type="button" className="btn btn-ghost" onClick={() => setFolderDialog(false)}>إلغاء</button><button type="button" className="btn btn-primary" disabled={!folderName.trim() || busy} onClick={createFolder}>إنشاء</button></div></div></div>}
      {destinationDialog && <div className="modal modal-open" role="dialog" aria-modal="true" aria-label="اختيار وجهة"><div className="modal-box max-w-md"><h3 className="text-lg font-bold">{destinationDialog === "copy" ? "نسخ إلى" : "نقل إلى"}</h3><label className="form-control mt-4"><span className="label-text mb-1">مسار المجلد الوجهة</span><input autoFocus value={destination} onChange={(event: any) => setDestination(event.target.value)} className="input input-bordered" dir="ltr" placeholder="ready/2026" /></label><div className="modal-action"><button type="button" className="btn btn-ghost" onClick={() => setDestinationDialog("")}>إلغاء</button><button type="button" className="btn btn-primary" disabled={busy} onClick={() => runAction(destinationDialog, { destination })}>تنفيذ</button></div></div></div>}
      <FileArchiveWizard
        open={archiveFiles.length > 0}
        onOpenChange={(open: any) => { if (!open) { setArchiveFiles([]); setArchiveQueueRecord(null); } }}
        initialStoredFiles={archiveFiles}
        contentTypes={contentTypes || []}
        videoItems={videoItems || []}
        addVideoItem={addVideoItem}
        showToast={showToast}
        onArchived={async ({ item }: any) => {
