...
function createLayoutOptions(nodeCount: any, requestedLayout: any = "auto") {
  ...
  if (layout === "concentric") {
    return {
      ...
      padding: 48,
      minNodeSpacing: 24,
      concentric: (node: any) => node.degree() + (node.data("kind") === "item" ? 0.5 : 0),
      levelWidth: () => 2
    };
  }
  return {
    ...
    padding: 48,
    nodeRepulsion: () => 9000,
    idealEdgeLength: (edge: any) => (edge.data("edgeKind") === "manual" ? 130 : 105)
  };
}
...
function GraphMetric({ label, value }: any) {
  return (
...
function NodeKindToggle({ kind, active, count, onToggle }: any) {
  const Icon = (KIND_ICONS as any)[kind] || Workflow;
  const meta = (NODE_KIND_META as any)[kind] || { label: kind, color: "#94a3b8" };
  return (
    <button
      type="button"
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap items-center gap-2">
          <select
            value={typeFilter}
            onChange={(event: any) => onTypeFilter(event.target.value)}
          ...
          >
            ...
            {typeOptions.map((option: any) => (
              <option key={option.key} value={option.key}>
                {(DOCUMENT_TYPE_LABELS as any)[option.key] || option.key} ({formatNumber(option.count)})
              </option>
            ))}
          </select>
          <select
            value={tagFilter}
            onChange={(event: any) => onTagFilter(event.target.value)}
          ...
          <select
            value={layoutMode}
            onChange={(event: any) => onLayoutMode(event.target.value)}
            aria-label="نمط تخطيط الخريطة"
            className="va-surface-muted min-h-9 rounded-[var(--va-radius-md)] border border-[var(--va-border-soft)] px-2 py-1 text-xs text-[var(--va-text)] outline-none"
          >
            {LAYOUT_OPTIONS.map((option: any) => (
      ...
      <div className="flex flex-wrap gap-2" aria-label="طبقات الخريطة">
        {EXPANDED_NODE_KINDS.map((kind: any) => (
          <NodeKindToggle
            key={kind}
            kind={kind}
...
function GraphSearchPanel({ query, onQuery, matchCount, onClear }: any) {
  return (
...
function SelectedNodePanel({ node, onOpen }: any) {
  if (!node) {
    return (
  ...
  const kindMeta = (NODE_KIND_META as any)[node.kind] || NODE_KIND_META.item;
  const TypeIcon = (KIND_ICONS as any)[node.kind] || Workflow;
  ...
  const typeLabel = node.kind === "item"
    ? ((DOCUMENT_TYPE_LABELS as any)[node.documentType] || node.documentType)
    : kindMeta.label;
  return (
    <div className="space-y-3">
      ...
      {node.tags?.length ? (
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {node.tags.slice(0, 10).map((tag: any) => (
...
export function GraphViewPage() {
  ...
  const nodeById = React.useMemo(() => new Map(model.nodes.map((node: any) => [node.id, node])), [model.nodes]);
  const selectedNode = selectedId ? nodeById.get(selectedId) || null : null;

  const openNode = React.useCallback((node: any) => {
    if (!node) return;
    if (node.kind === "item") {
  ...
  const queryMatchIds = React.useMemo(() => {
    ...
    return new Set(model.nodes
      .filter((node: any) => normalizeSearch(`${node.label} ${node.kind} ${node.documentType}`).includes(normalizedQuery))
    ...
    (async () => {
      try {
        ...
        const cy = cytoscape({
          ...
          elements: toCytoscapeElements(model) as any,
  ...
  React.useEffect(() => {
    ...
    if (!cy) return;
    (cy as any).elements().removeClass("cy-dimmed");
    if (!normalizedQuery) return;
    (cy as any).elements().addClass("cy-dimmed");
    queryMatchIds.forEach((id: any) => {
      const node = (cy as any).getElementById(id);
  ...
  const toggleKind = (kind: any) => {
    setSelectedId(null);
    setNodeKinds((current: any) => {
      if (kind === "item") return current.includes("item") ? current : ["item", ...current];
      const next = current.includes(kind) ? current.filter((item: any) => item !== kind) : [...current, kind];
  ...
  const rerunLayout = () => {
    const cy = cyRef.current;
    if (!cy) return;
    (cy as any).layout(createLayoutOptions(model.nodes.length, layoutMode)).run();
    (cy as any).fit(undefined, 48);
  };
      ...
      {!hasGraph ? (
        ...
        <section className="rounded-[var(--va-radius-xl)] border border-[var(--va-border-soft)] bg-[var(--va-elevated)] p-4 text-right shadow-[var(--va-elev-1)]">
          <GraphToolbar
            ...
            onFit={() => (cyRef.current as any)?.fit(undefined, 48)}
            onReset={rerunLayout}
          />
          ...
          {(model.truncated || model.entityTruncated) ? (
            <div className="mt-3 flex flex-wrap items-center gap-2 rounded-[var(--va-radius-md)] border border-[var(--va-border-soft)] bg-[var(--va-surface)] px-3 py-2 text-xs text-[var(--va-text-2)]">
              ...
              {model.truncated ? (
                <button
                  type="button"
                  onClick={() => setMaxNodes((current: any) => current + LOAD_MORE_STEP)}
            ...
            <aside className="rounded-[var(--va-radius-lg)] border border-[var(--va-border-soft)] bg-[var(--va-surface)] p-3 text-xs xl:order-3" aria-label="تفاصيل العقدة">
              ...
              <div className="mt-4 border-t border-[var(--va-border-soft)] pt-3">
                ...
                <div className="space-y-1.5">
                  {EXPANDED_NODE_KINDS.map((kind: any) => {
                    const meta = (NODE_KIND_META as any)[kind];
                    return (
