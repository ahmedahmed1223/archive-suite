...
function HistoryMetric({ action, label, value, active, onClick }: any) {
  const Icon = (ACTION_ICON as any)[action] || History;
  return jsxs("button", {
    type: "button",
    onClick,
    "aria-pressed": active,
...
function HistoryRecord({ record, itemTitle, index, settings }: any) {
  const tone = getHistoryActionTone(record.action);
  const Icon = (ACTION_ICON as any)[record.action] || History;
...
export function HistoryPage() {
  ...
  const itemTitleById = React.useMemo(() => new Map(videoItems.map((item: any) => [item.id, item.title || item.id])), [videoItems]);
    ...
    children: [
      ...
      jsxs("section", {
        ...
        children: [
          ...
          ...HISTORY_ACTIONS.map((item: any) => jsx(HistoryMetric, {
            action: item.id,
            label: item.label,
            ...
            children: [
              ...
              jsxs("select", {
                ...
                children: [
                  ...
                  ...HISTORY_ACTIONS.map((item: any) => jsx("option", { value: item.id, children: item.label }, item.id))
                ]
              }),
              jsx("select", {
                value: pageSize,
                ...
                className: "select select-bordered min-h-11 va-surface-deep rounded-xl px-3 text-sm text-[var(--va-text)]",
                children: [20, 50, 100].map((size: any) => jsx("option", { value: size, children: `${size} سجل` }, size))
              })
            ]
          }),
      ...
      isLoading && visibleRecords.length === 0 && changeHistory.length === 0 ? jsx("section", {
        ...
        children: Array.from({ length: 6 }).map((_: any, index: any) => jsx(SkeletonBlock, {
          className: "h-14 w-full"
        }, index))
      }) :
      visibleRecords.length ? jsx("section", {
        className: "space-y-3",
        children: visibleRecords.map((record: any, index: any) => jsx(HistoryRecord, {
          record,
