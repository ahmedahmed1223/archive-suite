...
function ItemCard({ item, transitions, onOpen, onMove }) {
  return jsxs("div", {
    className: "rounded-[var(--va-radius-md)] border border-[var(--va-border-soft)] bg-[var(--va-elevated)] p-2.5 shadow-[var(--va-elev-1)] transition-colors hover:border-emerald-500/25",
    children: [
      jsxs("button", {
        type: "button",
        onClick: () => onOpen(item),
        ...
        children: [
          jsx("p", { className: "min-w-0 truncate text-sm font-semibold text-[var(--va-text)]", dir: "auto", children: item.title || "بدون عنوان" }),
          jsx(ExternalLink, { className: "mt-0.5 h-4 w-4 shrink-0 text-[var(--va-text-muted)]" })
        ]
      }),
      transitions.length > 0 ? jsxs("label", {
        className: "mt-2 flex items-center gap-1.5 text-[11px] text-[var(--va-text-muted)]",
        children: [
          "نقل إلى:",
          jsxs("select", {
            value: "",
            ...
            className: "rounded-[var(--va-radius-sm)] border border-[var(--va-border-strong)] bg-[var(--va-surface)] px-2 py-1 text-xs text-[var(--va-text-2)]",
            children: [
    ...
    children: [
      ...
      total === 0 ? jsx(EmptyState, {
        ...
        children: columns.map((column) => jsxs("div", {
          role: "listitem",
          className: `flex min-w-[15rem] max-w-[18rem] shrink-0 flex-col rounded-[var(--va-radius-lg)] border ${COLUMN_ACCENT[column.color] || COLUMN_ACCENT.gray} bg-[var(--va-surface)] p-3`,
          children: [
            jsxs("header", { className: "mb-3 flex items-center justify-between", children: [
              jsx("h2", { className: "text-sm font-bold text-[var(--va-text)]", children: column.label }),
              jsx("span", { className: "font-[family-name:var(--va-font-mono)] rounded-full bg-[var(--va-surface-2)] px-2 py-0.5 text-xs text-[var(--va-text-2)]", children: formatNumber(column.items.length) })
            ] }),
            column.items.length === 0 ? jsx("p", {
              className: "rounded-[var(--va-radius-md)] border border-dashed border-[var(--va-border-soft)] px-3 py-6 text-center text-xs text-[var(--va-text-muted)]",
              children: "لا عناصر"
            }) : jsx("div", {
              className: "space-y-2",
