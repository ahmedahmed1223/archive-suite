...
function Section({ icon, title, children, actions }: any) {
  return (
...
function NewBoardForm({ onCreate }: any) {
...
function BoardSettings({ board, workspace, onPersist }: any) {
  const updateBoard = (patch: any) => {
    ...
    <Section icon={<Settings2 className="h-4 w-4 va-accent-text" />} title="إعدادات لوحة المشروع">
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)]">
        <label className="block">
          ...
          <input
            value={board.title}
            onChange={(event: any) => updateBoard({ title: event.target.value })}
            className="min-h-10 w-full rounded-[var(--va-radius-md)] border border-[var(--va-border-strong)] bg-[var(--va-surface-2)] px-3 py-2 text-sm text-[var(--va-text)] outline-none focus:border-[var(--va-action,#10b981)]"
          />
        </label>
        <label className="block">
          ...
          <input
            ...
            onChange={(event: any) => updateBoard({ description: event.target.value })}
...
function ColumnManager({ board, onBoardChange }: any) {
  const [title, setTitle] = React.useState("");
    ...
    <Section icon={<Columns3 className="h-4 w-4 va-accent-text" />} title="تخصيص الأعمدة">
      ...
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        {board.columns.map((column: any) => {
          const cardCount = board.cards.filter((card: any) => card.columnId === column.id).length;
            ...
            <div key={column.id} className="rounded-[var(--va-radius-md)] border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-3">
              <div className="flex items-center gap-2">
                <input
                  type="color"
                  value={column.color}
                  onChange={(event: any) => onBoardChange(updateKanbanColumn(board, column.id, { color: event.target.value }))}
                  aria-label={`لون عمود ${column.title}`}
                  className="h-8 w-9 shrink-0 rounded border border-[var(--va-border-soft)] bg-transparent"
                />
                <input
                  value={column.title}
                  onChange={(event: any) => onBoardChange(updateKanbanColumn(board, column.id, { title: event.target.value }))}
              ...
              <label className="mt-2 flex items-center gap-2 text-[11px] text-[var(--va-text-muted)]">
                ...
                <input
                  ...
                  onChange={(event: any) => onBoardChange(updateKanbanColumn(board, column.id, { wipLimit: event.target.value }))}
...
function FieldManager({ board, onBoardChange }: any) {
  ...
  const addField = (event: any) => {
    ...
    onBoardChange(addKanbanField(board, {
      label,
      type,
      options: options.split(",").map((option: any) => option.trim()).filter(Boolean)
    }));
    setLabel("");
    setOptions("");
  };
    ...
    <Section icon={<SlidersHorizontal className="h-4 w-4 va-accent-text" />} title="الحقول المخصصة">
      <form onSubmit={addField} className="grid gap-2 lg:grid-cols-[minmax(0,1fr)_12rem_minmax(0,1fr)_auto]">
        ...
        <select
          ...
          className="min-h-10 rounded-[var(--va-radius-md)] border border-[var(--va-border-strong)] bg-[var(--va-surface-2)] px-3 py-2 text-sm text-[var(--va-text)] outline-none"
        >
          {KANBAN_FIELD_TYPES.map((fieldType: any) => (
            <option key={fieldType} value={fieldType}>{(FIELD_TYPE_LABELS as any)[fieldType]}</option>
          ))}
        </select>
        <input
          value={options}
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {board.fields.length ? board.fields.map((field: any) => (
          <span key={field.id} className="inline-flex min-h-9 items-center gap-2 rounded-full border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] px-3 text-xs text-[var(--va-text-2)]">
            {field.label}
            <small className="text-[var(--va-text-muted)]">{(FIELD_TYPE_LABELS as any)[field.type]}</small>
            <button
              type="button"
...
function CardBuilder({ board, videoItems, onBoardChange }: any) {
  ...
  React.useEffect(() => {
    setDraft((value: any) => ({
      ...value,
      columnId: board.columns.some((column: any) => column.id === value.columnId) ? value.columnId : board.columns[0]?.id || ""
    }));
  }, [board.columns]);
    ...
    <Section icon={<ClipboardList className="h-4 w-4 va-accent-text" />} title="إضافة بطاقة يدوية">
      <form onSubmit={submit} className="grid gap-3 xl:grid-cols-2">
        <label className="block">
          ...
          <input
            value={draft.title}
            onChange={(event: any) => setDraft({ ...draft, title: event.target.value })}
            required
        ...
        <label className="block">
          ...
          <select
            ...
            onChange={(event: any) => setDraft({ ...draft, columnId: event.target.value })}
            className="min-h-10 w-full rounded-[var(--va-radius-md)] border border-[var(--va-border-strong)] bg-[var(--va-surface-2)] px-3 py-2 text-sm text-[var(--va-text)] outline-none"
          >
            {board.columns.map((column: any) => <option key={column.id} value={column.id}>{column.title}</option>)}
          </select>
        </label>
        <label className="block xl:col-span-2">
          ...
          <textarea
            value={draft.summary}
            onChange={(event: any) => setDraft({ ...draft, summary: event.target.value })}
            rows={3}
        ...
        <label className="block">
          ...
          <input
            value={draft.owner}
            onChange={(event: any) => setDraft({ ...draft, owner: event.target.value })}
            className="min-h-10 w-full rounded-[var(--va-radius-md)] border border-[var(--va-border-strong)] bg-[var(--va-surface-2)] px-3 py-2 text-sm text-[var(--va-text)] outline-none"
          />
        </label>
        <label className="block">
          ...
          <input
            type="date"
            value={draft.dueDate}
            onChange={(event: any) => setDraft({ ...draft, dueDate: event.target.value })}
            className="min-h-10 w-full rounded-[var(--va-radius-md)] border border-[var(--va-border-strong)] bg-[var(--va-surface-2)] px-3 py-2 text-sm text-[var(--va-text)] outline-none"
          />
        </label>
        <label className="block">
          ...
          <select
            ...
            onChange={(event: any) => setDraft({ ...draft, priority: event.target.value })}
            className="min-h-10 w-full rounded-[var(--va-radius-md)] border border-[var(--va-border-strong)] bg-[var(--va-surface-2)] px-3 py-2 text-sm text-[var(--va-text)] outline-none"
          >
            {Object.entries(PRIORITY_LABELS).map(([value, label]: any) => <option key={value} value={value}>{label}</option>)}
          </select>
        </label>
        <label className="block">
          ...
          <select
            ...
            onChange={(event: any) => setDraft({ ...draft, sourceItemId: event.target.value })}
            className="min-h-10 w-full rounded-[var(--va-radius-md)] border border-[var(--va-border-strong)] bg-[var(--va-surface-2)] px-3 py-2 text-sm text-[var(--va-text)] outline-none"
          >
            <option value="">بدون ربط</option>
            {videoItems.filter((item: any) => !item.isDeleted).slice(0, 300).map((item: any) => (
              <option key={item.id} value={item.id}>{item.title || item.id}</option>
            ))}
          </select>
        </label>
        {board.fields.map((field: any) => (
          <label key={field.id} className="block">
            <span className="mb-1 block text-xs text-[var(--va-text-muted)]">{field.label}</span>
            {field.type === "select" ? (
              <select
                value={(draft.fieldValues as any)[field.id] || ""}
                onChange={(event: any) => setFieldValue(field.id, event.target.value)}
                className="min-h-10 w-full rounded-[var(--va-radius-md)] border border-[var(--va-border-strong)] bg-[var(--va-surface-2)] px-3 py-2 text-sm text-[var(--va-text)] outline-none"
              >
                <option value="">بدون قيمة</option>
                {field.options.map((option: any) => <option key={option} value={option}>{option}</option>)}
              </select>
            ) : (
              <input
...
function ProjectCard({ card, board, linkedItem, onBoardChange, onOpenItem }: any) {
  return (
    <article className="rounded-[var(--va-radius-md)] border border-[var(--va-border-soft)] bg-[var(--va-elevated)] p-3 shadow-[var(--va-elev-1)]">
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        <span className={`rounded-full border px-2 py-0.5 text-[11px] ${(PRIORITY_CLASS as any)[card.priority] || PRIORITY_CLASS.medium}`}>
          {(PRIORITY_LABELS as any)[card.priority] || PRIORITY_LABELS.medium}
        </span>
      ...
      {board.fields.length > 0 && (
        <dl className="mt-3 grid gap-1 text-[11px]">
          {board.fields.map((field: any) => card.fieldValues?.[field.id] ? (
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <select
          value={card.columnId}
          onChange={(event: any) => onBoardChange(moveKanbanCard(board, card.id, event.target.value))}
...
function BoardColumns({ board, videoItems, onBoardChange, onOpenItem }: any) {
  ...
  return (
    <section className="overflow-x-auto pb-2" aria-label="لوحة بطاقات المشروع">
      <div className="grid min-w-[72rem] gap-3 xl:grid-cols-4" role="list">
        {board.columns.map((column: any) => {
          const cards = board.cards.filter((card: any) => card.columnId === column.id);
...
export function KanbanPage() {
  ...
  const board = workspace.boards.find((item: any) => item.id === workspace.activeBoardId) || workspace.boards[0] || null;
  const summary: any = React.useMemo(() => board ? getKanbanBoardSummary(board) : null, [board]);

  const persistWorkspace = React.useCallback(async (nextWorkspace: any, message?: any) => {
    await updateSettings?.({ ui: { kanbanWorkspace: normalizeKanbanWorkspace(nextWorkspace) } });
    if (message) showToast?.(message, "success");
  }, [showToast, updateSettings]);

  const persistBoard = React.useCallback((nextBoard: any, message: any) => {
    const nextWorkspace = {
      ...workspace,
      activeBoardId: nextBoard.id,
      boards: workspace.boards.map((item: any) => item.id === nextBoard.id ? nextBoard : item)
    };
    persistWorkspace(nextWorkspace, message);
  }, [persistWorkspace, workspace]);

  const createBoard = React.useCallback((input: any) => {
    persistWorkspace(addKanbanBoard(workspace, input), "تم إنشاء لوحة المشروع");
  }, [persistWorkspace, workspace]);

  const changeActiveBoard = React.useCallback((boardId: any) => {
    ...
    >
        ...
        actions={
          <div className="min-w-[14rem]">
            {workspace.boards.length > 0 && (
              <select
                value={board?.id || ""}
                onChange={(event: any) => changeActiveBoard(event.target.value)}
                aria-label="اختيار لوحة المشروع"
                className="min-h-10 w-full rounded-[var(--va-radius-md)] border border-[var(--va-border-strong)] bg-[var(--va-surface-2)] px-3 py-2 text-sm text-[var(--va-text)] outline-none"
              >
                {workspace.boards.map((item: any) => <option key={item.id} value={item.id}>{item.title}</option>)}
              </select>
            )}
          </div>
        }
      >
        <div className="mt-3 grid gap-2 sm:grid-cols-3">
          {PROJECT_BOARD_TIPS.map((tip: any) => (
      ...
      {!board ? (
        ...
        <>
          ...
          <CardBuilder board={board} videoItems={videoItems} onBoardChange={(nextBoard: any) => persistBoard(nextBoard, "تم حفظ البطاقة")} />
