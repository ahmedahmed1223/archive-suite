...
function TaskBuilder({ projects, items, users, activeProjectId, onProjectChange, onAdd }: any) {
  ...
  return jsxs("section", {
    ...
    children: [
      jsxs("div", { className: "mb-3 flex items-center justify-between gap-2", children: [
        ...
        activeProjectId && jsx("span", { className: "rounded-full border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] px-2 py-0.5 text-xs text-[var(--va-text-2)]", children: projectLabel(projects.find((project: any) => project.id === activeProjectId)) })
      ] }),
      jsxs("div", { className: "grid gap-3 sm:grid-cols-2", children: [
        jsxs("label", { className: "space-y-1 text-sm text-[var(--va-text-2)] sm:col-span-2", htmlFor: projectId, children: [
          ...
          jsx("select", {
            ...
            children: projects.map((project: any) => jsx("option", { value: project.id, children: projectLabel(project) }, project.id))
          })
        ] }),
        jsxs("label", { className: "space-y-1 text-sm text-[var(--va-text-2)] sm:col-span-2", htmlFor: titleId, children: [
          ...
          jsx("input", { id: titleId, value: title, onChange: (event: any) => setTitle(event.target.value), placeholder: "مثال: مراجعة لقطة الافتتاح", className: "input input-bordered w-full" })
        ] }),
        jsxs("label", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [
          jsx("span", { children: "الحالة" }),
          jsx("select", { value: status, onChange: (event: any) => setStatus(event.target.value), className: "select select-bordered w-full", children: PROJECT_TASK_STATUSES.map((key: any) => jsx("option", { value: key, children: (TASK_META as any)[key].label }, key)) })
        ] }),
        jsxs("label", { className: "space-y-1 text-sm text-[var(--va-text-2)]", htmlFor: itemIdField, children: [
          jsx("span", { children: "مادة مرتبطة" }),
          jsxs("select", { id: itemIdField, value: itemId, onChange: (event: any) => setItemId(event.target.value), className: "select select-bordered w-full", children: [
            jsx("option", { value: "", children: "بدون ربط" }),
            ...items.slice(0, 500).map((item: any) => jsx("option", { value: item.id, children: itemLabel(item) }, item.id))
          ] })
        ] }),
        jsxs("label", { className: "space-y-1 text-sm text-[var(--va-text-2)] sm:col-span-2", htmlFor: assigneeIdField, children: [
          ...
          jsxs("select", { id: assigneeIdField, value: assigneeId, onChange: (event: any) => setAssigneeId(event.target.value), className: "select select-bordered w-full", children: [
            jsx("option", { value: "", children: "غير مسندة" }),
            ...users.filter((user: any) => user.isActive !== false).map((user: any) => jsx("option", { value: user.id, children: user.displayName || user.username || user.id }, user.id))
          ] })
        ] })
      ] }),
...
function ProjectPicker({ projects, selectedId, onSelect }: any) {
  return jsx("section", {
...
function TaskBoard({ project, itemsById, usersById, onMoveTask, onRemoveTask }: any) {
  const grouped = getProjectTasksByStatus(project);
  return jsx("section", {
    className: "grid gap-3 xl:grid-cols-4",
    "aria-label": "لوحة مهام الإنتاج",
    children: PROJECT_TASK_STATUSES.map((status: any) => {
      const meta = (TASK_META as any)[status];
            ...
            children: tasks.map((task: any) => {
              ...
              return jsxs("article", {
                ...
                children: [
                  ...
                  jsx("select", { value: task.status, onChange: (event: any) => onMoveTask(task.id, event.target.value), className: "select select-bordered mt-3 w-full", children: PROJECT_TASK_STATUSES.map((key: any) => jsx("option", { value: key, children: (TASK_META as any)[key].label }, key)) })
                ]
              }, task.id);
            })
...
export function ProductionTasksPage() {
  ...
  const activeProjects = React.useMemo(() => projects.filter((project: any) => project.status !== "archived"), [projects]);
  const activeItems = React.useMemo(() => videoItems.filter((item: any) => !item.isDeleted), [videoItems]);
  const itemsById = React.useMemo(() => new Map(activeItems.map((item: any) => [item.id, item])), [activeItems]);
  ...
  const selectedProject = activeProjects.find((project: any) => project.id === selectedProjectId) || activeProjects[0] || null;

  React.useEffect(() => {
    if (selectedProjectId && activeProjects.some((project: any) => project.id === selectedProjectId)) return;
  ...
  const addTask = (taskPartial: any) => {
    if (!selectedProject) return;
    persist(addProjectTask(selectedProject, taskPartial));
    const assignee = usersById.get(taskPartial.assigneeId);
    showNotification?.(assignee ? `أُضيفت مهمة مسندة إلى ${(assignee as any).displayName || (assignee as any).username || "مستخدم"}.` : "أُضيفت مهمة إلى لوحة الإنتاج.", {
      type: "success",
      category: "task",
  ...
  const moveTask = (taskId: any, status: any) => {
    if (!selectedProject) return;
  ...
  const totalTasks = activeProjects.reduce((sum: any, project: any) => sum + (project.tasks?.length || 0), 0);
  const openTasks = activeProjects.reduce((sum: any, project: any) => sum + (project.tasks || []).filter((task: any) => task.status !== "done").length, 0);
  const reviewTasks = activeProjects.reduce((sum: any, project: any) => sum + (project.tasks || []).filter((task: any) => task.status === "review").length, 0);
  const doneTasks = activeProjects.reduce((sum: any, project: any) => sum + (project.tasks || []).filter((task: any) => task.status === "done").length, 0);
