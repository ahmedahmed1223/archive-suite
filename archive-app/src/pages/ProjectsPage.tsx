...
function formatClock(totalSeconds: any) {
...
const REVIEW_META = {
  raw: { label: "خام", tone: "border-[var(--va-border-soft)] bg-[var(--va-surface-2)] text-[var(--va-text-2)]" },
...
function formatReadiness(readiness: any) {
  if (readiness.status === "ready") return "جاهزة";
  if (readiness.status === "blocked") return "مصدر مفقود";
  return `ناقص: ${readiness.missing.map((item: any) => item.label).slice(0, 2).join("، ")}`;
}

function downloadJsonObject(payload: any, filename: any) {
...
// ── rough-cut builder ────────────────────────────────────────────────────────
function RoughCutBuilder({ items, onAdd }: any) {
  ...
  React.useEffect(() => {
    if (!items.some((i: any) => i.id === itemId)) setItemId(items[0]?.id || "");
  }, [items, itemId]);
  ...
  if (!items.length) {
    return jsx("p", {
      className: "rounded-xl border border-dashed border-[var(--va-border-soft)] bg-[var(--va-surface)] p-4 text-center text-sm text-[var(--va-text-muted)]",
  ...
  return jsxs("div", {
    ...
    children: [
      jsxs("p", { className: "mb-2 flex items-center gap-2 text-sm font-semibold text-[var(--va-text-2)]", children: [
        jsx(Scissors, { className: "h-4 w-4 va-accent-text" }), "إضافة قصاصة (rough cut)"
      ] }),
      jsxs("div", { className: "grid gap-2 sm:grid-cols-2", children: [
        jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)] sm:col-span-2", children: [
          ...
          jsx("select", {
            ...
            children: items.slice(0, 500).map((item: any) => jsx("option", { value: item.id, children: itemLabel(item) }, item.id))
          })
        ] }),
        jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [
          jsx("label", { htmlFor: inId, className: "block", children: "بداية (ث)" }),
          jsx("input", { id: inId, type: "number", min: "0", step: "0.1", value: inSec, onChange: (e: any) => setInSec(e.target.value), className: "input input-bordered w-full" })
        ] }),
        jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [
          jsx("label", { htmlFor: outId, className: "block", children: "نهاية (ث)" }),
          jsx("input", { id: outId, type: "number", min: "0", step: "0.1", value: outSec, onChange: (e: any) => setOutSec(e.target.value), className: "input input-bordered w-full" })
        ] }),
        jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)] sm:col-span-2", children: [
          jsx("label", { htmlFor: labelId, className: "block", children: "وصف القصاصة (اختياري)" }),
          jsx("input", { id: labelId, value: label, onChange: (e: any) => setLabel(e.target.value), placeholder: "مثال: لقطة الافتتاح", className: "input input-bordered w-full" })
        ] })
      ] }),
...
function SourceBin({ items, query, onQuery, selectedId, onSelect, projectItemIds = [] }: any) {
  ...
  const filtered = items
    .filter((item: any) => {
      ...
      return [itemLabel(item), item.type, item.subtype, item.path]
        .filter(Boolean)
        .some((value: any) => String(value).toLowerCase().includes(normalized));
    })
    .slice(0, 80);

  return jsxs("section", {
    className: "min-h-0 rounded-2xl border border-[var(--va-border-soft)] bg-[var(--va-surface)] p-3",
    children: [
      jsxs("div", { className: "mb-3 flex items-center justify-between gap-2", children: [
        jsxs("h3", { className: "flex items-center gap-2 text-sm font-semibold text-[var(--va-text)]", children: [jsx(ArchiveIcon, { className: "h-4 w-4 va-accent-text" }), "مكتبة المصادر"] }),
        jsx("span", { className: "rounded-full bg-[var(--va-surface-2)] px-2 py-0.5 text-xs text-[var(--va-text-muted)]", children: formatNumber(filtered.length) })
      ] }),
      jsx("input", {
        value: query,
        ...
        children: filtered.map((item: any) => {
          ...
          return jsxs("button", {
            ...
            className: [
              ...
              active ? "va-accent-border va-accent-bg-soft" : "border-[var(--va-border-soft)] bg-[var(--va-surface)] hover:border-[var(--va-border-soft)]"
            ].join(" "),
            children: [
              jsxs("div", { className: "flex items-start justify-between gap-2", children: [
                jsxs("div", { className: "min-w-0", children: [
                  jsx("p", { className: "truncate text-sm font-semibold text-[var(--va-text)]", children: itemLabel(item) }),
                  jsx("p", { className: "mt-1 truncate text-xs text-[var(--va-text-muted)]", dir: "ltr", children: itemMediaPath(item) || "لا يوجد مسار" })
                ] }),
      ...
      }) : jsx("p", { className: "rounded-xl border border-dashed border-[var(--va-border-soft)] p-4 text-center text-sm text-[var(--va-text-muted)]", children: "لا توجد مصادر مطابقة." })
    ]
  });
}

function MaterialImportPanel({ contentTypes = [], onImport }: any) {
  const firstType = contentTypes.find((type: any) => type.status !== "archived") || contentTypes[0];
  ...
  const selectedType = contentTypes.find((type: any) => type.id === typeId);
  const subtypes = selectedType?.subtypes || [];
  const canSubmit = title.trim() && path.trim() && !saving;

  React.useEffect(() => {
    if (typeId && contentTypes.some((type: any) => type.id === typeId)) return;
    setTypeId(firstType?.id || "");
  }, [contentTypes, firstType?.id, typeId]);

  React.useEffect(() => {
    if (!subtypeId || subtypes.some((subtype: any) => subtype.id === subtypeId)) return;
    setSubtypeId("");
  }, [subtypeId, subtypes]);
  ...
  const submit = async (event: any) => {
    ...
    const created = await onImport?.({
      ...
      subtype: subtypeId,
      notes: notes.trim(),
      tags: parseVideoTags(tags as any),
      sourceKind,
      media: {
  ...
  return jsxs("section", {
    className: "rounded-2xl border border-[var(--va-border-soft)] bg-[var(--va-surface)] p-3",
    "aria-label": "استيراد مواد للمونتاج",
    children: [
      jsxs("div", { className: "mb-3 flex items-center justify-between gap-2", children: [
        jsxs("h3", { className: "flex items-center gap-2 text-sm font-semibold text-[var(--va-text)]", children: [jsx(FileUp, { className: "h-4 w-4 va-accent-text" }), "استيراد مواد للمونتاج"] }),
        jsx("span", { className: "rounded-full bg-[var(--va-surface-2)] px-2 py-0.5 text-[11px] text-[var(--va-text-muted)]", children: "يرتبط بالمشروع" })
      ] }),
      jsxs("form", { onSubmit: submit, className: "space-y-3", children: [
        ...
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
          "اسم المادة",
          jsx("input", { value: title, onChange: (e: any) => setTitle(e.target.value), className: "input input-bordered mt-1 w-full", placeholder: "مثال: مقابلة المصدر الرئيسي" })
        ] }),
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
          sourceKind === "url" ? "رابط الفيديو أو نسخة الويب" : "مسار ملف المصدر",
          jsx("input", { value: path, onChange: (e: any) => setPath(e.target.value), dir: "ltr", className: "input input-bordered mt-1 w-full", placeholder: sourceKind === "url" ? "https://..." : "D:\\media\\clip.mp4" })
        ] }),
        jsxs("div", { className: "grid gap-2 sm:grid-cols-2", children: [
          jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
            "نوع المحتوى",
            jsx("select", { value: typeId, onChange: (e: any) => setTypeId(e.target.value), className: "select select-bordered mt-1 w-full", children: [
              jsx("option", { value: "", children: "بدون تصنيف" }, "empty"),
              ...contentTypes.filter((type: any) => type.status !== "archived").map((type: any) => jsx("option", { value: type.id, children: type.name }, type.id))
            ] })
          ] }),
          jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
            "الفرع",
            jsx("select", { value: subtypeId, onChange: (e: any) => setSubtypeId(e.target.value), className: "select select-bordered mt-1 w-full", children: [
              jsx("option", { value: "", children: "بدون فرع" }, "empty"),
              ...subtypes.map((subtype: any) => jsx("option", { value: subtype.id, children: subtype.name }, subtype.id))
            ] })
          ] })
        ] }),
        jsxs("details", { className: "rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-3", children: [
          jsx("summary", { className: "cursor-pointer text-xs font-semibold text-[var(--va-text)]", children: "جاهزية الوسائط وبيانات إضافية" }),
          jsxs("div", { className: "mt-3 grid gap-2", children: [
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: ["Thumbnail", jsx("input", { value: thumbnail, onChange: (e: any) => setThumbnail(e.target.value), dir: "ltr", className: "input input-bordered mt-1 w-full" })] }),
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: ["Audio key/path", jsx("input", { value: media.audioKey, onChange: (e: any) => setMedia((current: any) => ({ ...current, audioKey: e.target.value })), dir: "ltr", className: "input input-bordered mt-1 w-full" })] }),
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: ["Web proxy/derived", jsx("input", { value: media.derivedKey, onChange: (e: any) => setMedia((current: any) => ({ ...current, derivedKey: e.target.value })), dir: "ltr", className: "input input-bordered mt-1 w-full" })] }),
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: ["تفريغ مختصر", jsx("textarea", { value: media.transcription, onChange: (e: any) => setMedia((current: any) => ({ ...current, transcription: e.target.value })), className: "textarea textarea-bordered mt-1 w-full" })] }),
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: ["وسوم", jsx("input", { value: tags, onChange: (e: any) => setTags(e.target.value), className: "input input-bordered mt-1 w-full", placeholder: "archive, interview, b-roll" })] }),
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: ["ملاحظات", jsx("textarea", { value: notes, onChange: (e: any) => setNotes(e.target.value), className: "textarea textarea-bordered mt-1 w-full" })] })
          ] })
        ] }),
...
function MaterialInspector({ item, contentTypes = [], projectItemIds = [], onUpdate, onAttach }: any) {
  ...
  if (!item || !draft) {
    return jsxs("section", {
      className: "rounded-2xl border border-dashed border-[var(--va-border-soft)] bg-[var(--va-surface)] p-4 text-center",
      "aria-label": "إعدادات المادة المحددة",
      children: [
        jsx(Settings2, { className: "mx-auto h-8 w-8 text-[var(--va-text-muted)]" }),
        jsx("p", { className: "mt-2 text-sm text-[var(--va-text-muted)]", children: "اختر مادة لضبط إعداداتها وربطها بالمشروع." })
      ]
    });
  }

  const selectedType = contentTypes.find((type: any) => type.id === (draft as any).type);
  const subtypes = selectedType?.subtypes || [];
  const readiness = buildMediaReadiness({
    ...item,
    path: (draft as any).path,
    thumbnail: (draft as any).thumbnail,
    metadata: {
      ...(item.metadata || {}),
      media: {
        ...(item.metadata?.media || {}),
        thumbnailKey: (draft as any).thumbnail,
        audioKey: (draft as any).audioKey,
        transcription: (draft as any).transcription,
        derivedKey: (draft as any).derivedKey
      }
    }
  });
  const inProject = projectItemIds.includes(item.id);

  const patch = (key: any, value: any) => setDraft((current: any) => ({ ...current, [key]: value }));

  const save = async () => {
    setSaving(true);
    await onUpdate?.(item.id, {
      title: (draft as any).title.trim(),
      path: (draft as any).path.trim(),
      thumbnail: (draft as any).thumbnail.trim(),
      type: (draft as any).type,
      subtype: (draft as any).subtype,
      tags: parseVideoTags((draft as any).tags),
      notes: (draft as any).notes.trim(),
      metadata: {
        ...(item.metadata || {}),
        media: {
          ...(item.metadata?.media || {}),
          ...((draft as any).thumbnail.trim() ? { thumbnailKey: (draft as any).thumbnail.trim() } : { thumbnailKey: "" }),
          ...((draft as any).audioKey.trim() ? { audioKey: (draft as any).audioKey.trim() } : { audioKey: "" }),
          ...((draft as any).transcription.trim() ? { transcription: (draft as any).transcription.trim() } : { transcription: "" }),
          ...((draft as any).derivedKey.trim() ? { derivedKey: (draft as any).derivedKey.trim() } : { derivedKey: "" })
        },
  ...
  return jsxs("section", {
    ...
    children: [
      jsxs("div", { className: "mb-3 flex items-start justify-between gap-2", children: [
        jsxs("div", { className: "min-w-0", children: [
          jsxs("h3", { className: "flex items-center gap-2 text-sm font-semibold text-[var(--va-text)]", children: [jsx(Settings2, { className: "h-4 w-4 va-accent-text" }), "إعدادات المادة المحددة"] }),
          jsx("p", { className: "mt-1 truncate text-xs text-[var(--va-text-muted)]", dir: "ltr", children: itemMediaPath(item) || "لا يوجد مسار" })
        ] }),
      ...
      jsxs("div", { className: "grid gap-2", children: [
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: ["اسم المادة", jsx("input", { value: (draft as any).title, onChange: (e: any) => patch("title", e.target.value), className: "input input-bordered mt-1 w-full" })] }),
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: ["مسار/رابط المصدر", jsx("input", { value: (draft as any).path, onChange: (e: any) => patch("path", e.target.value), dir: "ltr", className: "input input-bordered mt-1 w-full" })] }),
        jsxs("div", { className: "grid gap-2 sm:grid-cols-2", children: [
          jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
            "نوع المحتوى",
            jsx("select", { value: (draft as any).type, onChange: (e: any) => patch("type", e.target.value), className: "select select-bordered mt-1 w-full", children: [
              jsx("option", { value: "", children: "بدون تصنيف" }, "empty"),
              ...contentTypes.filter((type: any) => type.status !== "archived").map((type: any) => jsx("option", { value: type.id, children: type.name }, type.id))
            ] })
          ] }),
          jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
            "الفرع",
            jsx("select", { value: (draft as any).subtype, onChange: (e: any) => patch("subtype", e.target.value), className: "select select-bordered mt-1 w-full", children: [
              jsx("option", { value: "", children: "بدون فرع" }, "empty"),
              ...subtypes.map((subtype: any) => jsx("option", { value: subtype.id, children: subtype.name }, subtype.id))
            ] })
          ] })
        ...
        jsxs("div", { className: "grid gap-2 sm:grid-cols-2", children: [
          jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: ["Thumbnail", jsx("input", { value: (draft as any).thumbnail, onChange: (e: any) => patch("thumbnail", e.target.value), dir: "ltr", className: "input input-bordered mt-1 w-full" })] }),
          jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: ["Audio", jsx("input", { value: (draft as any).audioKey, onChange: (e: any) => patch("audioKey", e.target.value), dir: "ltr", className: "input input-bordered mt-1 w-full" })] }),
          jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: ["Web proxy", jsx("input", { value: (draft as any).derivedKey, onChange: (e: any) => patch("derivedKey", e.target.value), dir: "ltr", className: "input input-bordered mt-1 w-full" })] }),
          jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: ["وسوم", jsx("input", { value: (draft as any).tags, onChange: (e: any) => patch("tags", e.target.value), className: "input input-bordered mt-1 w-full" })] })
        ] }),
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: ["تفريغ أو وصف صوتي", jsx("textarea", { value: (draft as any).transcription, onChange: (e: any) => patch("transcription", e.target.value), className: "textarea textarea-bordered mt-1 w-full" })] }),
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: ["ملاحظات المادة", jsx("textarea", { value: (draft as any).notes, onChange: (e: any) => patch("notes", e.target.value), className: "textarea textarea-bordered mt-1 w-full" })] }),
        jsxs("div", { className: "grid gap-2 sm:grid-cols-2", children: [
          jsxs("button", { type: "button", onClick: save, disabled: saving || !(draft as any).title.trim(), className: "btn btn-primary gap-2", children: [jsx(Save, { className: "h-4 w-4" }), saving ? "جار الحفظ..." : "حفظ إعدادات المادة"] }),
  ...
  return jsxs("section", {
    ...
    children: [
      jsxs("div", { className: "mb-3 flex flex-wrap items-center justify-between gap-2", children: [
        jsxs("h3", { className: "flex items-center gap-2 text-sm font-semibold text-[var(--va-text)]", children: [jsx(Eye, { className: "h-4 w-4 va-accent-text" }), "المعاينة وبناء القصاصة"] }),
        jsx("span", { className: "rounded-full bg-[var(--va-surface-2)] px-2 py-0.5 text-xs text-[var(--va-text-muted)]", children: `المؤشر ${formatClock(current)}` })
      ] }),
      canPreview ? jsx("div", { className: "overflow-hidden rounded-xl border border-[var(--va-border-soft)]", children: jsx(VideoPlayer, {
        videoRef,
        src: descriptor.source,
        onLoadedMetadata: (event: any) => setDuration(Number(event.currentTarget.duration) || 0),
        onTimeUpdate: (event: any) => setCurrentTime(event.currentTarget.currentTime)
      }) }) : jsxs("div", { className: "flex aspect-video items-center justify-center rounded-xl border border-dashed border-[var(--va-border-soft)] bg-black/50 p-6 text-center", children: [
        jsx("p", { className: "max-w-md text-sm leading-6 text-[var(--va-text-muted)]", children: item ? "المصدر غير قابل للمعاينة داخل المتصفح، لكن يمكنك استخدامه في القص والتصدير إذا كان متاحًا للخادم." : "اختر مصدرًا من مكتبة المواد." })
      ] }),
      jsxs("div", { className: "mt-3 grid gap-2 lg:grid-cols-[1fr_1fr_auto_auto]", children: [
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
          "In",
          jsx("input", { type: "number", min: "0", step: "0.1", value: markIn, onChange: (e: any) => onSetMarkIn(Number(e.target.value)), className: "input input-bordered mt-1 w-full" })
        ] }),
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
          "Out",
          jsx("input", { type: "number", min: "0", step: "0.1", value: markOut, onChange: (e: any) => onSetMarkOut(Number(e.target.value)), className: "input input-bordered mt-1 w-full" })
        ] }),
      ...
      jsxs("div", { className: "mt-2 grid gap-2 lg:grid-cols-[1fr_auto]", children: [
        jsx("input", { value: label, onChange: (e: any) => setLabel(e.target.value), placeholder: "اسم القصاصة أو وصف سريع", className: "input input-bordered w-full" }),
...
function ClipInspector({
  ...
  if (!clip) {
    return jsxs("aside", {
      className: "rounded-2xl border border-dashed border-[var(--va-border-soft)] bg-[var(--va-surface)] p-4 text-center",
      children: [
        jsx(SlidersHorizontal, { className: "mx-auto h-8 w-8 text-[var(--va-text-muted)]" }),
        jsx("p", { className: "mt-2 text-sm text-[var(--va-text-muted)]", children: "اختر قصاصة من الخط الزمني لتحرير خصائصها." })
      ]
    });
  }
  const meta = (REVIEW_META as any)[clip.reviewStatus] || REVIEW_META.raw;
  ...
  const patchNested = (key: any, patch: any) => onUpdate({ [key]: { ...(clip[key] || {}), ...patch } });
  const applyPreset = (preset: any) => onUpdate(buildMontagePresetClipPatch(preset));
  return jsxs("aside", {
    className: "rounded-2xl border border-[var(--va-border-soft)] bg-[var(--va-surface)] p-4",
    children: [
      jsxs("div", { className: "mb-3 flex items-start justify-between gap-2", children: [
        jsxs("div", { className: "min-w-0", children: [
          jsx("h3", { className: "truncate text-sm font-semibold text-[var(--va-text)]", children: clip.label || itemLabel(item) }),
          jsx("p", { className: "mt-1 truncate text-xs text-[var(--va-text-muted)]", children: itemLabel(item) })
        ] }),
        jsx("span", { className: `shrink-0 rounded-full border px-2 py-0.5 text-[11px] ${meta.tone}`, children: meta.label })
      ] }),
      jsxs("div", { className: "grid gap-2 sm:grid-cols-2", children: [
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)] sm:col-span-2", children: [
          "اسم القصاصة",
          jsx("input", { value: clip.label || "", onChange: (e: any) => onUpdate({ label: e.target.value }), className: "input input-bordered mt-1 w-full" })
        ] }),
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
          "In",
          jsx("input", { type: "number", min: "0", step: "0.1", disabled: clip.locked, value: clip.inSec, onChange: (e: any) => onUpdate({ inSec: Number(e.target.value) }), className: "input input-bordered mt-1 w-full" })
        ] }),
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
          "Out",
          jsx("input", { type: "number", min: "0", step: "0.1", disabled: clip.locked, value: clip.outSec, onChange: (e: any) => onUpdate({ outSec: Number(e.target.value) }), className: "input input-bordered mt-1 w-full" })
        ] }),
        ...
        jsxs("div", { className: "grid gap-2 rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-3 sm:col-span-2", children: [
          jsxs("div", { className: "flex items-center justify-between gap-2", children: [
            jsxs("p", { className: "flex items-center gap-2 text-xs font-semibold text-[var(--va-text)]", children: [jsx(Scissors, { className: "h-4 w-4 va-accent-text" }), "أدوات القطع" ] }),
            jsx("span", { className: "text-[11px] text-[var(--va-text-muted)]", children: `المدة ${formatClock(roughCutDuration(clip))}` })
          ] }),
        ...
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
          "المراجعة",
          jsx("select", { value: clip.reviewStatus || "raw", onChange: (e: any) => onUpdate({ reviewStatus: e.target.value }), className: "select select-bordered mt-1 w-full", children: MONTAGE_REVIEW_STATUSES.map((status: any) => jsx("option", { value: status, children: (REVIEW_META as any)[status]?.label || status }, status)) })
        ] }),
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
          "المسار",
          jsx("input", { value: clip.trackId || "v1", onChange: (e: any) => onUpdate({ trackId: e.target.value }), className: "input input-bordered mt-1 w-full" })
        ] }),
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
          "اللون",
          jsx("input", { type: "color", value: clip.color || "#22c55e", onChange: (e: any) => onUpdate({ color: e.target.value }), className: "mt-1 h-10 w-full rounded-lg border border-[var(--va-border-soft)] bg-transparent p-1" })
        ] }),
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
          "الصوت dB",
          jsx("input", { type: "number", min: "-60", max: "12", step: "1", value: clip.volumeDb ?? 0, onChange: (e: any) => onUpdate({ volumeDb: Number(e.target.value) }), className: "input input-bordered mt-1 w-full" })
        ] }),
        jsxs("label", { className: "flex items-center gap-2 rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-3 text-sm text-[var(--va-text-2)] sm:col-span-2", children: [
          jsx("input", { type: "checkbox", checked: Boolean(clip.locked), onChange: (e: any) => onUpdate({ locked: e.target.checked }), className: "checkbox checkbox-sm" }),
          jsx(Lock, { className: "h-4 w-4 text-[var(--va-text-muted)]" }),
          "قفل القصاصة من التحريك والقص"
        ] }),
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)] sm:col-span-2", children: [
          "ملاحظات زمنية",
          jsx("textarea", { value: clip.notes || "", onChange: (e: any) => onUpdate({ notes: e.target.value }), className: "textarea textarea-bordered mt-1 w-full", placeholder: "ملاحظة للمراجعة أو الاعتماد..." })
        ] }),
        jsxs("section", { className: "rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-3 sm:col-span-2", children: [
          ...
          jsxs("div", { className: "space-y-2", children: [
            ...
            jsx("div", { className: "flex flex-wrap gap-1.5", children: MONTAGE_TRANSITIONS.map((type: any) => jsxs("button", {
              type: "button",
              ...
              children: [
                jsx("span", { className: "font-mono text-[11px] leading-none", "aria-hidden": "true", dir: "ltr", children: (TRANSITION_ICONS as any)[type] }),
                jsx("span", { children: (TRANSITION_LABELS as any)[type] || type })
              ]
            }, type)) }),
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
              "مدة الانتقال",
              jsx("input", {
                ...
                onChange: (e: any) => patchNested("transition", { durationSec: Number(e.target.value) }),
        ...
        jsxs("section", { className: "rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-3 sm:col-span-2", children: [
          ...
          jsx("div", { className: "mb-3 flex flex-wrap gap-2", children: MONTAGE_LOOKS.map((look: any) => jsxs("button", {
            type: "button",
            onClick: () => applyPreset(look),
            title: (LOOK_LABELS as any)[look] || look,
          ...
          jsxs("div", { className: "grid gap-2 sm:grid-cols-3", children: [
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
              ...
              jsx("input", { type: "range", min: "-1", max: "1", step: "0.05", value: filters.brightness ?? 0, onChange: (e: any) => patchNested("filters", { brightness: Number(e.target.value) }), className: "range range-xs mt-2" })
            ] }),
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
              `Contrast ${filters.contrast ?? 1}`,
              jsx("input", { type: "range", min: "0", max: "3", step: "0.05", value: filters.contrast ?? 1, onChange: (e: any) => patchNested("filters", { contrast: Number(e.target.value) }), className: "range range-xs mt-2" })
            ] }),
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
              `Saturation ${filters.saturation ?? 1}`,
              jsx("input", { type: "range", min: "0", max: "3", step: "0.05", value: filters.saturation ?? 1, onChange: (e: any) => patchNested("filters", { saturation: Number(e.target.value) }), className: "range range-xs mt-2" })
            ] })
          ] })
        ] }),
        jsxs("section", { className: "rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-3 sm:col-span-2", children: [
          jsxs("div", { className: "mb-2 flex items-center justify-between gap-2", children: [
            jsxs("p", { className: "flex items-center gap-2 text-xs font-semibold text-[var(--va-text)]", children: [jsx(SlidersHorizontal, { className: "h-4 w-4 va-accent-text" }), "Transformation" ] }),
          ...
          jsxs("div", { className: "grid gap-2 sm:grid-cols-3", children: [
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
              "Scale",
              jsx("input", { type: "number", min: "0.1", max: "5", step: "0.05", value: transform.scale ?? 1, onChange: (e: any) => patchNested("transform", { scale: Number(e.target.value) }), className: "input input-bordered mt-1 w-full" })
            ] }),
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
              "X",
              jsx("input", { type: "number", step: "1", value: transform.x ?? 0, onChange: (e: any) => patchNested("transform", { x: Number(e.target.value) }), className: "input input-bordered mt-1 w-full" })
            ] }),
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
              "Y",
              jsx("input", { type: "number", step: "1", value: transform.y ?? 0, onChange: (e: any) => patchNested("transform", { y: Number(e.target.value) }), className: "input input-bordered mt-1 w-full" })
            ] }),
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
              "Rotation",
              jsx("input", { type: "number", min: "-180", max: "180", step: "1", value: transform.rotation ?? 0, onChange: (e: any) => patchNested("transform", { rotation: Number(e.target.value) }), className: "input input-bordered mt-1 w-full" })
            ] }),
            jsxs("label", { className: "text-xs text-[var(--va-text-muted)] sm:col-span-2", children: [
              `Opacity ${Math.round((transform.opacity ?? 1) * 100)}%`,
              jsx("input", { type: "range", min: "0", max: "1", step: "0.05", value: transform.opacity ?? 1, onChange: (e: any) => patchNested("transform", { opacity: Number(e.target.value) }), className: "range range-xs mt-2" })
            ] })
          ] })
        ] })
      ] }),
...
function TimelineSettingsPanel({ settings, onChange }: any) {
  const current = { ...DEFAULT_TIMELINE_SETTINGS, ...(settings || {}) };
  return jsxs("section", {
    className: "rounded-2xl border border-[var(--va-border-soft)] bg-[var(--va-surface)] p-3",
    children: [
      jsxs("h3", { className: "mb-3 flex items-center gap-2 text-sm font-semibold text-[var(--va-text)]", children: [jsx(Gauge, { className: "h-4 w-4 va-accent-text" }), "إعدادات المشروع"] }),
      jsxs("div", { className: "grid gap-2 sm:grid-cols-3", children: [
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
          "FPS",
          jsx("input", { type: "number", min: "1", step: "1", value: current.fps, onChange: (e: any) => onChange({ ...current, fps: Number(e.target.value) }), className: "input input-bordered mt-1 w-full" })
        ] }),
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
          "الدقة",
          jsx("input", { value: current.resolution, onChange: (e: any) => onChange({ ...current, resolution: e.target.value }), className: "input input-bordered mt-1 w-full" })
        ] }),
        jsxs("label", { className: "text-xs text-[var(--va-text-muted)]", children: [
          "النسبة",
          jsx("input", { value: current.aspectRatio, onChange: (e: any) => onChange({ ...current, aspectRatio: e.target.value }), className: "input input-bordered mt-1 w-full" })
        ] })
      ] })
    ]
  });
...
function MarkerPanel({ markers = [], currentTime = 0, onAddMarker }: any) {
  const [label, setLabel] = React.useState("");
  const add = () => {
  ...
  return jsxs("section", {
    ...
    children: [
      jsxs("div", { className: "mb-3 flex items-center justify-between gap-2", children: [
        jsxs("h3", { className: "flex items-center gap-2 text-sm font-semibold text-[var(--va-text)]", children: [jsx(Flag, { className: "h-4 w-4 va-accent-text" }), "Markers"] }),
        jsx("span", { className: "rounded-full bg-[var(--va-surface-2)] px-2 py-0.5 text-xs text-[var(--va-text-muted)]", children: formatClock(currentTime) })
      ] }),
      jsxs("div", { className: "grid gap-2 sm:grid-cols-[1fr_auto]", children: [
        jsx("input", { value: label, onChange: (event: any) => setLabel(event.target.value), placeholder: "علامة زمنية: افتتاح، ذروة، نهاية...", className: "input input-bordered input-sm w-full" }),
      ...
      markers.length ? jsx("div", {
        ...
        children: [...markers].sort((a: any, b: any) => a.atSec - b.atSec).slice(0, 6).map((marker: any) => jsxs("div", { className: "flex items-center justify-between gap-2 rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] px-3 py-2 text-xs", children: [
          jsx("span", { className: "truncate text-[var(--va-text-2)]", children: marker.label }),
          jsx("span", { className: "tabular-nums text-[var(--va-text-muted)]", children: formatClock(marker.atSec) })
        ] }, marker.id))
      }) : jsx("p", { className: "mt-3 text-xs text-[var(--va-text-muted)]", children: "لا توجد markers بعد." })
    ]
  });
}

function TemporalCommentsPanel({ clip, comments = [], currentTime = 0, onAddComment }: any) {
  ...
  return jsxs("section", {
    className: "mt-4 rounded-2xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-3",
    children: [
      jsxs("div", { className: "mb-2 flex items-center justify-between gap-2", children: [
        jsxs("h4", { className: "flex items-center gap-2 text-xs font-semibold text-[var(--va-text)]", children: [jsx(MessageSquare, { className: "h-4 w-4 va-accent-text" }), "تعليقات زمنية"] }),
        jsx("span", { className: "rounded-full bg-black/20 px-2 py-0.5 text-[11px] text-[var(--va-text-muted)]", children: formatClock(currentTime) })
      ] }),
      jsxs("div", { className: "grid gap-2", children: [
        jsx("textarea", { value: body, onChange: (event: any) => setBody(event.target.value), disabled: !clip, className: "textarea textarea-bordered min-h-20 w-full", placeholder: clip ? "تعليق مرتبط بهذه القصاصة والزمن الحالي..." : "اختر قصاصة أولًا" }),
      ...
      comments.length ? jsx("div", {
        className: "mt-3 space-y-2",
        children: comments.slice(0, 5).map((comment: any) => jsxs("article", { className: "rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface)] p-2 text-xs", children: [
          jsxs("div", { className: "mb-1 flex items-center justify-between gap-2", children: [
            ...
            jsx("span", { className: "tabular-nums text-[var(--va-text-muted)]", children: formatClock(comment.atSec) })
          ] }),
          jsx("p", { className: "leading-5 text-[var(--va-text-2)]", children: comment.body })
        ] }, comment.id))
      }) : jsx("p", { className: "mt-3 text-xs text-[var(--va-text-muted)]", children: "لا توجد تعليقات على هذه القصاصة." })
    ]
  });
}

function ExportCenter({ canExport, exportMode = "server", serverFfmpeg, hasValidCuts, exporting, onExport, onDownloadPackage, onOpenExportWizard, exportEvents = [] }: any) {
  ...
  return jsxs("section", {
    ...
    children: [
      jsxs("div", { className: "mb-3 flex items-center justify-between gap-2", children: [
        jsxs("h3", { className: "flex items-center gap-2 text-sm font-semibold text-[var(--va-text)]", children: [jsx(Download, { className: "h-4 w-4 va-accent-text" }), "مركز التصدير"] }),
      ...
      exportEvents.length ? jsx("div", {
        className: "mt-3 space-y-2",
        children: exportEvents.slice(0, 3).map((event: any) => jsxs("div", { className: "flex items-center justify-between gap-2 rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] px-3 py-2 text-xs", children: [
          jsx("span", { className: event.ok ? "text-emerald-200" : "text-red-200", children: event.label }),
          jsx("span", { className: "text-[var(--va-text-muted)]", children: event.time })
        ] }, event.id))
      }) : jsx("p", { className: "mt-3 text-xs text-[var(--va-text-muted)]", children: "لم يتم إنشاء تصديرات في هذه الجلسة بعد." })
    ]
  });
}

function ProductionBoard({ summary }: any) {
  const cells = [
  ...
  return jsx("section", {
    ...
    children: cells.map(([label, value, Icon]: any) => jsxs("div", { className: "rounded-2xl border va-surface-muted p-4 text-right", children: [
      jsxs("div", { className: "flex items-center justify-between gap-2", children: [
...
// ── timeline row ─────────────────────────────────────────────────────────────
function TimelineRow({ cut, index, total, itemsById, onMove, onRemove }: any) {
  ...
  return jsxs("div", {
    ...
    children: [
      ...
      jsxs("div", { className: "min-w-0", children: [
        jsx("p", { className: "truncate text-sm font-semibold text-[var(--va-text)]", children: title }),
        jsxs("p", { className: "mt-0.5 text-xs text-[var(--va-text-muted)]", children: [
          `${secondsToTimecode(cut.inSec)} ← ${secondsToTimecode(cut.outSec)}`,
          jsx("span", { className: "mx-2 text-[var(--va-text-muted)]", children: "·" }),
      ...
      jsxs("div", { className: "flex shrink-0 items-center gap-1", children: [
        jsx("button", { type: "button", disabled: index === 0, onClick: () => onMove(cut.id, index - 1), "aria-label": "تحريك لأعلى", className: "rounded-lg p-1.5 text-[var(--va-text-muted)] hover:bg-[var(--va-surface-2)] hover:text-[var(--va-text)] disabled:opacity-30", children: jsx(ChevronUp, { className: "h-4 w-4" }) }),
        jsx("button", { type: "button", disabled: index === total - 1, onClick: () => onMove(cut.id, index + 1), "aria-label": "تحريك لأسفل", className: "rounded-lg p-1.5 text-[var(--va-text-muted)] hover:bg-[var(--va-surface-2)] hover:text-[var(--va-text)] disabled:opacity-30", children: jsx(ChevronDown, { className: "h-4 w-4" }) }),
        jsx("button", { type: "button", onClick: () => onRemove(cut.id), "aria-label": "حذف القصاصة", className: "rounded-lg p-1.5 text-[var(--va-text-muted)] hover:bg-red-500/10 hover:text-red-300", children: jsx(Trash2, { className: "h-4 w-4" }) })
      ] })
    ]
  }, cut.id);
}
...
// ── project card ─────────────────────────────────────────────────────────────
function ProjectCard({ project, active, index, onOpen, onDelete }: any) {
  const cuts = project.roughCuts?.length || 0;
  const validCuts = (project.roughCuts || []).filter(isValidRoughCut).length;
  const openTasks = (project.tasks || []).filter((task: any) => task.status !== "done").length;
  ...
  return jsxs(motion.article, {
    ...
    onClick: onOpen,
    className: `va-entity-card cursor-pointer rounded-2xl border p-4 text-right transition-all ${active ? "va-accent-border va-accent-bg-soft" : "border-[var(--va-border-soft)] bg-[var(--va-surface)] hover:border-[var(--va-border-soft)]"}`,
    dir: "rtl",
    children: [
        ...
        jsxs("div", { className: "flex min-w-0 items-start gap-3", children: [
          ...
          jsxs("div", { className: "min-w-0", children: [
            jsxs("div", { className: "flex flex-wrap items-center gap-2", children: [
              jsx("h3", { className: "truncate text-base font-bold text-[var(--va-text)]", children: project.name || "مشروع بدون اسم" }),
              project.status === "archived" && jsx("span", { className: "rounded-full border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] px-2 py-0.5 text-xs text-[var(--va-text-muted)]", children: "مؤرشف" })
            ] }),
            project.description && jsx("p", { className: "mt-1 line-clamp-2 text-sm leading-relaxed text-[var(--va-text-muted)]", children: project.description })
          ] })
        ] }),
        jsx("div", { className: "flex shrink-0 gap-1", onClick: (e: any) => e.stopPropagation(), children: jsx("button", { type: "button", onClick: onDelete, className: "rounded-lg p-2 text-[var(--va-text-muted)] hover:bg-red-500/10 hover:text-red-300", "aria-label": `حذف ${project.name}`, children: jsx(Trash2, { className: "h-4 w-4" }) }) })
      ] }),
      jsxs("div", { className: "mt-3 grid grid-cols-3 gap-2 text-xs", children: [
        jsxs("span", { className: "rounded-lg border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-2", children: [jsx("b", { className: "block text-[var(--va-text)]", children: `${formatNumber(validCuts)}/${formatNumber(cuts)}` }), jsx("span", { className: "text-[var(--va-text-muted)]", children: "قصاصات" })] }),
        jsxs("span", { className: "rounded-lg border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-2", children: [jsx("b", { className: "block text-[var(--va-text)]", children: formatClock(duration) }), jsx("span", { className: "text-[var(--va-text-muted)]", children: "مدة" })] }),
        jsxs("span", { className: openTasks ? "rounded-lg border border-amber-500/20 bg-amber-500/10 p-2" : "rounded-lg border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] p-2", children: [jsx("b", { className: openTasks ? "block text-amber-100" : "block text-[var(--va-text)]", children: formatNumber(openTasks) }), jsx("span", { className: "text-[var(--va-text-muted)]", children: "مهام" })] })
      ] }),
      project.updatedAt && jsx("p", { className: "mt-3 text-xs text-[var(--va-text-muted)]", children: `آخر تحديث: ${formatDateTime(project.updatedAt)}` })
    ]
  }, project.id);
}
...
function ProjectEditor({
  ...
  const [selectedClipId, setSelectedClipId] = React.useState<any>(null);
  ...
  React.useEffect(() => {
    const nextSource = project?.itemIds?.find((id: any) => itemsById.has(id)) || items[0]?.id || "";
    setSelectedSourceId((current: any) => (current && itemsById.has(current) ? current : nextSource));
  }, [items, itemsById, project?.id, project?.itemIds]);

  React.useEffect(() => {
    const ordered = normalizedTimeline.roughCuts;
    setSelectedClipId((current: any) => (current && ordered.some((clip: any) => clip.id === current) ? current : ordered[0]?.id || null));
  ...
  if (!project) {
    return jsxs("section", {
      ...
      children: [
        jsx(Film, { className: "mx-auto h-12 w-12 text-[var(--va-text-muted)]" }),
        jsx("h2", { className: "mt-3 text-lg font-bold text-[var(--va-text)]", children: "اختر مشروعًا" }),
        jsx("p", { className: "mt-2 text-sm text-[var(--va-text-muted)]", children: "ستظهر هنا تفاصيل المشروع، بناء القصاصات، والخطّ الزمني." })
      ]
    });
  }

  const ordered = normalizedTimeline.roughCuts;
  const total = ordered.reduce((end: any, clip: any) => Math.max(end, (Number(clip.timelineStartSec) || 0) + roughCutDuration(clip)), 0);
  const selectedSource = itemsById.get(selectedSourceId) || items[0] || null;
  const selectedClip = ordered.find((clip: any) => clip.id === selectedClipId) || null;
  ...
  const approvedCount = ordered.filter((clip: any) => clip.reviewStatus === "approved").length;
  const mediaIssues = project.itemIds
    .map((id: any) => itemsById.get(id))
    .filter(Boolean)
    .map((item: any) => buildMediaReadiness(item))
    .filter((readiness: any) => readiness.status !== "ready").length;

  const updateClip = (patch: any) => {
    if (!selectedClip) return;
    onUpdateCut?.(selectedClip.id, patch);
  };

  const useCurrentOnClip = (field: any, value: any) => {
  ...
  const splitSelectedClip = (atSec: any) => {
    if (!selectedClip) return;
  ...
  const commitTimeline = ({ timelineTracks = normalizedTimeline.timelineTracks, roughCuts = normalizedTimeline.roughCuts }: any = {}) => {
    onPatch({
      timelineTracks,
      roughCuts,
  ...
  const handleTimelineCommand = (command: any) => {
    ...
    if (command.type === "track.reorder") {
      const tracks = [...normalizedTimeline.timelineTracks].sort((a: any, b: any) => a.order - b.order);
      const index = tracks.findIndex((track: any) => track.id === command.trackId);
    ...
    if (command.type === "clip.delete") {
      const roughCuts = normalizedTimeline.roughCuts
        .filter((clip: any) => clip.id !== command.clipId)
        .map((clip: any, order: any) => ({ ...clip, order }));
    ...
    const clip = normalizedTimeline.roughCuts.find((entry: any) => entry.id === command.clipId);
    if (!clip) return;
    const tracks = [...normalizedTimeline.timelineTracks].sort((a: any, b: any) => a.order - b.order);
  ...
  const workspaceHeader = jsxs("div", { className: "montage-project-header", children: [
    ...
    jsxs("div", { className: "montage-project-header__identity", children: [
      jsx("input", {
        value: project.name,
        onChange: (event: any) => onPatch({ name: event.target.value }),
      ...
      jsx("input", {
        ...
        onChange: (event: any) => onPatch({ description: event.target.value }),
  ...
  const mediaBin = jsxs("div", { className: "montage-workspace__panel-stack", children: [
    ...
    jsx(MaterialInspector, {
      ...
      onAttach: (itemId: any) => onAttachMaterial?.(project.id, itemId)
    })
  ] });
  ...
  const timeline = jsxs("div", { className: "montage-timeline-stage", children: [
    jsxs("div", { className: "montage-timeline-stage__bar", children: [
      ...
      jsxs("div", { className: "montage-timeline-stage__controls", children: [
        ...
        jsx("select", { value: timelineZoom, onChange: (event: any) => setTimelineZoom(Number(event.target.value)), className: "select select-bordered select-xs", "aria-label": "تكبير الخط الزمني", children: TIMELINE_ZOOM.map((zoom: any) => jsx("option", { value: zoom.value, children: zoom.label }, zoom.value)) })
      ] })
    ] }),
  ...
  const inspector = jsxs("div", { className: "montage-workspace__panel-stack", children: [
    ...
    jsx(TimelineSettingsPanel, {
      ...
      onChange: (timelineSettings: any) => onPatch({ timelineSettings })
    }),
    jsx(MarkerPanel, {
      markers: project.markers || [],
      currentTime,
  ...
  return jsx(MontageWorkspace, {
    ...
    toolbar: jsx(MontageToolStrip, {
      ...
      onPreferencesChange: (timelinePreferences: any) => onPatch({ timelinePreferences })
    }),
    mediaBin,
    monitor,
    inspector,
...
// ── create modal ─────────────────────────────────────────────────────────────
function ProjectForm({ onCancel, onSave }: any) {
    ...
    children: jsxs("div", {
      ...
      children: [
        jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [
          ...
          jsx("input", { id: nameId, ref: nameRef, "data-autofocus": true, value: name, onChange: (e: any) => setName(e.target.value), placeholder: "مثال: تقرير القدس", className: "input input-bordered w-full" })
        ] }),
        jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [
          jsx("label", { htmlFor: descId, className: "block", children: "الوصف (اختياري)" }),
          jsx("textarea", { id: descId, value: description, onChange: (e: any) => setDescription(e.target.value), placeholder: "وصف موجز للمشروع", className: "textarea textarea-bordered w-full" })
        ] })
      ]
    })
  });
...
export function ProjectsPage() {
  ...
  const [exportEvents, setExportEvents] = React.useState<any[]>([]);
  const [showForm, setShowForm] = React.useState(false);

  const activeItems = React.useMemo(() => videoItems.filter((i: any) => !i.isDeleted), [videoItems]);
  const itemsById = React.useMemo(() => new Map(activeItems.map((i: any) => [i.id, i])), [activeItems]);
  const filtered = React.useMemo(() => getFilteredProjects(projects, query), [projects, query]);
  const selected = projects.find((p: any) => p.id === selectedId) || filtered[0] || null;
  const openedProject = openedId ? projects.find((p: any) => p.id === openedId) || null : null;
  ...
  const totalTasks = React.useMemo(() => projects.filter((project: any) => project.status !== "archived").reduce((sum: any, project: any) => sum + (project.tasks?.length || 0), 0), [projects]);
  ...
  const mp4Enabled = canExportMp4({ backend: cloud.backend, token: getCloudToken(), wasmAvailable: wasmMp4Enabled } as any);
  const mp4Mode = cloud.backend !== "local" && getCloudToken() ? "server" : wasmMp4Enabled ? "wasm" : "unavailable";

  React.useEffect(() => {
    if (selectedId && projects.some((p: any) => p.id === selectedId)) return;
    setSelectedId((filtered[0] as any)?.id || null);
  }, [filtered, projects, selectedId]);

  // If the opened project disappears (deleted), fall back to the list view.
  React.useEffect(() => {
    if (openedId && !projects.some((p: any) => p.id === openedId)) setOpenedId(null);
  }, [openedId, projects]);
  ...
  const saveProject = async (draft: any, opts: any = {}) => {
    try {
  ...
  const attachMaterialToProject = async (projectId: any, itemId: any) => {
    const target = projects.find((project: any) => project.id === projectId);
  ...
  const importMaterial = async (projectId: any, draft: any = {}) => {
    const target = projects.find((project: any) => project.id === projectId);
  ...
  const updateMaterial = async (itemId: any, patch: any = {}) => {
    const current = itemsById.get(itemId);
    if (!current) return null;
    const metadata = patch.metadata
      ? { ...((current as any).metadata || {}), ...patch.metadata }
  ...
  const updateCut = (cutId: any, patch: any) => {
    if (!selected) return;
    const roughCuts = (selected.roughCuts || []).map((cut: any) => {
  ...
  const reorderClipsVisual = (nextClips: any) => {
  ...
  const removeProject = async (project: any) => {
    const itemCount = activeItems.filter((item: any) => item.project === project.id).length;
  ...
  const runExport = async (kind: any) => {
    if (!selected) return;
    ...
    try {
      if (kind === "json") {
        ...
        setExportEvents((events: any) => [{ id: `${Date.now()}-json`, ok: true, label: "JSON timeline", time: formatDateTime(new Date().toISOString()) }, ...events]);
      } else if (kind === "edl") {
        ...
        setExportEvents((events: any) => [{ id: `${Date.now()}-edl`, ok: true, label: "EDL CMX3600", time: formatDateTime(new Date().toISOString()) }, ...events]);
      } else if (kind === "mp4") {
        setExporting(true);
        ...
        setExportEvents((events: any) => [{ id: `${Date.now()}-mp4`, ok: true, label: "MP4 render", time: formatDateTime(new Date().toISOString()) }, ...events]);
      }
    } catch (error: any) {
      if (error instanceof CloudExportError) {
        showToast?.(error.message, "error");
        setExportEvents((events: any) => [{ id: `${Date.now()}-${kind}-error`, ok: false, label: `فشل ${kind.toUpperCase()}`, time: formatDateTime(new Date().toISOString()) }, ...events]);
      } else {
  ...
  const downloadDeliveryPackage = () => {
    ...
    if (ok) {
      ...
      setExportEvents((events: any) => [{ id: `${Date.now()}-delivery`, ok: true, label: "Delivery package", time: formatDateTime(new Date().toISOString()) }, ...events]);
    } else {
      showToast?.("تعذر تنزيل حزمة التسليم في هذه البيئة.", "error");
      setExportEvents((events: any) => [{ id: `${Date.now()}-delivery-error`, ok: false, label: "فشل حزمة التسليم", time: formatDateTime(new Date().toISOString()) }, ...events]);
    }
  };
  ...
  return jsxs(motion.div, {
    ...
    children: [
      ...
      projects.length > 0 && jsx("section", {
        ...
        ].map(([label, value, Icon]: any) => jsxs("div", { className: "va-metric-card rounded-2xl va-surface-muted border p-4 text-right", children: [
          jsxs("div", { className: "flex items-center justify-between gap-3", children: [
            jsx("span", { className: "text-sm text-[var(--va-text-muted)]", children: label }),
            jsx(Icon, { className: "h-5 w-5 va-accent-text" })
          ] }),
          jsx("p", { className: "mt-2 text-2xl font-bold text-[var(--va-text)]", children: value })
        ] }, label))
      }),
      ...
      projects.length === 0 ? jsx("div", {
        ...
        jsxs("label", { className: "va-filter-surface relative block rounded-2xl va-surface-muted border p-3", children: [
          ...
          jsx("input", { value: query, onChange: (e: any) => setQuery(e.target.value), placeholder: "بحث في المشاريع...", className: "input input-bordered w-full py-2 pl-3 pr-10 placeholder:text-[var(--va-text-muted)]" })
        ] }),
        filtered.length ? jsx("div", { className: "grid gap-3 sm:grid-cols-2 lg:grid-cols-3", children: filtered.map((project: any, index: any) => jsx(ProjectCard, {
          project, index,
          active: selected?.id === project.id,
          onOpen: () => openProject(project),
          onDelete: () => removeProject(project)
        }, project.id)) }) : jsx("div", { className: "va-card rounded-2xl border border-dashed border-[var(--va-border-soft)] bg-[var(--va-surface)]", children: jsx(EmptyState, {
          icon: jsx(Clapperboard, { className: "h-16 w-16" }),
