...
function StatusButton({ status, onCycle }: any) {
  return jsx("button", {
    type: "button",
    title: "تغيير الحالة",
...
function ListItemRow({ entry, onRemove, onCycleStatus, onNavigate }: any) {
  return jsxs("div", {
    className:
    ...
    children: [
      ...
      jsxs("div", {
        ...
        onKeyDown: (e: any) => e.key === "Enter" && onNavigate(entry),
        children: [
          jsx("p", {
...
function ListSection({ list, items, onRemoveItem, onCycleStatus, onDeleteList, onNavigate }: any) {
  const [collapsed, setCollapsed] = React.useState(false);
  const pending = items.filter((i: any) => i.status !== READING_LIST_STATUS.COMPLETED).length;
  const done = items.filter((i: any) => i.status === READING_LIST_STATUS.COMPLETED).length;
...
function NewListDialog({ onClose, onCreate }: any) {
...
export default function ReadingListsPage() {
  ...
  function handleNavigate(entry: any) {
    setSelectedItemId?.(entry.itemId);
    setCurrentPage?.("detail");
  }

  function handleCycleStatus(entry: any) {
  ...
  const totalPending = readingListItems.filter(
    (i: any) => i.status !== READING_LIST_STATUS.COMPLETED
  ).length;

  const filteredLists = React.useMemo(() => {
    if (filter === "all") return readingLists;
    return readingLists.filter((l: any) => {
      const items = readingListItems.filter((i: any) => i.listId === l.id);
      if (filter === "pending")
        return items.some((i: any) => i.status !== READING_LIST_STATUS.COMPLETED);
      if (filter === "completed")
        return items.some((i: any) => i.status === READING_LIST_STATUS.COMPLETED);
      return true;
    });
    ...
    children: [
      ...
      showNewList &&
        jsx(NewListDialog, {
          ...
          onCreate: (title: any) => createCustomReadingList({ title })
        })
    ]
  });
}
