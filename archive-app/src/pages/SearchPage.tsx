...
function SearchMetric({ label, value, hint }: any) {
  return jsxs("div", {
...
function SearchResultCard({ item, typeLabel, subtypeLabel, index, onOpen, onOpenSnippet, resultMode = "list", itemSize = "compact" }: any) {
        ...
        jsxs("div", {
          ...
          children: [
            ...
            item.transcriptSnippets?.length > 0 && jsxs("div", { className: "mt-3 rounded-xl border va-accent-border va-accent-bg-soft p-2.5", children: [
              ...
              jsx("div", { className: "space-y-2", children: item.transcriptSnippets.map((snippet: any) => jsxs("button", {
                type: "button",
...
function SearchInputWithSuggestions({ query, setQuery, showSuggestions, setShowSuggestions, recentSearches, videoItems, handleVoiceIntent, handleVoiceUnsupported, handleVoiceError, searchInputRef }: any) {
  ...
  const suggestions = React.useMemo(() => {
    ...
    videoItems.forEach((item: any) => {
      (item.tags || []).forEach((t: any) => { if (t.toLowerCase().includes(lc)) tagSet.add(t); });
    });
    const recentMatches = (recentSearches || []).filter((s: any) => s.toLowerCase().includes(lc) && s !== query);
    return [
      ...recentMatches.slice(0, 3).map((s: any) => ({ type: "recent", value: s })),
      ...[...tagSet].slice(0, 5).map((t: any) => ({ type: "tag", value: t }))
    ].slice(0, 7);
  }, [query, videoItems, recentSearches]);

  React.useEffect(() => {
    function handleClickOutside(e: any) {
      if (containerRef.current && !(containerRef.current as any).contains(e.target)) setShowSuggestions(false);
    }
  ...
  return jsxs("div", {
    ...
    children: [
      jsxs("label", {
        ...
        children: [
          ...
          jsx("input", {
            ref: searchInputRef,
            value: query,
            onChange: (event: any) => { setQuery(event.target.value); setShowSuggestions(true); },
      ...
      showSuggestions && suggestions.length > 0 && jsx("ul", {
        ...
        children: suggestions.map(({ type: sType, value }: any) => jsxs("li", {
          role: "option",
          children: [
            jsx("button", {
              type: "button",
...
export function SearchPage() {
  ...
  const [missingFieldsOnly, setMissingFieldsOnly] = React.useState((initialRouteState as any).missingFieldsOnly || false);
  ...
  const [resultMode, setResultMode] = React.useState((initialRouteState as any).viewMode || "list");
  const [itemSize, setItemSize] = React.useState((initialRouteState as any).itemSize || "compact");
  ...
  const typeById = React.useMemo(() => new Map(contentTypes.map((item: any) => [item.id, item])), [contentTypes]);
  const activeType = typeById.get(type);
  const subtypes = (activeType as any)?.subtypes || [];
  ...
  React.useEffect(() => {
    const applyRouteState = () => {
      ...
      setMissingFieldsOnly(!!(nextState as any).missingFieldsOnly);
      setDateFrom(nextState.dateFrom);
      setDateTo(nextState.dateTo);
      setPage(nextState.page);
      setPageSize(nextState.pageSize);
      setResultMode((nextState as any).viewMode || "list");
      setItemSize((nextState as any).itemSize || "compact");
    };
  ...
  const openItem = (item: any, seconds: any = null) => {
  ...
  const saveCurrentView = async (name: any, filters: any) => {
  ...
  const renameView = async (viewId: any, name: any) => {
  ...
  const typeLabel = (item: any) => (typeById.get(item.type) as any)?.name || item.type || "";
  const subtypeLabel = (item: any) => (typeById.get(item.type) as any)?.subtypes?.find((sub: any) => sub.id === item.subtype)?.name || item.subtype || "";
    ...
    children: [
      ...
      jsxs("section", {
        ...
        children: [
          ...
          jsx(SearchMetric, { label: "المفضلة", value: formatNumber(results.filter((item: any) => item.isFavorite).length), hint: "ضمن النتائج" }),
          jsx(SearchMetric, { label: "كل العناصر", value: formatNumber(videoItems.filter((item: any) => !item.isDeleted).length), hint: "نشطة في الأرشيف" })
        ]
      }),
      jsx(SavedViewsBar, {
        views: savedViews,
        ...
        children: [
          jsxs("div", {
            ...
            children: [
              ...
              jsxs("button", {
                type: "button",
                onClick: () => setShowFilterPanel((value: any) => !value),
                  ...
                  children: [
                    jsxs("select", {
                      ...
                      children: [
                        ...
                        ...contentTypes.map((item: any) => jsx("option", { value: item.id, children: item.name || item.id }, item.id))
                      ]
                    }),
                    jsxs("select", {
                      "aria-label": "تصفية البحث حسب الفرع",
                      value: subtype,
                      onChange: (event: any) => setSubtype(event.target.value),
                      ...
                      children: [
                        ...
                        ...subtypes.map((item: any) => jsx("option", { value: item.id, children: item.name || item.id }, item.id))
                      ]
                    }),
                    jsxs("select", {
                      "aria-label": "عدد نتائج البحث في الصفحة",
                      value: pageSize,
                      onChange: (event: any) => setPageSize(Number(event.target.value)),
                      className: "min-h-11 va-surface-deep rounded-xl border px-3 py-2 text-sm text-[var(--va-text)]",
                      children: PAGE_SIZE_OPTIONS.map((value: any) => jsx("option", { value, children: `${formatNumber(value)} نتيجة` }, value))
                    })
                  ]
                }),
                jsxs("div", {
                  ...
                  children: [
                    jsxs("label", {
                      ...
                      children: [
                        ...
                        jsx("input", { type: "date", value: dateFrom, onChange: (event: any) => setDateFrom(event.target.value), dir: "ltr", className: "input input-bordered w-full" })
                      ]
                    }),
                    jsxs("label", {
                      className: "block va-surface-muted rounded-xl border p-3",
                      children: [
                        ...
                        jsx("input", { type: "date", value: dateTo, onChange: (event: any) => setDateTo(event.target.value), dir: "ltr", className: "input input-bordered w-full" })
                      ]
                    }),
                    jsx("button", {
                      type: "button",
                      onClick: () => setFavoritesOnly((value: any) => !value),
                    ...
                    jsx("button", {
                      type: "button",
                      onClick: () => setMissingFieldsOnly((value: any) => !value),
                  ...
                  children: [
                    ...
                    jsxs("label", {
                      ...
                      children: [
                        ...
                        jsx("select", {
                          value: itemSize,
                          onChange: (event: any) => setItemSize(event.target.value),
                          ...
                          children: ARCHIVE_ITEM_SIZE_OPTIONS.map((option: any) => jsx("option", { value: option.value, children: (ARCHIVE_ITEM_SIZE_LABELS as any)[option.value] || option.label }, option.value))
                        })
                      ]
                    })
                  ]
      ...
      results.length === 0 ? jsxs("section", {
        ...
        children: [
          ...
          jsx(TagCloud, { videoItems, onSelect: (tag: any) => setQuery(tag), activeTag: query }),
          recentSearches.length > 0 && jsxs("div", {
            ...
            children: [
              ...
              jsx("div", {
                ...
                children: recentSearches.map((term: any) => jsxs("button", {
                  type: "button",
        ...
        children: [
          ...
          jsx("div", {
            ...
            children: visibleResults.map((item: any, index: any) => jsx(SearchResultCard, {
              item,
              index,
              ...
              onOpenSnippet: (snippet: any) => openItem(item, snippet.seconds)
            }, item.id))
          }),
          jsx(Pagination, { currentPage, totalPages, onPageChange: (nextPage: any) => setPage(Math.min(Math.max(nextPage, 1), totalPages)) })
        ]
      }),
      jsx(MobileActionBar, {
        label: "إجراءات البحث",
        actions: [
          { id: "filters", label: "فلاتر", icon: SlidersHorizontal, active: activeFilterCount > 0, onClick: () => (filtersRef.current as any)?.scrollIntoView({ block: "start", behavior: "smooth" }) },
