...
function createAppearanceDraft(settings: any = {}, fallbackTheme: any = "dark") {
...
function AppearanceStudioPreview({ draft, numberSystem = "latn" }: any) {
      ...
      jsxs("div", {
        ...
        children: [
          jsx("aside", {
            ...
            children: ["الأرشيف", "البحث", "المركز"].map((item: any, index: any) => jsxs("div", {
...
export function SettingsPage() {
  ...
  React.useEffect(() => {
    ...
    settings.theme,
    (settings.ui as any)?.cardStyle,
    (settings.ui as any)?.fontScale,
    settings.ui?.daisyTheme,
    (settings.ui as any)?.motionLevel,
    (settings.ui as any)?.sidebarLayout?.collapsed,
    (settings.ui as any)?.sidebarLayout?.mode,
  ...
  const saveSettings = React.useCallback(async (patch: any, successMessage?: any) => {
    const ok = await updateSettings?.(patch);
    if (ok !== false && successMessage) showToast?.(successMessage, "success");
    return ok;
  }, [showToast, updateSettings]);

  const patchUi = (uiPatch: any, message: any) => saveSettings({ ui: { ...(settings.ui || {}), ...uiPatch } }, message);
  const patchNotifications = (patch: any, message: any = "تم تحديث الإشعارات") => saveSettings({ notifications: { ...(settings.notifications || {}), ...patch } }, message);
  const toggleMutedCategory = (category: any, muted: any) => {
    const current = new Set<any>(settings.notifications?.mutedCategories || []);
    if (muted) current.add(category);
    else current.delete(category);
    return patchNotifications({ mutedCategories: [...current] });
  };
  const toggleToastType = (type: any, enabled: any) => patchNotifications({
    toastByType: {
      ...(settings.notifications?.toastByType || {}),
      [type]: enabled
    }
  });
  const patchAppearanceDraft = (patch: any) => setAppearanceDraft((current: any) => ({
    ...current,
    ...(typeof patch === "function" ? patch(current) : patch)
  }));
  const selectDaisyTheme = (value: any) => {
    ...
    patchAppearanceDraft((current: any) => ({
      daisyTheme,
    ...
    patchUi({ daisyTheme }, undefined);
  };
  const patchLiveThemeEditor = (patch: any) => {
    if (patch.daisyTheme) {
  ...
  const applyAppearanceDraft = () => {
    ...
    return saveSettings({
      ...
      ui: {
        ...
        customDaisyTheme,
        sidebarLayout: {
          ...((settings.ui as any)?.sidebarLayout || {}),
  ...
  const applyAppearancePreset = async (preset: any) => {
    ...
    const profile = {
      ...
      settings: {
        ...
        ui: {
          ...
          sidebarLayout: (settings.ui as any)?.sidebarLayout,
          fontScale: (settings.ui as any)?.fontScale,
          motionLevel: (settings.ui as any)?.motionLevel,
          cardStyle: (settings.ui as any)?.cardStyle,
  ...
  const importAppearanceProfile = async (event: any) => {
  ...
  const updateAutocompleteTrigger = (key: any, value: any) => {
  ...
  const openOnboardingWizard = () => {
    patchUi({ onboardingReplayRequestedAt: new Date().toISOString() }, undefined);
    ...
    children: [
        ...
        children: jsxs("div", {
          ...
          children: [
            jsxs("div", {
              ...
              children: [
                jsx(SegmentedChoices, { label: "العرض الافتراضي للأرشيف", value: settings.defaultView || "grid", options: VIEW_OPTIONS, onChange: (value: any) => saveSettings({ defaultView: value }, "تم تحديث العرض الافتراضي") }),
                jsx(SelectRow, {
                  label: "عدد العناصر الافتراضي",
                  value: String(settings.itemsPerPage || 24),
                  onChange: (value: any) => saveSettings({ itemsPerPage: Number(value) }, "تم تحديث عدد العناصر"),
                  options: [12, 24, 48, 96].map((value: any) => ({ value: String(value), label: `${formatNumber(value)} عنصر` })),
            ...
            jsx(ToggleRow, {
              ...
              checked: !!(settings.ui as any)?.editInSidePanel,
              onChange: (checked: any) => patchUi({ editInSidePanel: checked }, "تم تحديث وضع التعديل")
            })
          ]
        })
      }),
      jsx(SettingsCard, {
        ...
        children: jsx(RoleSelectionStep, {
          compact: true,
          value: normalizeRoleProfileId((settings.ui as any)?.roleProfile || currentUser?.role || "editor"),
          onChange: (value: any) => patchUi({ roleProfile: value }, "تم تحديث نمط الاستخدام")
        })
      })
    ]
  });
    ...
    children: [
          ...
          children: [
            jsxs("div", {
              ...
              children: [
                ...
                jsx(SegmentedChoices, { label: "المظهر", value: appearanceDraft.theme, options: THEME_OPTIONS, onChange: (value: any) => patchAppearanceDraft({ theme: value }) }),
                jsx(ColorChoices, { value: appearanceDraft.accentColor, onChange: (value: any) => patchAppearanceDraft({ accentColor: value }) }),
                jsx(SegmentedChoices, {
                  label: "كثافة الواجهة",
                  value: appearanceDraft.visualDensity,
                  options: DENSITY_OPTIONS,
                  columns: "sm:grid-cols-2",
                  onChange: (value: any) => patchAppearanceDraft({ visualDensity: value })
                }),
                jsx(SegmentedChoices, {
                  label: "حجم الخط",
                  ...
                  onChange: (value: any) => patchAppearanceDraft({ fontScale: value })
                }),
                jsx(SegmentedChoices, {
                  label: "مستوى الحركة",
                  ...
                  onChange: (value: any) => patchAppearanceDraft({ motionLevel: value })
                }),
                jsx(SegmentedChoices, {
                  ...
                  columns: "sm:grid-cols-3",
                  onChange: (value: any) => patchAppearanceDraft({ cardStyle: value })
                }),
                jsx(SegmentedChoices, {
                  ...
                  columns: "sm:grid-cols-3",
                  onChange: (value: any) => patchAppearanceDraft({ sidebarMode: value })
                }),
      ...
      jsx(SettingsCard, {
        ...
        aside: jsxs("div", { className: "flex flex-wrap gap-2", children: [
          ...
          jsx("button", { type: "button", onClick: () => (appearanceImportRef.current as any)?.click(), className: "rounded-xl border border-[var(--va-border-soft)] px-3 py-1.5 text-xs font-semibold text-[var(--va-text-2)] hover:bg-[var(--va-surface-2)]", children: "استيراد JSON" }),
        ...
        children: jsx("div", { className: "grid gap-3 md:grid-cols-2 xl:grid-cols-4", children: APPEARANCE_PRESETS.map((preset: any) => jsxs("button", {
          type: "button",
        ...
        children: jsxs("div", {
          ...
          children: [
            jsx(SegmentedChoices, {
              ...
              columns: "sm:grid-cols-2",
              onChange: (value: any) => saveSettings({ numberSystem: value }, "تم تحديث نظام الأرقام")
            }),
            jsx(SelectRow, {
              label: "اللغة",
              value: settings.language || "ar",
              onChange: (value: any) => saveSettings({ language: value }, "تم تحديث اللغة"),
    ...
    children: jsxs("div", {
      ...
      children: [
        jsx(SelectRow, {
          ...
          onChange: (value: any) => patchUi({ iconPickerLastTab: value }, "تم تحديث تفضيل منتقي الأيقونات"),
          options: [
        ...
        jsxs("div", {
          ...
          children: [
            jsx(TextInputRow, {
              ...
              onChange: (value: any) => updateAutocompleteTrigger("vocabulary", value),
              dir: "ltr",
            ...
            jsx(TextInputRow, {
              ...
              onChange: (value: any) => updateAutocompleteTrigger("tags", value),
              dir: "ltr",
    ...
    children: [
      ...
      jsx(SettingsCard, {
        ...
        children: jsx("div", {
          ...
          ].map(([method, detail]: any) => jsxs("div", {
            className: cx(
  ...
  const renderData = () => jsx(SettingsCard, {
    ...
    children: jsxs("div", {
      className: "space-y-3",
      children: [
        jsx(ToggleRow, { label: "الحفظ التلقائي", checked: !!settings.autoSave, onChange: (checked: any) => saveSettings({ autoSave: checked }, "تم تحديث الحفظ التلقائي"), description: "يحفظ التغييرات اليومية بدون خطوة إضافية." }),
        jsx(ToggleRow, { label: "النسخ الاحتياطي التلقائي", checked: !!settings.autoBackup, onChange: (checked: any) => saveSettings({ autoBackup: checked }, "تم تحديث النسخ التلقائي"), description: "ينشئ نسخاً احتياطية حسب الجدولة المحددة." }),
        jsxs("div", {
          className: "grid gap-3 md:grid-cols-2",
          children: [
            jsx(SelectRow, {
              ...
              onChange: (value: any) => saveSettings({ backupSchedule: value }, "تم تحديث جدولة النسخ"),
              options: [
            ...
            jsx(TextInputRow, {
              ...
              onChange: (value: any) => saveSettings({ backupInterval: Math.max(5, Number(value) || 60) }),
              dir: "ltr",
        ...
        jsxs("div", {
          ...
          children: [
            jsx("p", { className: "text-sm text-[var(--va-text-muted)]", children: (settings as any).lastBackupAt ? `آخر نسخة: ${formatDateTime((settings as any).lastBackupAt)}` : "لا توجد نسخة احتياطية مسجلة بعد." }),
        ...
        jsxs("div", {
          ...
          children: [
            jsx(TextInputRow, {
              ...
              onChange: (value: any) => saveSettings({ sessionTimeout: Math.max(1, Number(value) || 30) }),
              dir: "ltr"
            }),
            jsx(SelectRow, {
              label: "محاولات الدخول قبل القفل",
              value: String(authStore.maxLoginAttempts || 5),
              onChange: (value: any) => authStore.updateSecuritySettings?.(Number(value), authStore.lockoutDurationMs),
              options: [3, 5, 8, 10, 20].map((value: any) => ({ value: String(value), label: `${formatNumber(value)} محاولات` }))
            })
          ]
        }),
        jsx(ToggleRow, { label: "تفعيل مهلة الجلسة", checked: !!settings.enableSessionTimeout, onChange: (checked: any) => saveSettings({ enableSessionTimeout: checked }, "تم تحديث مهلة الجلسة") }),
        jsx(ToggleRow, { label: "تحذيرات المحتوى", checked: !!settings.contentWarningsEnabled, onChange: (checked: any) => saveSettings({ contentWarningsEnabled: checked }, "تم تحديث تحذيرات المحتوى") }),
        jsxs("div", {
    ...
    children: [
      ...
      jsx(SettingsCard, {
        ...
        children: jsxs("div", {
          className: "space-y-3",
          children: [
            jsx(ToggleRow, { label: "تفعيل الربط الخارجي", checked: !!settings.externalDb?.enabled, onChange: (checked: any) => saveSettings({ externalDb: { ...(settings.externalDb || {}), enabled: checked, mode: checked ? "bridge" : "disabled" } }, "تم تحديث الربط الخارجي") }),
            jsx(TextInputRow, {
              label: "عنوان الجسر المحلي",
              value: settings.externalDb?.bridgeUrl || "http://127.0.0.1:8766",
              onChange: (value: any) => saveSettings({ externalDb: { ...(settings.externalDb || {}), bridgeUrl: value } }),
              dir: "ltr",
        ...
        children: jsxs("div", {
          ...
          children: [
            jsx(ToggleRow, {
              ...
              onChange: (checked: any) => patchNotifications({ persistImportant: checked })
            }),
            jsx("div", { className: "grid gap-3 sm:grid-cols-2", children: [
              jsx(TextInputRow, {
                ...
                onChange: (value: any) => patchNotifications({ durationMs: Math.max(1000, Number(value) || 5500) }),
                dir: "ltr",
              ...
              jsx(TextInputRow, {
                ...
                onChange: (value: any) => patchNotifications({ retentionDays: Math.max(1, Math.min(365, Number(value) || 30)) }),
                dir: "ltr",
            ...
            jsxs("div", { className: "rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface)] p-3", children: [
              ...
              jsx("div", { className: "mt-3 grid gap-2 sm:grid-cols-2", children: NOTIFICATION_CATEGORIES.map((category: any) => jsx(ToggleRow, {
                label: NOTIFICATION_CATEGORY_LABELS[category] || category,
                checked: !((settings.notifications?.mutedCategories || []) as any[]).includes(category),
                onChange: (enabled: any) => toggleMutedCategory(category, !enabled)
              }, category)) })
            ] }),
            jsxs("div", { className: "rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface)] p-3", children: [
              jsx("p", { className: "text-sm font-semibold text-[var(--va-text)]", children: "أنواع تظهر كـ toast" }),
              jsx("div", { className: "mt-3 grid gap-2 sm:grid-cols-2", children: Object.entries(NOTIFICATION_TYPE_LABELS).map(([type, label]: any) => jsx(ToggleRow, {
                label,
                checked: (settings.notifications?.toastByType as any)?.[type] !== false,
                onChange: (enabled: any) => toggleToastType(type, enabled)
              }, type)) })
            ] }),
      ...
      jsx(SettingsCard, {
        ...
        children: jsx(TextInputRow, {
          ...
          value: String((settings as any).sharing?.defaultExpiryDays || 30),
          onChange: (value: any) => saveSettings({ sharing: { ...((settings as any).sharing || {}), defaultExpiryDays: Math.max(1, Math.min(365, Number(value) || 30)) } }, "تم تحديث المشاركة"),
          dir: "ltr",
    ...
    children: [
      ...
      jsxs("div", {
        ...
        children: [
          ...
          jsx("div", { className: "min-w-0", children: (tabContent as any)[activeTab]?.() || renderGeneral() })
        ]
      })
    ]
  });
