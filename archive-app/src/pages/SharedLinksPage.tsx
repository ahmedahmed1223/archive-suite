...
function LinkRow({ entry, onRevoke, onRemove, baseUrl, getToken }: any) {
  ...
  async function handleRevoke() {
    ...
    setRevokeError("");
    try {
      await revokeShareLink({ jti: entry.jti, baseUrl, getToken } as any);
              ...
              jsxs("div", {
                ...
                children: [
                  jsx("span", {
                    ...
                    children: (SCOPE_LABELS as any)[entry.scopeType] || entry.scopeType || "رابط"
                  }),
                  jsx("span", {
                    className: "badge badge-sm rounded-full va-accent-border va-accent-bg-soft va-accent-text-on-soft",
                    children: (PERMISSION_LABELS as any)[entry.permission] || entry.permission
                  }),
...
export default function SharedLinksPage() {
  const backend = useAppStore((s: any) => s.settings?.backendChoice);
  const getToken = useAppStore((s: any) => s.auth?.getToken);
  const baseUrl = useAppStore((s: any) => s.settings?.serverUrl || "");
  ...
  function handleRevoke(jti: any) {
    setLinks((prev: any) => prev.map((e: any) => (e.jti === jti ? { ...e, revoked: true } : e)));
  }

  function handleRemove(key: any) {
  ...
  const active = links.filter((l: any) => !l.revoked && !isExpired(l.expiresAt));
  const inactive = links.filter((l: any) => l.revoked || isExpired(l.expiresAt));
    ...
    children: [
      ...
      inactive.length > 0 && jsxs("section", {
        ...
        children: [
          ...
          jsx("ul", {
            className: "space-y-2",
            children: inactive.map((entry: any) =>
              jsx(LinkRow, {
                entry,
                onRevoke: handleRevoke,
                onRemove: handleRemove,
