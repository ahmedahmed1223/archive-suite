...
function addHistoryEntry(entry) {
  ...
  const idx = entries.findIndex((e) => e.token === entry.token || (e.url && e.url === entry.url));
...
function PermissionBadge({ permission }) {
  const Icon = PERMISSION_ICONS[permission] || Eye;
...
export default function SharedWithMePage({ openShareUrl } = {}) {
  ...
  function handleRemove(entry) {
    const updated = readHistory().filter((e) => e.token !== entry.token && e.url !== entry.url);
    saveHistory(updated);
    setHistory(updated);
  }
