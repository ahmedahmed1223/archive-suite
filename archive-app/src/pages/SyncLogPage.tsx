...
function ConnectionBadge({ state }: any) {
  const presentation = (CONNECTION_PRESENTATION as any)[state] || CONNECTION_PRESENTATION.online;
...
function SyncOpRow({ op }: any) {
  const status = (OP_STATUS_PRESENTATION as any)[op.status] || OP_STATUS_PRESENTATION.pending;
  return jsxs("div", {
    ...
    children: [
      jsxs("div", { className: "min-w-0", children: [
        jsxs("p", { className: "truncate text-sm font-semibold text-[var(--va-text)]", children: [
          (OP_ACTION_LABEL as any)[op.action] || op.action, " · ", op.entity
        ] }),
...
function ConflictRow({ conflict, onResolve }: any) {
  const title = conflict.local?.title || conflict.remote?.title || conflict.id;
  const typeLabel = (CONFLICT_TYPE_LABEL as any)[conflict.type] || conflict.type;
...
function SyncStatusPanel() {
  ...
  const handleResolve = (id: any, strategy: any) => {
  ...
  return jsxs("section", {
    ...
    children: [
      ...
      jsxs("div", { className: "space-y-2", children: [
        ...
        ops.length > 0
          ? jsx("div", { className: "space-y-2", children: ops.map((op: any) => jsx(SyncOpRow, { op }, op.id)) })
      ...
      jsxs("div", { className: "space-y-2", children: [
        ...
        conflicts.length > 0
          ? jsx("div", { className: "space-y-2", children: conflicts.map((conflict: any) => jsx(ConflictRow, { conflict, onResolve: handleResolve }, conflict.id)) })
...
function SyncEventRow({ entry, index }: any) {
  const presentation = (SYNC_EVENT_PRESENTATION as any)[entry.eventType] || {
  ...
  const toneClass = (TONE_CLASSES as any)[presentation.tone] || TONE_CLASSES.emerald;
      ...
      jsxs("div", { className: "flex flex-wrap items-start justify-between gap-3", children: [
        jsxs("div", { className: "flex min-w-0 items-start gap-3", children: [
          ...
          jsxs("div", { className: "min-w-0", children: [
            ...
            chips && chips.length > 0 && jsx("div", { className: "mt-2 flex flex-wrap gap-1.5", children: chips.map((chip: any, chipIndex: any) => jsx("span", {
...
function DeviceSummaryCard({ device, eventsByType }: any) {
...
export function SyncLogPage() {
  ...
  const syncEvents = React.useMemo(() => auditLogs
    .filter((log: any) => typeof log?.eventType === "string" && log.eventType.startsWith("sync."))
    .slice()
  ...
  const eventsByType = React.useMemo(() => {
    const counts = {};
    for (const entry of syncEvents) (counts as any)[entry.eventType] = ((counts as any)[entry.eventType] || 0) + 1;
    return counts;
  }, [syncEvents]);
  ...
  return jsxs(MotionPage, {
    ...
    children: [
      ...
      jsxs("section", { className: "grid gap-4 md:grid-cols-[minmax(0,1fr)_280px]", children: [
        syncEvents.length ? jsx("div", { className: "space-y-3", children: syncEvents.map((entry: any, index: any) => jsx(SyncEventRow, { entry, index }, entry.id)) }) : jsx("div", {
        ...
        jsxs("aside", { className: "space-y-3", children: [
          ...
          (eventsByType as any)["sync.deltaApply"] > 0 && jsxs("div", { className: "rounded-2xl border border-amber-500/25 bg-amber-500/10 p-3 text-xs leading-relaxed text-amber-100", children: [
