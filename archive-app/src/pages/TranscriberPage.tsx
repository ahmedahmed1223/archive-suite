...
function ModeButton({ active, onClick, icon, label, hint }: any) {
  return jsxs("button", {
    type: "button",
    onClick,
    "aria-pressed": active,
...
export function TranscriberPage() {
  ...
  const saveToArchive = async () => {
    ...
    const title = (file as any).name.replace(/\.[^.]+$/, "");
    const itemType = (file as any).type.startsWith("video/") ? "video" : "audio";
    try {
  ...
  const copy = async (withTimecodes: any) => {
  ...
  const segments = Array.isArray((result as any)?.segments) ? (result as any).segments : [];
    ...
    children: [
      ...
      jsxs("section", { className: "va-control-surface space-y-4 rounded-2xl va-surface-muted border p-4", children: [
        ...
        jsxs("label", { htmlFor: fileInputId, className: "block cursor-pointer rounded-2xl border border-dashed border-[var(--va-border-strong)] bg-[var(--va-surface)] p-6 text-center transition-colors hover:border-emerald-500/40", children: [
          ...
          jsx("p", { className: "mt-2 text-sm font-semibold text-[var(--va-text)]", children: file ? (file as any).name : "اختر ملف صوت أو فيديو" }),
          jsx("p", { className: "mt-1 text-xs text-[var(--va-text-muted)]", children: file ? formatFileSize((file as any).size) : "MP3 · WAV · M4A · MP4 · WEBM …" }),
          jsx("input", { id: fileInputId, type: "file", "aria-label": "اختيار ملف صوت أو فيديو للتفريغ", accept: "audio/*,video/*", className: "hidden", onChange: (e: any) => { setFile(e.target.files?.[0] || null); setResult(null); } })
        ] }),
      ...
      result ? jsxs("section", { className: "space-y-4 rounded-2xl va-surface-muted border p-4", children: [
        ...
        segments.length > 0 && jsx("div", { className: "max-h-72 space-y-1 overflow-y-auto", children: segments.map((seg: any, i: any) => jsxs("div", { className: "flex gap-3 rounded-lg border border-[var(--va-border-soft)] bg-[var(--va-surface)] px-3 py-1.5 text-sm", children: [
