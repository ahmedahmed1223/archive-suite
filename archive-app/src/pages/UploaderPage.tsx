...
function FileThumb({ row, fileStore }: any) {
  ...
  React.useEffect(() => {
    ...
    fileStore.getUrl(row.key)
      .then((next: any) => { if (alive) setUrl(next || ""); })
      .catch(() => { if (alive) setUrl(""); });
    return () => { alive = false; };
  }, [fileStore, row.key, row.type]);

  const meta = (TYPE_META as any)[row.type] || TYPE_META.file;
...
export function UploaderPage() {
  ...
  const autoProcessMedia = async (key: any, file: any) => {
  ...
  const open = async (key: any) => {
    try {
      const url = await fileStore?.getUrl?.(key);
      if (url && typeof window !== "undefined") window.open(url as any, "_blank", "noopener");
  ...
  return jsxs(motion.div, {
    ...
    children: [
      ...
      jsxs("section", { className: "va-control-surface space-y-3 rounded-2xl va-surface-muted border p-4", children: [
        jsxs("div", { className: "grid gap-3 md:grid-cols-2", children: [
          jsxs("label", { htmlFor: folderId, className: "block space-y-1 text-sm text-[var(--va-text-2)]", children: [
            ...
            jsx("input", { id: folderId, value: folder, onChange: (e: any) => setFolder(e.target.value), placeholder: "مثال: thumbnails", dir: "ltr", className: "input input-bordered w-full" })
          ] }),
          jsxs("label", { htmlFor: searchId, className: "block space-y-1 text-sm text-[var(--va-text-2)]", children: [
            ...
            jsxs("div", { className: "relative", children: [
              ...
              jsx("input", { id: searchId, value: query, onChange: (e: any) => setQuery(e.target.value), placeholder: "filename, folder, type", dir: "ltr", className: "input input-bordered w-full" })
            ] })
          ] })
        ] }),
        jsxs("label", { htmlFor: inputId, className: "block cursor-pointer rounded-2xl border border-dashed border-[var(--va-border-strong)] bg-[var(--va-surface)] p-6 text-center transition-colors hover:border-emerald-500/40", children: [
          ...
          jsx("input", { id: inputId, type: "file", "aria-label": "اختيار ملفات للرفع", multiple: true, className: "hidden", disabled: busy, onChange: (e: any) => upload(e.target.files) })
        ] })
      ] }),
      ...
      jsxs("section", { className: "space-y-3 rounded-2xl va-surface-muted border p-4", children: [
        ...
        }) : loading && !rows.length ? jsx(UXStateBlock, { state: "loading" }) : visibleRows.length ? jsx("div", { className: "grid gap-2 lg:grid-cols-2", children: visibleRows.map((row: any) => {
          const meta = (TYPE_META as any)[row.type] || TYPE_META.file;
