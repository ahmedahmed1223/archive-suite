...
function getRole(roleId: any) {
  return USER_ROLES.find((role: any) => role.id === roleId) || USER_ROLES[USER_ROLES.length - 1];
}

function UserForm({ user, users, onCancel, onSave }: any) {
  ...
  const usernameExists = !user && users.some((item: any) => item.username.trim().toLowerCase() === username.trim().toLowerCase());
  ...
  const submit = async (keepOpen: any) => {
    ...
    if (ok && keepOpen) {
      ...
      window.requestAnimationFrame(() => (displayNameRef.current as any)?.focus());
    }
  };
        ...
        jsxs("div", {
          ...
          children: [
            ...
            !user && jsxs("div", { className: "space-y-2 text-sm text-[var(--va-text-2)]", children: [
              jsxs("label", { className: "flex items-center gap-2 rounded-xl border border-[var(--va-border-soft)] bg-[var(--va-surface)] px-3 py-2 text-xs text-[var(--va-text-2)]", children: [
                jsx("input", {
                  ...
                  onChange: (event: any) => setInviteByEmail(event.target.checked),
            ...
            jsxs("div", { className: `space-y-2 ${user ? "md:col-span-2" : ""}`, children: [
              ...
              jsx("div", { role: "radiogroup", "aria-labelledby": roleGroupId, className: "flex flex-wrap gap-2", children: USER_ROLES.map((item: any) => jsxs("button", {
                type: "button",
                role: "radio",
...
function UserCard({ user, currentUser, users, index, recentOpsCount = 0, onEdit, onToggle, onDelete }: any) {
...
export function UsersPage() {
  ...
  const filteredUsers = React.useMemo(() => getFilteredUsers(users, query, roleFilter as any), [query, roleFilter, users]);

  const saveUser = async (draft: any, opts: any = {}) => {
    try {
      if (editingUser) {
        await updateUser?.(createUserValue({
          ...(editingUser as object),
          displayName: draft.displayName,
          role: draft.role,
          createdAt: (editingUser as any).createdAt
        }));
  ...
  return jsxs(MotionPage, {
    ...
    children: [
      ...
      ].map(([label, value, Icon]: any) => jsxs("div", { className: "card va-metric-card rounded-2xl va-surface-muted border p-4 text-right", children: [
        jsxs("div", { className: "flex items-start justify-between gap-3", children: [
          jsxs("div", { className: "min-w-0", children: [
      ...
      jsxs("section", { className: "va-filter-surface rounded-2xl va-surface-muted border p-4", children: [
        jsxs("div", { className: "grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]", children: [
          jsxs("label", { className: "relative block", children: [
            ...
            jsx("input", { value: query, onChange: (event: any) => setQuery(event.target.value), placeholder: "بحث بالاسم أو اسم المستخدم...", className: "input input-bordered w-full" })
          ] }),
          jsxs("select", { value: roleFilter, onChange: (event: any) => setRoleFilter(event.target.value), className: "select select-bordered min-h-11 va-surface-deep rounded-xl border px-3 text-sm text-[var(--va-text)] outline-none", children: [
            jsx("option", { value: "all", children: "كل الأدوار" }),
            ...USER_ROLES.map((role: any) => jsx("option", { value: role.id, children: role.label }, role.id))
          ] })
        ] }),
        jsxs("div", { role: "group", "aria-label": "تصفية حسب الدور", className: "mt-4 flex flex-wrap gap-2", children: [
          ...
          ...USER_ROLES.map((role: any) => jsxs("button", { type: "button", onClick: () => setRoleFilter(role.id), className: `btn btn-sm rounded-xl border px-3 py-2 text-sm gap-2 inline-flex items-center ${roleFilter === role.id ? "text-[var(--va-text)]" : "border-[var(--va-border-soft)] bg-[var(--va-surface)] text-[var(--va-text-muted)] hover:bg-[var(--va-surface-2)]"}`, style: roleFilter === role.id ? { borderColor: `${role.color}55`, backgroundColor: `${role.color}18` } : undefined, children: [
            jsx("span", { className: "inline-block h-2.5 w-2.5 rounded-full", style: { backgroundColor: role.color } }),
            role.label,
            jsx("span", { className: "badge badge-sm rounded-full bg-black/20 px-2 py-0.5 text-xs", children: formatNumber((summary.byRole as any)[role.id] || 0) })
          ] }, role.id))
        ] })
      ] }),
      filteredUsers.length ? jsx("section", { className: "grid gap-3 lg:grid-cols-2", children: filteredUsers.map((user: any, index: any) => jsx(UserCard, {
        user,
        users,
        currentUser,
        index,
