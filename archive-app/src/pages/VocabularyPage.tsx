...
function getCategoryInfo(categoryId: any) {
  return VOCABULARY_CATEGORIES.find((category: any) => category.id === categoryId) || VOCABULARY_CATEGORIES[VOCABULARY_CATEGORIES.length - 1];
}

function CategoryButton({ category, count, active, onClick }: any) {
  return jsxs("button", {
    type: "button",
    onClick,
...
function VocabularyForm({ entry, activeCategory, onCancel, onSave }: any) {
  ...
  const submit = async (keepOpen: any) => {
    ...
    if (ok && keepOpen) {
      ...
      window.requestAnimationFrame(() => (termRef.current as any)?.focus());
    }
  };
      ...
      children: [
        ...
        jsxs("div", { className: "space-y-1 text-sm text-[var(--va-text-2)]", children: [
          ...
          jsx("select", {
            ...
            children: VOCABULARY_CATEGORIES.map((item: any) => jsx("option", { value: item.id, children: item.label }, item.id))
          })
        ] }),
...
function VocabularyCard({ entry, index, onEdit, onDelete }: any) {
            ...
            children: [
              ...
              entry.aliases?.length > 0 && jsxs("div", {
                ...
                children: [
                  ...
                  entry.aliases.map((alias: any) => jsx("span", {
...
export function VocabularyPage() {
  ...
  const saveEntry = async (draft: any, opts: any = {}) => {
    try {
      if (editingEntry) {
        await updateVocabularyEntry?.(createVocabularyEntryValue({
          ...(editingEntry as object),
          ...draft,
          createdAt: (editingEntry as any).createdAt
        }));
  ...
  const createTermFromTag = async (tag: any) => {
    try {
  ...
  return jsxs(MotionPage, {
    ...
    children: [
      ...
      vocabulary.length > 0 && jsxs("section", { className: "grid gap-3 xl:grid-cols-3", children: [
        jsxs("div", { className: "rounded-2xl va-surface-muted border p-4 text-right", children: [
          ...
          workspace.duplicates.length ? jsx("div", { className: "mt-3 space-y-2", children: workspace.duplicates.slice(0, 4).map((group: any) => jsxs("div", { className: "rounded-xl va-surface-muted border p-3", children: [
            ...
            jsx("p", { className: "mt-1 text-xs text-[var(--va-text-muted)]", children: group.entries.map((entry: any) => entry.term).join("، ") }),
        ...
        jsxs("div", { className: "rounded-2xl va-surface-muted border p-4 text-right", children: [
          ...
          workspace.unusedTerms.length ? jsx("div", { className: "mt-3 flex flex-wrap gap-2", children: workspace.unusedTerms.slice(0, 10).map((entry: any) => jsx("button", { type: "button", onClick: () => { setQuery(entry.term); setCategory("all"); }, className: "rounded-full border border-[var(--va-border-soft)] bg-[var(--va-surface-2)] px-3 py-1 text-xs text-[var(--va-text-2)] hover:bg-[var(--va-elevated)]", children: entry.term }, entry.id)) }) : jsx("p", { className: "mt-3 text-sm text-[var(--va-text-muted)]", children: "كل المصطلحات تظهر في مواد أو وسوم." })
        ] }),
        jsxs("div", { className: "rounded-2xl va-surface-muted border p-4 text-right", children: [
          jsx("h2", { className: "text-sm font-bold text-[var(--va-text)]", children: "وسوم بلا مصطلح" }),
          workspace.tagsWithoutTerms.length ? jsx("div", { className: "mt-3 space-y-2", children: workspace.tagsWithoutTerms.slice(0, 6).map((tag: any) => jsxs("div", { className: "flex items-center justify-between gap-2 rounded-xl va-surface-muted border px-3 py-2", children: [
      ...
      vocabulary.length > 0 && jsx(EntityFoldersPanel, {
        ...
        getEntityLabel: (entry: any) => entry.term || entry.id,
        getEntityMeta: (entry: any) => getCategoryInfo(entry.category).label
      }),
          ...
          jsxs("div", {
            ...
            children: [
              ...
              jsx("select", {
                ...
                children: [24, 48, 96].map((size: any) => jsx("option", { value: size, children: `${size} مصطلح` }, size))
              })
            ]
          }),
          jsxs("div", {
            ...
            children: [
              ...
              ...VOCABULARY_CATEGORIES.map((item: any) => jsx(CategoryButton, { category: item, count: (counts as any)[item.id], active: category === item.id, onClick: () => setCategory(item.id) }, item.id))
            ]
          }),
      ...
      visibleEntries.length ? jsx("section", {
        ...
        children: visibleEntries.map((entry: any, index: any) => jsx(VocabularyCard, {
          entry,
          index,
          onEdit: () => {
