...
export const PAGE_NATIVE_MIGRATION: Record<string, any> = {
  ...
  "file-manager": { status: "native", notes: "Page JSX lives in src/pages/FileManagerPage.jsx; browses the file store with upload/move/copy and archive-when-ready actions." },
...
export function getPageMigrationStatus() {
  return (PAGE_MANIFEST as any[]).map((page: any) => {
...
export function getPageMigrationSummary(statusRows: any = getPageMigrationStatus()) {
  const total = statusRows.length;
  const wrappedPages = statusRows.filter((page: any) => page.status === "wrapped").length;
  const native = statusRows.filter((page: any) => page.status === "native").length;
  const heavyWrapped = statusRows.filter((page: any) => page.heavy && page.status === "wrapped").length;
