export interface ArchiveCsvExportData {
  contentTypes?: any[];
  videoItems?: any[];
...
export interface ArchiveCsvExportHelpers {
  isArchivedRecord?: (record: any) => boolean;
}

export function createArchiveCsvExportFiles(data: ArchiveCsvExportData = {}, helpers: ArchiveCsvExportHelpers = {}) {
  const { isArchivedRecord = (record: any) => Boolean(record?.archivedAt || record?.isArchived) } = helpers;
  const {
    contentTypes = [],
    videoItems = [],
    virtualCollections = [],
