...
const AI_RPC_METHODS = ["summarize", "suggestTags", "proofread", "autocompleteFields", "chat", "rankSearch"];
 ...
 * @param {() => string} [options.getToken] - returns the current JWT
 * @param {typeof fetch} [options.fetchImpl]
 * @param {() => void} [options.onUnauthorized] - called on 401
 * @returns {object} an AiProvider satisfying all 8 port methods
 */
export function createCloudAiProvider({ baseUrl = "", getToken, fetchImpl, onUnauthorized } = {}) {
  ...
  async function rpc(method, args) {
    ...
    let payload;
    try { payload = await response.json(); } catch { throw new CloudAiError(`استجابة غير صالحة من خدمة الذكاء (${method}).`, { status: response.status, method }); }
  ...
  const provider = {
    ...
    // segments }.
    async transcribe({ blob, mimeType, name } = {}) {
      if (!blob) throw new CloudAiError("لا يوجد ملف صوت للتفريغ.", { method: "transcribe" });
      const headers = { "Content-Type": mimeType || blob.type || "application/octet-stream" };
      ...
      let payload;
      try { payload = await response.json(); } catch { throw new CloudAiError("استجابة غير صالحة من خدمة التفريغ.", { status: response.status, method: "transcribe" }); }
      if (!response.ok || !payload?.ok) {
        const msg = response.status === 503
