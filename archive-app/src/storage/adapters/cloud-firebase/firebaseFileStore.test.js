...
function createFakeStorage() {
  ...
  const storageModule = {
    ...
    ref: (_storage, key) => ({ key }),
    uploadBytes: async (ref, blob, meta) => {
      blobs.set(ref.key, { blob, meta });
      return { ref };
    },
    getDownloadURL: async (ref) => `https://storage.example/${encodeURIComponent(ref.key)}`,
    getBytes: async (ref) => blobs.get(ref.key)?.blob || null,
