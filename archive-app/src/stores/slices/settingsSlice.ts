...
type StoreCtx = { set: any; get: () => any; getAuthStore?: () => any };
...
export function createSettingsActions({ set, get, getAuthStore }: StoreCtx) {
  return {
    updateSettings: async (patch: Record<string, any> = {}) => {
      const currentSettings: any = get().settings;
      const settings = mergeSettings(currentSettings, patch);
      set({ settings, isPasswordSet: get().isPasswordSet || !!(settings as any).masterPasswordHash });
    ...
    setMasterPassword: async (password: string) => {
      ...
      const settings = mergeSettings(get().settings as any, { masterPasswordHash: passwordHashValue, onboardingRequired: false, initialAdminPassword: null } as any);
      let users = get().users;
      let admin = users.find((user: any) => user.username === "admin");
      if (admin) {
        admin = { ...admin, passwordHash: passwordHashValue, role: "admin", isActive: true, mustChangePassword: false, updatedAt: nowIso() };
        users = users.map((user: any) => user.id === admin.id ? admin : user);
      } else {
    ...
    skipPasswordSetup: async () => {
      ...
      const settings = mergeSettings(get().settings as any, { ui: { onboardingSecurityMode: "quick", v1OnboardingCompleted: true }, onboardingRequired: false } as any);
      let users = get().users;
      ...
      const quickUser = users.find((user: any) => user.username === "admin" && user.isActive !== false) || users.find((user: any) => user.isActive !== false);
      if (quickUser) {
    ...
    unlockApp: async (password: string) => {
      const masterHash = (get().settings as any).masterPasswordHash;
    ...
    runSystemHealthCheck: async () => {
      ...
      const settings = mergeSettings(get().settings as any, { systemHealth: { lastCheckAt: nowIso(), checks } } as any);
