 ...
 */
export function readDeviceIdentity(fallback = null) {
  try {
    if (typeof window === "undefined") return fallback;
    const raw = window.localStorage.getItem(STORAGE_KEY);
    if (!raw) return fallback;
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== "object" || typeof parsed.deviceId !== "string") return fallback;
    return {
      deviceId: parsed.deviceId,
      deviceName: typeof parsed.deviceName === "string" && parsed.deviceName.trim() ? parsed.deviceName : getDefaultDeviceName()
    };
  } catch {
    return fallback;
  }
...
export function persistDeviceIdentity(identity) {
  try {
 ...
 */
export function ensureDeviceIdentity(seed = null) {
