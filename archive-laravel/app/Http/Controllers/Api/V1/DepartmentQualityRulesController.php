...
final class DepartmentQualityRulesController extends Controller
{
    ...
    public function upsert(Request $request): JsonResponse
    {
        ...
        if ($existing) {
            DB::table('department_quality_rules')->where('id', $existing->id)->update($values);
        } else {
            $values += ['id' => (string) Str::uuid(), 'department_id' => $v['departmentId'], 'type_id' => $v['typeId'] ?? null, 'created_at' => now()];
            DB::table('department_quality_rules')->insert($values);
        }
    ...
    public function preview(Request $request): JsonResponse
    {
        ...
        $metadata = $v['metadata'] ?? [];
        $missing = array_values(array_filter($fields, fn ($field) => ! array_key_exists($field, $metadata) || $metadata[$field] === null || $metadata[$field] === ''));
    ...
    {
        return ['id' => $r->id, 'departmentId' => $r->department_id, 'typeId' => $r->type_id, 'requiredFields' => json_decode($r->required_fields, true), 'enabled' => (bool) $r->enabled];
    }
}
