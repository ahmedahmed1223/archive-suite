...
class DropboxController extends Controller
{
    public function authorize(Request $request, DropboxConnectionService $dropbox): JsonResponse
    {
        ...
        if (! $dropbox->configured()) return response()->json(ApiError::envelope('Dropbox OAuth is not configured.', 409), 409);
    ...
    public function callback(Request $request, DropboxConnectionService $dropbox, DropboxGateway $gateway): JsonResponse
    {
        ...
        if (! is_array($pending) || ! isset($pending['user_id'], $pending['verifier'])) return response()->json(ApiError::envelope('Invalid or expired OAuth state.', 422), 422);
        $user = \App\Models\User::find($pending['user_id']);
        if (! $user) return response()->json(ApiError::envelope('OAuth user is unavailable.', 422), 422);
        try {
        ...
        } catch (\Throwable) { return response()->json(ApiError::envelope('Dropbox token exchange failed.', 502), 502); }
    }
    ...
    public function sync(Request $request, DropboxSyncService $sync): JsonResponse
    {
        ...
        catch (\Throwable) { return response()->json(ApiError::envelope('Dropbox sync failed.', 502), 502); }
    }
}
