...
class DropboxWebhookController extends Controller
{
    ...
    {
        return response((string) $request->query('challenge'), 200)->header('Content-Type', 'text/plain');
    }
    ...
    public function receive(Request $request, DropboxWebhookProcessor $processor)
    {
        ...
        if ($secret === '' || ! hash_equals(hash_hmac('sha256', $raw, $secret), $signature)) {
            return response()->json(ApiError::envelope('Invalid Dropbox signature.', 401), 401);
        }
        ...
        if ($accepted) {
            ProcessDropboxWebhook::dispatch($eventId, $payload);
        }
