...
class RecordAttachmentsController extends Controller
{
    ...
    private function recordExists(string $store, string $id): bool
    {
        return DB::table('storage_rows')->where(['store' => $store, 'uid' => $id])->exists();
    }
    ...
    {
        return ['id' => $row->id, 'recordStore' => $row->record_store, 'recordUid' => $row->record_uid, 'disk' => $row->disk, 'path' => $row->path, 'originalName' => $row->original_name, 'mimeType' => $row->mime_type, 'sizeBytes' => (int) $row->size_bytes, 'checksumSha256' => $row->checksum_sha256, 'isPrimary' => (bool) $row->is_primary, 'processingStatus' => $row->processing_status, 'createdAt' => $row->created_at];
    }
}
