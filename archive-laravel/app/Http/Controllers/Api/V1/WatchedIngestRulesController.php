...
final class WatchedIngestRulesController extends Controller
{
    ...
    {
        return response()->json(['ok' => true, 'rules' => DB::table('watched_ingest_rules')->orderBy('created_at')->get()->map(fn ($r) => $this->format($r))->all()]);
    }
    ...
    public function update(Request $request, string $id): JsonResponse
    {
        ...
        if (! DB::table('watched_ingest_rules')->where('id', $id)->exists()) {
            return response()->json(['ok' => false, 'error' => 'Watched ingest rule not found.', 'code' => 'not_found'], 404);
        }
        $v = $request->validate($this->rules());
    ...
    public function destroy(Request $request, string $id): JsonResponse
    {
        ...
        if (DB::table('watched_ingest_rules')->where('id', $id)->delete() < 1) {
            return response()->json(['ok' => false, 'error' => 'Watched ingest rule not found.', 'code' => 'not_found'], 404);
        }
    ...
    {
        return ['matchType' => ['required', 'in:path_prefix,filename_pattern'], 'pattern' => ['required', 'string', 'max:200'], 'metadataTemplateId' => ['nullable', 'string'], 'tags' => ['sometimes', 'array'], 'tags.*' => ['string'], 'stagingDirectory' => ['required', 'string', 'max:200', 'regex:/^[A-Za-z0-9_\/-]+$/'], 'enabled' => ['sometimes', 'boolean']];
    }

    private function format(object $r): array
    {
        return ['id' => $r->id, 'matchType' => $r->match_type, 'pattern' => $r->pattern, 'metadataTemplateId' => $r->metadata_template_id, 'tags' => json_decode($r->tags ?: '[]', true), 'stagingDirectory' => $r->staging_directory, 'enabled' => (bool) $r->enabled];
    }
}
