...
final class Cmx3600Codec
{
    ...
    public function preview(string $edl): array
    {
        ...
        foreach (preg_split('/\R/', $edl) ?: [] as $lineNumber => $line) {
            if (trim($line) === '' || str_starts_with(trim($line), 'TITLE:')) continue;
            if (! preg_match('/^\s*(\d+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\d\d:\d\d:\d\d:\d\d)\s+(\d\d:\d\d:\d\d:\d\d)\s+(\d\d:\d\d:\d\d:\d\d)\s+(\d\d:\d\d:\d\d:\d\d)/', $line, $m)) {
                $rejected[] = ['line' => (string) ($lineNumber + 1), 'reason' => 'invalid_cmx_event']; continue;
            }
            [, $event, $reel, $track, $transition, $sourceIn, $sourceOut, $recordIn, $recordOut] = $m;
            if ($track !== 'V') { $rejected[] = ['line' => (string) ($lineNumber + 1), 'reason' => 'audio_track_unsupported']; continue; }
            if ($transition !== 'C') { $rejected[] = ['line' => (string) ($lineNumber + 1), 'reason' => 'transition_unsupported']; continue; }
