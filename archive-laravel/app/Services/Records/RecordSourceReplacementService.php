...
final class RecordSourceReplacementService
{
    ...
    public function replace(string $id, UploadedFile $file, ?int $actorId): array
    {
        ...
        $directory = trim((string) config('ingest.directory'), '/').'/replacements/'.$id;
    ...
    public function restore(string $id, string $versionId): array
    {
        ...
        $data = json_decode($version->record_data, true, flags: JSON_THROW_ON_ERROR);
        ...
        $this->queueDerivative($id, $data['filePath'], $data['fileName']);
    ...
    {
        if (! in_array(strtolower(pathinfo($fileName, PATHINFO_EXTENSION)), (array) config('ingest.media_extensions', []), true)) {
            return;
        }
