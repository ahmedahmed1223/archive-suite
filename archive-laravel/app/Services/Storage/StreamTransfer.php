...
final class StreamTransfer
{
    ...
    {
        if ($chunkBytes < 1 || $maxRetries < 0) {
            throw new InvalidArgumentException('Invalid transfer limits.');
        }
    }
    ...
    public function copy(StorageAdapter $source, StorageAdapter $destination, string $sourcePath, string $destinationPath, string $conflictPolicy = 'skip', bool $replaceConfirmed = false, ?string $expectedSha256 = null, ?callable $cancelled = null): array
    {
        ...
        if (! in_array($conflictPolicy, ['skip', 'copy', 'replace'], true)) {
            throw new InvalidArgumentException('Unknown conflict policy.');
        }
        if ($destination->exists($destinationPath)) {
            if ($conflictPolicy === 'skip') {
                return ['status' => 'skipped', 'path' => $destinationPath, 'bytesCopied' => 0, 'offset' => $destination->size($destinationPath), 'sha256' => null];
            }
            if ($conflictPolicy === 'replace' && ! $replaceConfirmed) {
                throw new InvalidArgumentException('Replace requires explicit confirmation.');
            }
            if ($conflictPolicy === 'copy') {
                $destinationPath = $this->copyPath($destination, $destinationPath);
            }
        }
        ...
        if ($offset > $sourceSize) {
            throw new RuntimeException('Destination is larger than the source.');
        }
        $fullSha256 = $expectedSha256 === null ? null : $this->sha256($source, $sourcePath);
        if ($expectedSha256 !== null && ! hash_equals(strtolower($expectedSha256), $fullSha256)) {
            throw new RuntimeException('Source content checksum does not match expected SHA-256.');
        }
        ...
        try {
            while (! feof($stream)) {
                if ($cancelled !== null && $cancelled()) {
                    return ['status' => 'cancelled', 'path' => $destinationPath, 'bytesCopied' => $copied, 'offset' => $offset + $copied, 'sha256' => null];
                }
                $chunk = fread($stream, $this->chunkBytes);
                if ($chunk === false) {
                    throw new RuntimeException('Unable to read storage source stream.');
                }
                if ($chunk === '') {
                    continue;
                }
    ...
    private function writeChunk(StorageAdapter $destination, string $path, string $chunk, int $offset): void
    {
        for ($attempt = 0; ; $attempt++) {
            ...
            if ($stream === false) {
                throw new RuntimeException('Unable to create bounded transfer buffer.');
            }
            fwrite($stream, $chunk);
            rewind($stream);
            try {
                $destination->writeStream($path, $stream, $offset);
                fclose($stream);
