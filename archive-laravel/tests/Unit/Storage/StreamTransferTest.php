...
final class StreamTransferTest extends TestCase
{
    public function test_it_resumes_at_existing_destination_offset_and_verifies_sha256(): void
    {
        ...
        self::assertSame(3, $result['bytesCopied']);
        self::assertSame('abcdef', $destination->contents('out/archive.txt'));
        self::assertSame(hash('sha256', 'abcdef'), $result['sha256']);
    }

    public function test_it_cancels_between_chunks_without_reading_the_remainder(): void
    {
        $source = new MemoryAdapter(['source' => 'abcdef']);
        $destination = new MemoryAdapter;
        $checks = 0;
        $result = (new StreamTransfer(2))->copy($source, $destination, 'source', 'destination', cancelled: static function () use (&$checks): bool {
            return ++$checks > 1;
        });
        self::assertSame('cancelled', $result['status']);
        self::assertSame('ab', $destination->contents('destination'));
    }

    public function test_it_supports_skip_copy_and_confirmed_replace_conflict_policies(): void
    {
        $source = new MemoryAdapter(['a.txt' => 'new']);
        $destination = new MemoryAdapter(['a.txt' => 'old']);
        $transfer = new StreamTransfer;
        ...
        $transfer->copy($source, $destination, 'a.txt', 'a.txt', 'replace');
    }

    public function test_it_confines_folder_paths_and_redacts_audit_secrets(): void
    {
        $this->expectException(InvalidArgumentException::class);
        StoragePath::normalize('../outside');
    }
    ...
    {
        $events = [];
        $adapter = new AuditedStorageAdapter(new MemoryAdapter(['from' => 'file']), static function (string $action, array $payload) use (&$events): void {
...
final class MemoryAdapter implements StorageAdapter
{
    ...
    public function readStream(string $path, int $offset = 0)
    {
        ...
        fwrite($stream, substr($this->files[$path] ?? '', $offset));
        rewind($stream);
    ...
    public function writeStream(string $path, $stream, int $offset = 0): void
    {
        ...
        $this->files[$path] = substr($existing, 0, $offset).$chunk.substr($existing, $offset + strlen($chunk));
    }
