  ...
  return (
    <article className="panel">
      <div className="panel-section-header">
        <div>
          <h2>التوصيف الزمني</h2>
          <p className="helper-text">مقاطع وصفية مرتبطة بإطارات الفيديو، ولا تغيّر الملف أو المونتاج.</p>
        </div>
        <span className="badge">{segments.length}</span>
      </div>
      {canDescribe ? <form className="archive-toolbar-grid" onSubmit={createSegment}>
        <label><span>العنوان</span><input value={title} onChange={(e) => setTitle(e.target.value)} required /></label>
        <label><span>بداية الإطار</span><input type="number" min="0" value={startFrame} onChange={(e) => setStartFrame(e.target.value)} required /></label>
        <label><span>نهاية الإطار</span><input type="number" min="1" value={endFrame} onChange={(e) => setEndFrame(e.target.value)} required /></label>
        <label><span>الوصف</span><textarea value={description} onChange={(e) => setDescription(e.target.value)} /></label>
        <label><span>الأشخاص والموضوعات</span><input value={subjects} onChange={(e) => setSubjects(e.target.value)} placeholder="افصل بفواصل" /></label>
        <label><span>المكان</span><input value={place} onChange={(e) => setPlace(e.target.value)} /></label>
        <label><span>ملاحظة الحقوق</span><input value={rightsNote} onChange={(e) => setRightsNote(e.target.value)} /></label>
        <button className="button button-primary" disabled={!title || !endFrame}>إضافة مقطع</button>
      </form> : null}
      ...
      {segments.length ? <ol className="mobile-field-list">
        {segments.map((segment) => <li key={segment.id} id={`timed-description-${segment.id}`}>
          ...
          <span dir="ltr">{segment.startFrame}–{segment.endFrame} frames</span>
          {segment.description ? <small>{segment.description}</small> : null}
          {canDescribe ? <>
            <button className="button button-ghost button-sm" type="button" aria-label={`تعديل المقطع: ${segment.title}`} onClick={() => setEditingSegmentId(segment.id)}>تعديل</button>
            <button className="button button-ghost button-sm" type="button" aria-label={`حذف المقطع: ${segment.title}`} onClick={() => void deleteSegment(segment.id)}>حذف</button>
      ...
      </ol> : !error ? <EmptyState title="لا توجد مقاطع موصوفة بعد." description="أضف مقاطع وصفية بالإطارات من مساحة التوصيف." /> : null}
    </article>
  );
}
...
function TimedDescriptionSegmentEditor({ api, segment, onSaved, onCancel }: Readonly<{ api: ArchiveApiClient; segment: TimedDescriptionSegment; onSaved: (segment: TimedDescriptionSegment) => void; onCancel: () => void }>) {
  ...
  const [error, setError] = useState("");
  const labelSuffix = `: ${segment.title}`;
  ...
  async function save(event: FormEvent) {
    ...
    if (!response.ok) { setError(response.error || "تعذر حفظ التعديل."); return; }
    onSaved(response.segment);
  }

  return <form className="archive-toolbar-grid section-divider" onSubmit={save}><label><span>عنوان المقطع{labelSuffix}</span><input value={title} onChange={(event) => setTitle(event.target.value)} required /></label><label><span>بداية الإطار للمقطع{labelSuffix}</span><input type="number" min="0" value={startFrame} onChange={(event) => setStartFrame(event.target.value)} required /></label><label><span>نهاية الإطار للمقطع{labelSuffix}</span><input type="number" min="1" value={endFrame} onChange={(event) => setEndFrame(event.target.value)} required /></label><label><span>وصف المقطع{labelSuffix}</span><textarea value={description} onChange={(event) => setDescription(event.target.value)} /></label><label><span>الأشخاص والموضوعات للمقطع{labelSuffix}</span><input value={subjects} onChange={(event) => setSubjects(event.target.value)} /></label><label><span>مكان المقطع{labelSuffix}</span><input value={place} onChange={(event) => setPlace(event.target.value)} /></label><label><span>ملاحظة الحقوق للمقطع{labelSuffix}</span><input value={rightsNote} onChange={(event) => setRightsNote(event.target.value)} /></label><div className="archive-toolbar-actions"><button className="button button-primary" disabled={!title.trim() || !endFrame}>حفظ التعديل</button><button className="button button-secondary" type="button" onClick={onCancel}>إلغاء</button></div>{error ? <p className="form-status" role="alert">{error}</p> : null}</form>;
}
