...
export default function DepartmentQualityPanel({ departmentId }: Readonly<{ departmentId: string }>) {
  ...
  async function load() { if (!departmentId) { setRules([]); return; } const r = await api.departmentQualityRules(departmentId); if (r.ok) setRules(r.rules); else setMessage(r.error || "تعذر تحميل قواعد الجودة."); }
