...
function renderWithLocale(ui: ReactNode) {
  return render(<LocaleProvider initialLocale="ar" hasLocaleCookie>{ui}</LocaleProvider>);
}
...
describe("SearchAutocomplete", () => {
  ...
  it("selects an option with the keyboard", async () => {
    ...
    renderWithLocale(<SearchAutocomplete value="riy" onChange={() => {}} onSelect={onSelect} fetchSuggestions={async () => [{ kind: "record", label: "Riyadh archive interview", value: "Riyadh archive interview", recordId: "clip-001" }]} />);
  ...
  it("closes its suggestion list when focus leaves the field", async () => {
    renderWithLocale(<SearchAutocomplete value="riy" onChange={() => {}} onSelect={() => {}} fetchSuggestions={async () => [{ kind: "record", label: "Riyadh archive interview", value: "Riyadh archive interview", recordId: "clip-001" }]} />);
    const input = screen.getByRole("combobox");
    fireEvent.focus(input);
    await screen.findByRole("option", { name: "Riyadh archive interview" });
  ...
  it("does not restore suggestions when a pending request resolves after blur", async () => {
    ...
    renderWithLocale(<SearchAutocomplete value="riy" onChange={() => {}} onSelect={() => {}} fetchSuggestions={() => new Promise((resolve) => { releaseSuggestions = resolve; })} />);
    const input = screen.getByRole("combobox");
