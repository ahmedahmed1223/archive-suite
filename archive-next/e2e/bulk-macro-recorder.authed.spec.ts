...
test("editor must preview a saved macro before it can run, and selection changes invalidate it", async ({ roleSession }) => {
  ...
  await page.getByRole("textbox", { name: "اسم الماكرو" }).fill("وسم مهم");
  await page.getByRole("textbox", { name: "الوسم الجديد" }).fill("مهم");
  await page.getByRole("button", { name: "إضافة وسم" }).click();
  await page.getByRole("button", { name: "إضافة حالة" }).click();
  await page.getByRole("button", { name: "إضافة حذف" }).click();
  await page.getByRole("button", { name: "نقل الخطوة 3 للأعلى" }).click();
  await page.getByRole("button", { name: "إزالة الخطوة 2" }).click();
  await page.getByRole("button", { name: "حفظ الماكرو" }).click();
  await expect(page.getByText("تم حفظ الماكرو.")).toBeVisible();
  await expect(page.getByRole("button", { name: "تنفيذ الماكرو" })).toBeDisabled();
  expect(runs).toBe(0);
  await page.getByRole("button", { name: "معاينة التنفيذ" }).click();
  await expect(page.getByText("معاينة موقعة")).toBeVisible();
  await expect(page.getByText("قابل للتراجع").first()).toBeVisible();
  await expect(page.getByRole("button", { name: "تنفيذ الماكرو" })).toBeEnabled();
  await page.getByRole("checkbox", { name: /تحديد/ }).first().uncheck();
  await expect(page.getByRole("heading", { name: "مسجل الإجراءات الجماعية" })).toHaveCount(0);
  await page.getByRole("checkbox", { name: /تحديد/ }).first().check();
  await expect(page.getByRole("button", { name: "تنفيذ الماكرو" })).toBeDisabled();
  await page.getByRole("button", { name: "معاينة التنفيذ" }).click();
  await expect(page.getByRole("button", { name: "تنفيذ الماكرو" })).toBeEnabled();
  await page.getByRole("button", { name: "تنفيذ الماكرو" }).click();
  await expect(page.getByText(/نتيجة التنفيذ/)).toBeVisible();
  expect(runs).toBe(1);
});
