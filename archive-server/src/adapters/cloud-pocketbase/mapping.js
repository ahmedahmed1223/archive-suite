...
export function defaultKeyPathFor(store) {
...
export function toPbRecord(domainRecord, keyPath) {
...
export function fromPbRecord(pbRecord) {
  if (!pbRecord) return undefined;
  return pbRecord.data;
}

/** Build a safe PocketBase filter expression matching one uid. */
export function uidFilter(key) {
