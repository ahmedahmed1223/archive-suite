...
function memoryFileStore() {
  ...
  return {
    ...
    putBlob: async (key, value) => { files.set(key, Buffer.from(value)); return { key }; },
    getBlob: async (key) => files.get(key) || null,
...
describe("file browser API", () => {
  ...
  it("requires authentication and browses one folder level", async () => {
    ...
    expect(payload.result).toMatchObject({ path: "raw", entries: [{ name: "a.mp4", key: "raw/a.mp4", kind: "file" }] });
  });
