...
function makeReadBody(data) {
  return async () => data
}

function mockPrisma(records = []) {
    ...
    rightsRecord: {
      ...
      findMany: vi.fn(async ({ where }) => {
        ...
        if (where?.expiresAt) {
          ...
          return all.filter((r) => r.expiresAt && r.expiresAt > gt && r.expiresAt <= lte)
        }
        return all
      }),
...
function baseCtx({ prisma = mockPrisma(), authClaims = { sub: "u1", role: "editor" }, body = {} } = {}) {
  const res = makeRes()
  const sendJson = makeSendJson(res)

  const requireAuth = vi.fn((_req, _res) => authClaims)
  const requireEditor = vi.fn((_req, _res) => authClaims)
...
describe("GET /api/rights — unauthenticated", () => {
  it("returns 401 when requireAuth sends 401 and returns null", async () => {
    ...
    ctx.requireAuth = vi.fn((_req, res) => {
