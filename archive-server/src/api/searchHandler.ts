 ...
 */
function extractUserSub(req: any, authSecret: string | undefined): string {
 ...
 */
export async function handleSearch(req: any, res: any, { provider, prisma, authSecret }: SearchHandlerOptions = {}): Promise<void> {
  ...
  if (isSemantic && prisma) {
    try {
      const semanticResults = await semanticSearch(prisma as any, q, { store, limit });
      if (semanticResults.length > 0) {
        // Fetch full records for the found UIDs
        const uids = semanticResults.map((r: any) => r.uid);
        const records = await (prisma as any).storageRow.findMany({
          where: { uid: { in: uids }, store },
        });
        // Preserve similarity ordering from pgvector
        const byUid = Object.fromEntries(records.map((r: any) => [r.uid, r]));
        const ordered = uids.map((uid: string) => byUid[uid]).filter(Boolean)
          .map((r: any) => ({ uid: r.uid, ...r.data }));
      ...
      log.warn({ err: (semErr as any).message }, "Semantic search failed — falling back to full-text.");
    }
  }
  ...
  try {
    ...
    if (prisma) {
      ...
      const data = rows.slice(0, limit).map((r: any) => ({ uid: r.uid, ...r.data }));
      results = {
        q,
        store,
        data,
      ...
      const allRows = await (provider as any).getAll(store);
      ...
      const startIdx = cursor ? filtered.findIndex((r: any) => r.uid === cursor) + 1 : 0;
