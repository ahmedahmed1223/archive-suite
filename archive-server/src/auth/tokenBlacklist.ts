...
const memBlacklist = new Map<string, number>(); // jti → expiresAt (ms epoch)
...
async function getRedis(): Promise<any> {
  ...
  try {
    ...
    _redis = new Redis(url, {
      ...
      retryStrategy: (retries: number) => Math.min(retries * 100, 5000),
    });
    _redis.on("error", (err: Error) =>
      log.warn({ err: err.message }, "Redis blacklist error")
    );
    await _redis.ping();
    log.info("JWT blacklist connected to Redis");
  } catch (err) {
    const errMsg = err instanceof Error ? err.message : String(err);
    log.warn({ err: errMsg }, "Redis unavailable — JWT blacklist is memory-only");
    _redis = null;
  }
  return _redis;
}
...
async function hydrateFromRedis(): Promise<void> {
  try {
    ...
    const ttls = await Promise.all(keys.map((k: string) => redis.pexpiretime(k)));
    const now = Date.now();
    ...
    const errMsg = err instanceof Error ? err.message : String(err);
    log.warn({ err: errMsg }, "Redis hydration failed — starting with empty blacklist");
  }
}
 ...
 * @param expiresAt - Unix timestamp in SECONDS (JWT `exp`).
...
export function revokeToken(jti: string, expiresAt?: number): void {
  ...
  getRedis()
    ...
    .catch((err: Error) =>
      log.warn({ err: err.message, jti }, "Redis revokeToken failed")
    );
}
