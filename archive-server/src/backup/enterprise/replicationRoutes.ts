...
import { appendBackupManifestEntry, findRestorableEntry, type ManifestEntry } from "./manifest.js";
...
interface ReplicationRouteContext {
  req: Request;
  res: Response;
  url: string;
  send: (res: Response, status: number, data: unknown) => void;
  overLimit: (res: Response, type: string, req: Request) => boolean;
...
export async function handleReplicationRoute({
  ...
  requireAuth,
  requireAdmin,
}: ReplicationRouteContext): Promise<boolean> {
  ...
  if (req.method === "POST" && replicateMatch) {
    ...
    try {
      ...
      return send(res, (err as any)?.statusCode || 500, { ok: false, error: err instanceof Error ? err.message : "Replication failed" }), true;
    }
  }
  ...
  if (req.method === "POST" && smokeMatch) {
    ...
    try {
      ...
      return send(res, result.ok ? 200 : 422, { ...result }), true;
    } catch (err) {
      log.error({ err, replicaId }, "Restore smoke failed.");
      return send(res, 500, { ok: false, error: err instanceof Error ? err.message : "Smoke test failed" }), true;
    }
  }
...
function resolveBackupPath(backupId: string): string | null {
