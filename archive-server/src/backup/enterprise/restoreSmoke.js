...
/**
 * Run a DR smoke check against a replicated backup.
 *
 * @param {object} options
 * @param {object}  options.entry           Manifest entry (bucket, key, region, sha256, encryption)
 * @param {string}  [options.encryptionKey] Required when entry.encryption === "aes-256-gcm"
 * @param {string}  [options.tempDir]       Base temp dir (default: OS tmpdir)
 * @param {object}  [options.s3Client]      Injected S3 client for testing
 * @returns {Promise<{ ok: boolean, errors: string[], durationMs: number }>}
 */
export async function runRestoreSmoke({
  ...
  try {
    ...
    try {
      ...
      return { ok: false, errors: [`S3 download failed: ${err.message}`], durationMs: Date.now() - t0 };
    }

    await pipeline(Readable.from(s3Response.Body), createWriteStream(downloadPath));
    ...
    if (encryption === "aes-256-gcm") {
      ...
      try {
        ...
        return { ok: false, errors: [`Decryption failed: ${err.message}`], durationMs: Date.now() - t0 };
      }
    ...
    try {
      ...
      if (err.code === "ENOENT" || /not found|no such file/i.test(String(err.message))) {
      ...
      log.debug({ stderr: err.stderr }, "pg_restore --list non-zero exit (expected for non-pg_dump archives)");
    }
