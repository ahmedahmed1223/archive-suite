...
interface ControlAgentService {
  id: string;
  name?: string;
}

interface BackupReplicationConfig {
  enabled: boolean;
  bucket: string;
  region: string;
  prefix: string;
...
interface BackupConfig {
  replication: BackupReplicationConfig;
}
...
interface ServerConfig {
  ...
  jwtShareSecret: string;
  oauthStateSecret: string;
  jwtSecret: string;
  jwtTtlSec: number | undefined;
  ...
  vapidSubject: string;
  rateLimitRpcMax: number | undefined;
  rateLimitLoginMax: number | undefined;
  ...
  maxOcrBytes: number | undefined;
  pocketbaseUrl: string;
  backupEnabled: boolean;
  backupDir: string;
  backupIntervalHours: number | undefined;
  backupRetentionDays: number | undefined;
  backupRetentionWeeks: number | undefined;
  backupRetentionMonths: number | undefined;
  backupEncryptionKey: string;
  backup: BackupConfig;
  workflowDueRemindersEnabled: boolean;
  workflowDueCheckHours: number | undefined;
  controlAgentMode: string;
  controlAgentActions: string;
  controlAgentServices: ControlAgentService[];
  composeFile: string;
  publicApiStores: string;
  shareExpiryDays: number | undefined;
  pdfFontPath: string;
  mediaJobConcurrency: number | undefined;
...
export const config: Readonly<ServerConfig> = Object.freeze({
  ...
  logLevel: str(
    e.LOG_LEVEL,
    e.NODE_ENV === "production" ? "info" : "debug"
  ),
  ...
  backup: Object.freeze({
    replication: Object.freeze({
      ...
      drillIntervalHours: num(e.BACKUP_DR_DRILL_INTERVAL_HOURS, 24) ?? 24,
    }),
  } as BackupConfig),
  ...
  controlAgentActions: str(
    e.CONTROL_AGENT_ACTIONS ?? e.CONTROL_AGENT_ACTIONS_ENABLED,
    ""
  ),
  controlAgentServices: json(e.CONTROL_AGENT_SERVICES, []) as ControlAgentService[],
