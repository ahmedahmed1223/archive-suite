...
const PDF_FONT_CANDIDATES: string[] = [
  config.pdfFontPath,
...
function recordToRow(record?: StorageRecord | null): ExportRow {
  ...
  return {
    ...
    ocrText: String(d.ocrText ?? "").slice(0, 500),
    createdAt: String(d.createdAt ?? record?.createdAt ?? ""),
    updatedAt: String(d.updatedAt ?? record?.updatedAt ?? ""),
    fileUrl: String(d.fileUrl ?? d.url ?? ""),
    mimeType: String(d.mimeType ?? ""),
  };
}

function rowsToSheet(rows: ExportRow[]): any {
...
function buildWorkbook(rows: ExportRow[], { template = false }: BuildWorkbookOptions = {}): any {
...
async function loadPdfFont(pdfDoc: any): Promise<PdfFontInfo> {
...
function pdfText(text: unknown, unicode: boolean): string {
  const value = String(text ?? "");
  if (unicode) return value;
  return value.replace(/[^\x20-\x7E]/g, "").trim() || "Archive";
}

function wrapText(text: unknown, font: any, size: number, maxWidth: number, unicode: boolean): string[] {
...
async function buildPdfReport(rows: ExportRow[]): Promise<Buffer> {
  ...
  const draw = (text: unknown, x: number, size = 10, color = rgb(0.13, 0.16, 0.22), selectedFont = font) => {
    page.drawText(pdfText(text, unicode), { x, y, size, font: selectedFont, color });
  };
  const paragraph = (text: unknown, x: number, width: number, size = 10, leading = 15) => {
  ...
  const typeCounts: Record<string, number> = rows.reduce((acc, row) => {
...
interface ExportOptions {
  format?: "csv" | "xlsx" | "xlsx-template" | "pdf" | "zip" | "bibtex" | "ris";
  store?: string;
  ids?: (string | number)[] | null;
}

interface DataProvider {
  getAll(store: string): Promise<StorageRecord[]>;
}

export async function exportRecords(provider: DataProvider, { format = "csv", store = "videoItems", ids = null }: ExportOptions = {}): Promise<ExportResult> {
  ...
  if (format === "bibtex") {
    return {
      ...
      buffer: Buffer.from(recordsToBibtex(records as Array<Record<string, unknown> | null>), "utf-8"),
    };
  }
  ...
  if (format === "ris") {
    return {
      ...
      buffer: Buffer.from(recordsToRis(records as Array<Record<string, unknown> | null>), "utf-8"),
    };
  }
