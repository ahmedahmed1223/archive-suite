export const FILE_STORE_PROVIDERS = Object.freeze([
...
export function resolveFileStoreProviderStatuses(env = process.env, active = env.FILE_STORE || "disk") {
  return FILE_STORE_PROVIDERS.map((provider) => {
    const missingEnv = (provider.requiredEnv || []).filter((name) => !isSet(env, name));
