 ...
 * @param {string}   options.rootDir       - directory to watch
 * @param {number}   [options.intervalMs]  - polling interval (default 30 000 ms)
 * @param {Function} options.onIngest      - async (info) => void; called per new file
 * @returns {{ scan: () => Promise<void>, start: () => void, stop: () => void }}
 */
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        process.stderr.write(
          `[watchFolder] ingest error for ${filePath}: ${err?.message || err}\n`
        );
