...
export function createInMemoryMediaJobStore({ now = () => Date.now(), idFactory } = {}) {
  const jobs = new Map();
  let seq = 0;
...
export function parseFfmpegProgress(text, durationSec) {
  ...
  async function processJob(job) {
    ...
    try {
      const onProgress = (value) => {
        const progress = typeof value === "number" ? value : parseFfmpegProgress(value, job.params?.durationSec);
      ...
      if (eventBus) eventBus.publish({ type: "media.job.done", job: done });
    } catch (error) {
      const failed = store.update(job.id, {
        status: "error",
        error: error?.message || "Media job failed"
      });
      if (eventBus) eventBus.publish({ type: "media.job.error", job: failed });
    } finally {
      active -= 1;
    }
  }
...
async function defaultRunDerivative({ job, fileStore, onProgress }) {
...
export async function storeMontageOutput({ output, outputKey, fileStore }) {
...
export function montageOutputKey(job) {
