...
export class MediaError extends Error {
  ...
  constructor(message: string, { code, statusCode = 400 }: { code?: string; statusCode?: number } = {}) {
    super(message);
...
export function sanitizeMediaOutputKey(prefix: string, sourceKey: string, suffix: string, ext: string): string {
  return `${prefix}/${safeBaseName(sourceKey)}-${suffix}.${ext}`;
}

/** Project-controlled media staging directory (not os.tmpdir()). */
function mediaWorkDir(): string {
 ...
 */
async function writeBlobToFile(fileStore: any, key: string, destPath: string): Promise<boolean | null> {
...
async function withTempFileFromStore(fileStore: any, key: string, fn: (src: string, dir: string) => Promise<any>): Promise<any> {
...
export async function runMediaProbe({
  ...
  fileStore: any;
  ffprobePath?: string;
  runFfprobe?: (cmd: string, args: string[], opts: any) => Promise<{ stdout: string; stderr: string }>;
  timeoutMs?: number;
}): Promise<any> {
  return withTempFileFromStore(fileStore, key, async (src) => {
    ...
    try {
      ...
      throw new MediaError(`تعذّر تحليل ffprobe: ${(error as any).message}`, { code: "FFPROBE_PARSE", statusCode: 500 });
    }
  });
}

function argsFor(type: string, src: string, params: Record<string, unknown>, out: string, metadata: any): string[] {
  if (type === "thumbnail") {
    return buildThumbnailArgs(src, {
      ...params,
      atSec: (params.atSec as number) ?? smartThumbnailSecond(metadata),
      out
    });
  }
  if (type === "preview") return buildGifPreviewArgs(src, { ...(params as any), out });
  if (type === "audio") return buildAudioArgs(src, { ...(params as any), out });
  if (type === "transcode") return buildTranscodeArgs(src, { ...(params as any), out });
...
export async function runMediaDerivative({
  ...
  ffmpegPath?: string;
  ffprobePath?: string;
  runFfmpeg?: (cmd: string, args: string[], opts: any) => Promise<{ stdout: string; stderr: string }>;
  runFfprobe?: (cmd: string, args: string[], opts: any) => Promise<{ stdout: string; stderr: string }>;
  timeoutMs?: number;
  onProgress?: (value: string) => void;
}): Promise<{ outputKey: string; url: string | null; contentType: string }> {
  ...
  const outputKey = (params.outputKey as string) || sanitizeMediaOutputKey(config.prefix, key, config.suffix, ext);
...
function defaultTimeoutFor(type: string): number {
  if (type === "transcode") return 30 * 60 * 1000;
  if (type === "audio") return 5 * 60 * 1000;
  return 90_000;
}

export function defaultRunProcess(cmd: string, args: string[], { timeoutMs = 120_000, onProgress }: { timeoutMs?: number; onProgress?: (text: string) => void } = {}): Promise<{ stdout: string; stderr: string }> {
  return new Promise((resolve, reject) => {
    ...
    proc.on("error", (error) => {
      clearTimeout(timer);
      reject(new MediaError(`تعذّر تشغيل ${cmd}: ${(error as any).message}`, { code: "SPAWN", statusCode: 500 }));
    });
    proc.on("close", (code) => {
      clearTimeout(timer);
