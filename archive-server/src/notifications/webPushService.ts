/**
 * webPushService.ts — Web Push delivery (§20.2).
 *
...
const recentSends = new Map<string, number>(); // `${userId}:${tag}:${title}` → last send ms epoch
...
async function defaultSendImpl(subscription: any, payload: string): Promise<void> {
  if (!_webPush) {
 ...
 */
export async function saveSubscription(prisma: any, userId: string, subscription: any): Promise<unknown> {
...
export async function removeSubscription(prisma: any, userId: string, endpoint: any): Promise<unknown> {
...
async function getPushPrefs(prisma: any, userId: string): Promise<any> {
...
export function sendPushToUser({ prisma, userId, type = "system", title, body, url, tag, sendImpl = defaultSendImpl, onDone }: {
  ...
  url?: string;
  tag?: string;
  sendImpl?: (subscription: any, payload: string) => Promise<void>;
  onDone?: () => void;
}): void {
  setImmediate(async () => {
    try {
      ...
      log.warn({ err: (err as any)?.message }, "sendPushToUser failed");
    } finally {
