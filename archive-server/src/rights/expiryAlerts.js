/**
 * expiryAlerts.js — Rights expiry alert service.
 *
...
export function createExpiryAlertService({
  ...
  if (typeof auditLog !== "function") throw new Error("createExpiryAlertService: auditLog must be a function")
  ...
  async function scanAndAlert() {
    const now = new Date()
    const cutoff = new Date(now.getTime() + alertThresholdDays * 24 * 60 * 60 * 1000)
    ...
    try {
      ...
      logger.error({ err }, "expiryAlerts: failed to query expiring rights records")
      return { alerted: [] }
    }

    if (records.length === 0) {
      logger.info({ alertThresholdDays }, "expiryAlerts: no records expiring soon")
      return { alerted: [] }
    }
    ...
    for (const record of records) {
      const daysUntilExpiry = Math.ceil(
        (new Date(record.expiresAt).getTime() - now.getTime()) / (24 * 60 * 60 * 1000)
      )
      ...
      try {
        ...
        logger.error({ err, itemId: record.itemId }, "expiryAlerts: failed to emit audit log entry")
      }
    }
