/**
 * rightsEnforcement.js — Pure functions for rights/license enforcement.
 *
 ...
 */
export function checkRightsForExport({ record, requestingCountry, now = new Date() }) {
  ...
  if (record.embargoStart && record.embargoEnd) {
    const start = record.embargoStart instanceof Date ? record.embargoStart : new Date(record.embargoStart)
    const end = record.embargoEnd instanceof Date ? record.embargoEnd : new Date(record.embargoEnd)
  ...
  if (requestingCountry && Array.isArray(record.geoRestrictions) && record.geoRestrictions.length > 0) {
 ...
 * @returns {boolean}
 */
export function isExpiringSoon({ record, days = 30, now = new Date() }) {
 ...
 */
export function buildRightsSummary({ record, now = new Date() }) {
  if (!record) {
  ...
  if (record.embargoStart) {
    ...
    const end = record.embargoEnd
      ? (record.embargoEnd instanceof Date ? record.embargoEnd : new Date(record.embargoEnd))
      : null
  ...
  if (expiresAt) {
    ...
    if (daysUntilExpiry <= 30) {
      return { status: "expiring_soon", badge: `Expires in ${daysUntilExpiry}d`, daysUntilExpiry }
    }
