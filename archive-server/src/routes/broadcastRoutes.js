...
const broadcastMetaStore = new Map();

/**
 * Store broadcast metadata for an item. Called by ingest handlers.
 *
 * @param {string} itemId
 * @param {object} meta  - BroadcastMetadata + extractedAt
 */
export function storeBroadcastMeta(itemId, meta) {
 ...
 * @param {string} params.url          - normalised path, no query string
 * @param {Function} params.send       - (res, status, payload) => void
 * @param {Function} params.requireAuth   - () => claims|false (sends 401 on failure)
 * @param {Function} params.requireAdmin  - () => claims|false (sends 401/403 on failure)
 * @param {Function} params.readJsonBody  - (req) => Promise<object>
 * @param {Function} [params.resolveStorage] - () => StorageProvider (for item lookup)
 */
