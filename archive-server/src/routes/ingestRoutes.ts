 ...
 */
function buildArchiveItem({ filePath, size, mimeType, checksum }: IngestItemData): any {
  ...
  if (req.method === "POST" && url === "/api/ingest/scan") {
    try {
      ...
      const svc = createWatchFolderService({
        rootDir: watchDir,
        onIngest: async (info: IngestItemData) => {
  ...
  if (req.method === "POST" && url === "/api/ingest/ftp/pull") {
    try {
      ...
      const items = await Promise.all(
        pulled.map(async (filePath: string) => {
