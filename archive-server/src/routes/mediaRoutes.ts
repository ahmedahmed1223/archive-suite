...
function safeFileStoreInfo(info: any = {}): Record<string, any> {
...
async function buildFileStoreStatus(files: any): Promise<any> {
...
export async function handleMediaRoute({
  ...
  const derivativeRoutes: Record<string, string> = {
  ...
  if (req.method === "POST" && url.split("?")[0] === "/api/projects/export") {
    ...
    try {
      ...
      if (!outFile) {
        return send(res, 500, { ok: false, error: "Export failed to produce output file" }), true;
      }
