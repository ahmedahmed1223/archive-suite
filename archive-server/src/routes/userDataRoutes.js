...
export async function handleUserDataRoute({
  ...
  if (url === "/api/webhooks" && req.method === "POST") {
    ...
    try {
      ...
      const events = Array.isArray(body.events) ? body.events.filter((e) => ALLOWED_EVENTS.includes(e)) : ALLOWED_EVENTS;
  ...
  if (url.split("?")[0] === "/api/public/records" && req.method === "GET") {
    ...
    try {
      ...
      const sorted = [...all].sort((a, b) => String(a?.id ?? a?.uid ?? "").localeCompare(String(b?.id ?? b?.uid ?? "")));
      const startIdx = cursor ? sorted.findIndex((r) => String(r?.id ?? r?.uid ?? "") > cursor) : 0;
  ...
  if (url.split("?")[0] === "/api/workflow/transition" && req.method === "POST") {
    ...
    try {
      ...
      if (record?.ownerId && record.ownerId !== claims.sub) {
        sendPushToUser({ prisma, userId: record.ownerId, type: "system",
  ...
  if (req.method === "POST" && url.split("?")[0] === "/api/records/bulk") {
    ...
    try {
      ...
      if (body.action === "delete") {
        ...
        const toUpdate = Array.isArray(allRecords) ? allRecords.filter((r) => r && idSet.has(String(r.id ?? r.uid ?? ""))) : [];
