...
const CAPABILITY_KEYS = Object.freeze(["canView", "canComment", "canDownload", "canEdit"]);

/**
 * @param {{ resolvedShareSecret: string }} deps
 */
export function createSharePermissionService({ resolvedShareSecret } = {}) {
  /**
   ...
   */
  function fromRequest(req, { password } = {}) {
    if (!resolvedShareSecret) {
      return { ok: false, status: 501, error: "خدمة المشاركة غير مهيأة." };
    }
    ...
    if (!token) {
      ...
      const parts = auth.startsWith("Bearer ") ? auth.slice(7).trim().split(".") : [];
    ...
    try {
      ...
      return { ok: false, status, error: err?.message || "رمز المشاركة غير صالح." };
    }
  }
   ...
   */
  function allows(payload, capabilityKey) {
   ...
   * @param {string} itemId
   */
  function scopeIncludesItem(payload, itemId) {
    const scope = payload?.scope;
    if (!scope) return false;
    if (scope.type === "all") return true;
    if (scope.type === "items" || scope.type === "collection") {
      return Array.isArray(scope.ids) && scope.ids.includes(String(itemId || ""));
    }
    return false;
  }
