 ...
 */
function makeProvider(records = []) {
  return {
    getAll: vi.fn(async (store) => (store === "video_items" ? records : [])),
  };
}
...
describe("runDueDateCheck", () => {
  ...
  it("sends no notifications when no records have a dueDate", async () => {
    ...
    await runDueDateCheck(provider, prisma, sendPushToUser);
  ...
  it("skips records with no owner (no createdBy or ownerId)", async () => {
    const yesterday = new Date(Date.now() - 86_400_000).toISOString().slice(0, 10);
    const record = { ...makeRecord({ workflowDueDate: yesterday }), createdBy: undefined, ownerId: undefined };
  ...
  it("uses ownerId as fallback when createdBy is absent", async () => {
    const yesterday = new Date(Date.now() - 86_400_000).toISOString().slice(0, 10);
    const record = { ...makeRecord({ workflowDueDate: yesterday }), createdBy: undefined, ownerId: "owner-xyz" };
    const provider = makeProvider([record]);
    await runDueDateCheck(provider, prisma, sendPushToUser);
    expect(pushCalls).toHaveLength(1);
  ...
  it("skips records with invalid dueDate strings", async () => {
    const provider = makeProvider([makeRecord({ workflowDueDate: "not-a-date" })]);
    await runDueDateCheck(provider, prisma, sendPushToUser);
    expect(pushCalls).toHaveLength(0);
  });
  ...
  it("does not crash when provider.getAll throws", async () => {
    ...
    await expect(runDueDateCheck(provider, prisma, sendPushToUser)).resolves.toBeUndefined();
    expect(pushCalls).toHaveLength(0);
  });
  ...
  it("handles null/missing provider or prisma gracefully", async () => {
    ...
    await expect(runDueDateCheck(makeProvider([]), null, sendPushToUser)).resolves.toBeUndefined();
    expect(pushCalls).toHaveLength(0);
  });
  ...
  it("queries all five content stores", async () => {
    ...
    await runDueDateCheck(provider, prisma, sendPushToUser);
    const queried = provider.getAll.mock.calls.map(([s]) => s);
  ...
  it("deduplicates: same record gets at most one alert per type per day", async () => {
    ...
    const provider = makeProvider([makeRecord({ workflowDueDate: yesterday })]);
    await runDueDateCheck(provider, prisma, sendPushToUser);
    await runDueDateCheck(provider, prisma, sendPushToUser); // second run same day
