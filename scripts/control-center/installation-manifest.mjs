...
const SENSITIVE_KEY = /(password|secret|token|credential|authorization|cookie|dsn|connection|url)/i;
...
function rejectSensitive(value, field = "manifest") {
  if (typeof value === "string") {
    if (CREDENTIAL_URL.test(value) || SECRET_VALUE.test(value)) fail(`${field} contains a sensitive value or credential.`);
    return;
  }
...
function normalizeDataPaths(value) {
  ...
  for (const [name, path] of Object.entries(value)) {
    ...
    if (URL.test(safePath) || CREDENTIAL_URL.test(safePath)) fail(`dataPaths.${name} must be a local path without credentials.`);
...
function validateManifest(manifest) {
  ...
  if (!(typeof manifest.lastSuccessfulStep === "string" || manifest.lastSuccessfulStep === null)) fail("lastSuccessfulStep must be a string or null.");
  if (!(typeof manifest.previousVersion === "string" || manifest.previousVersion === null)) fail("previousVersion must be a string or null.");
  ...
  if (!(typeof operation.failedStep === "string" || operation.failedStep === null)) fail("operation.failedStep must be a string or null.");
...
export function decideInstallationResume({ path, input, fs = nodeFs }) {
  ...
  if (manifest.operation.status === "failed") return { action: "resume", step: manifest.lastSuccessfulStep };
...
export function beginInstallationOperation({ path, input, operation, fs = nodeFs }) {
  ...
  atomicWrite(path, next, fs);
  return { created: created.created, manifest: next, decision: decideInstallationResume({ path, input, fs }) };
}
