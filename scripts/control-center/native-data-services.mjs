// V1-210C / V1-211C: data services for Native installs (Windows and Linux
// share the exact same rules). PostgreSQL is either a locally managed
// instance under the install root or an external endpoint; queue and cache
// always run on the database baseline; a Redis-compatible endpoint is
// optional and only accepted after its probe succeeds. The gate runs BEFORE
// anything is installed, so an unhealthy endpoint blocks the install instead
// of producing a half-configured stack. Probes are injected
...
export function createNativeDataGate({ probes, startLocalPostgres } = {}) {
  ...
  return async function ensureDataServicesReady(plan) {
    ...
    return {
      ...
      message: "Data services are verified and ready for the Native install.",
