...
test("Windows data staging bundles PostgreSQL, pgvector, Redis, and the pgAdmin component manifest without build paths", () => {
  ...
  try {
    ...
    const result = stageWindowsDataServices({ destDir: out, postgresInstaller: installer, pgvectorDirectory: pgvector, redisDirectory: redis });
    ...
    assert.equal(manifest.components.redis.files.length, 1);
