...
export function evaluatePerformanceRun(contract, run) {
  ...
  errors.push(...datasetEvidenceErrors(contract, run?.datasetEvidence));
  const measurements = run?.measurements || {};
  for (const [metric, budget] of Object.entries(metricBudgets(contract))) {
    ...
    const actual = percentile(samples, contract.measurement.requiredMetrics[metric].percentile);
